...
\chapter{Reihen}
\label{cha:reihen}
\paragraph{Ziel.} Untersuche "`unendliche Summen"' $a_0 + a_1 + a_2 + \dotsb$ für eine gegebene Folge $(a_n)_{n \ge 0}$.

\section{Konvergenzkriterien}
\label{sec:reihen.konvkrit}
\begin{dfn}\label{dfn:reihen.reihe}
Sei $(a_j)_{j \ge 0}$ gegeben und $n \in \set{N}_0$. Dann heißt $s_n = \sum_{j=0}^{n} a_j$ \emph{$n$-te Partialsumme} und die Folge $(s_n)_{n \ge 0}$ heißt \emph{Reihe}. Man schreibt statt $(s_n)$ $\sum_{j=0} a_j$ (oder $\sum_{j} a_j$ oder $\sum a_j$). Die Reihe \emph{konvergiert} (bzw. \emph{divergiert}), wenn $(s_n)_{n \ge 0}$ konvergiert (bzw. divergiert). Wenn Konvergenz vorliegt, dann bezeichnet man den Grenzwert von $(s_n)$ mit $\sum_{j=0}^{\infty} a_j$ ("`Reihenwert"').
\end{dfn}

\begin{bsp}\label{bsp:reihen.reihe}
\begin{enumerate}
\item \label{bsp:reihen.reihe.a} Sei $a_j = \frac{1}{j!}$ ($j \in \set{N}_0$)\\
\folgt $s_n = \sum_{j=0}^n \frac{1}{j!} = 1 + 1 + \frac{1}{2} + \frac{1}{6} + \dotsb + \frac{1}{n!} \rightarrow e$ ($\ninf$)
 nach Bsp. \ref{bsp:konv.e} \folgt $\exists \sum_{j=0}^{\infty} \frac{1}{j!} = e$

\item \label{bsp:reihen.reihe.b} Geometrische Reihe:
Sei \kmplx{z} mit $\abs{z}<1$, $a_j = z^j$.\\
\noindent Also: $s_n = \sum_{j=0}^{\infty} z^j$, $(s_n) = (1, 1+z, 1+z+z^2, \dotsc)$\\
  Bsp. \ref{bsp:vor.geomet-summe}: $\sum_{j=0}^{n} z^j = \frac{1-z^{n+1}}{1-z} = \frac{1}{1-z} - \frac{z^{n+1}}{1-z}$, Übung: $z^{n+1} \ra 0$ ($\ninf$)\\
\noindent $\folgt \sum_{j=0}^{\infty} z^j = \frac{1}{1-z}$, $\abs{z}<1$

\emph{Anderer Beweis:} Sei $\abs{z}<1$. Setze $b_n \da \abs{z^{n+1}} = \abs{z}^{n+1}$.\\
\noindent Dann: $0 \le b_{n+1} = \abs{z} b_n \le b_n$.
Thm. \ref{thm:konv.mon-beschr}: $\exists\,b = \lim_{\ninf} b_n \ge 0$\\
\noindent Ferner folgt mit \ninf: $0 \le b = \abs{z} b \folgtwegen{\abs{z}<1} b=0$ $\quad \square$

\item \label{bsp:reihen.reihe.c}

\item \label{bsp:reihen.reihe.d}

\item \label{bsp:reihen.reihe.e}
