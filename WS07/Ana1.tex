...
\begin{bsp} \label{bsp:vor.geomet-summe} (geometrische Summenformel)
...
\begin{bsp}\label{bsp:vor.binsatz} (Binomischer Satz)
...
\textit{Hauptschritt}: Fall $m=1$. Sei $M = \{ x \in \set{R}_+ : x^n \le a \}$. Dann
\begin{enumerate}
...
\begin{proof}
\begin{enumerate}
\item Annahme: $w^n<a$. Sei $\varepsilon \in (0;1]$. Dann $\displaystyle(w+\varepsilon)^n \stackrel{\text{Bsp. \ref{bsp:vor.binsatz}}}{=} \sum^n_{j=0}\binom{n}{j}w^j\varepsilon^{n-j}$
\[= w^n+ \varepsilon \sum^{n-1}_{j=0}\binom{n}{j}\underbrace{w^j}_{\ge 0}\underbrace{\varepsilon^{n-j-1}}_{\le1}
\le w^n + \varepsilon \sum^n_{j=0}\binom{n}{j}w^i \nach{=}{\text{Bsp. \ref{bsp:vor.binsatz}}} w^n + \varepsilon (1+w)^n.\]
...
\begin{bsp}
  ...
  \item Sei $a_n=(-1)^n$.\\
    ...
    \begin{proof} Zu zeigen: $\forall \kmplx{a}\,\exists \ep_a>0
      ...
      \item[3.]Fall: $\kmplx{a}\backslash\{-1,1\}$. Wähle
  ...
  \begin{proof}
    ...
    \item Sei \nat{n}. Dann
      \begin{gather*}
        ...
        \nach{=}{\text{Bsp. \ref{bsp:vor.binsatz}}}
...
\begin{dfn}
  ...
  \[s_n = \sum^n_{j=0}a_j\]
  \textit{$n$-te Partialsumme} und die Folge $(s_n)_{n\geq 0}$ heißt \textit{Reihe}. Man schreibt statt $(s_n)$ $\sum_{j\geq 0}a_j$ (oder $\sum_j a_j$ oder $\sum a_j$).
  
  Die Reihe \textit{konvergiert} (bzw. \textit{divergiert}), wenn $(s_n)_{n\geq 0}$ konvergiert (bzw. divergiert). Wenn Konvergenz vorliegt, dann bezeichnet man den Grenzwert von $(s_n)$ mit $\displaystyle\sum_{j=0}^\infty a_j$ ("`Reihenwert"').
\end{dfn}
...
\begin{bsp}
  ...
  \[\folgt s_n = \sum^n_{j=0}\frac{1}{j!} = 1 + 1+\frac{1}{2}+\frac{1}{6}+\ldots+\frac{1}{n!} \lra \e\ (\ninf)\text{, nach Bsp.~\ref{bsp:konv.e}.}\]
  \[\folgt\exists\sum^\infty_{j=0}\frac{1}{j!} = \e.\]
  
  \item Geometrische Reihe: Sei $\kmplx{z}$ mit $|z|<1$, $a_j=z^j$.
  
  (Also $\displaystyle s_n = \sum^n_{j=0}z^j$, $(s_n) = 1, 1+z, 1+z+z^2\ldots$)
  ...
  \[\folgt s_n = \sum^n_{j=1}\frac{1}{j}=1+\frac{1}{2}+\frac{1}{3}+\ldots+\frac{1}{n},\ \nat{n}.\]
  \textit{Behauptung}: $\displaystyle s_{2^j}\geq 1+\frac{j}{2}$ $\forall\nat{j}$.\begin{itemize}
  ...
  \item Sei $a_j = (-1)^j$, $j\in\set{N}_0$. Für $n\in\set{N}_0$:
  \[s_n = \sum^n_{j=0}(-1)^j = \left\{\begin{array}{l @{\ } l}1-1+1-\ldots+1-1,&n\ \text{ungerade}\\1-1+1-\ldots+1-1+1,&n\ \text{gerade}\end{array}\right. = \left\{\begin{array}{l @{,\ n\ } l}0&\text{ungerade}\\1&\text{gerade}\end{array}\right.\]
