...
Definiere rekursiv eine "`Umordnung"' $(b_k)_{k\ge 1}$ von $a_k = (-1)^{k+1}\frac{1}{k}, k \in \set{N}$.
...
\exists N_\ep \in \set{N}: \forall n\ge N_\ep: \sum^n_{J = N_\ep+1} \abs{a_j} \le \ep\quad \text{nach Satz~\ref{satz:reihen.cauchy-krit}}
\end{equation}
...
= \exp(\underbrace{1+\dotsb+1}_{\text{$m$-mal}}) \nach{=}{a)} \mathrm{e}^m\]
