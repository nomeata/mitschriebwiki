...
Eine Halbnorm $p$ auf $X$ ist eine Abb. $p:X\rightarrow\mathbb{R}_{+}^{0}$ mit
	...
	\end{enumerate}
so heißt $p$ \underline{Norm} und $(X,p)$ \underline{normierter VR} (nVR).
...
\begin{Def}
	...
	\item [] Ein nVR $(X,||\cdot||)$ hei\ss t \underline{Banachraum}, wenn er \underline{vollst"andig} ist, d.h. jede Cauchy-Folge hat einen Grenzwert. Wenn die Norm klar ist, so schreibt man $X$ statt $(X,||\cdot||)$.
