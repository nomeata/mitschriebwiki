...
Sei $\varepsilon >0$ und $t,s \in [0,1]$ gegeben, wähle $n=N_\varepsilon$. Dann $\exists \delta=\delta (n) = \delta (\varespilon)$, so dass: $|f_n(t)-f_n(s)|\leq \varepsilon$ wenn $|t-s|\leq \delta \; (f_n$ glm. stätig) 
=======
$f:\; [0,1] \rightarrow \mathbb{K} $ mit $ \forall \varepsilon >0 \, \exists \,N_\epsilon \in \mathbb{N}: \quad ||f_n-f||_{\infty}\leq \varepsilon \, \forall n \geq N_\varepsilon$ \\
Sei $\varepsilon >0$ und $t,s \in [0,1]$ gegeben, wähle $n=N_\varepsilon$. Dann $\exists \delta=\delta (n) = \delta (\varespilon)$, so dass: $|f_n(t)-f_n(s)|\leq \varepsilon$ wenn |t-s<le \delta \; (f_n$ glm. stätig) 
>>>>>>> .r4956
\end{Bsp}
\end{document}
