...
$\{ P^{X_{n_k}}, k \in \N \}$ ist ebenfalls straft $\stackrel{S.5.8}{\Rightarrow} \exists$ Teilfolge $(X_{n_{k_j}})_{j \in \N}$ und ein W'ma"s $P_0$ mit $P^{X_{n_{k_j}}} \wto P_0$.\\
...
%Satz 5.11
\begin{Sa}[Zentraler Grenzwertsatz von Lindeberg-L\'erg] \index{Zentraler Grenzwertsatz von Lindeberg-L\'erg} \label{Sa5.11}
F"ur jedes $n \in \N$ seien $X_{nk}, k=1,\dots,r_n$ unabh ZV (nicht notwendig identisch verteilt) auf einem W'Raum $(\Sigma_n,\AA_n,P_n)$ mit $\var(X_{nk} = \sigma_{nk}^2 < \infty$ und $EX_{nk} = \mu_{nk} < \infty$. Es sei $s_n^2 := \sum_{k=1}^{r_n} \sigma_{nk}^2 > 0$. Ist dann die Lindeberg-Bedigung
\[
(L) \quad \lim_{n \rightarrow \infty} \frac1{s_n^2} \sum_{k=1}^{r_n} \int_{|x_{nk}-\mu_{nk}| > \eps s_n}\]
...
\item[2.] Der ZGWS hat eine lange \glqq Verbesserungsgeschichte \grqq hinter sich. Gelegentlich ist die Lyapunov-Bedingung einfacher zu verwenden: $\lim_{n \rightarrow \infty} \frac1{s_n^{2+\delta}} \sum_{k=1}^{r_n} E(|X_{nk} - \mu_{nk}|)^{2 + \delta} = 0$ f"ur ein $\delta > 0$.
...
\begin{Bew}
Sei $X_{nk} := Y_k, r_n = n, (\Omega_n,\SchwungA_n,P_n) = (\Omega,\SchwungA,P)$. Es gilt: $s_n^2 = n \sigma^2$ und
\[
\frac1{s_n^2} \sum_{k=1}^{r_n} \int_{|X_{nk}- \mu_{nk} > \eps s_n} (X_{nk} - \mu_{nk})^2 dPn = \frac1{\sigma^2} \int_{|Y_1-\mu_1| > \eps \sqrt{n} \sigma} (Y_1-\mu_1)^2 dP =: I_n
\]
Da $z_n := \ind_{(\eps \sqrt{n} \sigma, \infty} (|Y_1 - \mu_1|) (Y_1-\mu_1)^2 \leq (Y_1 - \mu_1)^2$ und $\lim_{n \rightarrow \infty} z_n = 0$ folgt mit major konv, dass $\lim_{n \rightarrow \infty} I_n = 0$. Also ist die Lindeberg-Bedingung erf"ullt und...Dank"o Evi f"ur fertig Mitschreiben. Der Rest folgt bald
\end{Bew}
