...
$$\mu_G ((a,b]) := G(b)-G(a)$$
für $a,b\in\R, a\le b$ definiert $\mu_G$ ein Maß auf $(\R, \BB(\R))$, das sogenannte \textbf{Lebesgue-Stiltjes-Maß}\index{Mass@Maß!Lebesgue-Stiltjes}\index{Lebesgue-Stiltjes-Mass@Lebesgue-Stiltjes-Maß} zu $G$. (Fortsetzungsproblem analog zu c) )\\
...
\begin{DefON}$\\$
Ist $f$ eine Elementarfuntktion mit Normaldarstellung $f=\sum_{j=1}^n\alpha_j \ind_{A_j},$ so heißt $\int f\d \mu:=\sum_{j=1}^n\alpha_j\mu(A_j)$ das \textbf{$\mu$-Integral}\index{$\mu$-Integral} von $f.$ Schreibweise $\int f\d \mu = \mu(t).$
\end{DefON}
...
F"ur jedes $n \in \N$ seien $X_{nk}, k=1,\dots,r_n$ unabhängige Zufallsvariablen (nicht notwendig identisch verteilt) auf einem Wahrscheinlichkeitsraum $(\Omega_n,\AA_n,P_n)$ mit $\var(X_{nk}) = \sigma_{nk}^2 < \infty$ und $EX_{nk} = \mu_{nk} < \infty$. Es sei $s_n^2 := \sum_{k=1}^{r_n} \sigma_{nk}^2 > 0$. Ist dann die \textbf{Lindeberg-Bedingung}\index{Lindeberg-Bedingung}
\[
...
\item[c)] Ist $\tau$ eine Stoppzeit und $(X_n)_{n\in\N_0}$ ein stochastischer Prozess, so ist $X^{\tau} = (X_n^{\tau})_{n\in\N_0}$ mit $X_n^{\tau} := X_{\tau\,1\,n} \quad\forall n\in\N_0$ der \textbf{gestoppte Prozess}\index{gestoppter Prozess}\index{Prozess!gestoppter}. \\
Da $\tau\,1\,n$ eine Stoppzeit ist, ist wegen Lemma \ref{Lem8.1}c) $X_{\tau\,1\,n}$ $\FF_{\tau\,1\,n}$-messbar und $(X_n^{\tau})$ ist $(\FF_{\tau\,1\,n})$-adaptiert.
\end{enumerate}
\end{BemON}
...
$\folgt (c_n)_{n\in\N_0}$ ist vorhersagbar. Da $X_0 + \sum_{k=1}^n c_k(X_k - X_{k-1}) = X_{\tau\,1\,n}$, folgt die Behauptung mit Satz \ref{Sa8.4}.
\end{Bew}

\begin{BemON} $\\$
Ist $X$ ein Martingal, so auch $X^{\tau}$ und damit gilt $EX_{\tau\,1\,n} = EX_0$. \\
Betrachte Bsp \ref{Bsp8.3} mit $\tau := \inf\{k\in\N_0\ |\ X_n \geq X_0 + c\}$ und $c_n := \ind_{\{\tau \geq n\}}$: \\
Solange $c$ nicht erreicht ist, wird eine Geldeinheit gesetzt, danach aufgehört. Spielt man maximal n-mal, so ist $X_{\tau\,1\,n}$ das Kapital am Ende. Im Mittel kann man das Kapital bei einem fairen Spiel nicht erhöhen.
\end{BemON}
...
\item $(R_,S_n)$ die Anzahl der roten / schwarzen Karten im Stapel, nachdem $n$ Karten aufgedeckt wurden.
...
nach Voraussetzung ist $v < \infty$. Ist $(X_n)_{n=1,\dots,N}$ ein $(\FF_n)_{n=1,\dots,N}$ ein Supermartingal, so folgt mit Satz \ref{Sa8.6}: $EX_1 \geq EX_{\tau} \quad\forall$ Stoppzeiten $\tau \leq N$. \\
...
$$P(R_1=r1,\dots,R_N=r_N) = \frac{1}{N!} \quad\forall r1,\dots,r_N,\ 1 \leq r_i \leq i,\ 1 \leq i \leq N$$
