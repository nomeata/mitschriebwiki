...
\end{cases}$$
(Hier fehlt ein Bild)\\
$\alpha_n$ ist $\BB$-messbar. $\alpha_n\uparrow$ und $\lim_{n\to\infty}\alpha_n(x)=x$ f¸r $n\to\infty.$ Sei $u_n:=\alpha_n\cdot f.$ Dann gilt $u_n\in\EE$ und $u_n\uparrow f$.
\end{Bew}
...
Sei $A_n := \{\omega \in \Omega | f(\omega) \ge \frac{1}{n}\} \folgt A_n \uparrow N$ und $\mu(N) = \lim_{n\to\infty}(\mu(A_n))$. Auﬂerdem gilt $0 = \int f\d\mu \ge \int\frac{1}{n}\cdot 1_{A_n} \d\mu = \frac{1}{n}\cdot\mu(A_n) \ge 0$ \\
$\folgt \mu(A_n) = 0$ $\forall n\in\N \folgt \mu(N) = 0$, also $f=0$ $\mu$-f.¸.
\item[(ii)] Annahme: $N$ ist $\mu$-Nullmenge. \\
Sei $g \in \EE$, $g(\Omega)=\{\alpha_1$, $\dots$, $\alpha_n\}$, $g \le f.$ \\
$\folgt g=\sum_{j=1}^n \alpha_j \circ 1_{A_j}$. \\
Falls $\alpha_j > 0 \folgt A_j \subset N \folgt \int g\d\mu=0 \folgtnach{L.1.3} \int f\d\mu=0$.
\end{enumerate}
\item[b)] Seien zun‰chst $f,g \ge 0$, $N:=\{f\neq g\} \folgtnach{a)}$ \\
\begin{eqnarray*}
...
Seien nun $f,g$ beliebig. Wegen $\{f^+ = g^+\}\supset\{f=g\}\subset\{f^- = g^-\}$ gilt auch $f^+ = g^+$ und $f^- = g^-$ $\mu$-f.¸. und mit dem vorigen Teil folgt die Behauptung.
\end{enumerate}
\end{Bew}

\begin{Bem}
Im Folgenden sei $L^1(\Omega, \AA, \mu):=\{f:\Omega\to\R$ $|$ $f$ ist messbar und $\mu$-integrierbar$\}$ (ist ein Vektorraum) und wir definieren \\
$f \sim_{\mu} g :\equizu f=g$ $\mu$-f.¸. und $\sim_{\mu}$ ist ƒquivalenzrelation auf $\{f:\Omega\to\R$ $|$ $f$ ist messbar$\}$. Sei $f^{[\mu]}$ die ƒquivalenzklasse zu $f$. \\
Mit Satz 2.5: Entweder alle oder keines der Elemente in $f^{[\mu]}$ ist $\mu$-integrierbar und die Integrale sind ggfs. gleich. Auﬂerdem gilt: \\
$f_1 \in f^{[\mu]}$, $g_1 \in g^{[\mu]} \folgt f_1 + g_1 \in (f+g)^{[\mu]}$. \\
$\folgt$ Man kann zum Raum der ƒquivalenzklassen ¸bergehen: $L^1(\Omega,\AA,\mu)/\sim_{\mu}$ \\
...
\begin{Def}
Es seien $\mu, \nu$ Maﬂe auf dem messbaren Raum $(\Omega,\AA)$. Gilt dann $\mu(A)=0 \folgt \nu(A) = 0$ $\forall$ $A\in\AA$, so heiﬂt $\nu$ \textbf{$\mu$-stetig}\index{$\mu$-stetig}, in Zeichen $\nu \ll \mu$. Man sagt auch, dass $\mu$ das Maﬂ $\nu$ dominiert.
\end{Def}

\begin{Sa}und Definition \\
Sei $(\Omega,\AA,\mu)$ ein Maﬂraum und $f:\Omega\to\R_+$ $(\AA, \BB)$-messbar. Dann wird durch $\nu:\AA\to\R_+\cup\{\infty\}$, $\nu(A):=\int_A f\d\mu$ ein Maﬂ auf $(\Omega,\AA)$ definiert. Man nennt $\nu$ das \textbf{Maﬂ mit der Dichte $f$}\index{Mass mit Dichte@Maﬂ mit Dichte} bzgl. $\mu$ und f eine \textbf{$\mu$-Dichte}\index{$\mu$-Dichte} von $\nu$. Schreibweise: $f=\frac{\d\nu}{\d\mu}$
\end{Sa}

\begin{Bew}
Wir weisen nach, dass $\mu$ ein Maﬂ ist: \\
$\nu \ge 0$ ist klar, da $f$ nach $\R_+$ abbildet; \\
\begin{enumerate}
\item[(i)] $\mu(\emptyset) = \int f\cdot 1_{\emptyset}\d\mu = 0$.
\item[(ii)] Seien $A_1,A_2,\dots$ paarweise disjunkt und $A=\sum_{n=1}^{\infty}A_n$. \\
Wegen $f\cdot 1_{\sum_{k=1}^{n}A_k} \uparrow f\cdot 1_A$ folgt mit Satz 2.1: \\
\begin{eqnarray*}
\nu(\sum_{n=1}^{\infty}A_n) &=& \int f\cdot 1_A\d\mu \\
 &=& \lim_{n\to\infty}(\int f\cdot \underbrace{1_{\sum_{k=1}^{n}A_k}}_{=\sum_{k=1}^n 1_{A_k}}\d\mu) \\
 &=& \lim_{n\to\infty}(\int \sum_{k=1}^n f\cdot 1_{A_k} \d\mu) \\
 &=& \lim_{n\to\infty}(\sum_{k=1}^{n}(\underbrace{\int f\cdot 1_{A_k}\d\mu}_{=\nu(A_k)})) \\
 &=& \sum_{k=1}^{\infty}\nu(A_k)
\end{eqnarray*}
\end{enumerate}
\end{Bew}

\begin{Sa}(Satz von Radon-Nikodym)\index{Satz!von Radon-Nikodym} \\
Seien $\mu,\nu$ Maﬂe auf dem messbaren Raum $(\Omega,\AA)$, $\mu$ sei $\sigma$-endlich. Dann gilt: \\
$\nu$ ist genau dann $\mu$-stetig, wenn $\nu$ eine Dichte bzgl. $\mu$ hat.
\end{Sa}

\begin{Bew}
$\nu$ hat Dichte bzgl. $\mu \folgt \nu(A)=\int_A f\d\mu=\int f\cdot 1_A\d\mu \folgtnach{S.2.5a)}\nu\ll\mu$. \\
Die andere Richtung siehe z.B. Henze, Stochastik II.
\end{Bew}

\begin{Sa}
Seien $\mu$ und $\nu$ Maﬂe auf $(\Omega,\AA)$, $\nu$ habe $\mu$-Dichte $f$. Dann gilt f¸r alle $(\AA,\BB)$-messbaren Abbildungen $g:\Omega\to\R$: \\
...
\begin{Bsp}
Sei $\mu=\lambda$ das Lebesgue-Maﬂ und $\nu=P^X$ die Verteilung einer Zufallsvariablen $X$. Ist $X$ absolutstetig, so gilt (Stochastik I): \\
\begin{displaymath}P^X (B)=\int_B f_X (x)\d x\end{displaymath} mit $f_X:\R\to\R_+\cup\{\infty\}$ und \\
\begin{displaymath}EX=\int_{\Omega}X\d P =\int_{\R}xP^X(\{x\})\d x =\int_{\R}x\cdot f_X (x)\d x.\end{displaymath} mit den S‰tzen 2.4 und 2.9.
\end{Bsp}

\section{Ungleichungen und R‰ume integrierbarer Funktionen}
Hier stellen wir einige Hilfsmittel f¸r sp‰ter zusammen. Der folgende Satz behandelt den Spezialfall von Wahrscheinlichkeitsmaﬂen.
...
\end{eqnarray*}
F¸r Teil 2 setze $\tilde X:=X-EX$ und $\gamma=2.$
\end{Bew}
...
Auﬂerdem gilt $\forall y\in I, \exists m\in\R$, mit
$$\Phi(x)\ge\Phi(y)+m(x-y)$$

\begin{Sa} (Jensensche Ungleichung)\index{Jensensche Ungleichung}\index{Ungleichung!Jensensche} \\
...
Mit der Vor¸berlegung folgt $(y=EX, x=X(\omega))$
$$\Phi(X)\ge\Phi(EX)+m(X-EX)\quad\text{P-f.s.}$$
f¸r ein $m\in\R.$ Erwartungswert auf beiden Seiten f¸hrt zur Behauptung (Nullmengen kˆnnen wir vernachl‰ssigen).
\end{Bew}

\begin{Bsp}$\\$
F¸r $\Phi(x)=|x|, \Phi(x) = x^2$ folgt: $|EX|\le E|X|, (EX)^2\le EX^2.$ ($\folgt EX^2-(EX)^2=\var X\ge 0$)\\
\end{Bsp}

Im Folgenden sei $(\Omega, \AA, \mu)$ wieder ein Maﬂraum.

\begin{Def}$\\$
Eine messbare Funktion $f:\Omega\to\R$ heiﬂt \textbf{$p$-fach $\mu$-integrierbar}\index{$p$-fach $\mu$-integrierbar}\index{$\mu$-integrierbar!$p$-fach}, wenn $\int |f|^p\d\mu< \infty$ mit $p>0.$ 
$$L^p(\Omega, \AA, \mu):=\{ f:\Omega\to\R|\int|f|^p\d\mu <\infty\}$$
$$||f||_p=\left(\int|f|^p\d\mu\right)^{\frac{1}{p}}$$
Wie im vorigen Abschnitt ist $L^p$ bzw. $L^p(\Omega,\AA,\mu)/\sim_\mu$ ein Vektorraum ¸ber $\R$ und $||f||_p$ auf den ƒquivalenzklassen eine Norm.
\end{Def}

\begin{Sa} $\\$
\begin{enumerate}
\item[a)] (Hˆldersche Ungleichung)\index{Holdersche Ungleichung@Hˆldersche Ungleichung}\index{Ungleichung!Holdersche@Hˆldersche} Es seien $p>1, f\in L^p(\Omega,\AA,\mu), g\in L^q(\Omega,\AA,\mu),$ wobei $\frac{1}{p}+\frac{1}{q}=1.$ Dann folgt: $f\cdot g\in L^1(\Omega,\AA,\mu)$ und es gilt:
...
\folgtnach{Int. ¸ber $\omega$} & ||f\cdot g||_1 & \le \frac 1 p ||f||_p^p||f||_p^{1-p}||g||_q + \frac 1 q ||g||_q^q||g||_q^{1-q}||f||_p\\
 & &= \frac 1 p ||f||_p||g||_q + \frac 1 q ||g||_q ||f||_p\\
\folgt & \text{Behauptung}
\end{eqnarray*}
\item[b)] Wegen $|f+g|\le|f|+|g|$ gilt $||f+g||_p\le|| |f| + |g| ||_p.$ Also gen¸gt es die Ungleichung f¸r $f+g\ge 0$ zu beweisen. Falls $p=1$ folgt $||f+g||_1=\int (f+g)\d\mu=\int f\d\mu + \int g\d\mu = ||f||_1 + ||g||_1.$ Sei also $p>1.$ Mit $(f+g)^p\le(2\cdot\max\{f,g\})^p\le 2^p(f^p + g^p)\folgt (f+g)\in L^p,$ also $||f+g||_p<\infty.$ Sei $q:=\frac{1}{1-\frac 1 p}.$ Anwendung von Teil a) liefert:
\begin{eqnarray*}
...
Lineare Algebra: $|\langle a, b\rangle|\le ||a||\cdot||b||\quad\forall a, b \in\R^n$
\end{Bem}
