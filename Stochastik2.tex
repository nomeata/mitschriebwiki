...
Auch ist Frau Bäuerle nicht verantwortlich für den Inhalt dieses Skriptes.
}
\date{Stand: \today}
...
\begin{Def}$\\$
Ist $f$ eine Elementarfuntktion mit Normaldarstellung $f=\sum_{j=1}^n\alpha_j 1_{A_j},$ so heißt $\int f\mbox{d}\mu:=\sum_{j=1}^n\alpha_j\mu(A_j)$ das \textbf{$\mu$-Integral}\index{$\mu$-Integral} von $f.$ Schreibweise $\int f\mbox{d}\mu = \mu(t).$
\end{Def}
...
\begin{enumerate}
\item[a)] $\int 1_A\mbox{d}\mu=\mu(A)$ für $A\in\AA$
...
\begin{Bem} Das Beweisverfahren (zuerst für $f\in\EE$ (bzw. $f=1_A$), dann für $f\in\EE^+,$ dann für $f$ beliebig) heißt \textbf{algebraische Indunktion}\index{algebraische Indunktion} und wird häufig verwendet.
\end{Bem}
...
Sei $A_n := \{\omega \in \Omega | f(\omega) \ge \frac{1}{n}\} \folgt A_n \uparrow N$ und $\mu(N) = \lim_{n\to\infty}(\mu(A_n))$. Außerdem gilt $0 = \int f\d\mu \ge \int\frac{1}{n}\cdot 1_{A_n} \d\mu = \frac{1}{n}\cdot\mu(A_n) \ge 0$ \\
...
\begin{BemON} Sei $J\subset I$, $\Pi_i:\Omega\to\times_{i\in I}\Omega_i$, $\Pi_i(\omega)(j)=\omega_j$ ($j\in J)$ die Projektion auf die $J$-Koordinaten, so bildet \\
\begin{displaymath}
...
Wegen $f_{A_1\times A_2,n}(\omega_1) = \mu_2(A_2\cap B_n)\cdot 1_{A_1}(\omega_1)$ ist $f_{A_1\times A_2,n}$ für $A_1\in\AA_1$, $A_2\in\AA_2$ messbar und daher $A_1\times A_2\in\mathcal{D}$. \\
...
$\Pi(A_1\times A_2)=\int \mu_2(A_2)\cdot 1_{A_1}\d\mu_1=\mu_2(A_2)\cdot\mu_1(A_1)$. \\
...
\begin{enumerate}
\item[(1)] Falls $f=\sum_{i=1}^n \alpha_i 1_{A_i}$ erhält man mit $(1_A)_{\omega_2}(\omega_1) = 1_{A_{\omega_2}}(\omega_1)$ die Beziehung
\begin{displaymath}
\int f_{\omega_2}\d\mu_1 \stackrel{lin.}{=} \sum_{i=1}^n\alpha_i \int 1_{\left(A_i\right)_{\omega_2}}\d\mu_1 = \sum_{i=1}^n\alpha_i \mu_1\left(\left(A_i\right)_{\omega_2}\right) $$\\$$
...
Da die Rechteckmengen ein durchschnittstabiler Erzeuger von $\AA$ sind, folgt wegen der Eindeutigkiet von $\mu$: \\
