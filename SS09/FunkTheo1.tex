...
Ferner:
\[0\leq d(w) \da \biggl\lvert\frac{f(w)-f(z)}{w-z} - g(z)\biggr\rvert\ = \biggl\lvert\sum_{n=1}^\infty a_n\underbrace{(\frac{w^n-z^n}{w-z} - z^{n-1})}_{\ad p_n(w)}\biggr\rvert\]
...
\item $\displaystyle n(\Gamma,z) \wegen{=}{\ref{satz2.16}c)} n(\Gamma,c) = \frac{1}{2\pi\i}\integral{0}{2\pi m}{\frac{\i r\e^{\i t}}{\underbrace{c+r\e^{\i t}}_{=\gamma(t)}-c}}{t} = m$, $\forall z\in B(c,r)$.
\end{enumerate}
\end{bsp}
...
\[I_\ep = \cup^n_{j=1}(\tau_j-\ep,\tau_j+\ep),\quad\Gamma_\ep = \gamma([a,b]\setminus I_\ep),\]
...
%Farbe?
\[\Im{n(\Gamma,0)} \ra\begin{cases}\pm\pi-(\pm\pi) = 0&\text{Berührpunkt,}\\+\pi-(-\pi) = 2\pi&\text{$\arg$ wächst,}\\-\pi-\pi=-2\pi&\text{$\arg$ fällt.}\end{cases}\qquad\text{für }\ep\ra 0.\]
