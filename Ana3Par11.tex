...
% Wer nennenswerte ƒnderungen macht, schreibt sich bei \author dazu

\begin{document}
\maketitle

In diesem Paragraphen sei $X$ stets ein Vektorraum (VR) ¸ber $\MdK$, wobei $\MdK=\MdR$ oder $\MdK = \MdC$.

\begin{definition}
Eine Abbildung $\|\cdot\|:X\to\MdR$ heiﬂt eine \begriff{Norm auf $X$} $:\equizu$
\begin{liste}
...
\end{liste}
\end{definition}
In diesem Fall heiﬂt $(X,\|\cdot\|)$ ein \begriff{normierter Raum} (NR). Meist schreibt man nur $X$ statt $(X,\|\cdot\|)$.

\begin{beispiele}
\item $X = \MdK^n$, f¸r $x=(x_1,\ldots,x_n)$: $\|x\| = \left( \sum_{j=1}^n |x_j|^2 \right)^\frac12$. Analysis II $\folgt$ $(X,\|\cdot\|)$ ist ein normierter Raum.
\item $A\subseteq \MdR^n$ sei beschr‰nkt und abgeschlossen. $X=C(A,\MdR^n)$; \\
$\|f\|_\infty = \max\{\|f(x)\|, x\in A\}$ $(f\in X)$. Dann ist $(X,\|\cdot\|_\infty)$ ein normierter Raum.
\item $X=L(\MdR^n)$. F¸r $f\in L(\MdR)$: $\|f\|_1 := \int_{\MdR^n} |f(x)|dx$; $\|f\|_2 := \left(\int_{\MdR^n} |f(x)|^2 dx\right)^\frac12 $;
Analysis II 16.1 $\folgt$ $\|\cdot\|_1$ hat die Eigenschaft (ii) und (iii) einer Norm, $\|f\|_1\ge 0$ aber $\|f\|_1=0 \equizu f=0$ fast ¸berall auf $\MdR^n$.\\
Es ist ¸blich, zwei Funktionen $f,g \in L(\MdR^n)$ als gleich zu betrachten, wenn $f=g$ fast ¸berall. In diesem Sinne: $(L(\MdR),\|\cdot\|_1)$ ist ein normierter Raum.
\end{beispiele}

F¸r den Rest des Paragraphen sei $(X,\|\cdot\|)$ stets ein normierter Raum. Wie in Analysis II zeigt man:
$$ \left| \|x\| - \|y\| \right| \le \|x-y\| \ \forall x,y\in X\, $$
$\|x-y\|$ heiﬂt Abstand von $x$ und $y$.

\begin{definition}
Sei $(x_n)$ eine Folge in X
\begin{liste}
\item $(x_n)$ heiﬂt konvergent $:\equizu$ $\exists x\in X: \|x_n - x\| = 0\ (n\to\infty)$\\
In diesem Fall ist $x$ eindeutig bestimmt (Beweis wie in $\MdR^n$) und heiﬂt der Grenzwert (GW) oder Limes von $(x_n)$. Man schreibt:
...
\item $\sum_{n=1}^\infty x_n$ bedeutet die Folge $(s_n)$ wobei $s_n := x_1+\cdots+x_n \ (n\in\MdN)$\\
 $\sum_{n=1}^\infty x_n$  heiﬂt konvergent $:\equizu$ $(s_n)$ ist konvergent.\\
 $\sum_{n=1}^\infty x_n$  heiﬂt divergent $:\equizu$ $(s_n)$ ist divergent.\\
 Im Konvergenzfall: $\sum_{n=1}^\infty x_n := \lim_{n\to\infty} s_n$
\end{liste}
\end{definition}
Wie ¸blich zeigt man: Aus $x_n\to x$ und $y_n\to y$ folgt:
\[x_n+y_n = x + y\]
...
\begin{liste}
\item $A$ heiﬂt \begriff{konvex} $:\equizu$ aus $x,y\in A$ und $t\in[0,1]$ folgt stets: $x+t(y-x) \in A$
\item $A$ heiﬂt \begriff{beschr‰nkt} $:\equizu$ $\exists c\ge0: \|x\|\le c \ \forall x\in A$
\item $A$ heiﬂt \begriff{abgeschlossen} $:\equizu$ der Grenzwert jeder konvergenten Folge aus $A$ gehˆrt zu $A$
\item $A$ heiﬂt \begriff{kompakt} $:\equizu$ jede Folge in $A$ enth‰lt eine konvergente Teilfolge, deren Grenzwert zu $A$ gehˆrt.
\item $(x_n)$ heiﬂt eine Cauchyfolge (CF) in $X$ $:\equizu$\\ $\forall \ep>0 \ \exists n_0\in\MdR: \|x_n-x_m\|<\ep \ \forall n,m\ge n_0$
\end{liste}
\end{definition}
...
\item Ist $A\subseteq \MdR^n$: $A$ ist kompakt $:\equizu$ $A$ ist beschr‰nkt und abgeschlossen (Analysis II, 2.2)
\item $A$ kompakt $\folgt$ $A$ abgeschlossen
\item $X=C[a,b]$ mit $\|\cdot\|_\infty$. Sei $(f_n)$ eine Folge in $X$ und $f\in X$. Dann $(f_n) \to f$ bez¸glich $\|\cdot\|_\infty$ $\equizu$ $(f_n)$ konvergiert auf $[a,b]$ gleichm‰ﬂig gegen $f$ (Analysis  I, ‹bungsblatt 10, Aufgabe 37)
\end{liste}
\end{bemerkung}
...
1, & \frac1n \le x \le 1
\end{cases}\]
In der ‹bung: $(f_n)$ ist eine Cauchyfolge in $X$, aber es existiert kein $f\in X: f_n\to f$ (bez¸glich $\|\cdot\|_2$)
\end{beispiel}

\begin{definition}
Ein normierter Raum $X$ heiﬂt \begriff{vollst‰ndig} oder ein \begriff{Banachraum} (BR) $:\equizu$ jede Cauchyfolge in $X$ ist konvergent.
\end{definition}

\begin{beispiele}
\item Sei $X$ und $\|\cdot\|_2$ wie im obigen Beispiel. Dann ist $X$ kein Banachraum.
\item $\MdR^n$ ist mit der ¸blichen Norm ein Banachraum (Siehe Analysis II)
\item $C[a,b]$ ist mit $\|\cdot\|_\infty$ ein Banachraum (Analysis I, ‹bungsblatt 10, Aufgabe 37)
...
\begin{itemize}
	\item[(1)] $U_{\epsilon}(x_0) := \{ x \in X: \Vert x - x_0 \Vert < \epsilon \}$ heiﬂt 		
	$\epsilon$ - Umgebung von $U$
	\item[(2)] $D \subseteq X$ heiﬂt offen $: \Leftrightarrow\ \forall x \in D \ \exists \epsilon
...
\begin{definition}[Operator]
 $X$ sei ein normierter Raum, $A \subseteq X$ und $T: A \to X$ eine Abbildung. $T$ heiﬂt auch ein \begriff{Operator} auf $A$, man schreibt meist $T_x$ statt $T(x)$ $(x \in A)$.
\begin{itemize}
	\item[(1)] $x^*$ heiﬂt ein \begriff{Fixpunkt} von $T$ $: \Leftrightarrow T_{x^*} = x^*$.
	\item[(2)] $T$ heiﬂt in $x_0 \in A$ stetig $: \Leftrightarrow $ f¸r jede Folge $(x_n)$ in 
	$A$. mit $x_n \to x_0: T_{x_n} \to T_{x_0}$. \\
	(‹bung: $\Leftrightarrow\ \forall \epsilon > 0 \ \exists \delta > 0: \Vert T_x - T_0 \Vert <
	\epsilon\ \forall x \in U_{\delta}(x_0) \cap A$)
	\item[(3)] $T$ heiﬂt stetig auf $A$ $: \Leftrightarrow T$ ist stetig in jedem $x \in A$.
	\item[(4)] $T$ heiﬂt auf $A$ \begriff{kontrahierend} $: \Leftrightarrow\ \exists \ L \in
...
\end{itemize}
$(x_n)$ heiﬂt \begriff{Folge der sukzessiven Approximation}.
\end{satz}
...
Sei $z \in A$ und $T_z = z$. $\| x^* -z \| = \|T_{x^*} - T_z \| \le L\|x^* - z \|$; w‰re  $\|x^* - z \| \ne 0 \folgt L \ge 1$, Wid., also $x^*=z$.
\end{beweis}
Ohne Beweis:
...
Dann enth‰lt $(y_n)$ eine auf $I$ gleichm‰ﬂig konvergente Teilfolge.
\end{satz}

\begin{beweis}
$\mathcal{F} := \{y_n : n \in \MdN \}$. $\mathcal{F}$ ist auf $I$ gleichstetig. $\forall n \in \MdN\ \forall x \in I: |y_n(x)| = |y_n(x)-y_0+y_0| \le | y_n(x)-y_0| + |y_0| = |y_n(x)-y_n(x_0)| + |y_0| \le M |x-x_0| + |y_0| \le M(b-a)\cdot |y_0| \folgt \mathcal{F}$ ist gleichm‰ﬂig beschr‰nkt. $ß1 \folgt$ Behauptung.
\end{beweis}
...
\item ‹bung: $A$ ist konvex.
\item Sei $(y_n)$ ein Folge in $A$. $11.4 \folgt (y_n)$ enth‰lt eine auf $I$ gleichm‰ﬂig konvergente Teilfolge $(y_{n_k})$, $y(x):=\lim_{n \to \infty} y_{n_k}(x)\ (x \in I)\ \folgtnach{A I} y \in C(I)$ \\
