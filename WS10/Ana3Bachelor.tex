...
Setze f\"ur $\hat f \in L^1(X)$: $\int_X \hat f(x) \text{ d}x := \int_X f(x) \text{ d}x$, dabei ist diese Definition unabh\"angig von der Wahl des Repr\"asentanten $f \in \fl^1(X)$ von $\hat f$, denn: ist auch noch $g \in \fl^1(X)$ und $\hat g = \hat f$, so ist $f - g \in \cn$, also $f-g = 0$ f.\"u. und damit: $\int_X f \text{ d}x = \int_X g \text{ d}x$.

F\"ur $\hat f \in L^p(X)$ definiere $\| \hat f \|_p := \| f \|_p$, wobei dieses Definition unabh\"angig ist von der Wahl des Repr\"asentanten $f \in \fl^p(X)$ von $\hat f$.

F\"ur $\hat f, \hat g \in L^2(X)$ setze $( \hat f | \hat g ) := \int_X f(x)g(x) \text{ d}x$ (auch diese Definition ist Repr\"asentanten-unabh\"angig) 

(Beachte: $f\cdot g \in \fl^(X)$ )

\textbf{Dann gilt:} \begin{enumerate} \item $L^p(X)$ ist unter $\| \cdot \|_p$ ein normierter Raum (NR).
\item F\"ur $\hat f, \hat g \in L^2(X)$ gilt:
\[ | ( \hat f | \hat g ) | = | \int_X f(x)g(x) \text{ d}x | \leq \int_X |fg| \text{ d}x = \| fg \|_1 \overset{16.?}{\leq} \| f \|_2 \| g \|_2 = \| \hat f \|_2 \| \hat g \|_2 \]
...
\begin{definition}
Sei $(B, \| \cdot \|)$ ein normierter Raum. Gilt mit einem Skalarprodukt $( \cdot | \cdot )$ auf $B$: $ \| v \| = \sqrt{(v | v)} \quad \forall v \in B$, so hei\"st $B$ ein Pr\"ahilbertraum. Ist $B$ ein Banachraum mit (*), so hei\"st $B$ ein Hilbertraum.

\textit{(leider wei\"s ich nicht, was hier (*) sein soll)}
\end{definition} 
