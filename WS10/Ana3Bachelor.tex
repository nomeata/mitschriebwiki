...
%Gibts hier vielleicht noch ne hübschere Lösung? Glaube nicht, dass dieser Fall in Ana schonmal eingetreten ist.
\begin{satz}[und Definition \text{(ohne Beweis)}] 
\label{Satz 8.5}
...
%Ich weiß nicht wie der Teil jetzt in der Vorlesung genannt wurde, aber hier heißt er einfach mal Bemerkung
\begin{bemerkung}
Für \(C\in\fb_d\) gilt:
\begin{align*}
...
\label{Satz 9.1}
Sei \(C\in\fb_d\). Dann:
\[ \lambda_d(C)=\int_{\mdr^k}\lambda_l(C^x)\,dx=\int_{\mdr^l}\lambda(C_y)\,dy \]
\end{satz}
Das heißt:
\[ \int_{\mdr^d}\mathds{1}_{C}(x,y) = \int_{\mdr^k}\left(\int_\mdr^l \mathds{1}_{C}(x,y)\,dy\right)\,dx = \int_{\mdr^l} \left(\int_{\mdr^k} \mathds{1}_{C}(x,y)\,dx\right)\,dy \]
...
\begin{enumerate}
	...
	\item 	Sei \(\varnothing\neq X\subseteq\mdr^d\). $X$ sei kompakt, also \(X\in\fb_d\). Weiter sei \(f\colon X\to[0,\infty)\) stetig, woraus mit \ref{Satz 4.11} \(f\in\mathfrak{L}^1(X)\) folgt.
		...
		\[\lambda_{d+1}(C) \overset{\ref{Satz 9.1}}= \int_{\mdr^d}\lambda_1(C^x)\,dx = \int_X\lambda_1(C^x)\,dx + \int_{R^d\setminus X}\lambda_1(C^x) = \int_Xf(x)\,dx \]
...
\textbf{§ 9: Das Prinzip von Cavalieri}: \\
