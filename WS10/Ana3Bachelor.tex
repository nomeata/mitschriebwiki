...
&\overset{\text{§\ref{Kapitel 10}}}= \int_{\rho}^{R} \left( \int_0^{2\pi} r \text{ d}r \right) \text{ d}\varphi\\
...
F\"ur \(j=1,2,3\):
\[
h_{j}(x,y):=\left(\underbrace{F_{j}(\phi(x,y))\frac{\partial\phi_{j}}{\partial y}(x,y)}_{=:u_{j}(x,y)},\underbrace{-F_{j}(\phi(x,y))\frac{\partial\phi_{j}}{\partial x}(x,y)}_{=:v_{j}(x,y)}\right)\quad((x,y)\in D)
\]
...
\begin{beispiel}
\begin{enumerate}
\item $f$ sei wie im Beispiel (2) vor \ref{Satz 18.7}. Wenn man sich ganz arg anstrengt, kommen dann lustige Ergebnisse raus wie z.B. $ \sum_{j=0}^\infty \frac{1}{(2j+1)^2} = \frac{\pi^2}{8}$.
