...
Genauso: \(\int_{X}{\lvert f\rvert\lvert f+g\rvert^{p-1}\mathrm{d}x}\leq\lVert g\rVert_{p}\lVert f+g\rVert_{p}^{p+1}\)
...
\begin{beweis}
\begin{enumerate}
\item Sei $f\in C_c(C,\mdr)$ und $K:=\supp(f)$, dann ist $K\subseteq X$ kompakt, also $K\in\fb_d$. Es gilt für alle $x\in X\setminus K$ $f(x)=0$ und damit folgt aus \ref{Folgerung 4.12} $\int_K |f|^p\text{ d}x<\infty$. Dann gilt:
...
\begin{enumerate}
	...
	\end{enumerate}
	\item 	Die Sätze \ref{Satz 16.1} bis \ref{Satz 16.3} und \ref{Satz 16.6} gelten in 
			\(L^p(X,\mdc)\).
...
\intertext{Damit ist}
	...
	\end{cases}
\end{align*}
Insbesondere ist \(\lvert\lvert b_k\rvert\rvert_2=1\). Das heißt \(\{b_k\mid k\in\MdZ\}\) ist ein
...
\begin{enumerate}
	\item 	Für \(n\in\mdn_0\) setze 
			\[s_n:=\sum^n_{k=-n}\alpha_k = \sum_{\lvert k\rvert\leq n}
			=\alpha_{-n}+\alpha_{-(n-1)}+\ldots+\alpha_0+\alpha_1+\ldots+\alpha_n\]
	...
	\item 	Für \(n\in\mdn_0\) setze 
			...
			\(\lvert\lvert f-\sigma_n\rvert\rvert_2\overset{n\to\infty}\longrightarrow 0\), so schreiben
			wir \[f\overset{\lvert\lvert\cdot\rvert\rvert_2}=\sum_{k\in\MdZ}f_k \ \ \ 
...
Sei \(\{b_k\mid k\in\MdZ\}\) wie in \ref{Beispiel17.2}. \(\{b_k\mid k\in\MdZ\}\) heißt eine
...
\[(\ast)\ \ \ \ \ \ \ \ \ f\overset{\lvert\lvert\cdot\rvert\rvert_2}=\sum_{k\in\MdZ}c_kb_k \]
\textbf{Frage:} Ist \(\{b_k\mid k\in\MdZ\}\) eine ONB von \(L^2([0,2\pi],\mdc)\)?\\
\textbf{Antwort:} Ja! In \ref{Satz 18.4} werden wir sehen, dass \((\ast)\) gilt mit 
\(c_k=(f\mid b_k)\).
\end{definition}
...
\(f\overset{\lvert\lvert\cdot\rvert\rvert_2}=\sum_{k\in\MdZ}c_kb_k \), so gilt:
...
\(\lvert\lvert \sigma_n-f\rvert\rvert_2\to 0\) für \(n\to\infty\). Sei \(j\in\MdZ\) und \(n\in\mdn\) mit
...
\item	\(S_nf:=\sum_{\lvert k\rvert\leq n}(f\mid b_k)b_k\) heißt
	...
	\[f\overset{\lvert\lvert\cdot\rvert\rvert_2}
	=\sum_{k\in\MdZ}(f\mid b_k)b_k\gdw\lvert\lvert f-S_nf\rvert\rvert_2
	\to0\]
...
\item	Gilt \(f_\mu\perp f_\nu\) für \(\mu\neq\nu\) (\(\mu,\nu=1,\ldots,n\)),
	...
	\[\lvert\lvert f_1+\ldots+f_n\rvert\rvert^2_2=
	\lvert\lvert f_1\rvert\rvert^2_2+\ldots+
	\lvert\lvert f_n\rvert\rvert^2_2\]
...
\item 	Die \textbf{Besselsche Ungleichung} lautet:
	\[\lvert\lvert S_nf\rvert\rvert^2_2
	=\sum_{\lvert k\rvert\leq n}\lvert(f\mid b_k)\rvert^2
	=\lvert\lvert f\rvert\rvert_2^2-\lvert\lvert(f-S_nf)\rvert\rvert^2_2
	\leq\lvert\lvert f\rvert\rvert^2_2\]
\item	Für alle \(v\in E_n\) gilt: 
	\[\lvert\lvert f-S_nf\rvert\rvert_2\leq\lvert\lvert f-v\rvert\rvert_2 
	\]
\end{enumerate}
\end{satz}
...
\item	Es genügt den Fall \(n=2\) zu betrachten, der Rest folgt induktiv.
	\begin{align*}
	\lvert\lvert f_1+f_2\rvert\rvert_2^2
	&= (f_1+f_2\mid f_1+f_2)						\\
	&= (f_1\mid f_1)+(f_1\mid f_2)+(f_2\mid f_1)+(f_2\mid f_2)		\\
	&= (f_1\mid f_1)+(f_2\mid f_2)						\\
	&=\lvert\lvert f_1\rvert\rvert^2_2+\lvert\lvert f_2\rvert\rvert^2_2	
	\end{align*}
\item	Übung!
\item	Es gilt
	\begin{align*}
	\lvert\lvert S_nf\rvert\rvert^2_2
	&= \left\lvert\left\lvert\sum_{\lvert k\rvert\leq n}(f\mid b_k)\right\rvert
		\right\rvert^2_2	
	\overset{(1)}=
		\sum_{\lvert k\rvert\leq n}\lvert\lvert(f\mid b_k)\rvert
		\rvert^2_2	
	= \sum_{\lvert k\rvert\leq n}\lvert(f\mid b_k)\rvert^2\lvert\lvert b_k\rvert
		\rvert^2_2	
	= \sum_{\lvert k\rvert\leq n}\lvert(f\mid b_k)\rvert^2
	\end{align*}
	und
	\begin{align*}
	\lvert\lvert f\rvert\rvert^2_2 
	= \lvert\lvert\underbrace{(f-S_nf)}_{\underset{(2)}\perp E_n}
		+\underbrace{S_nf}_{\in E_n}\rvert\rvert^2_2		
	= \lvert\lvert f-S_nf\rvert\rvert^2_2 + \lvert\lvert S_nf\rvert\rvert^2_2
	\end{align*}
\item	Sei \(v\in E_n\). Dann gilt:
	\begin{align*}
	\lvert\lvert f-v\rvert\rvert^2_2
	&= \lvert\lvert\underbrace{(f-S_nf)}_{\perp E_n}
		+\underbrace{(S_nf-v)}_{\in E_n}\rvert\rvert^2_2		\\
	&\overset{(1)}=
		\lvert\lvert f-S_nf\rvert\rvert^2_2
		+\lvert\lvert S_nf-v\rvert\rvert^2_2				\\
	&\geq \lvert\lvert f-S_nf\rvert\rvert^2_2
	\end{align*}
\end{enumerate}
\end{beweis}
...
\item \(f\in\mathrm{L}^{p}(I,\mdk)\) und \(\lVert f\rVert_{\infty}\leq(b-a)^{\frac{1}{p}}\lVert f\rVert_{\infty}\) (siehe \ref{Satz 16.2}).
...
\begin{satz}[Approximationssatz von Weiherstra\ss]
\label{Satz 18.4}
...
\(\lVert f\rVert_{2}^{2}=\sum_{k\in\mdz}{\lvert(f\mid b_{k})\rvert^{2}}\) (\textbf{Parsevalsche Gleichung}). Insbesondere gilt:
...
Setze \(g(0):=g(2\pi):=0\). Dann ist \(g\) stetig auf \([0,2\pi]\). Satz \ref{18.4} liefert nun: 
...
\lVert f-S_{n}f\rVert_{2}&=\lVert f-g+g-S_{n}g+S_{n}g-S_{n}f\rVert_{2}\\
    &\leq\lVert f-g\rVert_{2}+\lVert g-S_{n}g\rVert_{2}+\lVert S_{n}(g-f)\rVert_{2}\\
...
\[
\lVert f-S_{m}f\rVert_{2}<\lVert f-w\rVert_{2}=\lVert f-S_{n}f\rVert_{2}<\ep(2+\sqrt{2\pi})
\]
\end{beweis}
...
\begin{enumerate}
\item F\"ur \(k\in\mdn\) bezeichnen wie die Funktionen \(t\mapsto\cos(kt)\) und \(t\mapsto\sin(kt)\) mit \(\cos(k\cdot)\) bzw.
    \(\sin(k\cdot)\).
\item F\"ur \(k\in\mdn_{0}:\,\alpha_{k}:=\frac{1}{\pi}\int_{0}^{2\pi}{f(t)\cos(kt)\mathrm{d}t}=\frac{1}{\pi}\Re(f\mod e_{k})\).\\
F\"ur \(k\in\mdn:\,\beta_{k}:=\frac{1}{\pi}\int_{0}^{2\pi}{f(t)\sin(kt)\mathrm{d}t}=\frac{1}{\pi}\Im(f\mod e_{k}),\,\beta_{0}:=0\).
\end{enumerate}

\begin{definition}
\(f\) hei\ss t gerade (bez\"uglich \(\pi\)) genau dann, wenn gilt: \(f(t)=f(2\pi-t)\) f\"ur fast alle \(t\in[0,2\pi]\).\\
\(f\) hei\ss t ungerade (bez\"uglich \(\pi\)) genau dann, wenn gilt: \(f(t)=-f(2\pi-t)\) f\"ur fast alle \(t\in[0,2\pi]\).\\
...
Damit:
\[
f\overset{\lVert\cdot\rVert_{2}}{=}\frac{\pi}{2}-\frac{4}{\pi}\sum_{j=1}^{\infty}{\frac{\cos((2j+1)\cdot)}{(2j+1)^{2}}}
\]
\end{enumerate}
\end{beispiele}
...
\begin{enumerate}
\item Die Reihe $\sum_{k \in \MdZ} (f|b_k) b_k(t)$ konvergiert auf $[0, 2 \pi ]$ absolut und gleichmäßig.
Setzt man $g(t) := \sum_{k \in \MdZ} (f|b_k)b_k(t)$ für $t \in [0, 2\pi ]$, so ist $g$ stetig, $g(0)=g(2\pi )$ und $f=g$ f.ü. auf $[0,2 \pi ]$.
\item Ist $f$ stetig, so gilt $f=g$ auf $[0,2\pi ]$, also:
\[ f(t) = \sum_{k \in \MdZ} (f|b_k) b_k(t) \quad \forall t \in [0,2\pi ] \quad (*)\]
...
\begin{beweis}
\begin{enumerate}
\item $f_k(t) := (f|b_k)b_k(t)$;
\[ |f_k(t)| = |(f|b_k)| \cdot |b_k(t)| = \frac{1}{\sqrt{2\pi }} |(f|b_k)| \quad \forall t \in [0,2\pi ] \forall k \in \MdZ \]
...
Klar: $g(0) = g(2\pi )$.
\[ s_n(t) := \sum_{|k| \leq n} f_k(t) \quad (n \in \MdN_0, t \in [0,2\pi ]).\]
...
\begin{satz}
\label{Satz 18.8}
$f \in L^2_\MdR$ und die Folgen $(\alpha_k )$ und $(\beta_k )$ seien definiert wie im Abschnitt "Reelle Version". Weiter gelte: $\sum_{k=1}^\infty |\alpha_k| < \infty$ und $\sum_{k=1}^\infty |\beta_k | < \infty$. Dann gelten die Aussagen in 18.7 für die Reihen in 18.6.
\end{satz}

\begin{satz}
\label{Satz 18.9}
Sei $f:[0,2\pi ] \to \MdC$ \textbf{stetig db.} und $f(0)=f(2\pi )$.
\begin{enumerate}
\item Es ist $(f'|b_k) = ik(f|b_k) \quad \forall k \in\MdZ$
\item $\sum_{k\in\MdZ} |(f|b_k)|<\infty$  (d.h.: die Voraussetzungen von 18.7 sind erfüllt)
\end{enumerate}
\end{satz}
...
&\overset{P.I.}{=} \frac{1}{\sqrt{2\pi}} \left[ f(t)e^{-ikt} \right]_0^{2\pi} - \frac{1}{\sqrt{2\pi}}\int_0^\pi f(t)(-ik)e^{-ikt}\text{ d}t \\
...
\begin{align*}
\sigma_n - |(f|b_k)| &= \sum_{0<|k|\leq n} |(f|b_k)| \overset{(1)}{=} \sum_{0<|k|\leq n} \underbrace{\frac{1}{|k|}}_{:= u_k}\underbrace{(f'|b_k)}_{:= v_k} \\
&= \sum_{0<|k|\leq n} u_k v_k \overset{CS-Ugl.}{\leq} \left( \sum_{0<|k|\leq n} u_k^2 \right)^\frac{1}{2} \left( \sum_{0<|k|\leq n} v_k^2 \right)^\frac{1}{2}
...
\textbf{Beispiele:} \begin{enumerate}
\item $f$ sei wie im Beispiel (2) vor 18.7. Wenn man sich ganz arg anstrengt, kommen dann lustige Ergebnisse raus wie z.B. $ \sum_{j=0^\infty} \frac{1}{(2j+1)^2} = \frac{\pi^2}{8}$.
\item $f(t) = (t - \pi)^2 \quad (t \in [0,2\pi])$. $f$ ist gerade bzgl. $\pi$, also ist $\beta_k = 0$. Hier kommt dann sowas raus wie $\sum_{j=1}^\infty \frac{1}{j^2} = \frac{\pi^2}{6}$. 
Und wenn man beides geschickt kombiniert, erhält man $\sum_{j=1}^\infty \frac{(-1)^{j+1}}{j^2} = \frac{\pi^2}{12}.$
\textbf{Diese Beispiele werden noch ausführlicher aufgeschrieben.}
\end{enumerate}
...
\textbf{§ 18: Fourierreihen}: Jan Ihrens, Philipp Ost \\
\end{document}
