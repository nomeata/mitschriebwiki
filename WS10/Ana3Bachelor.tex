...
\end{displaymath}
Dann ist $\gamma$ ein stückweise stetig differenzierbarer Weg im $\MdR^2$, $\gamma$ ist geschlossen ($\gamma$ ist rektifizierbar). Es sei $B:= \{(x_0+r\cos t,y_0 + r\sin t): t\in [0,2\pi ], 0\le r\le R(t)\}$. Dann ist $B$ kompakt, also $B\in\fb_2 $. Weiter: $\partial B = \gamma([0,2\pi]) = \Gamma_\gamma$.\\
...
\begin{definition}
	...
	\begin{displaymath}
		...
		\end{pmatrix}
	\end{displaymath}
	Sei $(u_0,v_0)\in B$, $\gamma(t) := \phi(t,v_0)$, $\gamma'(t) = \phi_u(t,v_0)$, $\gamma'(u_0) = \phi_u(u_0,v_0)$. Weiter sei $\tilde{\gamma}(t):= \phi(u_0,t)$, $\tilde{\gamma}'(t) := \phi_v(u_0,v)$, $\tilde{\gamma}'(v_0) = \phi_v(u_0,v_0)$.$N(u_0,v_0) := \phi_u(u_0,v_0)\times\phi_v(u_0,v_0)$ \begriff{Normalenvektor} in $\phi(u_0,v_0)$. \\
	Seien $\Delta u,\Delta v >0$ (aber "`klein"'). $a:= \Delta u\phi_u(u_0,v_0)$, $b:= \Delta_v\phi_v(u_0,v_0)$\\ $P:= \{\lambda a+\mu b: \ \lambda,\mu\in [0,1]\}$ Aus der Linearen Algebra folgt: "`Inhalt"' von $P = \|a \times b\| = \Delta u\Delta v \|N(u_0,v_0)\|$.
	\begin{displaymath}
...
\item 	Wie in Analysis II zeigt man: gilt \(\lvert\lvert f_n-f\rvert\rvert_p=
		\lvert\lvert \hat f_n-\hat f\rvert\rvert_p\to 0\), so ist \((\hat f_n)\) eine Cauchyfolge
		in \(L^p(X)\).
