...
disjunkt (beachte Lemma \eqref{Lemma 2.1}, Punkt 3).
\[
\lambda_{d}(A):=\sum_{j=1}^{n}{\lambda_{d}(I_{j})}
\]
Wegen Lemma \eqref{Lemma 2.2} ist \(\lambda_{d}:\cf_{d}\to[0,\infty)\)
wohldefiniert.
\end{definition}
...
$\ce$ hat die Eigenschaften (i) und (ii) aus \eqref{Satz 2.6}, aus diesem Satz folgt dann, dass $\mu=\lambda_d$ auf $\fb_d$ ist.
\end{enumerate}
\end{beweis}
...
Aus (1) folgt, dass eine offene Menge $G\supseteq B\setminus A$ existiert mit $\lambda_d(G)\le\lambda_d(B\setminus A)+\ep$. Dann gilt nach \eqref{Satz 1.7}:
...
Setze $A_n:=A\cap\overline{U_n(0)}$. Dann ist $A_n$ für alle $n\in\mdn$ beschränkt, $A_n\subseteq A_{n+1}$ und $A=\bigcup_{n\in\mdn} A_n$. Nach \eqref{Satz 1.7} gilt:
...
\item Sei $X_0\in\fa$, dann gilt $\fa_{X_0}\subseteq\fa$ nach \eqref{Satz 1.5}. Nun sei $f:X\to Y$ $\fa$-$\fb$-messbar, dann ist $f_{\mid X_0}:X_0\to Y$ $\fa_{X_0}$-$\fb$-messbar.
\end{enumerate}
\end{bemerkung}
...
\item Ist $C$ wie in \eqref{Satz 2.11}, so ist $\mathds{1}_A$ nicht $\fb_d$-$\fb_1$-messbar.
