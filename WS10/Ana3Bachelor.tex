...
\end{displaymath}
Dann ist $\gamma$ ein stückweise stetig differenzierbarer Weg im $\MdR^2$, $\gamma$ ist geschlossen ($\gamma$ ist rektifizierbar). Es sei $B:= \{(x_0+r\cos t,y_0 + \sin t): t\in [0,2\pi ], 0\le r\le R(t)\}$. Dann ist $B$ kompakt, also $B\in $. Weiter: $\partial B = \gamma([0,2\pi]) = $.\\
Sind $B$ und $\gamma$ wie oben, so heißt $B$ \begriff{zulässig}.
\begin{beispiel}
 Sei $R$ konstant, also $R(t) = R > 0$, so ist $B = \overline{U_R(x_0,x_0)}$
\end{beispiel}

\begin{satz}[Integralsatz von Gauß im $\MdR^2$]
$B$ und $\gamma$ seien wir oben ($B$ also zulässig). Weiter sei $D\subseteq \MdR^2$ offen, $B\subseteq D$ und $f = (u,v) \in C1(D,\MdR^2)$. Dann
\begin{liste}
...
\begin{beispiel}
$B:= \{(x,y)\in\MdR2:x2+y2 \le R2\}$ $(R>0$, $\gamma(t) = (R\cos t,R\sin t)$ $(t\in[0,2\pi]$.\\
$\lambda_2(B) = \int_0^{2\pi} R\cos t\cdot R\cos t dt = R^2\int_0^{2\pi} \cos^2t dt = \pi R^2$
\end{beispiel}
...
\textbf{§ 13: Der Integralsatz von Gau\ss \ im \(\mdr^{2}\)}:\\ 
\textbf{§ 14: Flächen im \(\mdr^{3}\)}:\\
\textbf{§ 15: Der Integralsatz von Stokes}: Philipp Ost\\
\textbf{§ 16: \(\fl^{p}\)-Räume und \(\mathrm{L}^{p}\)-Räume}: \\
