...
\paragraph{Ein neues Symbol kommt hinzu: \(-\infty\)}{
Kommt noch\ldots
}
...
%Beginn22.11.2010
\textbf{-- Hier beginnt der Mitschrieb vom 22.11.2010 - bis jetzt noch ausschließlich die Sätze und Definitionen --}

\begin{beispiel}
\end{beispiel}

\begin{satz}
\label{Satz 4.9 (Charakterisierung der Integrierbarkeit)}
Sei $f: X \to \imdr$ messbar. Die folgenden Aussagen sind äquivalent:
\begin{enumerate}
 \item $f$ ist integrierbar.
 \item $\exists u, v: X \to [0,+\infty]$ mit: $u, v$ sind integrierbar, $u(x)=v(x)=\infty$ für \textbf{kein} $x \in X$ und $f=u-v$ auf $X$.
 \item $\exists g: X \to [0,+\infty]$ mit: $g$ ist integrierbar und $\lvert f \rvert \leq g$ auf $X$.
...
\[n \cdot \lambda(N) = \int_X n\mathds{1}_N \text{ d}x \stackrel{4.5}{\leq} \int_> \lvert f \rvert \text{ d}x \stackrel{4.9}{<} \infty \text{  für alle } n \in \mdn\]
Also: $0 \leq n\lambda(N) \leq \int_X \lvert f \rvert \text{ d}x \forall n \in \mdn \implies \lambda(N) = 0$ 
\end{beweis}
...
\begin{enumerate}
 ...
 \item Ist $f+g:X\to\imdr$ auf $X$ definiert, so ist $f+g$ integrierbar und $\int_X (f+g)\textbf{ d}x = \int_X f \textbf{ d}x + \int_X g \textbf{ d}x$. (Für $f=+\infty$ und $g=-\infty$ ist $f+g$ beispielsweise nicht definiert)
 \item $\fl^1(X)$ ist ein reeller Vektorraum und die Abbildung $f \mapsto \int_X f \textbf{ d}x$ ist linear auf $\fl^1(X)$.
 \item $\max\{f,g\}$ und $\min\{f,g\}$ sind integrierbar.
 \item Ist $f\leq g$ auf $X$, so ist $\int_X f \textbf{ d}x \leq \int_X g \textbf{ d}x$.
 \item $\lvert \int_X f \textbf{ d}x \rvert \leq \int_X \lvert f \rvert \textbf{ d}x$.
 \item Sei $Y \in \fb(X)$, $Y ¸neq \emptyset$. Dann sind die Funktionen $f_{|Y}: Y \to \imdr$ und $\mathds{1}_Y\cdot f: X \to \imdr$ integrierbar und
\[\int_Y f(x) \textbf{ d}x := \int_Y f_{|Y} (x) \textbf{ d}x = \lvert \int_X(\mathds{1}_Y \cdot f)(x) \textbf{ d}x\]
 \item Sei $y(X) < \infty$ und $h: X \to \mdr$ sei messbar und beschr\"ankt. Dann: $h \in \fl^1(X)$ und $\lvert \int_X h \textbf{ d}x\rvert \leq \|h\|_\infty \lambda(X) \quad$ (mit $\|h\|_\infty := \sup\{|h(x)> : x\in X\}$) 
\end{enumerate}
...
\begin{satz}
 \label{4.12 Folgerungen}
\begin{enumerate}
 \item Sind $A,B \in \fb(X)$, $A,B \neq \emptyset$ und $X = A \cup B$ und ist $f: X \to \imdr$ integrierbar (über $X$), so ist $f$ integrierbar über $A$ und integrierbar über $B$ und $\int_X f \textbf{ d}x = \int_A f \textbf{ d}x + \int_B f \textbf{ d}x$.
...
\textbf{-- Hier endet der Mitschrieb vom 22.11.2010 --}
%Ende22.11.2010
...
\textbf{§ 4: Konstruktion des Lebesgueintegrals}: Rebecca Schwerdt\\
