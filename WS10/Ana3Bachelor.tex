...
%Damit: \(\int_{\phi}{\rot F\cdot n\mathrm{d}\sigma}=\sum_{j=1}^{3}{\int_{B}{\div h_{j}(x,y)\mathrm{d}(x,y)}}=\sum_{j=1}^{3}{\int_{\gamma}{(u_{j}\mathrm{d}y-v_{j}\mathrm{d}x)}}=\int_{0}^{2\pi}{F_{j}(\varphi(t))\varphi_{j}'(t)\mathrm{d}t} % Diese Zeile hat 'nen Bug
\end{beweis}
