...
Hier kommt noch der Mitschrieb vom 8.11.
...
\[n \middot \lambda(N) = \int_X n\mathds{1}_N \text{ d}x \stackrel{4.5}{\leq} \int_> \lvert f \rvert \text{ d}x \stackrel{4.9}{<} \infty \text{  für alle } n \in \mdn\]
...
\begin{enumerate}
 ...
 \item Sei $Y \in \fb(X)$, $Y ¸neq \emptyset$. Dann sind die Funktionen $\f|_Y: Y \to \imdr$ und $\mathds{1}_Y\middot f: X \to \imdr$ integrierbar und
\[\int_Y f(x) \textbf{ d}x := \int_Y f|_Y (x) \textbf{ d}x = \lvert \int_X(\mathds{1}_Y \middot f)(x) \textbf{ d}x\]
