...
Sei $(\hat{x_n})_{n \in \N}$ eine CF in $X_{/_Y}$$. Dann ex eine Teilfolge $(\hat{x_n_k})_{k \in \N}$ mit $||\hat{x_m}-\hat{x_n_k}|| \leq 2^{-k}$ f"ur alle $x \geq n_k$. Dann ex $y_n_k \in Y$ mit $||x_n_{k+1}-x_n_k-y_n_k|| \leq 2 \cdot 2^{-k}$. Setze $v_N = x_n_1 + \sum_{k=1}^N z_k$, wobei $z_k = x_n_{k+1}-x_n_k-y_n_k.\ X$ BR $\Rightarrow \exists\, x:= \lim_{N \rightarrow \infty} v_n \in X$. Weiter gilt:
\[
v_N = x_n_{N+1} - \underbrace{\sum_{k=1}^N y_n_k}_{\in Y} \stackrel{\Pi \text{ stetog}}{=} \underbrace{\hat{v_N}}_{\hat{x_n_{N+1}}} \rightarrow \hat{x} \text{ in } X_{/_Y}
\]
\end{Bew}
\end{document}