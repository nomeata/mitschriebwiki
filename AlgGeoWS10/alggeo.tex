...
\node[right,text width=0.95\textwidth,rounded corners,fill=orange,inner sep=1ex]{%
  \hbox{\hspace{1em}\makebox[\width][t]{\raisebox{1.3ex}{\lhdbend}}\hspace{2em}%
  \begin{minipage}[c]{.88\textwidth}\smallskip\BODY\medskip\end{minipage}%
...
\begin{lem}[Schluss von Rabinowitsch]
  Aus dem \hyperref[satz2b]{schwache Hilbertsche Nullstellensatz} folgt der \hyperref[satz2c]{starke Hilbertsche Nullstellensatz}.
\end{lem}
\begin{proof}
...
\begin{enumerate}
\item\Label{1.4.1a} Seien $V\subseteq k^{n}, W\subseteq k^{m}$ affine Varietäten. Wir nennen eine Abbildung $f\colon V\ra W$ einen \emph{Morphismus}, wenn es Polynome $f_1,\dotsc,f_m \in k\polyx$ mit 
\[f(p)=(f_1(p),\dotsc,f_m(p)) \in k^{m}\text{ gibt.}\]
\item\Label{1.4.1b} Ein Morphismus $f\colon V\ra W$ heißt \emph{Isomorphismus}, wenn es einen Morphismus $g\colon W\ra V$ mit $g\circ f=\id_V$ und $f\circ g=\id_W$ gibt.
\item\Label{1.4.1c} Gibt es einen Isomorphismus zwischen $V$ und $W$, so heißen $V$ und $W$ \emph{isomorph}.
\end{enumerate}
\end{dfn}
...
So ist beispielsweise, nach \cref{1.2.4}, jede bijektive Abbildung von $k$ nach $k$ stetig.
\end{w}
...
\begin{proof}
  \begin{prooflist}
  \item\Label{1.5.10i} Ohne Einschränkung kann in \cref{1.5.3} $U_p=\D(g_p)$ für ein $g_{p}\in K[V]$ gewählt werden. Damit ist zunächst
...
\begin{db}\label{1.6.5} Seien $V\subseteq \A^n(K)$, $W\subseteq \A^m(K)$ affine Varietäten.
  ...
  \item\label{1.6.5e} Sind $V$ und $W$ irreduzibel, so induziert jede dominante rationale Abbildung \[f \colon V\ppf W\] einen Körperhomomorphismus $\alpha:=f^{\sharp} \colon K(W)\ra K(V)$ mit \[\alpha(g)=g\circ f.\]
...
\begin{db}\label{4.4.2}  Sei $\omega \in \Omega_{\CC}$, $\omega\neq 0$. 
  ...
  \item $\K$ heißt \emph{kanonischer Divisor}, wenn $\K=\div \omega$ für ein $w\in \Omega_{\CC}$ gilt.
...
\begin{kor}\label{4.4.3} \begin{enumerate}
  ...
  \item\Label{4.4.3b} Ist $D=\K$ ein kanonischer Divisor, so ist $\ll(\K)=g$ nach \ref{4.4.3a}. Also gilt hier \[\deg (\K)=g-1+\ll(D)-\ll(0)=2g-2.\]
 \end{enumerate}
\end{kor}
...
Sei dazu zunächst $a\in K$. Dann ist $X-a$ uniformisierend und es gilt also $\d X=\d (X-a)$. Also ist $\ord_a (\omega)=0$.

Nun sei $a=\infty$. Dann ist $\frac{1}{X}$ uniformisierend und mit Leibniz sieht man: \[\d\frac{1}{X}=-\frac{1}{X^2} \d X.\]
Also ist $\ord_{\infty}(\omega)=-2$ und damit ist $\K=-2\cdot \infty$ der kanonische Divisor zu $w$. Insbesondere ist $\deg \K=-2$. Setze $D=\K$. Nun liefert \cref{4.4.3b}: \[g(\P^1(K))=0.\] 
\end{bsp}
