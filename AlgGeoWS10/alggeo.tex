...
Seien $V,W$ irreduzible, affine Varietäten, $f \colon V\ra W$ ein Morphismus und $f^{\#} \colon K[W]\ra K[V]$ der reduzierte $K$-Algebrenhomomorphismus. Dann gilt:
  \begin{enumerate}
  \item\label{1.6.3a} $f^{\#}$ kann genau dann zu einem Körperhomomorphismus von $K(W)\ra K(V)$ fortgesetzt werden, wenn $f^{\#}$ injektiv ist.
  \item\label{1.6.3b} Weiter ist $f^{\#}$ genau dann injektiv, wenn $f(V)$ dicht in $W$ ist.
  \end{enumerate}
\end{prop}

\begin{proof}
  \begin{enumerate}
  \item[\ref{1.6.3a}] Die Notwendigkeit der Injektivität von $f^{\#}$ ist klar. Ist umgekehrt $f^{\#}$ injektiv, so definiert man $f^{\#}(\frac{a}{b})=\frac{f^{\#}(a)}{f^{\#}(b)}$. Die Injektivität impliziert die Wohldefiniertheit und man sieht, dass man auf diese Art einen     Körperhomomorphismus erhält.
  \item[\ref{1.6.3b}]  Mit $\alpha=f^{\#}$ gilt: Für jede Untervarietät $Z \subseteq V$ ist $\alpha^{-1}(\I(Z))=\I(f(Z))$. Denn:
  \[g \in \alpha^{-1}(\I(Z)) \iff g\circ f \in \I(Z) \iff g \circ f(x)=0 \, \, \forall x \in Z \iff g(y)=0 \, \, \forall y \in f(Z)\]
Nun gilt:
\[\alpha \text{ist injektiv} \iff 0=\alpha^{-1}(0)=\alpha^{-1}(\I(V)))=\I(f(V))=\I(\Bar{f(V)}) \iff \Bar{f(V)}=W\]  
  \end{enumerate}
\end{proof}
...
\begin{db}\label{1.6.5} Seien $V\subseteq \A^n(K)$, $W\subseteq \A^m(K)$ affine Varietäten.
  \begin{enumerate}
  \item\label{1.6.5a} Eine \emph{rationale Abbildung} $f \colon V\dots>W$ ist eine Äquivalenzklasse von Paaren $(U,f_U)$, wobei $U$ offen und dicht in $V$ und $f_U \colon U\ra W$ reguläre Abbildungen sind. Dabei:
  ...
  \item\label{1.6.5c} Jede rationale Abbildung $r \colon V\dots> W$ hat einen maximalen Definitionsbereich Def($r$), der offen ist.
  \item\label{1.6.5d} Sind $V,W$ irreduzibel, so ist die Kompositon von dominanten rationalen Abbildungen wieder eine dominante rationale Abbildung, d.h. für $U$ dicht in $V$ ist $\Bar{r(U)}=W$.
  \item\label{1.6.5e} Sind $V,W$ irreduzibel, so induziert jede dominante rationale Abbildung $r \colon V\dots> W$ einen Körperhomomorphismus $\alpha:=\r^{\#} \colon K(W)\ra K(V)$.
  \item\label{1.6.5f} Eine rationale Abbildung $f \colon V\dots> W$ heißt birational, wenn es eine rationale Abbildung $g \colon W\dots> V$ gibt, so dass $f\circ g$ und $g\circ f$ definiert sind und jeweils die Identität ergeben.
  \end{enumerate}
\end{db}
...
\begin{bsp}\label{1.6.6}
  ...
  \[f:V_1\dots>V_2, \quad (x,y)\mapsto x\]
  \[g:V_2\dots>V_3, \quad x\mapsto \frac{1}{x}\]
  ...
  Dann erhalten wir:\begin{itemize}
     \item $r_2\circ r_1=\id_{U_1}$ mit $U_1=\text{Def}(r_2\circ r_1)=\A^1(K)\setminus \{0\}$.
     \item $r_1\circ r_2=\id_{U_2}$ mit $U_2=\text{Def}(r_1\circ r_2)=V_2\setminus \{(0,0)\}$.
     \end{itemize}
...
\begin{itemize}
  ...
  \item $\Phi \colon V\ra K(V)$, $(f:V\ra W)\mapsto \alpha$, wobei $\alpha:K(W)\ra K(V)$ mit $\alpha(g)=g^{\#}=g\circ f$.
\end{itemize}
\end{nbem}
...
Zum zweiten Punkt: Zeige: Sind $V,W$ affine Varietäten und $\alpha \colon K(W)\ra K(V)$ $K$-Algebrenhomomorphismus, so gibt es eine rationale Abbildung $f=f_{\alpha}:V\dots> W$ mit $\alpha=f^{\#}$.
Für den Beweis dieser Aussage seien $g_1,\dotsc,g_m$ Erzeuger von $K[W]$. Dann ist $\alpha(g_i)\in K(V)$ und $\bigcup_{i=1}^{m} \text{Def}(\alpha(g_i))$ ist offen und dicht in $V$. Wähle $U=D(g)\subseteq \bigcup_{i=1}^{m} \text{Def}(\alpha(g_i))$ mit $g \in K[V]$.
Nach \cref{1.5.15} ist  $D(g)$ affin, also existiert eine affine Varietät $X$ und ein Isomorphismus $\Psi \colon X\ra (D(g)$.
%kommutatives Diagramm
Also: $\Psi^{\#}\circ \alpha \colon K[W] \ra \O(X)=K[X]$ induziert $\tilde{f} \colon X\ra W$.
Damit ist $f:=\tilde{f}\circ \Psi^{-1}$ eine reguläre Abbildung von $U$ nach $W$. Diese induziert eine rationale Abbildung $f \colon V\dots> W$. Da $\Psi^{\#}\circ \alpha$ injektiv ist, ist $f$ dominant und nach \cref{1.6.3} ist damit auch $\tilde{f}$ dominant.

Nun fehlt nur noch zu zeigen, dass $\Phi$ treu ist. Seien dazu $v_1$, $v_2 \colon V\dots>W$ rationale Abbildungen und $r_1^{\#}=r_2^{\#}=\alpha$. Wähle $U$ offen, affin und klein genung, so dass $r_1$, $r_2$ auf $U$ definiert sind.
...
\item\Label{2.2.3b} Die Abbildung
...
\item\Label{2.2.3c} Die Abbildung $\P^{n}(k)\setminus U_{i}\ra\P^{n-1}(k),$
...
\item[\ref{2.2.3b}] Wir geben eine Umkehrabbildung an:
\[\A^{n}(k)\ra U_{i},\quad(y_{1},\dotsc,y_{n})\mapsto(y_{1}:\dotsm:y_{i}:1:y_{i+1}:\dotsm:y_{n}).\]
...
\item[\ref{2.3.5d}] Hier wirken die Argumente aus Kapitel I, \cref{1.1.3} und \cref{1.1.5}.
\end{enumerate}\end{proof}
...
\begin{proof} Genau wie im Affinen. Siehe dazu \cref{1.2.1}, \cref{1.2.3}, \cref{1.2.9} und \cref{satz1} aus Kapitel I.
\end{proof}
...
für ein $j\in\N_{0}$, denn eventuell heben sich Summanden weg. Es gilt also\vspace*{-6pt}
\[H(f+g)=\sum_{i=0}^{d-j}(f_{i}+g_{i})X_{0}^{d-i-j}\]
\vspace*{-6pt}und damit\vspace*{-6pt}
\[X_{0}^{j}H(f+g)
...
\begin{w}Hier fehlt noch was.\end{w}
%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
Nun gilt die Aussage für die $\dach{W}\cap U_{i}\subseteq\A^{n}(k)$ nach \cref{1.5.9iii} und \cref{1.5.15i} aus Kapitel I, da die $\D(f)$ eine Basis der Topologie bilden und affin als abstrakte Varietäten sind.
...
Nach \cref{1.5.9ii} aus Kapitel I genügen aber für jedes $i$ endlich viele offene Mengen, also genügen auch insgesamt endlich viele.
\end{enumerate}\end{proof}

\sect{Reguläre Funktionen}

Seien, wie bisher, $k$ ein Körper, $n\in\N_{0}$, $U_{i}=\{(x_{0}:\dotsm:x_{n})\in\P^{n}(k)\mid x_{i}\neq 0\}$ und $\varphi_{i}\colon\A^{n}(k)\hookrightarrow\P^{n}(k)$, $(x_{0},\dotsc,\dach{x_{i}},\dotsc,x_{n})\mapsto(x_{0}:\dotsm:x_{i-1}:1:x_{i+1}:\dotsm:x_{n})$.
...
Sei $r\colon W\ra k$ eine Abbildung. Dann ist $r$ genau dann regulär, $r|_{W\cap U_{i}}$ regulär für jedes $i$ ist, d.h. $r\circ\varphi_{i}|_{\varphi_{i}^{-1}(W)}$ ist reguläre Funktion der quasi-affinen Varietät $\varphi_{i}^{-1}(W)$, definiert wie in Kapitel I, \cref{1.5.3a}.
\end{bem}
...
\begin{enumerate}
\item\Label{s4a} $V$ zusammenhängend $\implies\O_{V}(V)=K$.
\item\Label{s4b} Es sei $K[V]=\Quotient{K\ppolyx}{\I(V)}$ der homogene Koordinatenring und $F\in K[V]$ homogen mit $\deg F\geq 1$. Dann gilt:
...
\begin{proof}\begin{enumerate}
\item[\ref{s4b}] Wir definieren eine Abbildung
...
Sei dazu $r\in\O(\D(F))$. Falls $U_{i}\cap V\neq\leer$ ist, nach \cref{2.5.2}, $r\circ\varphi_{i}$ auf $\D(F)\cap U_{i}=\D(f_{i})$ regulär, wobei $f_{i}$ die Dehomogenisierung von $F$ bzgl. $X_{i}$ ist. Jetzt sind wir im Affinen und nach Kapitel I, \cref{1.5.11i}, gibt es dort ein $g_{i}\in K[X_{0},\dotsc,\dach{X_{i}},\dotsc,X_{n}]$ und $k_{i}\in\N_{0}$, so dass
...
\[r|_{U_{i}}=\frac{G_{i}}{F_{i}^{k_{i}}\cdot X_{i}^{e_{i}}},\text{ mit }G_{i}\in K\ppolyx\text{ und }e_{i}\in\N_{0}.\]
...
\begin{equation}\label{s4s}G_{i}\cdot X_{j}^{e_{j}}\cdot F\cdot X_{j}\cdot X_{i}=G_{j}\cdot X_{i}^{e_{i}}\cdot F\cdot X_{j}\cdot X_{i}\text{ auf ganz }V,\tag{$*$}\end{equation}
...
gilt. Wir setzen $\displaystyle G:=\sum_{i=0}^{n}h_{i}G_{i}X_{i}$ und mit Hilfe von \cref{s4s} lässt sich
...
einsehen. Somit gilt, auf $\D(F)\cap U_{j}$, gerade \[\frac{G}{F^{m+1}}=\frac{G_{j}}{X_{j}^{e_{j}}\cdot F}=r|_{U_{j}}.\] Also ist, nach \cref{2.5.2}, $\Psi\bigl(\frac{G}{F^{m+1}}\bigr)=r$, damit ist $\Psi$ surjektiv und die Isomorphie ist gezeigt.
\end{enumerate}\end{proof}
