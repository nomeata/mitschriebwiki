...
\item[\ref{1.2.5b}] {\sc Erinnerung:} Ein topologischer Raum $X$ heißt hausdorffsch$\iff$ Für alle $x,y\in X$ existieren offene Umgebungen $U_{x}\ni x$ und $U_{y}\ni y$ mit $U_{x}\cap U_{y}=\leer$.
...
\begin{motivation}
  ...
  \begin{align*}
    \V\colon& \mathpzc{J}_n \ra \mathpzc{V}_n, &I\mapsto\V(I)\\
    \I\colon& \mathpzc{V}_n \ra \mathpzc{J}_m, &V\mapsto\I(V)
  \end{align*}
...
Wir werden sehen, dass $\I\circ\V=\id$ gilt, falls $k$ algebraisch abgeschlossen ist. Die einzige ``Obstruktion'' ist,
dass $\V(I)=\leer$ gilt, obwohl $I\neq k\polyx$.

\begin{satz}[Hilbertscher Nullstellensatz]\label{1.3.2}\label{HNS}
  \begin{enumerate}
  \item {\bf Algebraische Form:}\label{1.3.2a} Sei $m$ ein maximales Ideal in $k\polyx$. Dann ist $\Quotient{k\polyx}{m}$ eine
    endliche algebraische Körpererweiterung von $k$.
  \item {\bf Schwacher Hilbertscher Nullstellensatz:}\label{1.3.2b} Ist $k$ algebraisch abgeschlossen und $I\subsetneq k\polyx$
    ein echtes Ideal, dann ist $\V(I)\neq\leer$.
  \item {\bf Starker Hilbertscher Nullstellensatz:}\label{1.3.2c} Ist $k$ algebraisch abgeschlossen und $I\subseteq k\polyx$ ein
    Ideal, dann ist $\I(\V(I))=\sqrt(I)$.
  \end{enumerate}
\end{satz}
\begin{proof}
  \begin{enumerate}
  \item Wir bezeichnen $x_1=\Bar{X}_1,\dotsc,x_n=\Bar{X}_n$. Ohne Einschränkung können wir nach einer eventuellen Umsortierung
    ...
    dabei ist $k(x_1,\dotsc,x_k)\cong\Quot(k[X_1,\dotsc,X_k])$ und $L$ ist endlich erzeugt als $S$-Modul.

    Die Aussage folgt dann aus den nachfolgenden Lemmata.
  \end{enumerate}
\end{proof}
...
\end{lem}
\begin{proof}
  Wir schreiben $x_i=\sum_{j=1}^m a_{ij}w_j$ mit $a_{ij}\in S$ und $w_iw_j=\sum_{k=1}^m b_{ijk}w_k$ mit $b_{ijk}\in S$. Sei 
\end{proof}
