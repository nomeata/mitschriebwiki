...
\begin{proof}
  ...
  Wir zeigen $\phi_0^{-1}(\V(J))=\V(I)$, also $\phi_{0}(\V(I))=\V(J)\cap U_{0}$. \cref{2.3.10} sagt $\phi_0(\V(I))=\V(I^*)\cap
  U_0$, also wissen wir $\phi_0(\V(I))\subseteq\V(J)\cap U_0$. Sei umgekehrt $z=(1:x_1:\dotsm:x_n)\in\V(J)\cap U_0$,
  $x=(x_1,\dotsc,x_n)$ und $f\in I$; sei dabei ohne Einschränkung $f$ ein Erzeuger, also $f=\DD(F)$ für ein $F\in J$. Es gilt
  dann $f(x_1,\dotsc,x_n)=F(1:x_1:\dotsm:x_n)=0$, also $x\in\V(I)$. Damit ist $\phi_0$ stetig. Wegen \cref{2.3.10} ist
  $\phi_0(\V(I)=\V(I^*)\cap U_0$ eine abgeschlossene Teilmenge von $U_0$, also ist $\phi_0^{-1}$ auch stetig.
\end{proof}

%Lemma 3.7, das ist jetzt verschoben
\begin{lem}\label{2.3.7}
Sei wieder $K$ algebraisch abgeschlossen, $I\subseteq K\polyx$ ein Radikalideal, $\phi_0$ und $I^{*}$ wie oben. Dann ist
...
\begin{proof}
  ...
  Dehomogenisierung. Dann gilt für alle $x\in\V(I)$: $F(\phi_0(x))=f(x)=0$, also $f\in\I(\V(I))=\sqrt{I}=I$. Damit folgt
...
\begin{proof}[Beweis von \cref{2.3.6}]\label{beweisprojhns}
  ...
  \item[\ref{2.3.6b}] Sei $I\neq K\ppolyx$ und $V=\V^{\mathrm{proj}}(I)=\V(I^{\mathrm{homogen}})\subseteq\P^n(K)$. Ist dann
    $V=\leer$, gilt $\{(0,\dotsc,0)\}=\tilde{V}=\V^{\mathrm{aff}}(I^{\mathrm{homogen}})=\V^{\mathrm{aff}}(I)$ und damit nach dem
    affinen Hilbertschen Nullstellensatz $\sqrt{I}=\I(\V^{\mathrm{aff}}(I))=(X_0,\dotsc,X_n)$. Die umgekehrte Implikation ist
    klar.
  \item[\ref{2.3.6a}] Wenn $I=K\ppolyx$, stimmt die Aussage. Sei also $I\neq K\ppolyx$ und $\sqrt{I}\neq(X_0,\dotsc,X_n)$. Nach
    \ref{2.3.6b} gilt dann $\V^{\mathrm{proj}}(I)\neq\leer$, also $\I(\V(I))=\I(\tilde{V})=\I(\V^{\mathrm{aff}}(I))=\sqrt{I}$.
  \end{enumerate}
...
Sei also $V$ irreduzibel. Dann ist $\I(V)$ ein Primideal und $K[V]$ nullteilefrei und demnach $L:=\Quot(K[V])$ ein Körper.
...
auf $\D(X_{i})\cap\D(X_{j})$ und das liegt dicht in $V$, denn $V$ ist irreduzibel. Damit sind sie nach \cref{kap1}, \cref{1.5.13b}, schon auf ganz $V$ gleich.
...
\begin{enumerate}
\item $K[V]_{d}\cdot f^{t}\subseteq K[V]_{d}\;\forall t\in\N$, denn sei
\item $K[V][f]\subseteq\frac{1}{X_{0}^{d}}\cdot K[V]$ und
\item Daraus folgt: $f$ ist ganz über $K[V]$.
\end{enumerate}
\end{enumerate}\end{proof}
...
Seien $W\subseteq \P^n(k)$, $W'\subseteq \P^m(k)$ quasi-projektive Varietäten.
  \begin{enumerate}
  \item\Label{2.6.1a} Eine Abbildung $f\colon W\ra W'$ heißt \emph{Morphismus}, wenn es ein $z \in W$, eine offene Umgebung $U_z \subseteq W$ von $z$ gibt und Polynome $F_0,\dotsc,F_n \in k\ppolyx$ vom gleichen Grad gibt, so dass für alle $y \in U_z$ gilt:
\[f(y)=(F_0(y),\dotsc,F_n(y))\]
  \item\Label{2.6.1b} Betrachte $f \colon \P^n(k)\supseteq W \ra W' \subseteq \P^n(k)$. Das liefert eine Abbildung $\A^n(k)\supseteq f^{-1}(W'\cap U_j) \cap U_i \ra W' \cap U_j$. 

Jetzt gilt:
$f$ ist genau dann ein Morphismus, wenn für alle $i,j$ mit $U_{ij}=f^{-1}(W'\cap U_j) \cap U_i \neq \leer$ die Abbildung $f\restrict{U_{ij}} \colon U_{ij} \ra W'\cap U_j$ ein Morphismus von quasi-affinen Varietäten ist (siehe \cref{1.5.14}).
...
{\sc Außerdem:}\begin{itemize}
    \item Ist $f$ Morphismus, so gilt lokal: $f(z)=(F(z)\cdot G(z))$. Also gilt lokal: $r(z)=\frac{G(z)}{F(z)}$ und also ist $r$ regulär.
    \item Ist umgekehrt $r$ regulär, so gilt lokal: $r(z)=\frac{G(z)}{F(z)}$. Damit: $f(z)=\Psi^{-1}(r(z))=(1,\frac{G(z)}{F(z)})=(F(z)\cdot G(z))$ und $f$ ist ein Morphismus.\end{itemize}

  \item[\ref{2.6.1d}] Zeige: Jeder Morphismus $f\colon W\ra W'$ ist stetig.

 {\sc Idee:} Führe Stetigkeit zurück in die affine Situation.
Wir wissen: $\bigcup_{i=0}^{n}U_i=\P^n$, $\bigcup_{i=0}^{m}U_j=\P^m$, $f\colon \P^n \supseteq W\ra W'\subseteq \P^m$
Betrachte also
\[ f_{ij}=f\restrict{U_{ij}} \colon U_{ij}=f^{-1}(W'\cap U_0)\ra W'\cap U_j=W_j'\subseteq U_j\cong \A^m\]
Nach \ref{2.6.1b} sind die $f_{ij}$ Morphismen im affinen Sinn und damit insbesondere stetig (siehe \cref{1.4.4}, \cref{2.3.11}).

Zeige nun: $U_{ij}$ ist offen in $W$. $f$ ist lokal (auf $U_z$) gegeben durch $f(w)=(F_0(w):\dotsm :F_m(w))$.
Also ist $U_{ij}\cap U_z=\{w \in U_z\cap U_i \mid f(w) \in U_j\}=\{w \in U_z\cap U_i \mid F_j(w)\neq 0\}$ offen. Damit ist aber auch $U_{ij}$ offen.
...
$f$ ist genau dann ein Morphismus, wenn $f$ stetig ist und für offenes $U\subseteq W'$, $g\in \O(U)$ gilt: $g\circ f \in \O(f^{-1}(U))$.
  \item\Label{2.6.2b} Für affine Varietäten stimmen die Definitonen von Morphismen überein, d.h. 
  \[W_1\subseteq \A^n(k) \inj U_0\subseteq \P^n(k)\]
  \[W_2\subseteq \A^m(k) \inj U_0\subseteq \P^m(k)\]
und $f$ ist genau dann ein Morphismus im Sinn von \cref{2.6.1}, wenn $f$ auch ein Morphismus im Sinn von \cref{1.4.1} ist.
  \end{enumerate}
\end{kor}
...
Nach \cref{1.5.12} ist $f_{ij}$ ein Morphismus und nach \cref{2.6.1b} ist damit auch $f$ ein Morphismus.
  \item[\ref{2.6.2b}] Aus \ref{2.6.2a} folgt: Ist $f$ ein Morphismus in der projektiven Welt, so ist $f$ stetig und respektiert die Strukturgarbe. Nach \cref{1.5.12} ist $f$ ein Morphismus im affinen Sinn.
\end{enumerate}
\end{proof}
...
\[f^{-1}(\{(\tilde{r}:\tilde{s})\})\cap U_2 \leftrightarrow \{(\lambda \tilde{r},\lambda \tilde{s}) \mid \lambda \in k\setminus \{0\}\} \subsetneqq \{(\lambda \tilde{r},\lambda \tilde{s}) \mid \lambda \in k\} \cong \A^1\]
Folglich enthält $\{(\lambda \tilde{r}, \lambda \tilde{s}) \mid \lambda \in k\setminus \{0\}\}$ unendlich viele Elemente und ist nicht isomorph zum Bild von $\A^1$ und damit nicht abgeschlossen in $\A^2$. Also kann $f$ nicht stetig auf ganz $\P^2$ sein.
  \item\Label{2.6.3b} Sei $E:=\V(X_0X_2^2-X_1^3+X_1X_0) \subseteq \P^2(k)$.

Also: $E\cap U_0=\{(1:x_1:x_2) \in U_0 \mid x_2^2-x_1^3+x_1=0\} \leftrightarrow \{(x,y)\in \A^2 \mid y^2=x^3-x\}$
