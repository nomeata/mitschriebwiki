...
\node[text width=0.8\textwidth,rounded corners,fill=orange,inner sep=1ex]{%
  ...
  \begin{minipage}[c]{.8\textwidth}\smallskip\raggedright\BODY\medskip\end{minipage}%
...
\begin{enumerate}
\item\Label{1.6.1a} Eine \emph{rationale Funktion} auf $V$ ist eine Äquivalenzklasse von Paaren $(U,f)$, wobei $U$ eine dichte, offene Teilmenge von $V$ mit $f\in \O_V(U)$ ist. Hierbei sei 
...
\begin{w}
Ist $V$ irreduzibel, so repräsentiert der gekürzte Bruch $\frac{g}{h} \in K(V)$ die rationale Funktion $r:=[(\D(h),\frac{g}{h})]$.
...
\begin{db}\label{1.7.2}
  ...
  \item\Label{1.7.2iv} Für $Z\subseteq\Spec R$ heißt $\I(Z):=\displaystyle\bigcap_{\wp\in Z}\wp$ das \emph{Verschwindungsideal von
      $Z$}.
  \end{enumerate}
\end{db}
\begin{proof}
...
\end{bem}
    ...
    \begin{w}
      Die Gegenrichtung stimmt nicht! Insbesondere ist $\Spec R$ nicht immer ein\par noetherscher topologischer Raum.
    \end{w}
...
\begin{align*} f(z)&=\left(1:\frac{f_1(y)}{g_1(y)}:\dotsm :\frac{f_m(y)}{g_m(y)}\right)\\&=(G_1(z):\dotsm :G_m(z)X_0^{e_0}:F_1(z)X_0^{e_1}: \dotsm :F_m(z)X_0^{e_m})\end{align*}
...
\begin{dfn}\label{3.1.1}
  ...
  \item\Label{3.1.1ii} Die Elemente in $\O_{W,p}$ heißen \emph{Funktionskeime}. 
...
Definiere $H=\V(h)\subseteq \A^{d+1}$ und 
\[K(H)=\Quot(\Quotient{K[X_1,\dotsc,X_d,Y]}{(h)})=K=K(W).\]
