...
%
\item $\bigwedge$ ist multilinear und alternierend, insbesondere gilt $v_{1}\wedge\dotsm\wedge v_{d}=0$, wenn $v_{i}=v_{j}$ für $i\neq j$. Genauer:
...
Für die andere ergänzen wir zunächst $v_{1},\dotsc,v_{k}$ zu einer Basis $\{e_{1},\dotsc,e_{n}\}$ von $V$ mit 
\[e_{1}=v_{1},\dotsc,e_{k}=v_{k}.\]
Für $w$ finden wir also
\[w=\!\!\!\!\!\!\!\!\!\!\sum_{\substack{\iq\in\{i_{1},\olddotsc,i_{d}\}\\1\leq i_{1}<\dotsm<i_{d}\leq n}}\!\!\!\!\!\!\!\!\! \lambda_{\iq}e_{i_{1}}\wedge\dotsm\wedge e_{i_{d}}.\]
Für $j\in\{1,\dotsc,k\}$ gilt dann:
...
und das ist eine Linearkombination von linear unabhängigen Vektoren. Also ist für $j\notin\{i_{1},\dotsc,i_{d}\}$ schon $\lambda_{\iq}=0$. Damit gilt
...
\begin{proof}
  ...
  \item Da $U_1$ affin ist, gilt $\O(U_1)=K[U_1]$. Seien $f_1,\dotsc,f_k$ Erzeuger von $K[U_1]$. Wir betrachten
    ...
    \end{tikzpicture} \]
    Insbesondere definiert ein injektiver Homomorphismus $K[U_1]\cong\O(U_1)\inj\O(U_2)\cong K[U_2]$ einen surjektiven
    Morphismus $U_2\surj U_1$ von affinen Varietäten.
    % 12.1.11
  \item Wir haben jetzt $p\in U_1\subseteq W_1$, $q\in U_2\subseteq W_2$ und ein kommutatives Diagramm
    ...
    U_2$. Analog ist $f\circ g$ die Einbettung $\tilde{\tilde{U}}_1\inj\tilde{U}_1$. Daraus folgt $f\circ
    g(\tilde{\tilde{U}}_1)\subseteq \tilde{\tilde{U}}_1$, also $g(\tilde{\tilde{U}})\subseteq
    f^{-1}(\tilde{\tilde{U}}_1)=\tilde{\tilde{U}}_2$, und analog $f(\tilde{\tilde{U}}_2)\subseteq\tilde{\tilde{U}}_1$. Hier gilt
...
\[\dim(X):=\sup \{d\in \N_0 \mid \exists \text{ Kette } \leer\neq X_0 \varsubsetneq  \dotsc \varsubsetneq X_d \text{ mit $X_i$ abg. und irred. in $X$}\}\]
...
\begin{enumerate}
  ...
  \item\Label{3.2.2b} Ist $X=\bigcup\limits_{i=1}^{n} X_i$ mit $X_i$ abgeschlossene Teilmengen von $X$, so gilt: $\dim(X)=\sup_{i} \dim(X_i)$.
\end{enumerate}
\end{bem}

\begin{proof}
  \begin{enumerate}
  \item[\ref{3.2.2a}] Sei $\leer \neq y_0 \varsubsetneq \dotsc \varsubsetneq y_k$ eine Kette von abgeschlossenen, irreduziblen Teilmengen in $Y$.

Dann ist auch $\leer \neq \Bar{y_0} \varsubsetneq \dotsc \varsubsetneq \Bar{y_n}$ Kette von abgeschlossenen, irreduziblen Teilmengen in $X$. Denn:
  \begin{itemize}
...
Wähle Kette $\leer \neq A_0 \varsubsetneq \dotsc \varsubsetneq A_k$ in $X$ von irreduziblen, abgeschlossenen Teilmengen.
...
Sei $R$ ein kommutativer Ring mit $1$.
  ...
  \[ \hoehe(\wp)=\sup \{n\in \N_0 \mid p_0 \varsubsetneq \dotsc \varsubsetneq p_n=\wp \text{ Kette von Primidealen}\}.\]
...
$\displaystyle \{\text{nichtleere irreduzible Untervarietäten von }V\} \lra \{\text{Primidealen in
    }K[V]\}$ durch 

$W\mapsto \I(W)$ bzw. $p \mapsto \V(p)$. Diese Zuordnungen sind inklusionsumkehrend, also entsprechen aufsteigende Primidealketten absteigenden Ketten von irreduziblen Varietäten.
\end{proof}

\begin{erinnerung}[aus Algebra {\sc ii}]\label{3.2.5}
Sei $K$ ein Körper, $A$ eine endlich erzeugte nullteilerfreie $K$-Algebra. \begin{itemize}
  \item $\dim(K\polyx)=n$
...
Für einen beliebigen Ring $R$ gilt: $\hoehe(m)=\dim(R_m)$.
\end{itemize}
\end{erinnerung}
