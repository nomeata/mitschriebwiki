...
Sei also $f\in K(\CC)^{\times}$ und $g_{1},\dotsc,g_{r}$ eine Basis von $K(C)$ über $K(f)=K(\frac{1}{f})$, also $r=\deg f$. Ohne Einschränkung können wir die $g_{i}$ ganz über $K[\frac{1}{f}]$ wählen.
...
\begin{db}\label{4.4.2}  Sei $w \in \Omega_{\CC}$, $w\neq 0$. 
  \begin{enumerate}
  \item\Label{4.4.2a} Sei $P\in \CC$, $t_p$ uniformisierend. Dann ist $w=f\cdot\d t_p$ und \[\ord_p (w):=\ord_p (f)\] ist wohldefiniert.
