...
\usepackage{latexki}
\lecturer{JProf. Dr. G. Weitze-Schmithüsen}
\semester{Wintersemester 10/11}
...
%\setdefaultlanguage[spelling=new,latesthyphen=true,babelshorthands=true]{german}
  ...
  \setsansfont[Mapping=tex-text,Numbers=OldStyle,Ligatures=Rare]{Linux Biolinum}
...
% "Standardumgebungen" alle in gabistyle enthalten...
...
\begin{equation*}p_{1},\dotsc,p_{r}\in k\polyx\end{equation*}
über einem Körper $k$ möchte man die Menge der Nullstellen
\begin{equation*}\{x=(x_{1},\dotsc,x_{n})\mid p_{i}(x)=0\text{ für alle }i\}\end{equation*}
analysieren.
\end{ziel}

\begin{nbsp}
\begin{enumerate}
\item Betrachte $ax^{2}+by^{2}=1\iff ax^{2}+by^{2}-1=0$ über $k=\R$. Das liefert eine Ellipse, für $a=b=1$ einen Kreis.
%Bild...
...
\item Die Lösungsmengen sind abhängig vom Körper, z.B. sehen wir, dass das Polynom $X^{3}-X$  für $k=\Z/3\Z$ als Lösungsmenge ganz $k$ hat.
\end{enumerate}
\end{nbsp}

\bigskip

\emph{Der Inhalt der Vorlesung wurde in großen Teilen von der Algebraischen-Geometrie-Vorlesung von Prof. Dr. Frank Herrlich inspiriert.}
...
mit \begin{equation*}V=\V(\F):=\{x=(x_{1},\dotsc,x_{n})\in k^{n}\mid f(x)=0\;\forall f\in\F\}\end{equation*} gibt.
\end{dfn}
...
das \emph{Radikal\index{Radikal}} von $(\F)$ ($\sqrt{(\F)}$ ist ein Ideal!). Ein Ideal mit $I=\sqrt{I}$ nennen wir \emph{Radikalideal\index{Radikalideal}}. Es gilt: $\V(\sqrt{(\F)})=\V(\F)$.
\item\Label{1.1.3d} Zu jeder affinen Varietät $V\subseteq k^{n}$ gibt es endlich viele Polynome $f_{1},\dotsc,f_{r}$ mit \begin{equation*}V=\V(\{f_{1},\dotsc,f_{r}\})=:\V(f_{1},\dotsc,f_{r}).\end{equation*}
\end{enumerate}
\end{bem}
...
\begin{equation*}V_{1}\cap V_{2}=\V(I_{1}\cup I_{2}) = \V(I_{1}+I_{2}).\end{equation*}
...
\begin{equation*}\bigl(f\cdot g\bigr)(x) = f(x)\cdot g(x) = 0\end{equation*}
...
\begin{equation*}\V(\I(M))\subseteq\V(\I(\V(J)))=\V(J).\end{equation*}
\end{proof}
...
\begin{enumerate}
\item\Label{1.2.5a} Wenn $k$ endlich ist, entspricht die Zariski-Topologie auf $\A^{n}(k)$ der diskreten Topologie.
...
\item $X$ heißt \emph{reduzibel\index{reduzibel}}, wenn es abgeschlossene echte Teilmengen \begin{equation*}A,B\subsetneq X\text{ mit }X=A\cup B\text{ gibt.}\end{equation*}
...
\begin{motivation}
  ...
  \begin{equation*} \VV_n = \{ V\subseteq k^n \mid V \text{ affine Varietät }\}\text{ und }
     \J_n = \{ I\subseteq k\polyx \mid I \text{ Radikalideal }\} \end{equation*}
...
Wir werden sehen, dass $\I\circ\V=\id$ gilt, falls $k$ algebraisch abgeschlossen ist. Das einzige \enquote{Problem} ist,
...
\begin{equation*} k \subseteq S = k(x_1,\dotsc,x_l) \subseteq L=\Quotient{k\polyx}{\m}, \end{equation*}
...
%X_i \text{ für } i\in\{1,\dotsc,n\},\ X_{n+1}\mapsto\frac1g \end{equation*}
...
\begin{equation*}f(p)=(f_1(p),\dotsc,f_m(p)) \in k^{m}\text{ gibt.}\end{equation*}
...
\item\Label{1.4.3b} Jedes $f \in k\polyx$ definiert einen Morphismus $\A^{n}(k)\ra \A^{1}(k)$.
...
\begin{bem}
\begin{enumerate}
\item\Label{1.4.6a} Der Koordinatenring $k[V]$ ist eine \emph{reduzierte\index{reduziert}} $k$-Algebra, das heißt aus $f^n=0$ folgt $f=0$.
...
%\end{equation*}\begin{tikzpicture}
...
% {%
%  X_{i}     &[-2em]       &[+1.5em]       &[-2em]\dach{\phi}(p_{i})\in\pi_{V}^{-1}\bigl(\phi(p_{i})\bigr) \\end{equation*}-2em]
%        &[-2em] k\polyx[m]     &[+1.5em] k\polyx&[-2em]                                             \\end{equation*}+1.5em]
%        &[-2em] \displaystyle k[W]=\Quotient{k[X_1,\dotsc,X_m]}{\I(W)}     &[+1.5em] \displaystyle k[V]=\Quotient{k\polyx}{\I(V)} &[-2em]                                             \\end{equation*}-2em]
...
\item\Label{1.4.8c} Ist $V$ ein Punkt, d.h. $V=\{(x_1,\dotsc,x_n)\}$, dann ist $\I(V)=(X_1-x_1,\dotsc,X_n-x_n)$. Also gilt dann $K[V]\cong K$. Das passt auch zu der Sichtweise, dass die maximalen Ideale in einem algebraisch abgeschlossenen Körper gerade den Punkten entsprechen.
...
\begin{equation*}f \in K[X,Y]\text{ mit }P^2(X,Y)=X+f(X,Y)(Y^2-X^3).\end{equation*}
Insbesondere wäre dann
 \begin{equation*}P^2(X,0)=X-f(X,0)\cdot X^3=X(1-f(X,0)X^2)\text{ in } K[X].\end{equation*}
...
\begin{db}\label{1.5.1}
  Sei $f\in K\polyx$ und \begin{equation*} \D(f) := \{ x\in\A^n(K) \mid f(x)\neq0 \} = \A^n(K)\setminus\V(f). \end{equation*} Das ist eine offene Teilmenge des
  $\A^n(K)$. Die Menge $\{\D(f) \mid f\in K\polyx\}$ ist eine Basis der Zariski-Topologie.
\end{db}
\begin{proof}
...
\begin{dfn}\label{1.5.3}
  ...
  \item\Label{1.5.3a} Sei $p\in U$. Eine Funktion $r\colon U\ra\A^1(K)$ heißt \emph{regulär in $p$\index{regulär}}, wenn es eine Umgebung $U_p$ von $p$ gibt
    und $f_p,g_p\in K[V]$ mit $g_p(x)\neq0$ für alle $x\in U_p$, sodass\begin{equation*}r(x)=\frac{f_p(x)}{g_p(x)}\text{ für alle }x\in U_p\text{ gilt.}\end{equation*} Die Funktion $r$ heißt \emph{regulär\index{regulär}}, wenn sie für alle $p\in U$ regulär ist.
  \item\Label{1.5.3b} Die Menge \begin{equation*}\O_V(U) = \{ r\colon U\ra \A^1(K) \mid r \text{ regulär}\}\end{equation*} heißt \emph{$K$-Algebra der regulären
...
\begin{bem}[Restriktion der regulären Funktionen]\label{1.5.5}
  ...
  \begin{enumerate}
  \item\Label{1.5.5a} Für in $V$ offene Mengen $U''\subseteq U'\subseteq U$ gilt $\rho^{U}_{U''}=\rho^{U'}_{U''}\circ\rho^{U}_{U'}$.
  \item\Label{1.5.5b} Ist $U$ offen in $V$ und $(U_i)_{i\in I}$ eine offene Überdeckung von $U$, so gilt
    \begin{enumerate}
    \item\Label{1.5.5bi} Für alle $f\in\O_V(U)$ gilt $f=0$ genau dann wenn $\rho^{U}_{U_i}(f)=0$ für alle $i\in I$.
...
\begin{dfn}\label{1.5.6}
  ...
  \item Sei $X$ ein topologischer Raum. Für jede offene Menge $U\subseteq X$ sei eine $K$-Algebra $\O(U)$ gegeben. Weiter sei
    ...
    $\{\O(U),\rho^{U}_{U'}\}_{U'\subseteq U\text{ offen}}$ eine \emph{Garbe von $K$-Algebren\index{Garbe}} auf $X$ und die $\rho^{U}_{U'}$ heißen
    \emph{Restriktionshomomorphismen\index{Restriktionshomomorphismus}}.
...
\begin{bem}\label{1.5.9}
  ...
  \item\Label{1.5.9ii} Offene Teilmengen $U$ von $V$ sind \emph{quasikompakt\index{quasikompakt}}, d.h. jede offene Überdeckung besitzt eine endliche
    Teilüberdeckung.
...
\begin{bem}\label{1.5.13}
  ...
  \item\Label{1.5.13b} (\enquote{Identitätssatz}) Sind $U_{1}, U_{2}\subseteq V$ offen und $r_1\in\O_V(U_1)$, $r_2\in\O_V(U_2)$ und gibt es ein $\schlange{U}\subseteq
...
\begin{bsp}\label{1.5.15}
  ...
  \item\Label{1.5.15ii} $\A^1(K)\setminus\{0\}=\D(X)$ ist nach \ref{1.5.15i} affin als abstrakte Varietät.
...
\begin{db}\label{1.6.5} Seien $V\subseteq \A^n(K)$, $W\subseteq \A^m(K)$ affine Varietäten.
  \begin{enumerate}
  \item\label{1.6.5a} Eine \emph{rationale Abbildung\index{rationale Abbildung}} $f \colon V\ppf W$ ist eine Äquivalenzklasse $(U,f_U)$, wobei die $U$ offen und dicht in $V$ liegen und die $f_U \colon U\ra W$ reguläre Abbildungen sind. Dabei ist
...
\begin{equation*}\alpha \colon K[W] \ra \O(U)\cong K[Z].\end{equation*} 
...
\begin{proof}
  ...
  \begin{equation*}p=(x_1,\dotsc,x_n)\mapsto \m_p=\I(\{p\})=(X_1-x_1,\dotsc,X_n-x_n).\end{equation*} 
  Für die andere sei $\m$ ein maximales Ideal in $K[V]$, dann gibt es einen Isomorphismus 
  \begin{equation*}\alpha\colon\Quotient{K\polyx}{\m}\ra K\end{equation*}
   und wir wählen $p=(\alpha(\Bar{X_1}),\dotsc,\alpha(\Bar{X_n}))$.
  \end{enumerate}
\end{proof}
...
\begin{bem}\label{1.7.9}
  ...
  \begin{enumerate}
  \item\Label{1.7.9i} $\alpha$ induziert eine stetige Abbildung \begin{equation*} f_\alpha\colon \Spec R'\ra\Spec R,\quad
...
%
    ...
    allen $R_{f_i}$ gilt. Da $g_i=g_j$ in $R_{f_if_j}$, gibt es ein genügend großes $N$, für das \begin{equation*}g_if_i^Nf_j^N=g_jf_i^Nf_j^N\end{equation*} gilt,
    und das ohne Einschränkung nicht von $i$ und $j$ abhängt. Insbesondere gibt es wie gerade eben ein $w\in\N$ mit
    \begin{equation*}f^w\in(f_1^N,\dotsc,f_m^N)\text{ und }f^w=\sum_{i=1}^ma_if_i^N.\end{equation*} 
    Wir definieren 
    \begin{equation*}g=\frac{1}{f^w}\sum_{i=1}^ma_if_i^Ng_i\in R_f.\end{equation*} 
    Dann gilt in $R_{f_i}$
    \begin{equation*} gf_j^N=\frac{1}{f^w}\sum_{i=1}^ma_if_i^Nf_j^Ng_j=\frac{1}{f^w}f^wg_jf_j^N=g_jf_j^N, \end{equation*}
...
\begin{equation*} \P^{n}(k):=\displaystyle\Quotient{k^{n+1}\!\setminus\!\{0\}}{\sim}\:, \end{equation*} wobei
...
\begin{equation*}(x_{0}:x_{1}:\dotsm:x_{n})\end{equation*}
...
\begin{equation*}\P^{n}(k)\ra k,\quad x=(x_{0}:\dotsm:x_{n})\mapsto f(x_{0},\dotsc,x_{n})\end{equation*}
...
\begin{equation*}\forall\;\lambda\in k^{\times}\colon  f(x_{0},\dotsc,x_{n})=0\iff f(\lambda x_{0},\dotsc,\lambda x_{n})=0.\end{equation*}
Das erfüllen gerade die homogenen Polynome.
...
%%%%%%%%%%%%%%%%%%%%%%%%%
\item\Label{2.3.1c} Die Voraussetzung \enquote{unendlich} kann in \ref{2.3.1b} nicht weggelassen werden: Seien $k=\FF_{3}$ und $f(X)=X^{3}+X$. Dann ist $f$ nicht homogen, aber es gilt
...
\begin{equation*}S_{d}:=\{f\in k\ppolyx\mid f\text{ homogen von Grad }d\},\end{equation*}
...
\item\Label{2.3.4b} Ein Ideal $I$ heißt \emph{homogen\index{homogen}}, wenn es von homogenen Elementen (nicht notwendigerweise von gleichem Grad) erzeugt wird.
\item\Label{2.3.4c} Die Summe, das Produkt, der Durchschnitt und die Radikale von homogenen Idealen sind wieder homogen.
\item\Label{2.3.4d} Es gilt: $I$ ist genau dann homogen, wenn
\vspace*{-10pt}
\begin{equation*}\forall f\in I\colon  f=\sum_{i=0}^{d}f_{i}\text{ mit }f_{i}\in S_{i}\implies f_{i}\in I.\end{equation*}
\end{enumerate}\end{erinnerung}
\begin{proof}Als Beispiel beweisen wir, dass das Radikal eines homogenen Ideals wieder homogen ist. Für die restlichen Beweise verweisen wir auf Algebra {\scshape ii}.

Sei also $I$ ein homogenes Ideal und $f\in\sqrt{I}$. Seien $f_{d}\in S_{d}$ mit $f=\sum_{d=0}^{n}f_{d}$. Dann gibt es ein $m\in\N$, so dass $f^{m}\in I$ und wir sehen:
\begin{equation*}f^{m}=f_{n}^{m}+\text{Terme von kleinerem Grad.}\end{equation*}
...
\item\Label{2.3.5a} Sei $V\subseteq\P^{n}(k)$. Dann nennen wir \begin{equation*}\I(V)=\langle\{f\in k\ppolyx\mid f\text{ homogen},f(x)=0\;\forall x\in V\}\rangle\end{equation*} das \emph{Verschwindungsideal von $V$\index{Verschwindungsideal}}.
\item\Label{2.3.5b} Für ein homogenes Ideal $I$ heißt
\begin{equation*}\V(I):=\{x\in\P^{n}(k)\mid f(x)=0\text{ für alle homogenen }f\in I\}\end{equation*}
...
\item\Label{2.3.9b} Sei $F=\H(f)$. Dann ist, nach Definition, $F$ homogen und es gilt $\deg F=d=\deg f$. Außerdem gilt:\vspace*{-6pt}
...
\item\Label{2.3.9c} $\DD$ ist ein $k$-Algebrenhomomorphismus und es gilt:
\begin{align*}
...
=\H(f)+\sum_{i=0}^{d}g_{i}X_{0}^{d-i}=\H(f)+\sum_{i=0}^{e}g_{i}X_{0}^{d-i}=\H(f)+X_{0}^{d-e}\cdot \H(g),\end{equation*}%\vspace*{-6pt}
...
\begin{equation*}\dach{W}=\bigcup_{\lambda\in\Lambda}U_{\lambda}\text{ mit }U_{\lambda}\subseteq \U_{i}=\{(x_{0}:\dotsm:x_{n})\in\P^{n}(k)\mid x_{i}\neq 0\},\end{equation*}
...
\begin{equation*}\dach{W}=\bigcup_{i=0}^{n}\dach{W}\cap \U_{i}.\end{equation*}
...
\begin{equation*}W=\bigcup_{i=0}^{n}\bigcup_{\lambda\in\Lambda}U_{\lambda}\cap \U_{i}.\end{equation*}
...
\begin{equation*}X_{0}^{\alpha_{0}}\dotsm X_{n}^{\alpha_{n}}\cdot f\in K[V]_{d}\cdot f\end{equation*}
und sogar in $K[V]$, denn $f=\frac{G_{i}}{X_{i}^{d_{i}}}$ und es gibt sicherlich ein $i$ mit $\alpha_{i}\geq d_{i}$. Damit sehen wir
\begin{equation*}X_{i}^{\alpha_{i}}\cdot f=X_{i}^{\alpha_{i}}\cdot\frac{g_{i}}{X_{i}^{d_{i}}}=X_{i}^{\alpha_{i}-d_{i}}\cdot g_{i}\end{equation*}
und $X_{i}^{\alpha_{i}-d_{i}}g_{i}$ hat gerade Grad $\alpha_{i}$, insgesamt hat der Ausdruck also wieder Grad $d$, wir erhalten also
\begin{equation*}K[V]_{d}\cdot f\subseteq K[V]_{d}\end{equation*}
...
\begin{equation*}f^{t}\in\frac{1}{X_{0}^{d}}\cdot K[V].\end{equation*}
...
Seien $W\subseteq \P^n(k)$, $W'\subseteq \P^m(k)$ quasi-projektive Varietäten.
  \begin{enumerate}
  \item\Label{2.6.1a} Eine Abbildung $f\colon W\ra W'$ heißt \emph{Morphismus\index{Morphismus}}, wenn es zu jedem $z \in W$, eine offene Umgebung $U_z \subseteq W$ von $z$ und Polynome $F_0,\dotsc,F_m \in k\ppolyx$ vom gleichen Grad gibt, so dass für alle $y \in U_z$ gilt:
...
%
\begin{equation*}f\text{ ist ein Morphismus}\iff\! %
\begin{minipage}[c]{\breite}\begin{center}
$f$ ist stetig und für offenes $U\subseteq W', g\in \O(U)$ gilt:\\
$g\circ f \in \O(f^{-1}(U)).$%
\end{center}\end{minipage}
\end{equation*}
  \item\Label{2.6.2b} Seien $W_{1}$ und $W_{2}$ affine Varietäten. Durch die Einbettungen 
  \begin{align*}W_1\subseteq \A^n(k)& \inj \U_0\subseteq \P^n(k)\\
  W_2\subseteq \A^m(k)& \inj \U_0\subseteq \P^m(k)\end{align*}
können wir sie als quasi-projektive Varietäten auffassen. Dann ist $f\colon W_{1}\ra W_{2}$ genau dann ein Morphismus im Sinn von \cref{2.6.1}, wenn $f$ auch ein Morphismus im Sinn von \cref{1.4.1} aus \cref{kap1} ist.
  \end{enumerate}
\end{kor}
...
\begin{db}\label{2.6.6} Seien $W_1$, $W_2$ quasi-projektive Varietäten.
  ...
  \item\Label{2.6.6c} Die Zuordnung $W\mapsto K(W)=\Rat(W)$  definiert eine kontravariante Äquivalenz von Kategorien.
    \begin{equation*}\left\{%
    \begin{minipage}[c]{\breite}\begin{center}%
...
\begin{proof}
\begin{enumerate}
  \item[\ref{2.6.6c}] folgt aus dem Affinen (vgl. \cref{satz4}) beziehungsweise \cref{2.6.5}.
...
\begin{equation*}\Phi(r\colon W_1\ppf W_2)=\begin{cases} K(W_2)\ra K(W_1)\\g\mapsto g\circ r \end{cases}\end{equation*} Nach der Überlegung zu Beginn des Beweises entsprechen die rationalen Abbildungen zwischen $W_1$ und $W_2$ bijektiv den rationalen Abbildungen zwischen den entsprechenden 
...
\item $d=1$: $G(1,n)$ entspricht dem $\P^{n-1}(k)$.
...
\begin{equation*}e_{1}=v_{1},\dotsc,e_{k}=v_{k}.\end{equation*}
Für $w$ finden wir also
\begin{equation*}w=\!\!\!\!\!\!\!\!\!\!\sum_{\substack{1\leq i_{1}<\dotsm<i_{d}\leq n\\\iq=(i_{1},\olddotsc,i_{d})}}\!\!\!\!\!\!\!\!\! \lambda_{\iq}e_{i_{1}}\wedge\dotsm\wedge e_{i_{d}}.\end{equation*}
Für $j\in\{1,\dotsc,k\}$ gilt dann $e_{j}\in\Kern\varphi_{w}$, also:
\begin{equation*}0=w\wedge e_{j}=\sum_{\iq}\lambda_{\iq}e_{i_{1}}\wedge\dotsm\wedge e_{i_{d}}\wedge e_{j}=\!\!\!\!\sum_{\substack{\iq\\ j\notin\{i_{1},\olddotsc,i_{d}\}}}\!\!\!\!\!\lambda_{\iq}e_{i_{1}}\wedge\dotsm\wedge e_{i_{d}}\wedge e_{j}\end{equation*}
...
\begin{equation*}w=\sum_{\iq}\lambda_{\iq}e_{1}\wedge\dotsm\wedge e_{k}\wedge e_{i_{k+1}}\wedge\dotsm\wedge e_{i_{d}}=(e_{1}\wedge\dotsm\wedge e_{k})\wedge\biggl(\sum_{\iq}\lambda_{\iq}e_{i_{k+1}}\wedge\dotsm\wedge e_{i_{d}}\biggr)\end{equation*}
...
\begin{equation*}\S_{d}:=\{e_{i_{1}}\wedge\dotsm\wedge e_{i_{d}}\mid 1\leq i_{1}<\dotsm<i_{d}\leq n\}\end{equation*}
als zugehörige Basis von $\bigwedge^{d}V$. Sei wieder, für $w\in\bigwedge^{d}V$,
\begin{equation*}\varphi_{w}\colon V\ra\bigwedge^{d+1}V,\quad v\mapsto w\wedge v,\end{equation*}
...
\begin{equation*}w\in\Bild\Psi\!\iff\! \dim(\Kern\varphi_{w})\geq d\iff\Rang\varphi_{w}\leq n-d\iff\det(l_{ij}(w))_{I,J}=0,\end{equation*}
...
\begin{dfn}\label{3.1.1}
  ...
  \item\Label{3.1.1i} Es sei
    \begin{equation*} \O_{W,p} = \Quotient{\{ (U,f) \mid p\in U\subseteq W\text{ offen, } f\in\O_{W}(U) \}}{\sim}, \end{equation*}
...
\begin{bem}\label{3.1.3}
  \begin{enumerate}
  \item\Label{3.1.3i} Die Abbildung \begin{equation*} \phi_{p}\colon \O_{W,p}\ra K,\quad f_p=[(U,f)]\mapsto f_p(p):=f(p), \end{equation*}
    ist ein wohldefinierter $K$-Algebrenhomomorphismus und heißt \emph{Auswertung\index{Auswertung}}.
  \item\Label{3.1.3ii} $\O_{W,p}$ ist ein lokaler Ring mit maximalem Ideal \begin{equation*} \m_p = \{ [(U,f)]\in\O_{W,p}\mid f(p)=0\}. \end{equation*}
  \end{enumerate}
\end{bem}
\begin{proof}
  \begin{enumerate}
...
\begin{bem}\label{3.1.4}
  Die Abbildung \begin{equation*} \psi_p\colon \O_W(U)\ra\O_{W,p},\quad f\mapsto[(U,f)]=f_p, \end{equation*} heißt \emph{kanonische Keim-Abbildung\index{kanonische Keim-Abbildung}}.
...
\begin{proof}
  ...
  \item Wir haben jetzt $p\in U_1\subseteq W_1$, $q\in U_2\subseteq W_2$ und ein kommutatives Diagramm
    ...
    Abbildungen \begin{equation*} f\circ g\colon \dach{U_1}\ra U_1,\qquad g\circ f\colon\dach{U_2}\ra \schlange{U_2}. \end{equation*}
    ...
    \begin{equation*}f\circ g(\dach{U_1})\subseteq \dach{U_1},\text{ also }g(\dach{U_1})\subseteq
    f^{-1}(\dach{U_1})=\dach{U_2},\end{equation*} 
...
\begin{bem}\label{3.1.8}
  ...
  \item\Label{3.1.8i} Morphismen von Varietäten induzieren Homomorphismen zwischen den lokalen Ringen: Sei $\phi\colon W_1\ra
    ...
    \begin{equation*} \phi_x^\sharp\colon \O_{W_2,\phi(x)}\ra\O_{W_1,x},\quad [(U,f)] \mapsto\relax [(\phi^{-1}(U),f\circ \phi)], \end{equation*}
...
\sect{Dimension von Varietäten}
\label{3.2}

{\scshape Wünsche:} \begin{itemize}
 \item $\dim \A^n=n$
 \item $\dim \V(XZ,YZ)=2$ im $\A^{3}$ (\enquote{Ebene})
 \item $\dim \V(X^2+Y^2-Z^2)=2$ im $\A^{3}$
 \item $\dim \V(X^2+Y^2-1)=1$ im $\A^{2}$ (\enquote{Kreis})
\end{itemize}

\begin{dfn}\label{3.2.1}
Sei $X$ ein topologischer Raum. Dann heißt 
\begin{equation*}\dim(X):=\sup \{d\in \N_0 \mid \exists \text{ Kette } \leer\neq X_0 \subsetneq  \dotsm \subsetneq X_d \text{ mit $X_i$ abg. und irred. in $X$}\}\end{equation*}
...
\end{w}
% 
\begin{equation*}\text{Ab jetzt: Mit Dimension ist stets die Krulldimension gemeint!}\end{equation*}
...
\begin{enumerate}
  ...
  \begin{equation*}\dim(X)=\sup_{i} \dim(X_i).\end{equation*}
\end{enumerate}
\end{bem}
...
\begin{equation*}\text{Dann gilt: }\dim \V(f)=n-1.\end{equation*}
...
Nach \cref{3.2.2} ist also $\dim V=2$.
  ...
  \item Genauso ist $\dim \V(X^2+Y^2-1)=1$, da Hyperfläche.
   ...
   Die Wünsche, die zu Beginn des \hyperref[3.2]{Abschnitts} geäußert wurden, sind also erfüllt.
  \end{enumerate}
\end{bsp}
...
\begin{nbsp}
\begin{enumerate}
\item Wir betrachten die elliptische Kurve $V=\V(Y^{2}-X^{3}+X)$ und $P=(0,0)$. Die Tangente an $V$ in $P$ ist die $Y$-Achse, also $\V(X)$.
% hier bräuchte man ein schönes Bild von einer elliptischen Kurve...
\item Nun betrachten wir den Newton-Knoten $V=\V(Y^{2}-X^{3}+X^{2})$. Dann gibt es in $P=(0,0)$ zwei \enquote{Tangenten}. Aber wie könnte der Tangentialraum aussehen?
% hübsches Bild vom Newtonknoten
\item Jetzt die Neilsche Parabel: $V=\V(Y^{2}-X^{3})$. Dann ist die $X$-Achse \enquote{doppelte Tangente} in $P=(0,0)$, aber auch hier ist der Tangentialraum nicht intuitiv klar.
% neilsche Parabel....
\end{enumerate}\end{nbsp}
...
\item\Label{3.3.2b} Sei $V\subseteq\A^{n}(K)$ eine affine Varietät, $p\in V$ und $I:=\I(V)$. Seien zusätzlich
...
\item\Label{3.3.4c} Sei $V\subseteq\A^{n}(K)$ eine affine Varietät, $I=\I(V)=\langle f_{1},\dotsc,f_{r}\rangle$ und $p\in V$. Dann ist
\begin{itemize}
...
\item\Label{3.3.7b} $\DD$ ist bereits durch $\DD\restrict{\m_{p}}$ für das maximale Ideal $\m_{p}=\{f\mid f(p)=0\}$ festgelegt, denn für $c$ konstant ist $\DD(c)=0$, also gilt, für beliebiges $f\in\O_{V,p}$, $\schlange{f}:=f-f(p)$ ist in $\m_{p}$ und $\DD(\schlange{f})=\DD(f)$.
...
\item\Label{3.3.7d} Jedes solche $\l$ kommt von einer Derivation her. Dazu definieren wir \begin{equation*}\DD(f_{p}):=\l(f_{p}-f_{p}(p)).\end{equation*}
%
...
\item\Label{3.3.11a} Sei $V=\V(X^{2}+Y^{2}-Z^{2})\subseteq\A^{3}$. Dann ist $\dim V=\dim_{p}V$ für alle Punkte $v\in V$. Für die Jacobi-Matrix gilt:
...
\begin{equation*}\Sing W=\{p \in W \mid p \text{ singulär}\}\end{equation*} der \emph{singuläre Ort\index{singulärer Ort}}.
\begin{equation*}\Reg W=\{p \in W \mid p \text{ regulär}\}=W\setminus \Sing W\end{equation*} 
heißt \emph{regulärer Ort\index{regulärer Ort}}.

Wenn $\Sing W$ leer ist, nennen wir $W$ \emph{regulär\index{regulär (Varietät)}} oder \emph{nicht-singulär}.
\end{dfn}

\begin{db}\label{3.4.2}
  \begin{enumerate}
  \item\Label{3.4.2a} Ein noetherscher, lokaler Ring $R$ mit maximalem Ideal $\m$ heißt \emph{regulär\index{regulär (Ring)}}, wenn 
  $\dim_k \Quotient{\m}{\m^2}=\dim R$. Dabei ist $k=\Quotient{R}{\m}$.
  \item\Label{3.4.2b} Sei $W$ quasi-projektive Varietät, $p \in W$. Dann gilt:
  \begin{equation*}p \text{ ist regulär} \iff \O_{W,p} \text{ ist regulär}.\end{equation*}
  \end{enumerate}
\end{db}
...
\begin{lem}\label{3.4.3} Sei $(R,\m)$ ein lokaler, noetherscher Ring.
  ...
  \end{enumerate}
\end{lem}
\begin{proof}siehe \cref{3.5.4} und \cref{3.5.5}.
\end{proof}
...
\begin{ziel} Sei $I=(f_1,\dotsc,f_k)$, $\hoehe I:=\{\inf \hoehe\wp \mid \wp \supseteq I, \wp$ ist Primideal\}.
...
Sei $R$ noethersch, $q \in \Spec R$, $x\in q$, $\hoehe q \ge l \ge 1$. Dann existiert eine Primidealkette
\begin{equation*}x\in \wp_0'\subsetneq\dotsm \subsetneq \wp_{l-1}'=q.\end{equation*}
\end{lem}
...
\begin{enumerate}
  \item\Label{3.5.4a} Für $m_1,\dotsc,m_n\in\m$ gilt: 
  \begin{equation*}\begin{minipage}[c]{\breite}\begin{center}%
    $\{m_1,\dotsc,m_n\}$ ist minimales\\
    Erzeugendensystem von $\m$%
  \end{center}\end{minipage}%
    \!\!\!\!\!\iff \{\Bar{m_1},\dotsc,\Bar{m_n}\} \text{ ist Basis von }\Quotient{\m}{\m^2}.\end{equation*}
  \item\Label{3.5.4b} Alle minimalen Erzeugendensysteme von $\m$ haben dieselbe Anzahl: $\dim_k \Quotient{\m}{\m^2}$.
  \item\Label{3.5.4c} Es ist $\dim_k \Quotient{\m}{\m^2}\ge \dim R$.
\end{enumerate}
\end{lem}
\begin{proof}
\begin{enumerate}
  \item[\ref{3.5.4a}] Sei zuerst $ \langle m_1,\dotsc,m_n\rangle =\m$. Betrachte die Projektionen 
  \begin{equation*}\pi_1\colon R\ra k=\Quotient{R}{\m},\quad \pi_2\colon \m\ra \Quotient{\m}{\m^2}.\end{equation*}
  Dann ist $\Bar{r m_1}:=\pi_1(r)\pi_2(\m)$ wohldefiniert und $\{\Bar{m_1},\dotsc,\Bar{m_n}\}$ ist ein Erzeugendensystem von $\Quotient{\m}{\m^2}$. Da $\Quotient{\m}{\m^{2}}$ ein $k$-Vektorraum ist, ist $\{\Bar{m_{1}},\dotsc,\Bar{m_{n}}\}$ also -- wie gewünscht -- eine Basis.
  ...
  \begin{equation*}\hoehe \m\le \card{\{\text{minimales Erzeugendensystem von }\m\}}=\dim_{k} \Quotient{\m}{\m^2}.\end{equation*}
\end{enumerate} 
\end{proof}
...
Sei $\CC$ immer eine reguläre, projektive, zusammenhängende Kurve über einem algebraisch abgeschlossenen Körper $K$.
...
\begin{equation*}D=\sum_{i=1}^{k}n_{i}P_{i}\end{equation*}
mit $k\in\N$, $n_{i}\in\Z$ und $P_{i}\in \CC$. Die Menge
\begin{equation*}\Div\CC:=\{D\mid D\text{ ist Divisor auf }\CC\}\end{equation*}
...
Sind zwei zusammenhängende reguläre projektive Kurven $\CC_{1}$ und $\CC_{2}$ birational äquivalent, so sind sie bereits isomorph.
\end{kor}
...
In diesem Abschnitt seien $\CC_{1}$ und $\CC_{2}$ reguläre projektive zusammenhängende Kurven und $f\colon\CC_{1}\surj\CC_{2}$ ein surjektiver Morphismus.

\begin{db}\label{4.2.1}
\begin{enumerate}
\item\Label{4.2.1a} Sei $Q\in\CC_{2}$, $P\in f^{-1}(Q)$ und $t$ Uniformisierende in $Q$, also $\m_{Q}=\langle t\rangle$. Dann heißt
\begin{equation*}e_{P}:=e_{P}(f):=\ord_{P}(t\circ f)=\nu_{P}(t\circ f)\end{equation*}
\emph{Verzweigungsgrad von $f$ in $P$\index{Verzweigungsgrad}}.
\item\Label{4.2.1b} Wir definieren einen Gruppenhomomorphismus $f^{*}\colon\Div\CC_{2}\ra\Div\CC_{1}$ durch
\begin{equation*}Q\mapsto\sum_{P\in f^{-1}(Q)}\!e_{P}\cdot P.\end{equation*}
...
\begin{bem}[ohne Beweis]\label{4.2.5}
...
\begin{lem}\label{4.2.8} Seien $P_{i}\in\CC_{1}$ und $\schlange{\O}:=\displaystyle \bigcap_{i=1}^{k} \O_{P_i} \subseteq K(\CC_1)$. Dann gilt:
  ...
  \item\Label{4.2.8c} Jedes $v\in \schlange{\O}$ lässt sich eindeutig schreiben als \begin{equation*}v=u\cdot t_1^{e_1}\dotsm t_k^{e_k}\end{equation*} mit $u\in \schlange{\O}^{\times}$, $e_i=\nu_{P_i} (v)$.
  \end{enumerate}
\end{lem}
\begin{proof} \begin{enumerate}
...
\end{enumerate}
\end{proof}
\begin{proof}[Beweis von \cref{s7a}] \hypertarget{bews7}{Zu} zeigen ist: \begin{equation*}n=\deg (f)=\sum_{i=1}^{k} e_{P_i} = \sum_{i=1}^{k} \ord_{P_i} (t\circ f),\end{equation*} wobei $(t)=\m_Q$.
...
\begin{equation*}\Quotient{\schlange{\O}}{(t\circ f)}\cong \bigoplus_{i=1}^{k} \Quotient{\schlange{\O}}{(t_i^{e_{P_i}})}\cong \bigoplus_{i=1}^{k} K^{e_{P_i}}.\end{equation*}
Also ist $\dim_K \Quotient{\schlange{\O}}{(t\circ f)}=\displaystyle \sum_{i=1}^{k} e_{P_i}$.

Andererseits ist $\schlange{\O} \cong \O_Q^n\ra (\Quotient{\O_a}{(t)})^n\cong K^n$. 

Damit gilt $\Quotient{\schlange{\O}}{(t\cdot \schlange{\O})}=\Quotient{\schlange{\O}}{(t\circ f)}\cong K^n$ und damit ist $\dim_K \Quotient{\schlange{\O}}{(t\circ f)}=n$.
\end{proof}
%7.2.11
...
\begin{equation*}\L(D):=\{f\in K(\CC)^{\times}\mid D+\div(f)\text{ ist \hyperref[4.1.1c]{effektiv}}\}\cup\{0\}\end{equation*}
den \emph{Riemann-Roch-Raum von $D$\index{Riemann-Roch-Raum}}. $\L(D)$ ist ein $K$-Vektorraum, da für $P\in\CC$ immer
\begin{equation*}\ord_{P}(f+g)\geq\min\{\ord_{P}(f),\ord_{P}(g)\}\text{ gilt.}\end{equation*}
...
\item\Label{s8b} Es gibt ein $\gamma\in\N_{0}$, so dass für alle $D\in\Div\CC$
...
\begin{equation*}\div g_{i}+\gamma_{0}\cdot N\geq 0\end{equation*}
gilt. Damit zeigen wir nun, dass $\ll(k\cdot N)\geq \deg(k\cdot N)-r\cdot(\gamma_{0}-1)$, wir also \begin{equation*}\gamma:=r(\gamma_{0}-1)+1\end{equation*} 
finden. Sei dazu $h_{ij}:=\frac{g_{i}}{f^{j}}$, für $j\in\{0,\dotsc,k\}$. Damit gilt
\begin{equation*}\div h_{ij} + (k+\gamma_{0})\cdot N=\div g_{i}-j\cdot \div f +k\cdot N +\gamma_{0}\cdot N\geq (k-j)\cdot N\geq 0,\end{equation*}
...
\begin{equation*}\ll((k+\gamma_{0})\cdot N)\geq r\cdot (k+1).\end{equation*}
Wenn wir \cref{4.3.5} iterativ anwenden, sehen wir, \hyperlink{bews8e3}{ähnlich wie oben}, dass
\begin{equation*}\ll(k\cdot N)\geq\ll((k+\gamma_{0})\cdot N)-\deg(\gamma_{0}\cdot N)\geq r\cdot (k+1)-\gamma_{0}r=kr-r\cdot(\gamma_{0}-1),\end{equation*}
...
\begin{db}\label{4.4.2}  Sei $\omega \in \Omega_{\CC}$, $\omega\neq 0$. 
  \begin{enumerate}
  \item\Label{4.4.2a} Sei $P\in \CC$, $t_P$ uniformisierend. Dann ist $\omega=f\cdot\d t_P$ und \begin{equation*}\ord_P (\omega):=\ord_P (f)\end{equation*} ist wohldefiniert.
...
\begin{kor}\label{4.4.3} \begin{enumerate}
  \item\Label{4.4.3a} Ist $\K$ ein kanonischer Divisor sowie $D=0$, so ist $\deg D=0$ und $\ll(D)=1$ nach \cref{4.3.2a}. Also ist in diesem Fall $\ll(\K)=\g$.
  \item\Label{4.4.3b} Ist $D=\K$ ein kanonischer Divisor, so ist $\ll(\K)=\g$ nach \ref{4.4.3a}. Also gilt hier \begin{equation*}\deg (\K)=\g-1+\ll(D)-\ll(0)=2\g-2.\end{equation*}
 \end{enumerate}
\end{kor}
