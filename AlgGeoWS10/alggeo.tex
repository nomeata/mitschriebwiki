...
\item\Label{3.3.7b} $\DD$ ist bereits durch $\DD\restrict{\m_{p}}$ für das maximale Ideal $\m_{p}=\{f\mid f(p)=0\}$ festgelegt, denn für $k$ konstant ist $\DD(k)=0$, also gilt, für beliebiges $f\in\O_{V,p}$, $\tilde{f}:=f-f(p)$ ist in $\m_{p}$ und $\DD(\tilde{f})=\DD(f)$.
