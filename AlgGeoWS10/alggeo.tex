...
Sei $1\leq d\leq n$, $V=K^{n}$, $\Lambda^{d}V$ die \emph{$d$-te äußere Potenz} und
...
\[\Psi\colon G(d,n)\ra\P(\Lambda^{d}V),\quad U=[u_{1},\dotsc,u_{d}]\mapsto[u_{1}\wedge\dotsm\wedge u_{d}]\]
wohldefiniert und injektiv.
\end{db}

\begin{nerinnerung}\begin{itemize}[leftmargin=*,labelindent=\parindent]
\item $\Lambda^{d}V$ hat die Basis $\{e_{i_{1}}\wedge\dotsm\wedge e_{i_{d}}\mid 1\leq i_{1}<\dotsm<i_{d}\leq n\}$.
Insbesondere ist $\dim\Lambda^{d}V=\binom{n}{d}$.
%
\item $\Lambda$ ist multilinear und alternierend, insbesondere gilt $v_{1}\wedge\dotsm\wedge v_{d}=0$, wenn $v_{i}=v_{j}$ für $i\neq j$. Genauer:
