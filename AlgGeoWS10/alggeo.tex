...
und $\displaystyle\biggl(\sum_{\iq}\lambda_{\iq}e_{i_{k+1}}\wedge\dotsm\wedge e_{i_{d}}\biggr)$ liegt in $\bigwedge^{d-k}$, ist also eine geeignete Wahl für $w'$.
\end{proof}
...
\[\S_{d}:=\{e_{i_{1}},\dotsc,e_{i_{d}}\mid 1\leq i_{1}<\dotsm<i_{d}\leq n\}\]
als zugehörige Basis von $\P(\bigwedge^{d}V)$. Sei wieder
\[\varphi_{w}\colon V\ra\bigwedge^{d+1}V,\quad v\mapsto w\wedge v\]
und $\L_{w}:=(l_{ij}(w))_{i,j}$ die Abbildungsmatrix von $\varphi_{w}$ bzgl. $\S_{d}$ und $\S_{d+1}$. Außerdem sind die $l_{ij}$ linear in $w$.

Aus \cref{2.7.3} folgt, dass
\[w\in\Bild\Psi\iff \dim(\Kern\varphi_{w})=d\iff\Rang\varphi_{w}=n-d\iff\det(l_{ij}(w))_{I,J}=0,\]
für alle $\card{I}=\card{J}=n-d+1$, also alle $n-d+1$-Minoren der Matrix $0$ sind.
...
Sei $R$ ein kommutativer Ring mit $1$.
  ...
  \[ \hoehe(\wp)=\sup \{n\in \N_0 \mid p_0 \varsubsetneq \dotsm \varsubsetneq p_n=\wp \text{ Kette von Primidealen}\}.\]
