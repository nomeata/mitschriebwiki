...
\item Somit ist $v$ nun aus $A=K[V]$ und damit ganz über $B$. Also gibt es $b_0,\dotsc,b_{n-1} \in B$ mit \[v^n+b_{n-1}v^{n-1}+\dotsm+b_0=0,\]d.h. $v=-b_{n-1}-\frac{b_{n-2}}{v}-\dotsm-\frac{b_0}{v^{n-1}}$.
...
\begin{lem}\label{4.2.8} Sei $\schlange{\O}:=\displaystyle \bigcap_{i=1}^{k} \O_{P_i} \subseteq K(\CC_1)$. Dann gilt:
  ...
  \item\Label{4.2.8b} Es gibt $t_1\dotsc,t_k \in \schlange{\O}$ mit $\ord_{P_i} (t_j)=\delta{ij}$.
...
Wie im Beweis von \cref{4.2.6a} kann $\schlange{t_i}$ ohne Einschränkung als regulär vorrausgesetzt werden.
...
Also ist $v_{P_1}(t_1)=1$, d.h.$t_1$ tut Gewünschtes. Analog konstruiert man $t_2,\dotsc,t_k$.
  \item[\ref{4.2.8c}] folgt aus \ref{4.2.8b}, denn setze \[u=\frac{v}{t_1^{\ord P_1(v)}\dotsm t_k^{\ord P_k(v)}}.\] Dann ist für jedes i $\ord P_i(u)=0$, also ist $u\in \schlange{\O}$.
