...
\[f(y)=(F_0(y),\dotsc,F_n(y))\]
  \item\Label{2.6.1b} Betrachte $f \colon \P^n(k)\supseteq W \ra W' \subseteq \P^n(k)$. Das liefert eine Abbildung $\A^n(k)\supseteq f^{-1}(W'\cup U_j) \cup U_i \ra W' \cup U_j$. 

Jetzt gilt:
$f$ ist genau dann ein Morphismus, wenn für alle $i,j$ mit $U_{ij}=f^{-1}(W'\cup U_j) \cup U_i \neq \leer$ die Abbildung $f\restrict{U_{ij}} \colon U_{ij} \ra W'\cup U_j$ ein Morphismus von quasi-affinen Varietäten ist (siehe \cref{1.5.14}).
...
Wir wissen: $\cup_{i=0}^{n}U_i=\P^n$, $\cup_{i=0}^{m}U_j=\P^m$, $f\colon \P^n \supseteq W\ra W'\subseteq \P^m$
Betrachte also
\[ f_{ij}=f\restrict{U_{ij}} \colon U_{ij}=f^{-1}(W'\cup U_0)\ra W'\cup U_j=W_j'\subseteq U_j\cong \A^m\]
...
Also ist $U_{ij}\cup U_z=\{w \in U_z\cup U_i \mid f(w) \in U_j\}=\{w \in U_z\cup U_i \mid F_j(w)\neq 0\}$ offen. Damit ist aber auch $U_{ij}$ offen.
