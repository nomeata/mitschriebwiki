...
Wie im Beweis von \cref{4.2.7} kann $\schlange{t_i}$ ohne Einschränkung als regulär vorrausgesetzt werden.

Um die $t_i$ zu konstruieren, wählen wir zuerst $g_i\in A=K[\schlange{V}]$ mit: \[g_i(P_i)\neq 0 \text{ und } g_i(P_j)=0\; (i,j\in \{1,\dotsc,k\}\] Sei $t_1=\schlange{t_1}+\displaystyle \sum_{j=2}^{k} \alpha_j \cdot g_j^2$ mit $\alpha_j \in K$, $\alpha_j \neq \frac
...
\begin{proof}\begin{enumerate}
\item[\ref{4.3.2a}] Nach \cref{satz5} sind die regulären Funktionen alle konstant.
...
\[\deg D+1-\gamma\leq\l(D).\]
\end{enumerate}\end{satz}

\begin{dfn}\label{4.3.3}
Das kleinste $\gamma$ für das \ref{s8b} erfüllt ist, nennen wir das \emph{Geschlecht von $\CC$}. Wir schreiben auch $\g(\CC)$ oder $\g$.
\end{dfn}
...
\[\div f=-D=\div g.\]
Damit ist auch $\div\bigl(\frac{f}{g}\bigr)=0$ und $\frac{f}{g}$ somit konstant, da 
\item[\ref{s8b}]
\end{enumerate}
