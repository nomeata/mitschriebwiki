...
%\chardef\hash=`\#
%\def\args{\begingroup\catcode`\$=6\relax\catcode`\#=11\relax#\endgroup}
...
\newcommand{\definetheorem}[3]{
  ...
  \BeforeBeginEnvironment{#1}{\makeatletter%
     ...
     %\expandafter\edef\csname ctr@#1@\roman{chapter}@\roman{section}@\arabic{thmcnt}@name \endcsname{#2~\thethmcnt}
     ...
     \immediate\write\blah{\string\crefname{\labelcode}{#2~\thethmcnt}{}}\relax%
    % links klappen leider nicht, weil sich # nicht in Datei schreiben lässt und sonst der counter sich nicht merken kann, wo er sein soll...
    % Möglichkeit: in Datei schreiben und manuell ## durch # ersetzen....
    %\immediate\write\blah{\noexpand\crefformat{\labelcode}{##2#2~\thethmcnt~##1##3}}%\endgroup%
    % hier nimmt thmcnt den Wert an, wo wir sind, nicht wo wir hinlinken:
    %\crefformat{\labelcode}{##2#2~\thethmcnt~##1##3}
