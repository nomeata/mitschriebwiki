...
\item[\ref{1.4.4b}] Das folgt direkt aus der \autoref{1.4.1}, da Polynome global definierbar. 
\end{enumerate}
\end{proof}
...
\item Sei $\phi: k[W]\ra k[V]$ ein $k$-Algebrenhomomorphismus. $p_i: W\ra k$ sei die Projektion auf die i-te Koordinate.
Definiere nun $f:V\ra W$ durch $x=(x_1,\dotsc,x_n)\mapsto (\phi(p_1)(x),\dotsc,\phi(p_m)(x)$.Zeige:
\begin{itemize}
...
Zeige also: Für alle $g \in k[W]$ gilt $g(f(x))=0$. Wir wissen, dass $g$ als Element in $k[W]$ 0 ist. Daraus folgt $\phi(g)=0$ in $k[V]$. Also folgt die Behauptung, da $g(\phi(p_1)(x),\dotsc,\phi(p_m)(x))=\dach\phi(g)(x)$ für $g \in \I(W)$, wobei $\dach\phi$ so gewählt ist, das folgendes Diagramm kommutiert.
\[\begin{tikzpicture}
...
\end{itemize}
Alternativ könnte man auch zeigen: Es existiert ein Funktor $\Psi: \RedAlg_k\ra \AffVar_k$ mit $\Psi\circ \Phi\cong \id_{\AffVar_k}$ und $\Phi\circ \Psi\cong \id_{\RedAlg_k}$.
Nach \autoref{1.4.7} ist $\Phi$ volltreu. Sei also nun $A$ eine endlich erzeuge $K$-Algebra mit Erzeugern $a_1,\dotsc,a_n$.
Definiere den Homomorphismus $\phi: K\polyx\ra A$ durch $X_i\mapsto a_i$. Aus der Reduziertheit von $A$ folgt, dass $I=\Kern(\phi)$ ein Radikalideal ist. Setze nun $V=\ V(I)$. Da $K$ algebraisch abgeschlossen ist, gilt $\I(V)=\sqrt{(I)}=I$.Insgesamt:
...
\item\label{1.4.8e} Wir betrachten die Neilsche Parabel aus \autoref{1.4.3}. Wir haben also $V=\V(Y^2-X^3)$ sowie $I=(Y^2-X^3)$, $\V(I)=\{(t^2,t^3) \mid t\in K\}$.
\[\text{Folglich:}K[V]\cong \Quotient{K[X,Y]}{(Y^2-X^3)}\] 
...
\[r_1: \begin{cases} \A^1(K)\ra V \\ t\mapsto (t^2,t^3) \end{cases} \text{ und \quad  } r_2: \begin{cases} V\ra \A^1(K) \\ (x,y) \mapsto y/x\end{cases}\]
Dann sind $r_1$ und $r_2$ sind birationale Abbildungen und induzieren $\alpha_1: K(V)\ra K(\A^1(K))=K(t)$ sowie $\alpha_2: K(\A^1(K))\ra K(V)$, wobei $\alpha_1(X)=t^2$ und $\alpha_2(t)=Y/X$. Also gilt in $K(V): Y^2/X^2=X$.
...
\[(U,f)\textasciitilde(U',f') \iff f\restrict{U\cap U'}=f'\restrict{U\cap U'}\]
...
\begin{proof}
\begin{enumerate}
\item[\ref{1.6.1a}] Zu zeigen: $\textasciitilde$ definiert eine Äquivalenzrelation. Es bleibt nur die Transitivität zu zeigen.
Seien $(U_1,f_1)\textasciitilde (U_2,f_2)$ und $(U_2,f_2)\textasciitilde (U_3,f_3)$. Dann stimmen $f_1$ und $f_3$ auf $U_1\cap U_2\cap U_3$ überein. Da $U_1\cap U_2\cap U_3$ dicht in $V$ ist, folgt mit dem Identitätssatz \autoref{1.5.13}:
\[f_1\restrict{U_1\cap U_3}=f_3\restrict{U_1\cap U_3}\]
\item[\ref{1.6.1b}] Ist $(U,f)$\textasciitilde $(U',f')$, so kann auf $U\cup U'$ die reguläre Funktion $\dach{f}$ definiert werden, indem man 
\[\dach{f}(x)=\begin{cases} f(x),\text{ falls x$\in$ U} \\ f'(x),\text{falls x$\in$ U'}\end{cases}\] setzt.

Setze also $U_{max}=U\cup U'$, wobei$U'$ alle offenen Mengen durchläuft für die es ein $f'\in \O(U')$ gibt mit $(U',f')\textasciitilde (U,f)$.
...
\[\alpha: K(V)\ra \Rat(V), \qquad \frac{g}{h}\mapsto [(D(h),\frac{g}{h}]\] 
