...
\definetheorem{defprop}{Definition/Proposition}{thmcnt}
...
\newtheorem{satz}{Satz}
% den ganzen Spaß nochmal für Sätze....
  \BeforeBeginEnvironment{satz}{\makeatletter%
     \addtocounter{satz}{1}% noch nicht passiert
     \edef\labelcode{ctr@satz@\roman{chapter}@\roman{section}@\arabic{satz}}\relax%
     % formatierungszeugs wird in datei geschrieben, so dass es beim nächsten mal rechtzeitig eingelesen und verwendet werden kann...
     \immediate\write\blah{\string\crefname{\labelcode}{Satz\noexpand~\thesatz}{}}\relax%
    % jetzt noch die Formatierungsinformationen mit rein...
    \immediate\write\blah{\string\crefformat{\labelcode}{\string#2 Satz\noexpand~\thesatz\noexpand~\hashchar1\hashchar3}}%
      % neuer Befehl: Label setzt richtigen label für Aufzählungen in Umgebungen
      \edef\Label#1{\noexpand\label[\labelcode]{#1}}%
      \addtocounter{satz}{-1} % schnell wieder zurück :)
  \makeatother}%
...
\newcommand{\ra}[1][]{\raisebox{-1pt}{\tikz{\draw[thin,shorten >=3pt, ->,font=\scriptsize] (0,0) node{\hspace*{-2pt}} -- (0.5,0) node[above] {#1} -- node{} (1,0);}}\penalty1000\relax}
\renewcommand{\mapsto}{\raisebox{-1pt}{\tikz{\draw[thin,shorten >=3pt, |->] (0,0) node{\hspace*{0pt}} -- node{} (1,0);}}\penalty1000\relax}
\newcommand{\inj}[1][]{\raisebox{-1pt}{\tikz{\draw[thin,shorten >=3pt, right hook->,font=\scriptsize] (0,0) node{\hspace*{0pt}} -- (0.5,0) node[above] {#1} -- node{} (1,0);}}\penalty1000\relax}
\newcommand{\surj}[1][]{\raisebox{-1pt}{\tikz{\draw[thin,shorten >=3pt, ->>,font=\scriptsize] (0,0) node{\hspace*{0pt}} -- (0.5,0) node[above] {#1} -- node{} (1,0);}}\penalty1000\relax}
...
\newcommand{\lra}[1][]{\raisebox{-1pt}{\tikz{\draw[thin,shorten >=3pt, <->,font=\scriptsize] (0,0) node{\hspace*{-2pt}} -- (0.5,0) node[above] {#1} -- node{} (1,0);}}\penalty1000\relax}
% ``punktierte'' Pfeile...
...
Definiere nun $f\colon V\ra W$ durch \[x=(x_1,\dotsc,x_n)\mapsto (\phi(p_1)(x),\dotsc,\phi(p_m)(x)).\]
Zeige:
\begin{itemize}
...
Nach \cref{1.4.7} ist $\Phi$ volltreu. 

Sei also nun $A$ eine endlich erzeuge $K$-Algebra mit Erzeugern $a_1,\dotsc,a_n$.
...
\begin{proof}
  ...
  \item\Label{1.5.10i} Ohne Einschränkung kann in \cref{1.5.3} $U_p=\D(g_p)$ gewählt werden. Damit ist zunächst
    $r(x)=\frac{f_p(x)}{g_p(x)}$ für alle $x\in U_p$. Wir wählen ein $\schlange{g}_p\in K[V]$ mit $x\in \D(\schlange{g}_p)$. Also ist
    $\D(\schlange{g}_p)\subseteq U_p=\D(g_p)$ und damit $\schlange{g}_p\in\sqrt{(g_p)}$, es gibt also ein $h\in K[V]$ und ein $d\in\N$
    mit $\schlange{g}_p=h\cdot g_p$. Wir wählen $\schlange{\schlange{f}}=f_p\cdot h$ und $\schlange{\schlange{g}}_p=\schlange{g}_p^d$ und haben
    dann \[r(x)=\frac{f_p(x)}{g_p(x)} = \frac{f_p(x)h(x)}{g_p(x)h(x)} = \frac{\schlange{\schlange{f}}_p(x)}{\schlange{\schlange{g}}_p(x)}\]
    auf $\D(\schlange{\schlange{g}}_p)=\D(\schlange{g}_p)\subseteq \D(g_p)$.
  \item Nach \cref{1.5.9ii} genügen endlich viele der $\D(g_p)$, also können wir $U_p=\D(g_1)\cup\dotso\cup
    ...
    \begin{ziel}
      Bastle $\schlange{f}_i,\schlange{g}_i\in K[V]$ mit $\displaystyle r=\frac{\schlange{f}_i}{\schlange{g}_i}$ auf $\D(\schlange{g}_i)=\D(g_i)$ und
      $\schlange{f}_i\schlange{g}_j=\schlange{f}_j\schlange{g}_i$ in $K[V]$.
    \end{ziel}
    ...
    $\schlange{f}_i=f_ig_i$ und $\schlange{g}_i=g_i^2$. Dann ist \[r(x)=\frac{f_i(x)g_i(x)}{g_i^2(x)} =
    \frac{\schlange{f}_i(x)}{\schlange{g}_i(x)}\] auf $\D(g_i)=\D(\schlange{g}_i)$ und $\schlange{f}_i\schlange{g}_j-\schlange{f}_j\schlange{g}_i=0$ auf
    ganz $V$.
...
\[\alpha\colon K(V)\ra \Rat(V), \quad \frac{g}{h}\mapsto\relax [(\D(h),\frac{g}{h}]\] 
...
%Verbesserung: Hier fehlt noch Text
\end{itemize}
\end{nbem}
...
Also: $\Psi^{\sharp}\circ \alpha \colon K[W] \ra \O(X)=K[X]$ induziert $\schlange{f} \colon X\ra W$.
Damit ist $f:=\schlange{f}\circ \Psi^{-1}$ eine reguläre Abbildung von $U$ nach $W$. Diese induziert eine rationale Abbildung $f \colon V\ppf W$. Da $\Psi^{\sharp}\circ \alpha$ injektiv ist, ist $f$ dominant und nach \cref{1.6.3} ist damit auch $\schlange{f}$ dominant.
...
\begin{proof}
  ...
  \[ \F_U = \varprojlim_{\substack{\schlange{U}\subseteq U\\\schlange{U}\in\B}}\F(\schlange{U}) =
     ...
     \rho_{\schlange{\schlange{U}}}^{\schlange{U}}(f_{\schlange{U}})=f_{\schlange{\schlange{U}}}\text{ für }
     \schlange{\schlange{U}},\schlange{U}\in\B \text{ mit } \schlange{\schlange{U}}\subseteq\schlange{U}\subseteq U \Bigr\}. \]
     Mit den Restriktionsabbildungen
     $\rho_{U'}^U\colon (f_{\schlange{U}})_{\substack{\schlange{U}\subseteq U\\\schlange{U}\in\B}} \mapsto
     (f_{\schlange{U}})_{\substack{\schlange{U}\subseteq U'\\\schlange{U}\in\B}}$ (wobei $U'\subseteq U$) ist $\F$ eine
     Garbe. Außerdem stimmt für $U\in\B$ das neue $\F(U)$ mit dem ursprünglichen $\F(U)$ überein via
     dem Isomorphismus $(f_{\schlange{U}})_{\substack{\schlange{U}\subseteq U\\\schlange{U}\in\B}} \mapsto f_U$.
...
\begin{prop}\label{2.3.12prop}
  ...
  \begin{enumerate}
  \item\Label{2.3.12a} Der affine Kegel $\schlange{V}$ über $V$ ist eine affine Varietät in $\A^{n+1}(k)$. Genauer: ist
...
\begin{proof}
  ...
  \item[\ref{2.3.12b}] Wir zeigen $\I(V)=\I(\schlange{V})$, falls $V\neq\leer$.
    ...
    Es bliebt noch zu zeigen, dass $\I(\schlange{V})$ ein homogenes Ideal ist. Dazu zeigen wir: ist $f\in\I(\schlange{V})$ mit
    ...
    alle $x\in\schlange{V}$: \[ 0=f(x)=f(\lambda x)=\sum_{i=0}^d f_i(\lambda x) = \sum_{i=0}^d \lambda^i f(x). \] Wenn wir das als
...
\begin{db}\label{2.6.6} Seien $W_1$, $W_2$ quasi-projektive Varietäten.
  \begin{enumerate}
  \item\Label{2.6.6a} Eine \emph{rationale Abbildung} $f\colon W_1\ppf W_2$ ist eine Äquivalenzklasse von Paaren $(U,f_U)$, wobei $U\subseteq W_1$ offen und dicht, $f_U\colon U\ra W_2$ ein Morphismus ist und
   \[(U,f_U)\sim (\schlange{U},f_{\schlange{U}} \iff f_U\restrict{U\cap \schlange{U}}=f_{\schlange U}\restrict{U\cap \schlange{U}}\]
  \item\Label{2.6.6b} $f$ heißt \emph{dominant}, wenn das Bild in $W_2$ für einen Repräsentanten $(U,f_U)$ (und damit für alle) dicht ist. 
...
\[F(r\colon W_1\ppf W_2)=\begin{cases} K(W_2)\ra K(W_1)\\g\mapsto g\circ r \end{cases}\] Nach der Überlegung zu Beginn des Beweises entsprechen die rationalen Abbildungen zwischen $W_1$ und $W_2$ bijektiv den rationalen Abbildungen zwischen den entsprechenden 
affinen Varietäten und diese entsprechen nach \cref{satz4} den Morphismen zwischen $K(U_2)\cong K(W_2)$ (wegen \cref{2.6.5}) und $K(U_1)\cong K(W_1)$. 
\end{enumerate}
\end{proof}
...
\begin{dfn}\label{3.1.1}
  ...
  \item\Label{3.1.1i} Es sei
    ...
    wobei $(U_1,f_1)\sim(U_2,f_2)$ bedeuten soll, dass es eine offene Teilmenge $U_3\subseteq U_1\cap U_2$ mit $p\in U_3$,
    sodass $f_1\restrict{U_3}=f_2\restrict{U_3}$ gilt. Das ist eine $K$-Algebra und heißt \emph{lokaler Ring von $W$ in $p$}.
  \item\Label{3.1.1ii} Die Elemente in $\O_{W,p}$ heißen \emph{Funktionskeime}. Für die Äquivalenzklasse eines $(U,f)$ schreiben
    wir auch $f_p$.
  \end{enumerate}
\end{dfn}
...
\begin{proof}
  ...
  \item Da $U_1$ affin ist, gilt $\O(U_1)=K[U_1]$. Seien $f_1,\dotsc,f_k$ Erzeuger von $K[U_1]$. Wir betrachten
    $\phi((f_1)_p),\dotsc,\phi((f_k)_p)$ in $\O_{W_2,q}\cong K[\schlange{U}_2]_{\m_q^{\schlange{U}_2}}$: es ist
    \[ \phi((f_i)_p) = \frac{g_i}{h_i} \text{ mit } g_i,h_i\in K[U_2] \text{ und } h_i\notin m_q^{\schlange{U}_2}. \]
  ...
  \item Wir haben jetzt $p\in U_1\subseteq W_1$, $q\in U_2\subseteq W_2$ und ein kommutatives Diagramm
    ...
    erhalten wir einen Morphismus $g\colon \schlange{U}_1\ra\schlange{U}_2$ (es ist ohne Einschränkung $\schlange{U}_1\subseteq U_1$,
    $\schlange{U_2}\subseteq U_2$) mit $g(p)=q$, $g^\sharp=\phi^{-1}\colon K[\schlange{U}_2] \ra K[\schlange{U}_1]$ und
    $g_p^\sharp=\phi^{-1}\colon \O_{W_2,q}\ra\O_{W_1,p}$.

    Wir setzen jetzt $\schlange{\schlange{U}}_1=\schlange{U}_1$ und $\schlange{\schlange{U}}_2=f^{-1}(\schlange{U})\subseteq U_2$. Wir haben dann
    Abbildungen \[ f\circ g\colon \schlange{\schlange{U}}_1\ra U_1,\qquad g\circ f\colon\schlange{\schlange{U}}_2\ra \schlange{U}_2. \]
    Auf den lokalen Ringen induziert $g\circ f$ die Identität, also ist $g\circ f$ die Einbettung $\schlange{\schlange{U}}_2\inj
    U_2$. Analog ist $f\circ g$ die Einbettung $\schlange{\schlange{U}}_1\inj\schlange{U}_1$. Daraus folgt 
    \[f\circ g(\schlange{\schlange{U}}_1)\subseteq \schlange{\schlange{U}}_1,\text{ also }g(\schlange{\schlange{U}})\subseteq
    f^{-1}(\schlange{\schlange{U}}_1)=\schlange{\schlange{U}}_2,\] 
    und analog $f(\schlange{\schlange{U}}_2)\subseteq\schlange{\schlange{U}}_1$. Hier gilt
    dann $f\circ g=\id_{\schlange{\schlange{U}}_1}$ und $g\circ f=\id_{\schlange{\schlange{U}}_2}$.
...
Angenommen es gäbe ein Primideal $\wp$ mit $0\varsubsetneq \wp \varsubsetneq (f)$. Wähle nun $h\in \wp$ mit minimalem Grad. Da $f$ irreduzibel ist, folgt $h \in (f)$.
...
\begin{enumerate}
\item\Label{3.3.7a} Wir nennen $\DD:\O_{V,p}\ra K$ eine \emph{Derivation an $p$}, wenn $\DD$ $K$-linear ist und 
...
\[f_{p}^{(1)}(a)=\sum_{i=1}^{n}\frac{\partial f}{\partial X_{i}}(p)\cdot\l(\Bar{X_{i}-p_{i}})=\l\biggl(\sum_{i=1}^{n}\Bar{\frac{\partial f}{\partial X_{i}}(p)\cdot(X_{i}-p_{i})}\biggr)=\l\bigl(\Bar{f_{p}^{(1)}-f_{p}^{(1)}(p)}\bigr).\]
...
$\beta_{p}$ ist also, wie behauptet, die Umkehrabbildung, die Vektorräume somit isomorph.
\end{enumerate}\end{proof}

\begin{db}\label{3.3.10}
Sei $V$ eine quasi-projektive Varietät, $p\in V$.
\begin{enumerate}
\item\Label{3.3.10a} $\displaystyle \bigl(\Quotient{\m_{p}}{\m_{p}^{2}}\bigr)^{\vee}$ heißt \emph{(Zariski-)Tangentialraum}.
\item\Label{3.3.10b} Der Punkt $p$ heißt \emph{regulär} (bzw. \emph{nicht-singulär}), wenn $\dim\T_{V,p}=\dim_{p} V$ ist.
...
\[\J=\left(\!\!\begin{array}{r}2x\\-2y\\0\end{array}\!\right)\]
...
\[\Sing W=\{p \in W \mid p \text{ singulär}\}\] der \emph{singuläre Ort}.
\[\Reg W=\{p \in W \mid p \text{ regulär}\}=W\setminus \Sing W\] 
heißt \emph{regulärer Ort}.
\end{dfn}

\begin{db}\label{3.4.2}
  \begin{enumerate}
  \item\Label{3.4.2a} Ein noetherscher, lokaler Ring $R$ mit maximalem Ideal $\m$ heißt \emph{regulär}, wenn 
  \[\dim_k \Quotient{\m}{\m^2}=\dim R\; (k=\Quotient{R}{\m})\]
  \item\Label{3.4.2b} Sei $W$ quasi-projektive Varietät, $p \in W$. Dann gilt:
  \[p \text{ regulär } \iff \O_{W,p} \text{ ist regulär}.\]
  \end{enumerate}
\end{db}
...
\begin{nbem}
Ein Primideal $\wp$ heißt \emph{minimales Primoberideal} von einem Ideal $I$, wenn $\wp\supseteq I$ und $\wp$ minimal mit dieser Eigenschaft ist. Zorn's Lemma zeigt, dass es zu jedem $x\in R\setminus R^{\times}$ ein minimales Primoberideal von $(x)$ gibt.
\begin{enumerate}
  \item (Krullsches Hauptideallemma, vgl. Brodmann {\scshape iii}.10.15)
  
   Ist $R$ noetherscher Ring, $x\in R\setminus R^{\times}$, $\wp$ minimales Primoberideal von $(x)$, dann gilt $\hoehe \wp \le 1$.
  ...
  Sind $\wp_1,\dotsc,\wp_n$ Primideale in $R$ und $I\nsubseteq \wp_i$, $I$, $J$ Ideale, $I\nsubseteq J$, dann gilt auch $I\nsubseteq J\cup \wp_1\dotsm\cup \wp_n$. 
\end{enumerate} 
\end{nbem}

\begin{lem}\label{3.5.1}
Sei $R$ noethersch, $q \in \Spec R$, $x\in q$, $\hoehe q \ge l \ge 1$. Dann exisitert eine Primidealkette
\[x\in \wp_0'\varsubsetneq\dotsm \varsubsetneq \wp_{l-1}'=q.\]
\end{lem}
\begin{proof} Mache vollständige Induktion über $l$.

$l=1$: Wähle $\wp_0'=q$.

$l\ge 2$: Da $\hoehe q\le l$ gilt, gibt es eine Primidealkette $\wp_0\varsubsetneq\dotsm\varsubsetneq \wp_l=q$. Ist $x \in \wp_{l-1}$, so kann man die Induktionsvorraussetzung anwenden und erhält die Behauptung.

Ab jetzt: $x\in \wp_{l-1}$. Definiere $I=(x) + \wp_{l-2}\subseteq q$. Sei $S$ minimales Primoberideal von $I$ mit $I\subseteq S\subseteq q$.

Gilt nun $S\neq q$, $\wp_{l-2}\varsubsetneq \wp_{l-1}\varsubsetneq q$, so folgt $\Bar{0}\varsubsetneq\Bar{\wp_{l-1}}\varsubsetneq \Bar{q}$ in $\Quotient{R}{\wp_{l-2}}$.

Daraus folgt $\hoehe \Bar{q}\le 2$, d.h $\Bar{q}$ ist kein minimales Primoberideal von $(\Bar{x})$ wegen Krull's Hauptideallemma. Damit ist $q$ auch kein minimales Primoberideal von $(x)+\wp_{l-2}=I$.

Nach Induktionsvorraussetzung angewandt auf $S$ gibt es eine Primidealkette $x\in \wp_0'\varsubsetneq \dotsm\varsubsetneq \wp_{l-2}$ in $S$. Verlängere diese durch $q$ um 1.
\end{proof}
...
Sei $R$ noetherscher Ring, $I=\langle x_1,\dotsc,x_k\rangle$ echtes Ideal in R. Dann gilt: $\hoehe \wp\ge k$ für jedes minimale Primideal von $I$. Insbesondere ist $\hoehe I\ge k$.
\end{prop}
...
Zeige:$l\ge k$. Nach \cref{3.5.1} finden wir Primidealkette $\wp_0'\varsubsetneq \dotsm \varsubsetneq \wp_{l-1}'=\wp$. Also gilt $\Bar{\wp}_0'\varsubsetneq \dotsm \varsubsetneq \Bar{\wp}_{l-1}'=\Bar{\wp}$ und $\Bar{\wp}$ ist minimales Primoberideal von $I=\langle\Bar{x_1},\dotsc,\Bar{x_{k-1}}\rangle$. Mit der Induktionsvoraussetzung folgt: $l-1\ge \hoehe \Bar{\wp}\ge k-1$ und damit die Behauptung.
\end{proof}

\begin{lem}\label{3.5.4} Sei $(R,\m)$ noetherscher, lokaler Ring, $k=\Quotient{R}{\m}$.
\begin{enumerate}
  \item\Label{3.5.4a} Für $\m_1,\dotsc,\m_n$ gilt: \[\{\m_1,\dotsc,\m_n\} \text{ minimales Erzeugendensystem von }\m \iff \{\m_1,\dotsc,\m_n\} \text{ ist Basis von }\Quotient{\m}{\m^2}\]
  \item\Label{3.5.4b} Alle minialen Erzeugendensysteme von $\m$ haben dieselbe Anzahl, nämlich $\dim_k \Quotient{\m}{\m^2}$.
  \item\Label{3.5.4c} Es ist $\dim_k \Quotient{\m}{\m^2}\ge \dim R$.
\end{enumerate}
\end{lem}
\begin{proof}
\begin{enumerate}
  \item[\ref{3.5.4a}] Sei zuerst $ \{\m_1,\dotsc,\m_n\} =\m$. Betrachte die Projektionen $\pi_1\colon R\ra k=\Quotient{R}{\m}$, $\pi_2\colon \m\ra \Quotient{\m}{\m^2}$. Dann ist $\Bar{r\m_1}:=\pi_1(r)\pi_2(\m)$ wohldefiniert und $\{\Bar{\m_1},\dotsc,\Bar{\m_n}\}$ ist ein Erzeugendensystem von $\Quotient{\m}{\m^2}$.

  Sei jetzt $\{\Bar{\m_1},\dotsc,\Bar{\m_n}\}=\Quotient{\m}{\m^2}$. Definiere $N=\langle\m_1,\dotsc,\m_n\rangle$. Dann ist $\m=N+\m^2$ und damit $N=\m$ nach dem Nakayama-Lemma aus Algebra 2.
  \item[\ref{3.5.4b}] folgt aus \ref{3.5.4a}.
  \item[\ref{3.5.4c}] Da $R$ lokal ist, folgt $\dim R=\hoehe \m$. Außerdem folgt $\hoehe m\le | \{$ minimales Erzeugendensystem von $\m\}|=\dim \Quotient{\m}{\m^2}$ mit \cref{3.5.3} und \ref{3.5.4b}.

Insbesondere haben wir damit auch \cref{3.4.3} bewiesen.
\end{enumerate} 
\end{proof}
...
\end{prop}
\begin{proof} Erinnerung aus \cref{3.4.2}: \[(R,\m) \text{ ist regulär } \iff \dim_k \Quotient{\m}{\m^2}=\dim R \iff \m\text{ kann durch } \dim R\text{ viele Elemente erzeugt werden}\]
Wir beweisen die Aussage nun via Induktion über $d:=\dim R=\dim\Quotient{\m}{\m^2}$.
...
$\pi\colon R\ra \Quotient{R}{(x)}$ bezeichne die kanonische Projektion. Nach \cref{3.5.2} gilt \[\dim \Quotient{R}{(x)}=d-1.\] Ferner ist für das maximale Ideal $\pi(\m)$ in $\Quotient{R}{(x)}$ \[\dim \Quotient{\pi(\m) }{\pi(\m)^2} \le |\{\text{ Erzeuger von }\pi(\m)\}|\le d-1\]
nach \cref{3.5.4}.
...
Damit gibt es ein $\wp_i$ mit $(x)\varsupsetneq \wp_i$. Sei nun $b\in \wp_i$. Also ist $b=a\cdot x$ mit $a\in R$. Da $\wp_i$ Primideal, $x\notin \wp_i$, muss $a\in \wp_i$ gelten. Also ist \[\wp_i=\wp_i x\text{ und damit }\wp_i=\wp_i \m\]
...
\begin{enumerate}
\item\Label{4.1.1a} Ein \emph{Divisor} auf $\CC$ ist eine formale Summe
\[D=\sum_{i=1}^{k}n_{i}P_{i}\]
mit $k\in\N$, $n_{i}\in\Z$ und $P_{i}\in \CC$. Die Menge
\[\Div\CC:=\{D\mid D\text{ ist Divisor auf }\CC\}\]
ist mit \enquote{$+$} die freie abelsche Gruppe über $\CC$. Sie heißt \emph{Divisorengruppe}.
\item\Label{4.1.1b} Für $\displaystyle D=\sum_{i=1}^{k}n_{i}P_{i}\in\Div\CC$ heißt $\displaystyle\deg D:=\sum_{i=1}^{k}n_{i}$ der \emph{Grad von $D$}.

$\deg\colon\Div\CC\ra\Z$ ist also ein Gruppenhomomorphismus.
\item\Label{4.1.1c} $D=\sum n_{i}P_{i}$ heißt \emph{effektiv}, wenn alle $n_{i}\geq 0$ sind. Wir schreiben dann $D\geq 0$.
\end{enumerate}\end{dfn}
...
\item\Label{4.1.3b} $\displaystyle\div f:=\sum_{P\in\CC}\ord_{P}(f)\cdot P$ heißt \emph{Divisor zu $f$}.
\item\Label{4.1.3c} Wir nennen $D\in\Div\CC$ \emph{Hauptdivisor}, wenn es $f\in K(\CC)^{\times}$ gibt, so dass $D=\div f$.
\item\Label{4.1.3d} Die Hauptdivisoren bilden eine Untergruppe $\Divh\CC$ von $\Div\CC$.
\item\Label{4.1.3e} $\displaystyle\Cl(\CC):=\Quotient{\Div\CC}{\Divh\CC}$ heißt \emph{Divisorenklassengruppe}.
\item\Label{4.1.3f} $D,D'\in\Div\CC$ heißen \emph{linear äquivalent}, wenn $D-D'\in\Divh\CC$ liegt.
...
\emph{Verzweigungsgrad von $f$ in $P$}.
...
\begin{proof}
Sei $f\in K(C)$. Dann lässt $f$ sich nach \cref{4.1.7} zu einem Morphismus von $\CC$ nach $\P^{1}$ fortsetzen. Der Beweis ist irgendwie komisch....
%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%
\end{proof}
...
&\iff\text{ Für jedes Primideal }\wp\neq 0\text{ ist }R_{\wp}\text{ ein diskreter Bewertungsring.}
\end{align*}
\end{nerinnerung}
...
und bezeichnen so den ganzen Abschluss von $B$ in $K[\CC_{1}]$ mit $A$. Aus Algebra {\scshape ii} wissen wir, dass $A$ dann ein endlich-erzeuger $B$-Modul ist (Algebra {\scshape ii}, Satz 15, bzw. Shafarevich {\scshape ii}.5, Thm. 4). $A$ ist also eine endlich-erzeugte $K$-Algebra und nullteilerfrei, da $A\subseteq K(\CC_{1})$. Es gibt also ein affines $\schlange{V}$ mit $A=K[\schlange{V}]$ und
...
Angenommen es gibt $P_0\in \CC_1\setminus \schlange{V}$ mit $f(P_0)=Q\in V$. Seien $P_1\dotsc,P_k$ alle Urbilder von $Q=f(P_0)$, die auch in $\schlange{V}$ liegen. Nach \cref{4.2.10b} kann man ein $v\in K(\CC_1)$ mit $v\notin \O_{P_0}$ und $v\in \O_{P_i} \; \forall i\in \{1,\dotsc,k\}$ wählen.
\begin{enumerate}
\item Wir zeigen: Man kann ohne Einschränkung annehmen, dass $v$ keine Polstelle in $\schlange{V}$ hat.

Ist nämlich $x\in \schlange{V}$ eine Polstelle, so setzt man $y=f(x)+Q$. Wähle nun $h\in B=K[V]$ mit $h(Q)\neq 0$, $h(y)=0$, d.h. $h\in \m_y^v\setminus \m_Q^v$. Für $v':=v\cdots (h\circ f)$gilt damit:
...
\item Somit ist $v$ nun aus $A=K[V]$ und damit ganz über $B$.

 Also gibt es $b_0,\dotsc,b_{n-1} \in B$ mit \[v^n+b_{n-1}v^{n-1}+\dotsm+b_0=0,\]d.h. $v=-b_{n-1}-\frac{b_{n-2}}{v}-\dotsm-\frac{b_0}{v^{n-1}}$.

Da $v\notin \O_{P_0}$ gilt, ist die linke Seite nicht in $P_0$ definiert, aber es ist $\frac{1}{v}\in \O_{P_0}$. Demnach ist die rechte Seite in $P_0$ definiert, da $b_i\circ f$ auf ganz $f^{-1}(V)$ regulär sind, was ein Widerspruch ergibt.
\end{enumerate}
\end{prooflist}\end{proof}
...
Um die $t_i$ zu konstruieren, wählen wir zuerst $g_i\in A=K[\schlange{V}]$ mit: \[g_i(P_i)\neq 0 \text{ und } g_i(P_j)=0\; \text{ wobei }i,j\in \{1,\dotsc,k\}\] Sei $t_1=\schlange{t_1}+\displaystyle \sum_{j=2}^{k} \alpha_j \cdot g_j^2$ mit $\alpha_j \in K$, $\alpha_j \neq \frac
{-\schlange{t_1}(P_j)}{(g_j(P_j))^2}$.

Dann: $t_1(P_j)=\schlange{t_1}(P_j)+\alpha_j(g_j(P_j))^2\neq 0$, $t_1(P_1)=0$ und $\schlange{t_1}+\displaystyle \sum_{j=2}^{k} \alpha_j \cdot g_j^2 \in \m_{P_1} \setminus \m_{P_1}^2$, da die $g_j\in \m_{P_1}^2$ und $\schlange{t_1} \in \m_{P_1}\setminus \m_{P_1}^2$. 

Also ist $v_{P_1}(t_1)=1$, d.h.$t_1$ tut Gewünschtes. Analog konstruiert man $t_2,\dotsc,t_k$.
  \item[\ref{4.2.8c}] folgt aus \ref{4.2.8b}, denn setze \[u=\frac{v}{t_1^{\ord P_1(v)}\dotsm t_k^{\ord P_k(v)}}.\] Dann ist $\ord P_i(u)=0$ (für jedes $i$), also ist $u\in \schlange{\O}$.
...
\begin{lem}\label{4.2.9} Es gilt:
  ...
  \item\label{4.2.9b} $\schlange{\O}$ ist freier $\O_Q$-Modul vom $\Rang n=\deg f$. Dabei fasst man wiederum $\O_Q$ via $f^*$ als Teilring von $\schlange{\O}$ auf.
  \end{enumerate}
\end{lem}

\begin{proof}
\begin{enumerate}
\item[\ref{4.2.9a}] Seien $w\in \schlange{\O}$, $x_1,\dotsc,x_r$ die Polstellen von $w$, $y_1,\dots,y_r$ ihre Bilder und $l_i=\ord_{x_i} (w),-n=\min\{l_1,\dotsc,l_r\}$.

Wähle $h'\in B$ mit $h'(y_i)=0$ und $h'(Q)\neq 0$. Sei $h=(h')^N$. 
...
Sei $\{\alpha_1,\dotsc,\alpha_n\}$ eine Basis von $\Quotient{K(\CC_1)}{K(\CC_2)}$, $l=\max\{$Polstellenordnung in den $P_i's\}$. Dann sind $\alpha_1\cdot t^l,\dotsc,\alpha_n\cdot t^l$ linear unabhängig und in $\schlange{\O}$. 
...
Andererseits: $\O \cong \O_Q^n\ra (\Quotient{\O_a}{(t)})^n\cong K^n$. 

Damit: $\Quotient{\schlange{\O}}{(t\cdot \schlange{\O})}=\Quotient{\schlange{\O}}{(t\circ f)}\cong K^n$ und damit $\dim_K \Quotient{\O}{(t\circ f)}=n$.
\end{proof}
%7.2.11
...
\[D\sim D'\implies s(D)=s(D')\]
gilt und dass mit \cref{4.3.5} schon
\[D'\leq D\implies s(D')\leq s(D)\]
gilt, da 
\end{enumerate}
\end{proof}
...
\begin{db}\label{4.4.1} Sei $\Omega_{\CC}=\Omega_{\Quotient{K(\CC)}{K}}$ der $K(\CC)$-Vektorraum der $K$-Differentiale von $K(\CC)$ (siehe auch Algebra 2).
\begin{enumerate}
 \item\Label{4.4.1a} $\Omega_{\CC}$ heißt auch Vektorraum der \emph{rationalen Differentiale}.
...
Außerdem wird $\Omega_{\CC}$ von $\d\Bar{x}, \d\Bar{y}$ erzeugt und die notwendige Bedingung $\d F(\Bar{x},\Bar{y})=0$ erzwingt $\frac{\d F}{\d x} \d \Bar{x}+\frac{\d F}{\d y} \d \Bar{y}=0$.
Der Lösungsraum dieses linearen Gleichungssystems ist eindimensional. \end{enumerate}
\end{proof}
\begin{db}\label{4.4.2}  Sei $w \in \Omega_{\CC}$, $w\neq 0$. 
  \begin{enumerate}
  \item\Label{4.4.2a} Sei $P\in \CC$, $\t_p$ uniformisierend. Dann ist $w=f\cdot\d t_p$ und \[\ord_p (w):=\ord_p (f)\] ist wohldefiniert.
  \item Man definiert $\dim W=\displaystyle \sum_{P\in \CC}^{} \ord_p (w) \cdot P$.
  \item $\K$ heißt \emph{kanonischer Divisor}, wenn $\K=\div w$ für ein $w\in \Omega_{\CC}$ gilt.
  \item Je zwei kanonische Divisoren sind linear äquivalent.
  \end{enumerate}
\end{db}
\begin{w} Für den Beweis von \cref{4.4.2a} muss man die Unabhängigkeit der Ordnung von $t_p$ zeigen. Das ist schwierig!

Die restlichen Aussagen folgen dann aus \cref{4.4.2a}
\end{w}
\begin{satz}[von Riemann-Roch]
Sei $\K$ ein kanonischer Divisor auf $\CC$ und $D\in \Div(\CC)$. Dann gilt: \[\l(D)-\l(\K\setminus D)=\deg D+1-g\]
\end{satz}

\begin{kor}\label{4.4.3} \begin{enumerate}
  \item\Label{4.4.3a} Ist $\K$ ein kanonischer Divisor sowie $D=0$, so ist $\deg D=0$, $\l(D)=1$ nach \cref{4.3.2a}. Also ist in diesem Fall $\l(K)=g$.
  \item\Label{4.4.3b} Ist $D=\K$ ein kanonischer Divisor, so ist $\l(K)=g$ nach \ref{4.4.3a}. Also gilt hier \[\deg (\K)=g-1+\l(D)-\l(0)=2g-2.\]
 \end{enumerate}
\end{kor}
\begin{bsp}\label{4.4.4}
Sei $\CC=\P^1(K)$. Dann ist $K(\CC)=K(X)$ und $\Omega_{\CC}=K(X)\d X$, $w=\d X$ ist uniformisierend.

Wir suchen den kanonischen Divisor $\K=\div w$, d.h. wir müssen für jeden Punkt $P$ die Ordnung $\ord_p(w)$ bestimmen (vgl. \cref{4.4.2}).

Sei dazu zunächst $a\in K$. Dann ist $X-a$ uniformisierend und es gilt also $\d X=\d (X-a)$. Also ist $\ord_a (w)=0$.

Nun sei $a=\infty$. Dann ist $\frac{1}{X}$ uniformisierend und mit Leibniz sieht man: \[\d\frac{1}{X}=-\frac{1}{X^2} \d X\]
Also ist $\ord_{\infty}=-2$ und damit ist $\K=-2\cdot \infty$ der kanonische Divisor zu $w$. Insbesondere ist $\deg K=-2$. Setze $D=\K$. Nun liefert \cref{4.4.3b}: \[g(\P^1(K))=0\] 
\end{bsp}
\end{document}