...
$\GR(G,S)$\index{$\GR(G,S)$} von $G$ bzgl. $S$ wird wie folgt definiert:
\begin{align*}
E(\GR(G,S)) &:= G, \\
...
\]
\item
Es ist $K(\GR_H(G,S))=G\times S-S'\times\{-1,1\}$ mit
...
Weiter gelten:
\begin{enumerate}
\item Für $Gk:=\rho(G)_K(k)$\index{$Gk$} ist $\bar{Gk}=G\bar{k}$.
...
heißt \emph{baryzentrische Unterteilung}\index{baryzentrische Unterteilung}\index{Unterteilung}\index{Graph!Unterteilung}\index{Graph!baryzentrische Unterteilung}\index{$\GRsub$}
von $\GR$.
\begin{center}
...
\index{freie Gruppe}\index{Gruppe!frei}\index{$F(X)$}
