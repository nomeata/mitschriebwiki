...
\begin{align*}
P(N_1\ge u) 
&\ge P(N_{\frak kn} - N_{\frak {k-1}n} \ge 1 \text{ für } k =1,\ldots,n)\\
...
\begin{align*}
\sum_{k=1}^n X_{nk} \sim B(n,1-e^{-\lambda \frac tn}) \tomit d P_0(\lambda t)
\end{align*}
...
\begin{align*}
S_u &: D_0 \to D_0,  & f &\mapsto f(\cdot\wedge n)\\
Z_u &: D_0 \to D_0,  & f &\mapsto f(n+\cdot)-f(n)
\end{align*}
wobei $(\cdot \wedge n)$ die Abbildung $m\mapsto \min (m,n)$ darstellt.
...
\[
Z_\tau(N) = (N_{\tau + t} - N_\tau)_{\tau \ge 0}
\]
