...
\begin{satz}
\label{satz3.1}
% Hier satz 3.1 eintragen
\end{satz}


\begin{beweis}
\begin{enuma}
\item % Hier Beweis von Satz 3.1 a) eintragen
\item % Hier Beweis von Satz 3.1 b) eintragen
\item 
...
\begin{beweis}
Wir benutzen ein sogenanntes „Kopplungsargument“: Sei $(Y_n)$ eine weitere Markov-Kette, unabhängig von $(X_n)$ mit gleicher Übergangsmatrix und Startverteilung $\pi$, also $Y_n\sim \pi$ für alle $n\in \MdN$. Es sei
\[
...
Betrachte $(Z_n)_{n\in\MdN_0}$ definiert durch
\[
Z_n= 
\begin{cases}
X_n, &\text{ für } n\le T \\
...
Nach dem Satz $\ref{satz3.1}$ genügt es zu zeigen: 
...
\begin{centering}
\Large Hier bitte Vorlesung vom 7.5. eintragen:
\end{centering}
...
\begin{bemerkung}
Bemerkung: Ist $h:S\to \mathbb R$ eine beschränkte Funktion, so wird durch
\begin{align*}
