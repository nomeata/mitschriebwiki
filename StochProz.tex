...
\]
gilt, also $P(t)$ rechtseitig stetig ist, dann heißt sie \emph{Standardübergangsmatrizenfunktion}\index{Standardübergangsmatrizenfunktion}.
\end{bemerkung}
...
\draw[color=blue] (0,0) -- node[above] {$X_2(t)$} (2,1.25) -- (4,1.5) --  (6,-1.2) -- (8,-2.5);
\end{tikzpicture}
\end{center}
...
\[
\mathcal F_\tau:=\{A\in\mathcal F\vert A\cap\{\tau\leq t\}\in\mathcal F_t\text{ für alle } t\geq0\}
\]
...
\]
$\borel[0,t]\otimes\mathcal F_t$-messbar.
...
\[
\{\tau_n\leq t\}=\left\{\tau\leq\underbrace{\frac{\lceil 2^nt\rceil}{2^n}}_{\leq t}\right\}\in\mathcal F_t\text{ für alle }t\geq0.
  \]
...
\]
ist $\borel[0,t]\otimes\mathcal F_t$-messbar für $0\leq u<v\leq t$. Denn: Sei $A\in\borel$ mit $0\notin A$, dann ist
\[
(1_{[u,v)}X_v)^{-1}(A)=\underbrace{[u,v)}_{\in\borel[0,t]}\times\underbrace{X_v^{-1}(A)}_{\mathcal F_v\subset\mathcal F_t}
\]
...
\]
$X^{(n)}$ ist eine linear Kombination von Abbildungen vom obigen Typ, also $\borel[0,t]\otimes\mathcal F_t$-messbar.
\item Es ist zu zeigen: Für alle $A\in\borel$ und $t\geq0$ ist
\[
X_t^{-1}(A)\cap\{\tau\leq t\}\in\mathcal F_t
\]
...
\end{align*}
mit der $\sigma$-Algebra $\borel[0,t]\otimes\mathcal F_t$ auf $[0,t]\times\Omega$. $g$ ist $(\mathcal F_t,\borel[0,t]\otimes\mathcal F_t)$-messbar, da für alle $A\in\mathcal F_t, s\leq t$ gilt:
\[
\{\omega\in\Omega\vert(\tau(\omega)\wedge t,\omega)\in[0,s]\times A\}=A\cap\underbrace{\{\tau\leq s\}}_{\in\mathcal F_s}\in\mathcal F_t
\]
Die Messbarkeit von $h$ folgt aus b.). Es gilt also:
\begin{align*}
  & h(g(\omega))=h(\tau(\omega)\wedge t,\omega)=X_{\tau(\omega)\wedge t}(\omega)\text{ ist $(\mathcal F_t,\borel)$-messbar} \\
\implies & X_{\tau\wedge t}^{-1}(A)\in\mathcal F_t\text{ für alle }A\in\borel \\
\implies & X_\tau^{-1}(A)\cap\{\tau\leq t\}=\underbrace{X_{\tau\wedge t}^{-1}(A)}_{\in\mathcal F_t}\cap\underbrace{\{\tau\leq t\}}_{\in\mathcal F_t}\in\mathcal F_t
\end{align*}
  \end{enuma}
\end{beweis}
\begin{satz}
  \label{satz:13.2}
Es sei $(B_t,\mathcal F_t)_{t\geq0}$ eine Brownsche Bewegung, $\tau$ eine endliche Stoppzeit. Dann sind die $\sigma$-Algebra $\mathcal F_t$ und der Prozess $(B_{\tau+t}-B_{\tau})_{t\geq0}$ stochastisch unabhängig und $(B_{\tau+t}-B_{\tau})_{t\geq0}$ hat dieselbe Verteilung wie $(B_t)_{t\geq0}$.
\end{satz}
