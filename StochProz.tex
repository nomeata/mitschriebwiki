...
Sei $Y_n$ die Nachfrage nach einem gelagerten Produkt im Zeitintervall $(n-1,n]$. $(Y_n)$ sei u.i.v. und $Y_n\in \MdN_0$. Die Auffüll-Politik sei eine $(z,Z)$-Politik mit $z\le Z$, $z,Z\in \MdN$, die wie folgt funktioniert: Falls der der Lagerbestand zur Zeit $n\le z$ ist, dann fülle auf $Z$ auf, sonst tue nichts.
...
\intertext{
Mit der Stirling-Formel ($n! \simeq \frac n l ^n\cdot \sqrt{2\pi n}$) erhält man dann}
p_{00}^{(2n)} &\approx (pn)^n\cdot \frac{(\frac{2n}l)^{2n} \sqrt{4\pi n}}{(\frac nl) ^{2n} 2\pi n} = \frac{(4pq)^n}{\sqrt{\pi n}}
\end{align*}

\paragraph{Fall 1} Ist $p=q=\frac12$, so ist $p_{00}^{(2n)} \approx \frac1{\pi n}$, also ist $\sum_{n=0}^\infty p_{00}^{(2n)} = \infty$ und die Markovkette ist rekurrent.

\paragraph{Fall 2} Ist dagegen $p\ne q$, also $pq<\frac14$, so ist $\sum_{n=0}^\infty p_{00}^{(2n)} \le \text{konstanter Term } \sum_{n=0}^\infty (4pq)^n < \infty$, also ist die Markovkette transient.
\end{beispiel}

\begin{bemerkung}
Betrachtet man die symmetrische Irrfahrt auf $\MdZ^d$, also $p_{ij}= \frac1{2d}$ für $\|i-j\|=1$, mit $\|\cdot\|$ der $l^1$-Norm und $i,j\in \MdR^d$, so ist die Irrfahrt rekurrent für $d=1,2$ und transient sonst.
\end{bemerkung}
...
&= \delta_{ij} + \sum_{k=1}^\infty f_{ij}^{(k)} \sum_{n=0}^\infty p_{jj}^{(n-k)} \\
&= \delta_{ij} + f_{ij}^* \underbrace{\sum_{n=0}^\infty p_{jj}^{(n-k)}}_{<\infty\text{ da $j$ transient}}  <\infty
\end{align*}
\end{beweis}

\begin{satz}
Ist eine Klasse $K\subseteq S$ rekurrent, so ist $K$ abgeschlossen beziehungsweise $(p_{ij},\, i,j\in K)$ ist stochastisch.
\end{satz}
...
\]
Denn für $n>N$ gilt
$P_i(X_N=j,X_n=i)=p_{ij}^{(N)} p_{ji}^{(n-N)} = 0$ und für $n<N$ gilt: $P_i(X_N=j,X_n=i)=p_{ij}^{(n)} p_{ji}^{(N-n)} = 0$, da $N-n<N$.

Also ist $P(T_i\le m,X_N=j) = \sum_{n=1}^m P_i(T_i=n,X_N=j) \le \sum_{n=1}^m P_i(X_n=i, X_N=j) = 0$ und damit 
\begin{align*}
\sum_{n=1}^m f_{ii}^{(n)} 
&= P_i(T_i\le m) \\
&= P_i(T_i\le m, X_N\ne j) \\
...
\end{align*}
Für $n\to\infty$ folgt dann
\begin{align*}
1=f_{ii}^* = \sum_{n=1}^\infty \le 1-p_{ij}^{(N)} < 1
\end{align*}
...
\begin{beweis}
Da $(p_{ij},\,i,j\in K)$ stochastisch ist folgt induktiv, dass $(p_{ij}^{(n)},\, i,j\in K)$ ist stochastisch für alle $n\in\MdN$. Angenommen, $K$ wäre transient. Sei dann $j\in K$, dann ist nach Lemma \ref{lem:2.6} $p_{ij}^{(n)}\to 0$ für $n\to\infty$ und alle $i\in S$. Für $i\in K$ folgt also: $1=\sum_{j\in K} p_{ij}^{(n)} \to 0$ für $n\to\infty$, im Widerspruch zur Abgeschlossenheit von $K$.
\end{beweis}

\begin{bemerkung}
Insbesondere gilt: Ist $S$ endlich und $P$ irreduzibel, so ist die Markovkette rekurrent.
\end{bemerkung}
...
Sei $(X_n)_{n\in\MdN}$ eine Markov-Kette mit Zustandsraum $Z$ und Übergangsmatrix $P=(p_{ij})$. Aufgrund der bisherigen Ergebnisse können wir $S$ zerlegen in rekurrente Klassen $K_1,K_2,\ldots$ und eine Menge von transienten Zuständen $T$, also $S=T\cup K_1\cup K_2\cup\ldots$.

Es sei $\tau \da \inf\{n\ge 0 \mid X_n\ne T\}$  die Austrittszeit aus der Menge der transienten Zustände. Für $i\in T$, $k\in T^c$ interessiert uns $u_{ik}=P_i(X_\tau = k)$, vorausgesetzt $P_i(\tau <\infty)=1$.

Wir unterteilen $P=(p_ij)$ in 
\[
P = 
\begin{pmatrix}
Q & R \\
...
\end{pmatrix}
\]
wobei $Q$ die Einschränkung von $P$ auf die transienten Zustände ist, also $Q=(q_{ij}, i,j\in T)$.

\begin{satz}
\label{satz:2.9}Für $i\in T$, $j\in T^c$ gilt:
\[
u_{ij} = \sum_{k\in T} q_{ij}u_{kj} + p_{ij}
\]
\end{satz}
...
&= \sum_{k\in T}\sum_{n\ge 2} p_{ik} P_i(X_2\in T,\ldots,X_{n-1}\in T, X_{n-1}=j) \\
&= \sum_{k\in T}\sum_{n\ge 2} p_{ik} P_i(\tau = n-1, X_\tau = j) \\
&= \sum_{k\in T} p_{ik} P_k(X_\tau = j) \\
&= \sum_{k\in T} p_{ik} u_{kj}
\end{align*}
Da für $i,j\in T$ gilt: $p_{ik}=q_{ik}$, folgt die Behauptung.
\end{beweis}

\begin{bemerkung}
Es sei $U=(u_{ij})_{i\in T, j\in T^c}$. Dann lässt sich Satz \ref{satz:2.9} schrieben als $U=QU+R$ beziehungsweise $U-QU=R$, also $(I-Q)U=R$. Falls $I-Q$ invertibel ist, ist $U=(J-Q)^{-1}R$
\end{bemerkung}
...
\begin{definition}
  ...
  \begin{align*}
    \sum_{i \in S} \nu(i) \cdot p_{i,j} = \nu(j)
  \end{align*}
  Ist $\nu$ eine Verteilung und invariant, so nennt man $\nu$ auch \emph{stationäre Verteilung}\index{stationäre Verteilung!zeitdiskret} oder \emph{Gleichgewichts-Verteilung}.
\end{definition}
...
\begin{beweis}
  ...
  \begin{align*}
    ...
    =& P(Y\in B\mid X_j=j) \cdot P(X^\tau\in A\mid X_\tau=j) \cdot P(X_\tau=j) 
  \end{align*}
...
\begin{bemerkung}
  ...
  Die Mengen der Form
  \[
  A(t_1,\dots,t_n,i_1,\dots,i_n):=\{f\in D_0\mid f(t_j)-f(t_{j-1})=i_j \text{ für } f=1,\dots,n\}
  \]
...
\begin{definition}
Sei $\tau$ eine $(\cF_t)_{t\ge 0}$-Stopptzeit\index{Stoppzeit}, das heißt $\{\tau \le t \} \in \cF_t$. Der Prozess 
\[
...
\begin{satz}
\label{satz:11.1}
Ist $(B_t)_{t\ge 0}$ eine Brownschen Bewegung auf einem Wahrscheinlichkeitsraum $(\Omega, \cF, P)$, so gilt für $P$-fast-alle $\omega\in \Omega$, dass 
\begin{align*}
...
\begin{satz}
Mit $(X^{(n)})$ definiert wie oben und Zeitpunkte $0\le t_1<\cdots<t_l<\infty$ gilt:
\begin{align*}
