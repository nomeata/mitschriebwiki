...
\paragraph{Fall 1} Ist $p=q=\frac12$, so ist $p_{00}^{(2n)} \approx \frac1{\sqrt{\pi n}}$, also ist $\sum_{n=0}^\infty p_{00}^{(2n)} = \infty$ und die Markovkette ist rekurrent.
...
\begin{beweis}
Da $(p_{ij},\,i,j\in K)$ stochastisch ist folgt induktiv, dass $(p_{ij}^{(n)},\, i,j\in K)$ ist stochastisch für alle $n\in\MdN$. Angenommen, $K$ wäre transient. Sei dann $j\in K$, dann ist nach Lemma \ref{lem:2.6} $p_{ij}^{(n)}\to 0$ für $n\to\infty$ und alle $i\in S$. Für $i\in K$ folgt also: $1=\sum_{j\in K} p_{ij}^{(n)} \to 0$ für $n\to\infty$. Widerspruch.
\end{beweis}
...
&= \sum_{k\in T}\sum_{n\ge 2} p_{ik} P_k(X_2\in T,\ldots,X_{n-2}\in T, X_{n-1}=j) \\
...
\begin{bemerkung}
Es sei $U=(u_{ij})_{i\in T, j\in T^c}$. Dann lässt sich Satz \ref{satz:2.9} schrieben als $U=QU+R$ beziehungsweise $U-QU=R$, also $(I-Q)U=R$. Falls $I-Q$ invertierbar ist, ist $U=(I-Q)^{-1}R$
\end{bemerkung}
...
\item Zunächst gilt:
  ...
  \begin{align*}
    \gamma_k(i) &= \sum_{n=1}^\infty \sum_{j \in S} P_k(X_n = i, X_{n-1} = j, n \leq T_k)\\
                ...
                &= \sum_{j \in S} p_{ji} E_k[\sum_{n=0}^{T_{k-1}} 1_{[X_n = j]}]\\
                &= \sum_{j \in S} p_{ji} E_k[\sum_{n=0}^{T_k} 1_{[X_n = j]}]
