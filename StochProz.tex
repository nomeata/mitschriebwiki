...
\begin{align*}
  (Ph)(i) := \sum_{j \in S} p_{i,j} \cdot h(j) \qquad \forall i \in S
\end{align*}
...
\end{lemma}
\begin{beweis}
  Wir prüfen nach, dass $(Ph)(X_n)$ ein bedingter Erwartungswert von $h(X_{n+1})$ bzgl. $\cF_n$ ist.
...
\begin{beweis}
  ...
  \begin{align*}
    E[h(X_{n+1})|\cF_n] =Ph(X_n)= h(X_n) + (Ph - h)(X_n)
  \end{align*}
  $\leadsto$ Behauptung.
\end{beweis}
...
$0\leq t_0\leq t_1 \leq \dots \leq t_n, n\in \mathbb{N}$ sind die Zufallsvariablen
\[
...
\begin{satz}
  \label{satz:7.1}
  Es sei $N=(N_t)_{t\geq 0}$ ein stochastischer Prozess, der den Bedingungen (A1)-(A4) genügt.
...
&= 
\begin{cases}
1, &\text{falls mindestens ein Ereignis in $\big[\frac{(k-1)t}n, \frac{kt}n\big)$ eintritt}\\
0, &\text{sonst}
\end{cases}
\end{align*}
...
\begin{beweis}
Zunächst nehmen wir an, dass $\tau$ Werte in $\mathbb Q_+$ annimmt. Für $0=:s_0\le s_1\le\cdots\le s_k<\infty$, $0=:t_0\le t_1\le \cdots\le t_l<\infty$ und beliebige $i_1,\ldots,i_k,j_1,\ldots,j_l\in \mathbb N_0$ gilt
\begin{align*}
