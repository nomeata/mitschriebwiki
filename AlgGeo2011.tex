...
\item[iii)]
	Folgt aus (iv)
\item[iv)]
	Sei $V'$ affine Variet\"at mit $V\subseteq V'$. Sei $V'=V(I')$ f\"ur ein Ideal $I'$.\\
	\emph{Behauptung:} $I\subset I(V)$\\
	Dann ist $V' = V(I')\supseteq V(I(V)) = \bar{V}$\\
	\emph{Beweis der Behauptung:} $f\in I'$\\
	$\Rightarrow$ f\"ur alle $x\in V'$ ist $f(x)=0$\\
	$\Rightarrow f(x)=0$ f\"ur alle $x\in V \Rightarrow f\in I(V)$
\end{enumerate}\end{Bew}
...
\item
	Ein Ring $R$ hei\ss t \begriffspez{graduiert}{graduiert!Ring}, wenn es eine Zerlegung $R=\bigoplus\limits_{d=0}^{\infty}R_d$ gibt mit abelschen Gruppen $R_d$, sodass $R_dR_e\subseteq R_{d+e}$ \textcolor{red}{(fehlt da etwas?)} f\"ur alle $d,e\ge0$
...
\begin{description}
\item\emph{Behauptung:} $\mathcal J\subseteq I^*$ (denn dann ist $V=V(\mathcal J)\supseteq V(I^*)$)

\item\emph{denn:} Sei $F\in \mathcal J$ homogen, $x=(y_1,\ldots ,y_n)\in V(I)$.\\
