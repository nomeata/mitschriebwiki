...
\item
	$H:\left\{\begin{array}{rcl}
		...
		f=\Sum_{i=0}^d &\mapsto& \Sum_{i=0}^d f_iX_0^{d-1}
...
\item[d)]
	$\widetilde F$ ist homogen. Schreibe $\widetilde F=\Sum_{i=0}^df_iX_0^{d-1}$ mit $f\in f[X_1,\ldots ,X_n]$ homogen vom Grad $i$.\\
	$f_d\ne0$, weil $X\nmid \widetilde F$\\
...
\item[a)]
	...
	\[\begin{array}{rcl}
		f^m-\Sum_{j=0}^{m-1}a_jf^j &=& 0 \qquad |\cdot X^{d_j\cdot m}\\
...
\textcolor{red}{Es gibt im Skript zwei Verweise auf eine Bemerkung 13.8, kann es also sein, dass mir hier etwas fehlt?}
...
\item
	Es ist
		\[f^*(\ddiv g) = \Sum_{Q\in C_2} \ord_Q(g)\cdot f^* P = \Sum_{Q\in C_2}\ord_Q(g) \cdot \Sum_{P\in f'(Q)}e_P(f)\cdot P\]
	und
		\[\ddiv(g\circ f) = \Sum_{P\in G}\ord_P(g\circ f)P = \Sum_{Q\in C_2} \Sum_{P\in \overline{f}'(Q)} \ord_P(g\circ f)\cdot P\]
	...
	$\Rightarrow u\cdot t_P^s = g\circ f = (v\cdot t_Q^r) \circ f = (v\circ f) \cdot (T_Q^P\circ f)^r = \underbrace{(v\circ f)}_{\in \calO_{C_1,P}} \cdot u'^rt_P^{r\cdot e_P(f)} \Rightarrow s = r \cdot e_P(f)$
\item
	folgt aus b)
\end{enumerate}\end{Bew}
...
Da $\dim C_1 = \dim C_2 = 1$, ist $\trdeg(k(C_1)) = \trdeg(k(C_2)) = 1$ (Folgerung \ref{19.8}), also $k(C_1)/k(C_2)$ algebraisch. Au\ss erdem ist $k(C_1)/k(C_2)$ endlich erzeugt, weil $k(C_{\textcolor{red}{1}})$ schon \"uber $k$ endlich erzeugt ist.
\end{Bew}
...
\item
	...
	Dann gilt f\"ur jedes $Q\in C_2$:
		\[ \deg(f^*(Q)) = \sum_{P\in f'(Q)} e_P = \deg f =: n \]
\end{enumerate}\end{Satz}
...
Also: $\deg(f) = n$\\
F\"ur $x_0 \in \textcolor{red}{k (=\A^1(k))}$ ist $f^{-1}(x_0) = \{ (x_0,y) \in k^2: h(x_0,y)=0\}$.\\
