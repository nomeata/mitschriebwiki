...
\newcommand{\IP}{\mathbbm{P}}%\P bereits belegt
...
\section*{Benannte SÃ¤tze}
\pdfbookmark[1]{Benannte SÃ¤tze}{contents}
...
\item
	Eine Abbildung $f: U_1 \to U_2$ zwischen quasi-affinen Variet\"aten $U_1,U_2$ hei\ss t \begriff{Morphismus} (oder \begriffspez{regul\"are Abbildung}{Abbildung!-rugul\"are}), wenn $f$ stetig ist und f\"ur jedes offene $U \subseteq U_2$ und jedes $g\in\mathcal O_{U_2}(U)$ gilt:
...
\item
	Eine \begriff{rationale Funktion} auf $V$ ist eine Ãquivalenzklasse von Paaren $(U,f)$, 
	wobei $U\subseteq V$ offen und dicht und $f\in\mathcal O(U)$ ist.\\
	Dabei ist $(U,f)\sim(U',f'):\Leftrightarrow f|_{U\cap U'}=f'|_{U\cap U'}$
\item In jeder Ãquivalenzklasse gibt es ein maximales Element $(U_{\max},f_{\max}), U_{\max}=:\Ddef(f)$ hei\ss t \begriff{Definitionsbereich} der nat\"urlichen Funktion.\\
...
Seien $V,W$ affine VarietÃ¤ten.\begin{enumerate}[a)]
\item
	Eine \begriff{rationale Abbildung}\index{Abbildung!rationale-} $f:V\dashrightarrow W$ ist eine Ãquivalenzklasse von Paaren $(U,f_U)$, wobei $U\subseteq V$ offen und dicht, $f_U:U\longrightarrow W$ regul\"ar.\\
...
\item
	Eine \begriffspez{rationale Funktion} auf $V$ ist eine \"Aquvalenzklasse von Paaren $(U,f)$, wobei $U\subseteq V$ offen, dicht, $f\in\calO_V(U)$.\\
	Dabei ist $(U,f)\sim(U',f'):\Leftrightarrow f|_{U\cap  U'}=f'|_{U\cap  U'}$
\item
...
Sei $R$ Ring (kommutativ mit Eins), $A$ eine $R$-Algebra, $M$ ein $A$-Modul. Eine $R$-lineare Abbildung $D:A\to M$ hei\ss t \begriff{$R$-Derivation}, wenn gilt:
...
Sei $k$ algebraisch abgeschlossener K\"orper. $C$ sei irreduzible projektive Variet\"at der Dimension \textcolor{red}{1/$k$}.
\begin{tabbing}
