...
% Einige Anstrengungen, um den Â§ vor die Section-Nummer zu stellen
% \renewcommand{\thesection} allein fÃ¼hrt zu einem Konflikt mit ntheorem-hyper
\makeatletter
...
\textcolor{red}{Die Vorlesung ist beendet und dieses Skript ist erst in der Endphase. Der Inhalt sollte bald vollst\"andig sein. Stellen bei denen ich unsicher war sind rot markiert.}
\section*{Wer}
...
\item Abschnitt \"uber Divisoren vervollst\"andigen (sehr wichtig)
...
$k^n = V(0) = V(\emptyset)$\\
$V(X(X-1)y(Y-1))$ affine Variet\"at
\end{nnBsp}
...
\item\label{bem2.4.iv}
	...
	\[ V1 \subseteq V_2 \Leftrightarrow I(V_1) \supseteq I(V_2)\]
	Also insbesondere: $V_1 = V_2 \Leftrightarrow I(V_1) =I(V_2)$
\end{enumerate}\end{Bem}

\begin{Def}
Ein Ideal $I$ in eine, Ring $R$ hei\ss t \begriff{Radikalideal}, wenn gilt:\\
...
\item[iv)]
	...
	\item[Behauptung:] $I\subset I(V)$\end{description}
...
\item
	Sei $g \in A(V)$ mit $g^d=0$ f\"ur ein $d>0$, sei $\in k[X_1,\dots ,X_n]$ mit $\bar f = g$
...
\item
	Sei $X$ ein toologischer Raum, $Y \subseteq X$.\\
...
$\Rightarrow V_j\subseteq V_j \Rightarrow i=j\Rightarrow W=V_i$\begin{description}[\setlabelstyle{\itshape}]
...
Sei $k$ ein K\"orper, $V\subseteq \A ^n(k)$, $W\subseteq \A ^m(k)$ affine Variet\"at.\begin{enumerate}[a)]
\item
	Eine Abbildung $f: V \to W$ hei\ss t \begriff{Morphismus}, wenn es Polynome $f_1,\dots ,f_m \in k[X_1,\dots ,X_n]$ gibt mit \[
		f(x) = (f_1(x),\dots ,f_n(x))\]
	f\"ur alle $x\in V$.

\item
	Ein Morphismus $f: V\to W$ hei\ss t \begriffspez{Isomorphismus}{Morphismus!Iso-}\index{Isomorphismus}, wenn es einen Morphismus $g:W\to V$gibt mit \[ g\circ f = \id_W \textrm{ und } f\circ g = \id_V\]
...
\item
	Sei $V=\A ^1(k)$, $W =V(Y^2\cdot X^3)\subseteq \A ^2(k)$.\\
	...
	$g(f(x))=g(x^2,x^3) = \frac{x^3}{x2}=x$ (f\"ur $x\not= 0$)\\
...
\item
	...
	Die Fixpunkte fon $f$ sind genau die Punkte, deren Koordninaten alle in $\F_p$ liegen (\quot{$\F_p$-wertige Punkte})
...
\item
	$k[V]:=\{f:V\to \A^n(k) : f \textrm{ ist Morphismus}\}$ hei\ss t \begriffspez{affiner Koordinatenring}{Koordinatenring!affiner-} von V.
...
\item
	Jeder Morphismus $\varphi = f:V\to W$ induziert $k$-Algebrahomomorphismus \[f^{\#}:k[W]\to k[W], g\mapsto g\circ f\]
\item
	Die Abbildung $\Mor(V,W)\to \Hom_k(k[W],k[V]), f\mapsto f^\ast$ ist bijektiv.
...
Da die $p_i$ $k[V]$ erzeugen (also $k$-Algebra), folgt $f^{\#} = \varphi$\\
...
Dann gilt: $\overline h$ ist Einheit in $k[V] \Leftrightarrow V(k)\cup V=\emptyset$
\end{Bem}

\begin{Bew}\[\begin{array}{rl}
V\cup K(k) &=V(I(V)+(h))=\emptyset \overset{\textrm{HNS}}{\Longleftrightarrow}I(V)+(h)=k[X_1,\dots ,X_n]\\
...
\item
	F\"ur jedes offene $U\subseteq V$ ist
		\[k[V]\to\mathcal O_V(U), f\mapsto f/U\]
	ein $k$-Algebra-Homomophismus.\\
	Zusatz: Ist $U$ dicht, so ist $\alpha_n$ injektiv (\"Ubung?)
\end{enumerate}\end{DefBem}
...
\item
	$f\in k[X-1,\dots ,,X_n] \Rightarrow \frac{1}{f}\in \mathcal O_{\A^n(k)}(D(f))$
\end{enumerate}\end{nnBsp}

\begin{Bem}
Sei $V\subseteq\A^n(k)$ affine Variet\"at, $U\subseteq V$ offen.\begin{enumerate}[a)]
\item
	F\"ur offene Teilmengen $U''\subseteq U' \subseteq U$ gilt:\begin{itemize}
	\item[i)] F\"ur $f\in\mathcal O_V(U)$ ist \[f=0 \Leftrightarrow \underbrace{f_{\vert U_i}}_{=\varrho_{U_i}^{U''}}=0 \forall i\in I\]
	\end{itemize}
\item
	F\"ur jedes $i\in I$ sei $f_i\in \mathcal O_V(U_i)$ gegeben.\\
	Ist $f_{i\vert U_i \cup U_j} =f_{j\vert U_i \cup U_j}$ f\"ur alle $i,j$, so gibt es $f\in \mathcal O_V(U)$ mit $f_{\vert U_i}=f_i$ f\"ur alle$i\in I$.
\end{enumerate}\end{Bem}
...
F\"ur $U\subseteq X$ offen sein $\mathcal F(U)=R, \varrho^U_{U'}=\id_R$\\
...
\item
	...
	Setze $\hat g_p = h g_p, \hat h = \tilde h_p^d = h\cdot h_r$\\
	Dann gilt f\"ur jedes $x\in D(\hat h_p)=D(\tilde h_p)$
		\[g(x)=\frac{g_p(x)}{h_p(x)}=\frac{g_p(x)\cdot h_p(x)}{h_p(x)\cdot h_p(x)}=\frac{\hat g_p(x)}{\hat h_p(x)}\]
...
\item
	...
	\[g\circ f \in \mathcal O_{U_1}(\bar{f}'(U))\]
...
\item
	Die quasi-affinen Variet\"aten ber $k$ bilden eine Kategorie, die $\underline{\Aff(k)}$ als volle Unterkategorie enth\"alt.
...
\begin{Bew}
Sei $g=f\cdot X_{n+1}-1\in k[X_1,\dots ,X_{n-1}]$ und $V=V(g)\subseteq\A^{n+1}(k)$, $V$ ist affine Variet\"at, $\varphi:D(f) \to V, x\mapsto(x,\frac{1}{f(x)})$ ist Morphismus mit Umkehrabbildung\dots \textcolor{red}{[DER REST FEHLT MIR]}
\end{Bew}
...
\item
	Die irreduziblen affinen Variet\"at bilden mit dne dominanten rationalen Abbildungeneien Kategorie.
...
\item
	...
	$g\circ f$ ist auf $f^{-1}(D(x))$ regul\"ar. Das ist \emph{nicht dicht} in $V$!
...
Zu zeigen bleibt:\begin{enumerate}[i)]
	\item zu jeder endlich erzeugten K\"orpererweiterung $K|k\ni V$ mit $k(V)\cong K$
...
\begin{nnBsp}\begin{enumerate}[1)]
\item[0)]\begin{description}
	\item $n=0$: $\IP^0(k)$ hat genau einen Punkt

\end{description}\item\begin{description}
	\item $n=1$: $\IP^1(k)=k\cup\{\infty\}$

\end{description}\item\begin{description}
	\item $k=\R$, $n=1$: $\IP^1(\R)=S^1/\{\pm 1\}$
	\item \textcolor{white}{$k=\R$, }$n=2$: $\IP^2(\R)$ \quot{Kreuzhaube} (nicht orientierbare geschlossene Fl\"ache)

\end{description}\item\begin{description}
	\item $k=\C$: $\IP^1(\C) \overset{1)}{=} \C \cup \{\infty\}$

\end{description}\item\begin{description}
	\item $k=\F_2, n=2$: $\IP^2(\F_2)$ hat 7 Punkte
\end{description}\end{enumerate}\end{nnBsp}
...
\item
	F\"ur $(x_o,\dots ,x_n)\in k^{n+1}$ und $\lambda\in k$ gilt:
		\[f(\lambda x_o,\dots ,\lambda x_n)=\lambda^d\cdot f(x_o,\dots ,x_n)\]
\item
	$f$ hat wohlbestimmte Nullstellenmenge $V(f)\subset\R^n(k)$
\end{enumerate}\end{Bem}

\begin{Def}
Eine Teilmenge $V\subseteq\IP^n(k)$ hei\ss t \begriffspez{projektive Variet\"at}{Variet\"at!projektive-}, wenn es eine Menge $F\subset k[X_o,\dots ,X_n]$ von homogenen Polynomen gibt mit $V=\{x\in\IP^n(k):f(x)=0\forall x\in V\}$
\end{Def}

\begin{Bsp}\begin{enumerate}[a)]
\item
	$V(X_o,\dots ,X_n)=\emptyset$
...
\item
	...
	$V\cap U_2 = V(\frac{x_0}{x_1}\frac{x_1}{x_2}-1)\subset U_2=\A^2(k)$\\
	$\textcolor{white}{V\cap U_2} = V(xy-1)$ Hyperbel
\end{enumerate}\end{Bsp}

\linetitle{Warnung:} Ist $V\subset\IP^n(k), v\neq 0$, so ist
	\[I_o(V):=\{f\in k[X_0,\dots ,X_n]:f \text{ homogen}, f(x)=0 \forall x\in V\}\]
kein Ideal!\\
...
\item
	$I$ homogen $\Leftrightarrow I=\bigoplus\limits_{d=0}^{\infty}(I\cap R)\Leftrightarrow$ f\"ur jedes $a\in I, a=\sum\limits_{d=0}^n a_d$, $a_d\in R_d$ ist $a_d\in  I$ f\"ur jedes $d=0,\dots ,n$
...
Dan ist $(0,\ldots ,0)\in V_{\text{aff}}(I)$, aber $\{(0,\ldots ,0)\}\not=V_{\text{aff}}(I)$.\\
...
\item
	$H:\left\{\begin{array}{rcl}
		...
		f=\sum\limits_{i=0}^d &\mapsto& \sum\limits_{i=0}^d f_iX_0^{d-1}
...
\item[$\varrho$ stetig:]$\ $
	...
	\item[denn:] Sei $f=\sum\limits_{i=0}^df_i$, $x=(x_1,\ldots ,x_n)\in V(f)\Leftrightarrow$ f\"ur $\varphi(x)=(1:x_1:\ldots:x_n)$ gilt $0=H(f)(\varphi(x))=\sum\limits_{i=0}^df_iX_0^{d-1}(1:x_1:\ldots :x_n)=\sum\limits_{i=0}^df_i(x)=f(x)$
	\end{description}
...
$\widetilde F = H_0(D_0(F))\in I^* \overset{F=X_0^\text{V}}{=}F\in I^* \Rightarrow \mathcal J \subseteq I^*$\\
\textcolor{red}{(Das \"uber dem letzten Gleichheitszeichen \"uberpr\"ufen)}
...
F\"ur $i=0,\ldots, n$ ist $U\cap U_i$ offen in $U_i\cap W$ und damit in der affinen Variet\"at $\overline{U_i\cap W}$ (Zarinski-Abschluss in $\A^n(k)=\varrho_i(U_i)$).\\
...
\item[b)]
	...
	\item$\bullet$ $\psi$ surjektiv:
		...
		Homgenisiere bez\"uglich $X_i$: erhalte $\frac{G_i}{F^{r_i}X_i^{e_i}}$, \OE $r_i=1$ (sonst ersetze $F$ durch $F^{r_i}$)\\
		...
		das hei\ss t $F^{m+1}=\sum\limits_{i=0}^n H_iFX_i^{e_i+1}, H_i\in k[X_0,\ldots ,x_n]$ homogen\\
		Setze $G:=\sum\limits_{i=0}^nH_iG_iX_i$\\
		$\Rightarrow F^{m+1}G_jX_j=\sum\limits_{i=0}^nH_iFX_i^{e_i+1}G_jX_j \overset{(*)}{=}\sum\limits_{i=0}^nH_iFX_j^{e_j+1}G_iX_i=G\cdot FX_j^{e_j+1}$\\
...
\item[a)]
	...
	Ist f\"ur $j=i$ auch $V\cap U_j\not=\emptyset$, so ist $V\cap U_i\cap U_j$ dicht in $V$ und $\frac{G_i}{X_i^{d_i}}=\frac{G_j}{X_j^{d_j}}=:f\in L$.
	
	\emph{Behauptung 1}: $f$ ist ganz \"uber $k[V]$\\
	Dann gibt es ein $m\ge 1$ und $a_0,\ldots ,a_{m-1}\in k[V]$, so dass
	\[\begin{array}{rcl}
		f^m-\sum\limits_{j=0}^{m-1}a_jf^j &=& 0 \qquad |\cdot X^{d_j\cdot m}\\
		\underbrace{G_i^m}_{\deg = d_i\cdot m} + \sum\limits_{j=0}^{m-1}a_j\cdot \underbrace{G_i^j\cdot X_i^{d_i(m-j)}}_{\deg=d_ij+d_im-d_ij=d_im} &=& 0 \end{array}\]
	$\Rightarrow $ \OE $a_j\in k$ f\"ur alle $j$\\
	$\Rightarrow f$ algebraisch \"uber $k$ $\overset{k \text{ alg. abg.}}{\Longrightarrow} f\in k$\\
	\emph{Bew. von Beh. 1}: Sei $d:=\sum\limits_{i=1}^n d_i$ und $k[V]_d$ die homogenen Elemente vom Grad $d$.\\
	...
	\emph{Bew. von Beh. 2}: $k[V]_d$ wird erzeugt von den $X_0^{j_0}\cdot\ldots\cdot X_n^{j_n}$ mit $\sum\limits_{i=1}^nj_i=d=\sum\limits_{i=1}^nd_i$.\\
...
Dann gibt es homogene Polynnome $F_0,\ldots ,F_m$ in $k[X_0,\ldots ,X_n]$ mit $f(x)=(F_0(x):\ldots :F_m(x))$ f\"ur alle $x\in \IP^n(k)$
\end{Bem}
...
\item
	Ist $V$ irreduzibel, so ist $k(V)=\Quot(k[U])$ f\"ur jede offene dichte Teilmenge $U\subseteq V$.
\item
	Ist $V$ irreduzibel und projektiv mit homogenem Koordinatenring $k[V]$, so ist $k(V)=\{\frac{f}{g}\in\underbrace{\Quot(k[V])}_{=:\Quot_0(k[V])}: f,g \text{ homogen vom gleichen Grad}\}$.
\end{enumerate}\end{DefBem}
...
\item[c)]
	...
	\textcolor{red}{REST FEHLT}
\item[d)]
	$\begin{array}{rcl}
		\Quot_0(k[V]) &\to& \Rat(V)\\
		\frac{f}{g} &\mapsto& [(D(g), x\mapsto\frac{f}{g}(x))]
	\end{array}$
	Ist Homomophismus von $k$-Algebren.
\end{enumerate}\end{Bew}
...
$\begin{array}{rcl}
	(v_1,\ldots ,v_d)&\mapsto& \sum\limits_{\substack{1\le i_1<\ldots <i_d\le n\\\sigma \in S_d}}(-1)^{\sign(\sigma)}e_{i_1}\wedge\ldots \wedge e_{i_d}\cdot \lambda_{1i_1}\cdot \lambda_{2i_2}\cdot\ldots \lambda_{di_d} \\ &&\\ %leere Zeile, sonst ist es gequetscht
...
$\Rightarrow v_1\wedge\ldots \wedge v_d = \sum\limits_{i=1}^da_{1i}u_i\wedge\ldots  \wedge \sum\limits_{i=1}^d a_{di}u_i \overset{\text{s. o.}}{=} \det(A) u_1\wedge \ldots \wedge u_d$\\
...
Sei $L_{\omega}$ die Abbildungsmatrix von $\varphi_\omega$ bez\"uglich der Basen $e_1,\ldots ,e_n$ und $e_{i_1} \wedge \ldots \wedge e_{i_1}$.\\
...
$[\omega]\in\Bild(\Psi) \Leftrightarrow \dim(\Kern(\varphi_\omega))\ge d \Leftrightarrow \Rang(L_\omega)\le n\cdot d$\\
...
$\Rightarrow \calF_{d,n}(k)$ ist die Menge aller Paare $(\omega,v)$ mit $f_{IJ}(\omega)=0$ f\"ur alle $I,J$ wie oben \emph{und} $\sum\limits_{j=1}^n l_{ij}(\omega)v_j=0$
\end{enumerate}\end{Bew}
...
\item
	...
	Ist $y=T_{V,x}$ und $f\in I(V)$, so ist $D_y(f)=0$ nach Definition, denn $D_y(f)=f_x^1(y)$.\\
...
\item
	...
	$V_i$ ist irreduzibel anch \"Ubung 2, Aufgabe 5.\\
	$\overline{V_i}\cap Y=V_i$ weil $V_i$ abgeschlossen in $Y$ ist. $\Rightarrow \overline{V_i}\subsetneq\overline{V}_{i+}$\\
...
\proofle
	...
	$V_d$ irreduzible $\Rightarrow \exists i$ mit $V_d\subseteq X_i$\\
...
\item
	...
	Dann hei\ss t
		\[\Ht(\mathfrak p):=\sup\{n\in\N:\exists \p_0\subseteq\ldots \subseteq\p_n=\p \text{ Kette von Primelemten in }R\}\]
	\begriff{H\"ohe} von $\p$.
\item
...
\begin{Prop}
Ist $k$ algebraisch abgeschloss, so gilt f\"ur jede affine Variet\"aten $V\subseteq\A^n(k)$: $\dim(V)=\dim k[V]$
\end{Prop}
...
\item
	Ist $\varphi:k[X_1,\ldots ,X_n]\to A$ surjektiver Homomophismus in $k$-Algebra, so ist \[\dim A +\Ht(\Kern(\varphi))=n\]
\item
	Jede rationale (nicht verl\"angerbare) Kette von Primidealen in $A$ hat die L\"ange $A$.\textcolor{red}{(\"uberpr\"ufen)}
\end{enumerate}\end{Satz}

\begin{nnErinn}
$S|R$ \begriff{ganze Ringerweiterung} $\Leftrightarrow$ jedes $a\in S$ ist Nullstelle eines \emph{normierten} Polynoms mit Koeffizinten aus $R$
\end{nnErinn}
...
\item
	Ist $v$ diskrete Bewertung auf $K$, so ist $\calO_v:=\{x\in K: x=0\text{ oder }v(x)\ge 0\}$ lokaler Ring mit maximalem Ideal $m=\{x\in K: x=0\text{ oder }v(x)>0\}$. \textcolor{red}{(w\"are $m_v$ nicht bessere Bezeichnung?)}
\item
...
\begin{Def}\begin{enumerate}[a)]
\item
	Ein \begriff{Divisor} auf $C$ ist eine formale Summe $D=\sum\limits_{i=1}^nn_iP_i$ mit $n\in \N$, $n_i\in\Z$, $P_i\in C$.
\item
	$\Div(C)=\{D=\sum\limits_{i=1}^nn_iP_i \ | D$ ist Divisor auf $C\}$ mit der formalen Addition hei\ss t \begriff{Divisorengruppe} von $C$.
\item
	F\"ur $D=\sum\limits_{i=1}^nn_iP_i$ hei\ss t $\deg(D):= \sum\limits_{i=1}^nn_i$ der \begriff{Grad} von $D$.
\item
	$D=\sum\limits_{i=1}^nn_iP_i$ hei\ss t \begriff{effektiv}, wenn alle $n_i\ge0$ sind.\\
	Schreibweise: $D\ge0$
\end{enumerate}\end{Def}

\begin{DefBem}\begin{enumerate}[a)]
\item
	F\"ur $f\in k(C)^\X$ hei\ss t $\Ddiv(f):=\sum\limits_{P\in C}\ord_P(f)_P$ der \begriffspez{Divisor von f}{Divisor!-von $f$}
\item
...
\item
	\begin{description}[\setlabelstyle{\itshape}]
	\item[zu zeigen:]$e_P(f)$ h\"angt nicht vond er Wahl von $f$ ab.
	\end{description}
...
% 06. 02. 2012
\textcolor{red}{Hier fehlt der Rest des Abschnitts}
...
\begin{DefBem}
Sei $D=\sum\limits_{P\in C}n_P\cdot P \in \Div(C)$\begin{enumerate}[a)]
\item
...
\end{enumerate}
Insbesondere ist $L(D)$ endlich dimensionalr Vektorraum\end{Prop}
...
\item
	...
	\item[Beweis 1:]
		...
		Seien $P_1,\ldots ,P_r \in C$ die Punkte mit $\ord_P(1) \le 0$ und $n_P > 0$.\\
		$h_i:= \frac{1}{f} - \frac{1}{f}(P_i) \in m_{P_i}$\\
		$h:= \prod\limits_{i=1}^r h_i^{-\textcolor{red}{n}} P_i$ tut's \textcolor{red}{(wo kommt das $n$ her? ist das $n_P$?)}
	\item[Beweis 2:]
		$[k(C) : k(f)] = \deg(f) =: r$\\
