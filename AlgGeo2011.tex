...
$\tilde{J} := \Pi^{-1}(J)$ ist nach Voraussetzung endlich erzeugbar. Die Bilder der Erzeuger von $\tilde{J}$ in $J$ erzeugen $J$.
\end{Bew}
...
\proofforward
	...
	$I$ ist Ideal. $I$ ist endlich erzeugbar, $I=(a_1,\dots ,a_r),a_i \in I_{n_1}$, $n_0=\max_{i=1}^r n_i$	$\Rightarrow I_n = I_{n_0}$ f\"ur $n \ge n_0$
...
\item
	Eine \deftermspec{rationale Abbildung}{Abbildung!rationale-} $f:V\dashrightarrow W$ ist eine \"Aquivalenzklasse von Paaren $(U,f_U)$, wobei $U\subseteq V$ offen und dicht, $f_U:U\longrightarrow W$ regul\"ar.\\
...
\item
	...
	$I(V)$ hei\ss t \deftermspec{Verschwindungsideal}{Ideal!Verschwindungs-}.
...
\item
	Eine $k$-Algebra $S$ hei\ss t \deftermspec{graduiert}{graduiert!$k$-Algebra}, wenn $S=\bigoplus\limits_{d=0}^{\infty}S_d$ graduierter Ring ist und $S_0=k$.\\
...
\item
	...
	Diese hei\ss t \deftermspec{Zariski-Topologie}{Topologie!Zariski-}.
...
Dann hei\ss t $K[V]:=\FakRaum{k[X_0,\ldots ,X_n]}{I(V)}$ \deftermspec{homogener Koordinatenring}{Ring!homogener Koordinaten-}\index{homogen!Koordinatenring} zu $V$.
\end{Def}
...
\item[denn:] Sei $F\in \calJ$ homogen, $x=(y_1,\ldots ,y_n)\in V(I)$.\\
...
\item
	Eine \deftermspec{rationale Funktion}{Funktion!rationale-} auf $V$ ist eine \"Aquvalenzklasse von Paaren $(U,f)$, wobei $U\subseteq V$ offen, dicht, $f\in\calO_V(U)$.\\
...
\item
	Die Elemente von $\calO_{V,x}$ hei\ss en \deftermspec{Keime}{Keim} von regul\"aren Funktionen.\\
...
$x\in V$ hei\ss t \defterm{regul\"ar} (oder \deftermspec{nichtsingul\"ar}{singul\"ar!nicht-}), wenn $\dim T_{V,x}=\dim_x V$, andernfalls hei\ss t x \defterm{singul\"ar}. $V$ hei\ss t nichtsingul\"ar, wenn jeder Punkt $x\in V$ regul\"ar ist.
\end{Def}
...
\item
	...
	$R$ hei\ss t \defterm{regul\"ar}, wenn $\dim R=\dim_k(m/m^2)$.
\item\label{19.3b}
...
Dann ist $\calO_{C,P}$ diskreter Bewertungsring. Die zugeh\"orige diskrete Bewertung auf $k(C)=\Quot(\calO_{C,P})$ hei\ss t $\ord_P$. $\ord_P$ hei\ss t die \defterm{Ordnung} von $f$ in $P$.
\end{Def}
