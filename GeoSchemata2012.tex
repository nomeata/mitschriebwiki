...
\newglossaryentry{Keim}{name=Keim,plural=Keime,description={Die Elemente eines lokalen Rings, Schreibweise \ensuremath{f_x := (U,f)_\sim}}}
...
\newglossaryentry{Amalgam}{name=Amalgam,plural=Amalgame,description={\ldots }}
...
\proofforward
  $\checkmark$
\proofreverse
  F\"ur jedes $x\in U$ gibt es Umgebung $U_x$ mit $s|_{U_x} = 0$. ($s|_{U_x} = \rho_{U_x}^U$). Die $(U_x)_{x\in U}$ bilden offene \"Uberdeckungen, die $s|_{U_x}$ bilden konsistente Familie, $s$ und $0$ sind beides \glspl{Amalgam} $\xRightarrow[\text{eigenschaft}]{\text{Gruppen-}} s= 0$
