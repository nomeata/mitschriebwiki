...
\documentclass[paper = A4, fontsize=12pt, numbers=noendperiod, chapterprefix=true, index=toc]{scrbook}
...
\newcommand{\defterm}[1]{{\sindex[idx]{#1}}\emp{#1}}
\newcommand{\deftermspec}[2]{{\sindex[idx]{#2}}\emp{#1}}
...
\item
  Ein geringter Raum $(X, \calO_X)$ hei\ss t \defterm{lokal}\sindex[idx]{geringter Raum!lokal} geringter Raum, wenn f\"ur jedes $x\in X$ der Halm $\calO_{X,x}$ ein lokaler Ring ist.
\end{enumerate}\end{Def}
...
\begin{enumerate}[a)]
\item
	Der von $\id_X$ induzierte Morphismus $\Delta = \Delta_f: X \to X \X_S X$ hei\ss t \deftermspec{Diagonalmorphismus}{Morphismus!Diagonal-} (oder Diagonale\sindex[idx]{Diagonale}) zu $f$.
	\begin{center}\begin{tikzpicture}
...
\begin{erinn}
$X = \Spec R$ affines Schema, $I \subseteq R$ Ideal, $f \in R$, $\tilde I (D(f)) = I \cdot R_f = I \cdot \calO_X(D(f))$, $\tilde I$ hei\ss t quasikoh"arente\sindex[idx]{koh"arent!quasi-} Idealgarbe\sindex[idx]{Garbe!Ideal-}.
\end{erinn}
