...
\item
	...
	\begin{minipage}[c]{0.4\textwidth}\begin{tikzpicture}[baseline=0]
		...
		\draw[->, cyan] (4) to [in=150, out=30] node[below]{$\psi$} (2);
...
\item
	...
	\emph{Erhalte:} $h:X \X_S Y \to X \X_S T$ aus \begin{tikzpicture}[baseline = 0]
		...
		\draw[->, cyan] (4) to [in=150, out=30] node[below]{\mbox{\small $g \circ \pr_Y$}} (2);
	...
	\begin{tikzpicture}[baseline=0]
		...
		\draw[->, cyan] (4) to [in=160, out=30] node[below]{$\psi$} (2);
	\end{tikzpicture}
...
\item
	...
	\draw[->, font=\scriptsize] (4) to[in=135, out=45] node[above]{$\Delta = \Delta_f$} (0);
	\draw[->, font=\scriptsize] (4) -- node[below]{$\id$} (1);
	\draw[->, font=\scriptsize] (4) to[in=165, out=15] node[below]{$\id$} (2);
	\end{tikzpicture}
...
\begin{Satz}
Sei $f: X \to Y$ ein Morphismus von Schemata. Dann gilt:
\begin{enumerate}[a)]
\item
	$f$ ist genau dann $\left\{\begin{array}{c}\text{separiert}\\ \text{eigentlich}\end{array}\right\}$, wenn es zu jedem Bewertungsdiagramm \begin{tikzpicture}[baseline=0]
		\node (0) at (-1,0.5) {$U$};
		\node (1) at (1,0.5) {$X$};
		\node (2) at (-1,-0.5) {$T$};
		\node (3) at (1,-0.5) {$Y$};
		...
		\draw[->, font=\scriptsize] (2) -- node[below]{$h_1$} (3);
		\draw[->, dashed, font=\scriptsize, green] (2) -- node[above]{$h$}(1);
	\end{tikzpicture} $\left\{\begin{array}{c}\text{h"ochstens}\\ \text{mindestens}\end{array}\right\}$ einen Morphismus $h: T \to X$ gibt, der das Diagramm kommutativ macht.
\item
...
%	\node at (2.5,0) {$\schraffiert$};
	...
	\draw[->, font=\scriptsize] (0) to[in=135, out=225] node[left, align=right]{UAE $Y \X_S T$:\\ $\exists! f \X_S T$} (4);
\end{tikzpicture}\end{center}
\emph{Noch zu zeigen:} \textcolor{red}{$\id_X \X_S T = \id_X \X_S T$}, $(f \circ g) \X_S T = (f \X_S T) \circ (g \X_S T)$
\begin{center}\begin{tikzpicture}
