...
\item
  Viele Zeichnungen haben sich \"uberschneidende Pfeile. Es s"ahe besser aus, wenn die Pfeile sich nicht einfach \"uberkreuzen w"urden, sondern die unten liegenden Pfeile L"ucken h"atten. Das kann man machen, indem man direkt unter die oberhalb liegenden Pfeile eine etwas etwas dickere wei\ss e Linie zeichnet. (Erledigt f"ur die Vorlesung, "Ubung folgt)
\item
...
\begin{center}\begin{tikzpicture}
	...
	\draw[->,font=\scriptsize] (1) to [out=270,in=180] node[left]{$\rho_U^{f^{-1}(V)}$} (5);
	\draw[->,font=\scriptsize] (2) to [out=45,in=135] node[above]{$\rho_U^{f^{-1}}(V)$} (4);
	\draw[->,font=\scriptsize] (4) to [out=315,in=0] node[right]{$\hat \alpha(U)$}(5);
\end{tikzpicture}\end{center}
 ...
 \begin{minipage}{9cm}
  ...
  \begin{tikzpicture}
    ...
    (m-1-3) edge node[auto] {$d^{n}$} (m-1-5)
			edge node[auto,swap] {$\alpha^n-\beta^n - e^{n-1} h^n$}  (m-3-3)
			edge [bend right=100] node[auto,swap] {$h^n$}  (m-3-1)
	(m-3-1) edge node[auto,swap] {$e^{n-1}$} (m-3-3)
	;
	\path[->,dashed, font=\footnotesize]
