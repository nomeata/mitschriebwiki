...
\begin{Aufg}
 Ein Schemamorphismus $f\colon X\to Y$ heiÃt \textbf{quasi-endlich}, wenn $f^{-1}(y)$ fÃ¼r alle $y\in Y$ eine endliche Menge ist.
 
 Finde ein Beispiel fÃ¼r einen Schemamorphismus $f\colon X\to Y$, so dass
\begin{enumerate}%[topsep=0cm, parsep=0cm, label=\alph*)]
...
\begin{Aufg}
Ein Schemamorphismus $f\colon X \rightarrow Y$ heiÃt \textbf{projektiv}, wenn es fÃ¼r ein $n \in \N_0$ ein kommutatives Diagramm 
\begin{minipage}{4cm}
\begin{tikzpicture}
...
\begin{Aufg}
Es sei $X$ eine nichtsingul"are, projektive Kurve Ã¼ber einem algebraisch abgeschlossenen K"orper $k$. Zeige:
\begin{enumerate}%[topsep=0cm, parsep=0cm, label=\alph*)]
 \item FÃ¼r jedes $f \in k(X)^\times$ ist $\mathcal{L}(\textrm{div}(f))$ isomorph zu $\mathcal{O}_X$.
 \item FÃ¼r jeden Divisor $D$ auf $X$ gibt es eine offene Ãberdeckung $X = \bigcup_{i \in I} U_i$ %von $X$ 
 und rationale Funktionen $f_i \in k(X)$, sodass $\mathcal{L}(D)(U_i) = \frac{1}{f_i} \mathcal{O}_X(U_i)$ fÃ¼r jedes $i \in I$ gilt.
 \item FÃ¼r je zwei Divisoren $D_1$ und $D_2$ auf $X$ ist $\mathcal{L}(D_1 + D_2)$ isomorph zu $\mathcal{L}(D_1) \otimes_{\mathcal{O}_X} \mathcal{L}(D_2)$.
\end{enumerate}
\end{Aufg}
