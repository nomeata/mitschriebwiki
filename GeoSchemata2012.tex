...
\documentclass[paper = A4, fontsize=12pt, numbers=noendperiod, chapterprefix=true, bibliography=totoc, headsepline]{scrbook}
...
% "bibliography=totoc" erstellt einen nicht nummerierten Eintrag fuer das Lteraturver-
% zeichnis im Inhaltsverzeichnis
...
% hyperref kommen.
% Von Joachim deaktiviert, da es sonst mit Fehler baut
...
% F\"ur TikZ-Diagramme
\usepackage{tikz}
\usetikzlibrary{matrix,arrows,calc,intersections, positioning, patterns}
...
\newglossaryentry{Kategorie}{name=Kategorie,description={Eine Kategorie \ensuremath{\calC} besteht aus einer Klasse von Objekten \ensuremath{\Ob(\calC)}, Mengen \ensuremath{\Mor_\calC(X, Y)} von Morphismen von Objekten und Verkn"upfungsabbildungen \ensuremath{\Mor_\calC(Y, Z) \X \Mor_\calC(X, Y) \to \Mor_\calC(X, Z), (g, f) \mapsto g \circ f} (das \ensuremath{\circ} wird h"aufig weggelassen). Dabei ist das neutrale Element der Identit"atsmorphismus \ensuremath{\id_X: X \to X}. Mit \ensuremath{\dom(f)} bezeichnet man die Quelle (domain) eines Morphismus \ensuremath{f}, mit \ensuremath{\cod(f)} das Ziel (co-domain)}}

\newglossaryentry{Morphismus}{name=Morphismus,plural=Morphismen,description={Allgemein eine Abbildung zwischen zwei Objekten \ensuremath{X} und \ensuremath{Y} einer Kategorie \ensuremath{\calC}. Die Menge der Morphismen einer Kategorie wird mit \ensuremath{\Mor_\calC(X, Y)} bezeichnet (siehe auch Definition \ref{1.5})}}

\newglossaryentry{Funktor}{name=Funktor,plural=Funktoren,description={Eine Abbildung zwischen Kategorien. Es wird zwischen kovarianten Funktoren und kontravarianten Funktoren unterschieden. F"ur einen kovarianten Funktor \ensuremath{F} gilt \ensuremath{F: \Mor_\calC(X, Y) \to \Mor_\calD(F(X), F(Y))}, im kontravarianten Fall hingegen gilt \ensuremath{F: \Mor_\calC(X, Y) \to \Mor_calD(F(Y), F(X))}}}

\newglossaryentry{Varietaet}{name=Variet\"at,plural=Variet\"aten,description={Eine algebraische Variet"at ist ein geometrisches Objekt, das durch Polynomgleichungen beschrieben werden kann}}

\newglossaryentry{Affine Varietaet}{name=Affine Variet"at,plural=affine Variet"aten,parent=Varietaet,description={Eine irreduzible affine algebraische Menge "uber einem K"orper. Affine algebraische Mengen sind definiert als Teilmengen \ensuremath{\{x \in K^n | f_1(x)= \ldots f_k(x) = 0\}} eines affinen Raumes \ensuremath{K^n}, wobei \ensuremath{K} ein K"orper und \ensuremath{\{f_1,\ldots ,f_k\}} eine Menge von Polynomen in \ensuremath{K[X_1,\ldots ,X_n]} ist. Eine quasi-affine Variet"at ist eine offene Teilmenge einer affinen Variet"at},text=affine Variet\"at}

\newglossaryentry{Projektive Varietaet}{name=Projektive Variet\"at,plural=projektive Variet"aten,parent=Varietaet,description={Eine irreduzible projektive algebraische Menge "uber einem K"orper. Projektive algebraische Mengen sind definiert als Teilmengen \ensuremath{\{ x \in \IP^n | f_1(x) = \ldots = f_k(x) = 0\}} eines Projektiven Raumes \ensuremath{\IP^n} "uber einem K"orper \ensuremath{K}. Dabei sind \ensuremath{f_1,\ldots, f_k} homogene Polynome in \ensuremath{K[X_0,\ldots,X_n]} und \ensuremath{x = [x_0: \ldots :x_n]} ein Punkt in \ensuremath{\IP^n}. Eine quasi-projektive Variet"at ist eine offene Teilmenge einer projektiven Variet"at},text=projektive Variet\"at}

\newglossaryentry{Regulaere Funktion}{name=Regul\"are Funktion,plural=Regul\"are Funktionen,description={Eine Abbildung \ensuremath{f: U \to \A^1(K)}, die regul"ar ist f"ur alle \ensuremath{p \in U}, wobei \ensuremath{U \subseteq V} offen und \ensuremath{V \subseteq \A^n(K)} eine affine Variet"at ist. \ensuremath{f} hei\ss t dabei regul"ar in \ensuremath{p}, wenn es eine Umgebung \ensuremath{U_p \subseteq U} von \ensuremath{p} gibt und \ensuremath{g, h \in K[V]} mit \ensuremath{h(x) \ne 0} und \ensuremath{f(x) = \frac{g(x)}{h(x)}} f"ur alle \ensuremath{x \in U_p}. Dieser Begriff wurde in der Vorlesung \quot{Algebraische Geometrie I} im Semester zuvor definiert},text=regul\"aren Funktionen}

\newglossaryentry{Keim}{name=Keim,plural=Keime,description={Die Elemente eines Helmes, Schreibweise \ensuremath{f_x := (U,f)_\sim}}}

\newglossaryentry{Holomorphe Funktion}{name=Holomorphe Funktion,plural=Holomorphe Funktionen,description={In jedem Punkt aus \ensuremath{U \subseteq \C} komplex differenzierbare Funktion. Ist sie auf ganz \ensuremath{\C} komplex differenzierbar, wird sie auch eine ganze Funktion genannt},text=holomorphen Funktionen}

\newglossaryentry{Isomorphismus}{name=Isomorphismus,plural=Isomorphismen,description={Ein Morphismus \ensuremath{f: X \to Y} mit beidseitigem Inversen \ensuremath{g: Y \to X}, das hei\ss t \ensuremath{f \circ g = \id_Y} und \ensuremath{g \circ f = \id_X}. Allgemein gesprochen ist ein Isomorphismus eine Abbildung, die zwei algebraische Strukturen umkehrbar eindeutig aufeinander abbildet, beide sind also \quot{sozusagen gleich}. Dazu muss die Abbildung ein bijektiver Homomorphismus sein},parent=Morphismus}

\newglossaryentry{Homomorphismus}{name=Homomorphismus,plural=Homomorphismen,description={Eine Abbildung zwischen zwei algebraischen Strukturen, die die Verk"upfungen erh"alt},parent=Morphismus}

\newglossaryentry{Modul}{name=Modul,plural=Moduln,description={"Uber einem kommutativen Ring mit Eins eine additiv abelsche Gruppe, deren Multiplikationen mit Ringelementen eines Ringes wieder im Modul liegen. Es gelten die Assoziativit"at und das Distributivgesetz. Man kann das ganze als \quot{Multiplikation mit Skalaren} interpretieren, wobei die Ringelemente die \quot{Skalare} und die Modulelemente die \quot{Vektoren} sind}}

\newglossaryentry{Ideal}{name=Ideal,plural=Ideale,description={Eine Untergruppe der additiven Gruppe eines Ringes die abgeschlossen bez"uglich der Linearkombination ist (also ein Modul). Das bedeutet, dass f"ur jedes \ensuremath{r \in R}, \ensuremath{a \in I} stets \ensuremath{ra \in I} ist, man also mit der Multiplikation nicht aus dem Ideal herauskommen kann. In nicht kommutativen Ringen muss zwischen Links- und Rechtsidealen unterschieden werden, dabei werden Ideale, die sowohl Links- und Rechtsideale sind, als zweiseitige Ideale bezeichnet}}

\newglossaryentry{Primideal}{name=Primideal,plural=Primideale,description={Ein Ideal, bei dem f"ur jedes Produkt auch mindestens ein Faktor darin liegt, also f"ur \ensuremath{ab \in \p} ist \ensuremath{a \in \p} oder \ensuremath{b \in \p}},parent=Ideal}

\newglossaryentry{Lokaler Ring}{name=Lokaler Ring,plural=Lokale Ringe,description={},text=lokaler Ring}

\newglossaryentry{Halm}{name=Halm,plural=Halme,description={Ein Halm \ensuremath{\calO_{V,x}} einer quasiprojektiven Variet"at \ensuremath{V} "uber einem K"orper \ensuremath{K} in \ensuremath{x \in V} ist definiert als \ensuremath{\calO_{V,x} = \{ [(U,f)]_{\sim}: U} offene Umgebung von \ensuremath{x, f \in \calO_V(U)\}} wobei \ensuremath{(u,f) \sim (U',f') \Leftrightarrow f_{U\cap U'} = f'_{U\cap U'}}}}

\newglossaryentry{integer}{name=integer,description={irreduzibel und reduziert (auf englisch \quot{integral})}}
...
Dies ist ein Mitschrieb der Vorlesung \quot{Geometrie der Schemata (Algebraische Geometrie II)} von Prof. Dr. F. Herrlich im Sommersemester 2012 am Karlsruher Institut f"ur Technologie (KIT). Prof. Dr. F. Herrlich ist f"ur  den Inhalt nicht verantwortlich. Die Vorlesung ist beendet und der Inhalt ist vollst"andig. Abgesehen von Designverbesserungen und eventuellen Korrekturen plane ich keine weiteren "Anderungen.

\section*{Wer}
Getippt wurde das Skript von Aleksandar Sandic. Es basiert auf meinem Skript zur Vorlesung Algebraische Geometrie aus dem Wintersemester 2011/12. Die L"osungen der "Ubungsbl"atter wurden teilweise von Myriam Finster, der "Ubungsleiterin, getippt, teilweise von mir. Die "Ubungsbl"atter wurden komplett von ihr getippt.
...
\item
	Eintr"age f"ur das Glossar sammeln.
\item
	Ich muss Farben finden, die sich auch bei einem SW-Ausdruck unterscheiden lassen und diese als Standard definieren. Farbvergleich: \textcolor{red}{Red}, \textcolor{green}{Green}, \textcolor{blue}{Blue}, \textcolor{cyan}{Cyan}, \textcolor{magenta}{Magenta}, \textcolor{yellow}{Yellow}, \textcolor{gray}{Grey}. Red, Blue und Gray sind bereits reserviert (Fehler, Links und Anmerkungen). Yellow ist zu hell, Cyan ist zu nahe an Green und Magenta ist zu dunkel um sich eindeutig von Black abzuheben.
\end{itemize}
...
\begin{Def}
Sei $X$ ein \gls{topologischer Raum}, $\mathcal C$ eine \gls{Kategorie}. Eine \deftermspec{Pr\"agarbe}{Garbe!Pr\"a-} $\mathcal F$ auf $X$ mit Werten in $\mathcal C$ besteht aus einer Abbildung $\Off(X) \to \Ob \mathcal C$, $U \mapsto \mathcal F(U)$ und \glspl{Morphismus} $\rho_U^{U'}: \mathcal F(U') \to \mathcal F(U)$ f\"ur alle $U\subseteq U'$ offen, sodass gilt:
\begin{enumerate}[i)]
\item
...
\begin{Bem}
Eine Pr\"agarbe $\mathcal F$ auf $X$ mit Werten in $\mathcal C$ ist dasselbe wie ein kontravarianter \glspl{Funktor} $\mathcal F: \Off(X) \to \mathcal C$.
\end{Bem}
...
Es gibt \emph{genau ein} $s \in \mathcal F(U)$ mit $\rho_{U_i}^U(s) = s_i$ f\"ur alle $i \in I$. Dieses $s$ wird als \defterm{Amalgam} bezeichnet.
\end{Def}

\begin{bspe}\label{bsp1.3}\begin{enumerate}[1)]
\item
  $X$ quasi-\gls{Projektive Varietaet} \"uber einem K\"orper $k$, $\mathcal O(U) = \{f: U \to \A^1(k): f \text{ regul"ar}\}$ Ring der \gls{Regulaere Funktion} auf $U$.
...
\item\label{bsp1.3.3}
  Sei $X$ topologischer Raum, $G$ Gruppe, $\mathcal G(U) := G$ f\"ur alle $U \subseteq X$ offen, $\rho_U^{U'} = \id_G$. Seien $U, U'$ offen in $X$ mit $U\cap U' = \emptyset$
...
\begin{Bem}
Sei $X$ topologischer Raum, $\mathcal F$ Garbe auf $X$. Dann ist $\mathcal F(\emptyset)$ einelementig.
\end{Bem}
...
\begin{center}\begin{tikzpicture}[baseline=0]
  ...
  \node (0,0){$\schraffiert$};
  \draw[->,font=\scriptsize] (a) -- node[above] {$\varphi_{U'}$} (b);
  \draw[->,font=\scriptsize] (c) -- node[below] {$\varphi_U$} (d);
  \draw[->,font=\scriptsize] (a) -- node[left] {$\rho_U^{U'}$} (c);
  \draw[->,font=\scriptsize] (b) -- node[right] {$\rho_U^{U'}$} (d);
...
\begin{enumerate}[a)]
\item
  F\"ur ein $x\in X$ sei ein \defterm{Halm} definiert als
    \[\mathcal F_x = \lim_{x\in U \in \Off(X)} \mathcal F(U) = \FakRaum{\{(U,s): x\in U\in \Off(X), s\in \mathcal F(U)\}}{{\sim}}\]
  wobei $(U,s)\sim(U',s') :\Leftrightarrow \exists x\in U'' \subseteq U\cap U'$ mit $\rho_{U''}^U(s) = \rho_{U''}^{U'}(s')$. $\mathcal F_x$ heißt \defterm{Halm} von $F$ in $x$.
\item
...
\item
  ...
  \begin{tikzpicture}
    ...
    \draw[->,font=\scriptsize] (a) -- node[above] {$\varphi_{U}$} (b);
    \draw[->,dashed,font=\scriptsize] (c) -- node[below] {$\varphi_x$} (d);
    \draw[->,font=\scriptsize] (a) -- (c);
    \draw[->,font=\scriptsize] (b) -- (d);
  \end{tikzpicture}
...
\begin{Bem}\label{1.7}
Sei $\calF$ Garbe von abelschen Gruppen auf $X$, $U\subseteq X$ offen, $s\in \calF(U)$. Dann gilt:
...
\proofforward
  $\checkmark$
\proofreverse
  F\"ur jedes $x\in U$ gibt es Umgebung $U_x$ mit $s|_{U_x} = 0$. ($s|_{U_x} = \rho_{U_x}^U$). Die $(U_x)_{x\in U}$ bilden offene \"Uberdeckungen, die $s|_{U_x}$ bilden konsistente Familie, $s$ und $0$ sind beides Amalgam $\xRightarrow[\text{eigenschaft}]{\text{Gruppen-}} s= 0$
...
\item
  ...
  \proofforward
    ...
    $\exists$ Umgebung von $x$ und $s\in \calF(U)$ mit $s_x =$ \gls{Keim} von $s$ in $x$ mit $\varphi_x(s_x)=$ Keim von $\varphi_U(s)$ in $x = \varphi_U(s)_s$
    \begin{center}\begin{tikzpicture}
      ...
      \draw[->,font=\scriptsize] (F) -- node[above] {$\varphi_{U}$} (G);
      \draw[->,font=\scriptsize] (Fx) -- node[above] {$\varphi_{x}$} (Gx);
      \draw[->] (F) -- (Fx);
      \draw[->] (G) -- (Gx);
      
      \draw[|->] (s) to [out=15,in=165]  (phis);
      \draw[|->] (s) to [out=240,in=120] (sx);
      \draw[|->] (sx) to [out=345,in=195] (0);
...
\item
  \begin{twosidedproof}
  \proofforward
    Sei $g_x\in\calG_x$, $(U,g)$ Repr"asentant $\Rightarrow \exists s\in \calF(U)$ mit $\varphi_U(s)=g \Rightarrow \varphi_x(s_x) = g$
    \begin{center}\begin{tikzpicture}
      \node (F) at (-2,1) {$\calF(U)$};
      \node (G) at (2,1) {$\calG(U)$};
      \node (Fx) at (-2,-1) {$\calF_x$};
      ...
      \node[anchor=base] (s) at (-2.5,1.5) {$s$};
      \node[anchor=base] (phis) at (2.5,1.5) {$\varphi_U(s)$};
      \node[anchor=base] (g) at (3.5,1.5) {$= g$};
      \node[anchor=base] (sx) at (-2.5,-1.5) {$s_x$};
      \node[anchor=base] (0) at (2.5,-1.5) {$0$};
      \node[anchor=base] (gx) at (3.5,-1.5) {$g_x$};
  ...
  \end{twosidedproof}
  \begin{bsp}
  Sei $X = \C$, $\calO$ die Garbe der \gls{Holomorphe Funktion} auf $\C$, $\calO^\X$ die Garbe der invertierbaren holomorphen Funktionen. $\varphi = \exp$, das hei\ss t f\"ur $f\in \calO(U)$ sei $\varphi(f) = e^{2\pi if}$.
...
\item
  ...
  \proofreverse
    ...
    F\"ur jedes $x\in U$ gibt es $s_x\in \calF_x$ mit $\varphi_x(s_x) = g_x$. W\"ahle Repr\"asentanten $(U_x, s^{(x)})$ von $s_x$, sodass $\varphi_{U_x}(s^{(x)}) = g|_{U_x}$ (das geht!) (\emph{denn}: Sei $(U,\tilde s)$ Repr\"asentant von $s_x \Rightarrow \varphi_U(\tilde s)\sim_x g|_U \Rightarrow \exists x \in U_x \subset U: \varphi_U(\tilde s)|_{U_x} = g|_{U_x}$)
    
    Die $U_x$ bilden offene \"Uberdeckungen von $U$, die $s^{(x)}$ bilden konsistente Familie (*) $\Rightarrow$ Es gibt ein Amalgam $s\in \calF(U)$ mit $\varphi_U(s)|_{U_x} = \varphi_{U_x}(s^{(x)}) = g|_{U_x} \Rightarrow \varphi_U(s) = g$.
    
    (*) zu zeigen: $s^{(x)}|_{U_x\cap U_y} = s^{(y)}|_{U_x\cap U_y}$
    \begin{description}[\setlabelstyle{\itshape}]
    \item[denn:]
		$\varphi_{U_x\cap U_y}(s^{(x)}|_{U_x\cap U_y} = \varphi_{U_x\cap U_y}(s^{(y)}|_{U_x\cap U_y}$, $\varphi_{U_x\cap U_y}$ injektiv nach Voraussetzung und a) $\Rightarrow $ Behauptung
    \end{description}
  \end{twosidedproof}
...
\begin{enumerate}[a)]
\item
  Es gibt genau eine Garbe $\calF^+$ auf $X$ und einen Morphismus $\vartheta: \calF \to \calF^+$, sodass $\vartheta_x:\calF_x \to \calF_x^+$ f\"ur jedes $x\in X$ ein \gls{Isomorphismus} ist.
\item
...
\item
	...
	\begin{center}\begin{tikzpicture}
		...
		\draw[->,font=\scriptsize] (F) -- node[above]{$\vartheta$} (F+);
		\draw[->,font=\scriptsize] (F) -- node[left]{$\varphi$} (G);
		\draw[->,dashed,font=\scriptsize] (F+) -- node[right]{$\exists!$} (G);
	\end{tikzpicture}\end{center}
...
\item \textcolor{white}{a} %<-- ohne das wird das Design zerschossen
  \begin{center}\begin{tikzpicture}
    ...
    \draw[->,font=\scriptsize] (FU') --node[above]{$\varphi_{U'}$} (GU');
    \draw[->,font=\scriptsize] (FU') --node[left]{$\rho_{U}^{U'}$} (FU);
    \draw[->,font=\scriptsize] (GU') --node[right]{$\rho_{U}^{U'}$} (GU);
    \draw[->,font=\scriptsize] (FU) --node[above]{$\varphi_{U}$} (GU);
  \end{tikzpicture}\end{center}
\item
  Sei $(U_i)_{i\in I}$ offene \"Uberdeckung von $U\in \Off(X)$, $s_i \in \Kern(\varphi_{U_i}) \subseteq \calF(U_i)$ konsistente Familie.
  
  Es gibt ein Amalgam $s\in \calF(U)$. $\varphi_x(s_x) = 0$ f\"ur jedes $x\in U$ $\xRightarrow{\ref{1.8} \ref{1.8a})} \varphi_U(s) = 0$
%25.4.
\item[e)]
...
Sei $f_1 \in \calF(U_1)$ mit $f_1|_{D_1} = 0, f_1|_{D_2} = 1, 0 = f_2 \in \calF(U_2) \Rightarrow f_1|_{U_1\cap U_2} \in \calG(U_1\cap U_2)$ (!)

$\bar{f_1} = \bar{f_2}$ in $\FakRaum{\calF(U_1\cap U_2)}{\calG(U_1\cap U_2)} \Rightarrow (\bar{f_i} \in \FakRaum{\calF(U_i)}{\calG(U_i)})_{i=1,2}$ ist konsistente Familie.
...
\begin{bew}
(*) Sei $0 \to \F' \overset{\varphi}{\to} \F \overset{\psi}{\to} \F'' \to 0$ exakte Sequenz in $\Sh(X)$. \emph{Achtung:} Das bedeutet \emph{nicht}, dass f\"ur jedes $\widetilde U \in \Off(X)$ die Sequenz $0 \to \F'(\widetilde U) \to \F(\widetilde U) \to \F''(\widetilde U) \to 0$ exakt sein muss.

\emph{Aber:} (*) ist \"aquivalent zu: $0 \to \F'_y \overset{\varphi_y}{\to} \F_y \overset{\psi_y}{\to} \F''_y \to 0$ ist exakt f\"ur jedes $y \in X$
...
\item[c)]
  ...
  \item
    ...
    \begin{tikzpicture}[baseline = 0]
      ...
      \draw[->] (FV) -- (FV');
      \draw[->,font=\scriptsize] (FV) -- node[right]{$\varphi_V$} (GV);
      \draw[->] (GV) -- (GV');
      \draw[->,font=\scriptsize] (FV') -- node[right]{$\varphi_{V'}$} (GV');
...
Sei $f:X\to Y$ stetig, $\F$ eine Garbe auf $X$, $\G$ Garbe auf $Y$. Dann gibt es eine (nat\"urliche) Bijektion
...
\begin{description}
\item[Behauptung:]
  $\alpha:R\to R'$ Ringhomom, $\p\subset R'$ \gls{Primideal} $\Rightarrow \alpha^{-1}(\p)$ Primideal in $R$
\item[Beweis:]
...
\item
  ...
  \proofsubseteq
    Es ist $\bar{V} = \bigcap\limits_{V\subseteq V(I)}V(I)$ Ist $I$ Ideal in $R$ mit $V\subseteq V(I)$, so ist $I\subseteq I(V) = \bigcap\limits_{\p \in V} \p$.
...
\begin{Prop}
Sei $V\subseteq \Spec R$ abgeschlossen, $V\ne 0$. Dann gilt: $V$ irreduzibel $\Leftrightarrow I(V)$ Primideal
\end{Prop}
...
\begin{bew}
\emph{Noch zu zeigen:} $f_\alpha$ stetig.

Sei $V=V(I) \subseteq \Spec R \Rightarrow f_\alpha^{-1}(V) = \{\p \in \Spec R': \alpha^{-1}(\p) \supseteq I\} = \{ \p: \p \supseteq \alpha(I)\} = V(\alpha(I))$
\end{bew}

\begin{Bem}
Sei $k$ algebraisch abgeschlossener K\"orper, $V\subseteq \A^n(k)$ \gls{Affine Varietaet}.
...
\item[injektiv:]
	$\checkmark$
\item[$m$ stetig:]
	Sei$V(I) \subseteq \Spec k[V]$ abgeschlossen.
...
\begin{bew}
Sei $U\subseteq \Spec R$ offen, $V= \Spec R - U \Rightarrow \exists I \subseteq R$ Ideal mit $V=V(I)$. F\"ur $f\in I$ ist $f\in \p$ f\"ur jedes $\p \in V$, das hei\ss t $V(I) \subseteq V(f) \Rightarrow D(f) \subseteq U$
\end{bew}
...
\begin{bew}
Sei $(U_i)_{i\in I}$ offene \"Uberdeckung von $\Spec R$. \OE $U_i = D(f_i)$ f\"ur ein $f_i\in R$.
...
\item
  Es gibt eine eindeutig bestimmte Garbe $\calO_X$ von Ringen auf $X$ mit $\calO_X(D(f)) = R_f$ f\"ur jedes $f\in R$. $\calO_X$ hei\ss t \deftermspec{Strukturgarbe}{Garbe!Struktur-} auf $X$.
\item
  F\"ur beliebiges $U\subseteq X$ offen ist $\calO_X(U) = \{ s: U \to \bigsqcup\limits^{\cdot}_{\p\in U} R_\p \vert s(\p) \in R_\p$ f\"ur alle $\p \in U\}$; f\"ur jedes $\p \in U$ gibt es Umgebung $U_\p \subseteq U$ von $\p$ und $f,g \in R$ mit $g\notin \q$ f\"ur alle $\q \in U_\p$ sodass $s(\q) = \frac{f}{g}(\q)$ f\"ur alle $\q \in U_\p\}$
...
$\Rightarrow $ erhalte \gls{Homomorphismus} $\begin{array}{ccc} R_g &\to& R_f \\ \frac{a}{g^k} &\mapsto& \frac{a\cdot h^k}{f^{d\cdot k}}\end{array}$
...
\begin{Def}
Sei $R$ ein Ring, $X = \Spec R$, $\calO_X$ die Strukturgarbe. Dann hei\ss t $(X,\calO_X)$ \deftermspec{affines Schema}{Schema!affines}.
\end{Def}
...
Sei $(X,\calO_X)$ affines Schema, $X= \Spec R$. Dann ist f\"ur jedes $f\in R$ auch $(D(f), \calO_X(f))$ affines Schema. \emph{Genauer:} $(D(f), \calO_X(D(f))) = (\Spec R_f, \calO_{\Spec R_f})$
\end{Bem}
...
\item
  Ein geringter Raum $(X, \calO_X)$ hei\ss t \deftermspec{lokal}{geringter Raum!lokal} geringter Raum, wenn f\"ur jedes $x\in X$ der \gls{Halm} $\calO_{X,x}$ ein \gls{Lokaler Ring} ist.
\end{enumerate}\end{Def}
...
Ist $(X,\calO_X)$ affines Schema, $X= \Spec R$, so ist $R = \calO_X(X)$. Ein Morphismus $\Spec S \to \Spec R$ induziert Homomophismus
...
$V\subseteq \IP^n(k)$, $k$ K\"orper quasi-projektiver Variet\"aten. $V$ besitzt endliceh \"Uberdeckung durch affine Variet\"aten $V= \bigcup_{i=1}^r X_i$.
...
\item
	$t$ setzt sich fort zu volltreuem, auf Objekten injektivem Funktor
		\[ \FakRaum{\text{quasiprojektive Variet\"aten}}{k} \to \text{ Schemata} \]
...
\begin{enumerate}[a)]
\item
	F\"ur $U\subseteq X$ offen sei $I(U):= I \cdot \calO_X(U)$, das Bild von $I$ unter Restriktion. $\mathcal I$ ist Garbe von Idealen auf $X$.
\item
...
\begin{Folg}
In der Situation \ref{4.2} wird $j:Z \to X$ zum Schemamorphismus, wobei \begin{tikzpicture}[baseline=-20]\node (0) at (-1.5, 0) {$j^\#\calO_X$}; \node (1) at (1.5,0) {$j_*\calO_Z$}; \node (2) at (0, -1) {$\calO/\mathcal I$}; \draw[->] (0) -- (1);\draw[->] (0) -- (2);\draw[->,font=\scriptsize] (2) -- node[right]{$\sim$ b)}(1);\end{tikzpicture} die Quotientenabbildung $\calO_X \to \FakRaum{\calO_X}{\mathcal I}$ ist.
\end{Folg}
...
\item
	Ist $X = \Spec R$ affines Schema, so hei\ss t eine Idealgarbe $\calI$ auf $X$ \deftermspec{quasikoh"arent}{koh"arent!quasi-}, wenn es ein Ideal $I$ in R gibt mit $\calI(U) = I\calO_X(U)$ f\"ur jedes offene $U\subseteq X$.
\item
	Eine Idealgarbe $\calI$ auf $X$ hei\ss t \defterm{koh\"arent}, wenn f\"ur jedes offene affine Unterschema $U \subseteq X$ die Einschr\"ankung $\calI|_U$ quasikoh\"arent ist.
\end{enumerate}\end{Def}
...
\item
	Sei $X$ ein Schema, $\calN_X$ die durch $\calN_X(U) =$ Nilradikal in $\calO_X(U)$ definierte Idealgarbe. Dann gilt: $\calN_X$ ist quasikoh\"arent.
\item
...
\proofsubseteq
	$x = \frac{a}{f^{d}} \in R_f, x^n = 0 \Rightarrow \frac{a^n}{f^{dn}} = 0 \Rightarrow a^n = 0$
...
\item\label{5.1c}
	...
	\begin{center}\begin{tikzpicture}
		...
		\draw[->, color2,font=\scriptsize] (0) -- node[above] {$\psi$} (1);
		\draw[->, color2,font=\scriptsize] (0) -- node[below] {$\varphi$} (2);
		
		\draw[->, dashed, color1,font=\scriptsize] (0) to [out=90, in=135] node[above] {$\exists! h$} (3);
		\draw[->, color2,font=\scriptsize] (0) to [out=270, in=225] (4);
		
		\draw[->,font=\scriptsize] (3) -- node[above] {$\pr_Y$} (1);
		\draw[->,font=\scriptsize] (3) to [out=315, in=0] node[right] {$\pr_X$} (2);
		\draw[->,font=\scriptsize,preaction={draw,ultra thick,white}] (1) to [out=0, in=90] node[left] {$g$} (4);
	\end{tikzpicture}\end{center}
\item
	...
	\begin{center}\begin{tikzpicture}
		...
		\draw[->,font=\scriptsize] (0) -- node[below] {$f$} (2);
		\draw[->,font=\scriptsize] (1) -- node[below] {$g$} (2);
	\end{tikzpicture}\end{center}
\end{enumerate}\end{DefBem}
...
\item
	...
	\emph{Erinnerung:} Das Tensorprodukt $M \otimes_R N$ von $R$-\glspl{Modul} $M,n$ \quot{linearisiert} die bilinieare Abbildung
	\begin{center}\begin{tikzpicture}
		...
		\draw[->,font=\scriptsize] (2) -- node[left] {$\Phi$ bilinear}(4);
		\draw[->, dashed, color2,font=\scriptsize] (3) -- node[right] {$\exists! \varphi$ linear} (4);
	\end{tikzpicture}\end{center}
	...
	\item
		...
		\begin{center}\begin{tikzpicture}
			...
			\draw[->, color1,font=\scriptsize] (1) -- node[below] {$\alpha$} (0);
			
			\draw[->] (4) -- (1);
			\draw[->] (4) -- (2);
			\draw[->,font=\scriptsize] (1) -- node[right] {$\sigma_A$} (3);
			\draw[->,font=\scriptsize] (2) -- node[left] {$\sigma_B$}(3);
			\draw[->, color1,font=\scriptsize,preaction={draw,ultra thick,white}] (2) to [in=15, out=165] node[above] {$\beta$} (0);
			
			\draw[->, color1,font=\scriptsize] (4) to [in=315, out=180] (0);
			\draw[->, dashed, color2,font=\scriptsize] (3) to [in=45, out=180] node[above] {$\exists! \gamma$} (0);
		\end{tikzpicture}\end{center}
...
\item
	...
	\begin{center}\begin{tikzpicture}
		...
		\draw[->,font=\scriptsize] (1) -- node[left] {$f|_{X_i}$} (4);
		\draw[->,font=\scriptsize] (2) -- node[right] {$g$}(4);
		\draw[->,font=\scriptsize] (3) -- node[right] {$\pr_{X_i}$} (1);
		\draw[->,font=\scriptsize] (3) -- node[left] {$\pr_{Y_i}$}(2);
		
		\draw[->, color1,font=\scriptsize] (0) -- node[below] {$\varphi$} (1);
		\draw[->, color1,font=\scriptsize,preaction={draw,ultra thick,white}] (0) to [in=165, out=15] node[above] {$\psi$} (2);
		
		\draw[->, color1,font=\scriptsize] (0) to [in=180, out=315] (4);
		\draw[->, color2,font=\scriptsize] (0) to [in=180, out=45] node[above] {$h$} (3);
	\end{tikzpicture}\end{center}
...
\item
	...
	\begin{minipage}[c]{0.4\textwidth}\begin{tikzpicture}[baseline=0]
		...
		\draw[->,font=\scriptsize] (0) --node[above]{$\id$} (3);
		\draw[->,font=\scriptsize] (0) --node[above]{$f$} (2);
		\draw[->,font=\scriptsize] (3) --node[below]{$f$} (1);
		\draw[->,font=\scriptsize] (2) --node[below]{$\id$} (1);
		\draw[->, color1,font=\scriptsize] (4) --node[below]{$\varphi$} (3);
		
		\draw[->, dashed, color1,font=\scriptsize] (4) to [in=180, out=45] node[above]{$\varphi$} (0);
		\draw[->, color1,font=\scriptsize,preaction={draw,ultra thick,white}] (4) to [in=165, out=15] node[above, near start]{$\psi$} (2);
...
\item
	...
	\emph{Erhalte:} $h:X \X_S Y \to X \X_S T$ aus \begin{tikzpicture}[baseline = 0]
		...
		\node (2) at (1.5,0) {$T$};
		\node (3) at (0,-1.5) {$S$};
		\node (4) at (-4.5,0) {$X \X_S Y$};
		
		\draw[->] (0) -- node[near start,left,font=\scriptsize]{$\pr_X$} (1);
		\draw[->] (0) -- node[near start,right,font=\scriptsize]{$\pr_T$} (2);
		\draw[->] (1) -- node[left,font=\scriptsize]{$f$} (3);
		\draw[->] (2) -- node[right,font=\scriptsize]{$f \circ g$} (3);
		
		\draw[->, color1] (4) -- node[below,font=\scriptsize]{$\pr_X$} (1);
		\draw[->, dashed, color1] (4) to [in=180, out=45] node[above,font=\scriptsize]{$h$} (0);
		\draw[->, color1,preaction={draw,ultra thick,white}] (4) to [in=165, out=15] node[above,near start,font=\scriptsize]{$g \circ \pr_Y$} (2);
	...
	\begin{tikzpicture}[baseline=0]
		...
		\node (4) at (-4.5, 0) {\textcolor{color1}{$Z$}};
		
		\draw[->,font=\scriptsize] (0) --node[left,near start]{$h$} (3);
		\draw[->,font=\scriptsize] (0) --node[right,near start]{$\pr_Y$} (2);
		\draw[->,font=\scriptsize] (3) --node[below]{$\pr_T$} (1);
		\draw[->,font=\scriptsize] (2) --node[below]{$g$} (1);
		\draw[->, color1,font=\scriptsize] (4) --node[below]{$\varphi$} (3);
		
		\draw[->, dashed, color1,font=\scriptsize] (4) to [in=180, out=45] node[above]{$\exists! \Phi$} (0);
		\draw[->, color1,font=\scriptsize,preaction={draw,ultra thick,white}] (4) to [in=165, out=15] node[above,near start]{$\psi$} (2);
	\end{tikzpicture}
	...
	\begin{center}\begin{tikzpicture}
		...
		\draw[->,font=\scriptsize] (0) --node[left,near start]{\textcolor{color1}{$\pr_X$}} (3);
		\draw[->,font=\scriptsize] (0) --node[right,near start]{$\pr_Y$} (2);
		\draw[->,font=\scriptsize] (3) --node[left]{$f$} (1);
		\draw[->,font=\scriptsize] (2) --node[right]{$t \circ g$} (1);
		\draw[->,color1,font=\scriptsize] (4) --node[below]{$\textcolor{black}{\pr_X} \circ \varphi$} (3);
...
Dann ist die Abbildung $\left\{ \begin{array}{ccc} X \X_S Y &\to& \{(x,y) \in X \X Y: f(x)=g(y)\} \\ z &\mapsto& (\pr_X(z), \pr_Y(z))\end{array} \right.$ surjektiv.
\begin{center}
\begin{tikzpicture}
	...
	\draw[->,font=\scriptsize] (0) -- node[above]{$\pr_X$}(1);
	\draw[->,font=\scriptsize] (0) -- node[above]{$\pr_Y$}(2);
	\draw[->,font=\scriptsize] (1) -- node[below]{$f$}(3);
	\draw[->,font=\scriptsize] (2) -- node[below]{$g$}(3);
\end{tikzpicture}
\end{center}
\end{Bem}
...
\item[b)]
	...
	\begin{center}\begin{tikzpicture}
		...
		\draw[->,font=\scriptsize] (0) --node[left,near start]{$\pr_X$} (3);
		\draw[->] (0) -- (2);
		\draw[->] (3) -- (1);
		\draw[->] (2) -- (1);
		\draw[->,font=\scriptsize] (4) --node[below]{$\iota$} (3);
		
		\draw[->,font=\scriptsize] (4) to [in=180, out=45] node[above]{$h_1$} node[below]{$h_2$} (0);
		\draw[->,font=\scriptsize,preaction={draw,ultra thick,white}] (4) to [in=165, out=15] node[above]{$\psi$} (2);
	\end{tikzpicture}\end{center}
...
\item
	...
	$U$ ist quasikompakt $\Rightarrow $ endlich viele der $g_{ijk}$ gen"ugen zum \"Uberdecken. Nenne sie $g_1, \ldots, g_r$. Sei nun $I_1 \subseteq I_2 \subseteq \ldots $ Kette von idealen in $R$. F\"ur $i=1,\ldots ,r$ sei $\varphi_i: R \to R_{g_i}$ der nat\"urliche Homomorphismus $\Rightarrow \varphi_i(I_1) \cdot R_{g_i} \subseteq \varphi_i(I_2) \cdot R_{g_i} \subseteq \ldots$ wird station"ar
	...
	\proofsupseteq
		...
		F\"ur jedes $i = 1,\ldots ,r$ gibt es $a_i \in I$, $n\in \N$ mit $\varphi_i(b) = \frac{b}{1} = \frac{a_i}{g_i^n}$ in $R_{g_i}$.	$\Rightarrow \exists m_i$ mit $g_i^{m_i} (g_i^{n_i}b - a_i) = 0$ in $R$ $\Rightarrow g_i^{m_i + n_i} b = g_i^{m_i} a_i \in I$ $\Rightarrow \exists M$ mit $g_i^M b \in I$ f\"ur $i = 1,\ldots ,r$
...
$B$ ist nach Voraussetzung endl. erzeugter $A$-Modul. Weiter ist $f^{-1}(y) = \Spec (B \otimes_A \kappa(y))$. $B \otimes_A \kappa(y)$ ist endlich-dimensionaler $\kappa(y)$-Vektorraum $\Rightarrow B \otimes_A \kappa(y)$ hat nur endlich viele Primideale
\end{bew}
...
\item
	...
	\draw[->,font=\scriptsize] (0) -- node[left]{$\pr_1$} (1);
	\draw[->,font=\scriptsize] (0) -- node[right]{$\pr_2$} (2);
	\draw[->,font=\scriptsize] (1) -- node[below]{$f$} (3);
	\draw[->,font=\scriptsize] (2) -- node[below]{$f$} (3);
	
	\draw[->,font=\scriptsize] (4) to[in=150, out=45] node[above]{$\Delta = \Delta_f$} (0);
	\draw[->,font=\scriptsize] (4) -- node[left]{$\id$} (1);
	\draw[->,font=\scriptsize,preaction={draw,ultra thick,white}] (4) to[in=165, out=15] node[above,near start]{$\id$} (2);
	\end{tikzpicture}
...
Sei $X = $ \begin{tikzpicture}[baseline=-4]
  ...
  \draw[fill, font=\scriptsize] (0,-0.25) circle(0.05) node[right] {$0_1$};
\end{tikzpicture} $\A_k^1$ mit doppeltem Nullpunkt (\S \ref{par3} Beispiel 2), $F:X \to \Spec k$ der Strukturmorphismus. $f$ ist nicht separiert, da $\Delta(X)$ nicht abgeschlossen is in $ X \X_k X$, denn $(0_1,0_2) \notin \Delta$ aber $\in \Delta$.
\end{bsp}
...
\begin{bew}
Sei $i: U \hookrightarrow X$ offene abgeschlossene Einbettung. $\Rightarrow U \X_X U \cong U$ und f\"ur $\Delta: U \to U \X_X U \cong U$ gilt $\Delta = \id_U$.
\end{bew}
...
\item
	...
	\proofreverse
		...
		\end{enumerate}\end{PropDef}
		\begin{bew}
		\cite{AM} Chapter 5, Theorem 5.11
		\end{bew}
		Sei also $R \subset K$ Bewertungsring, der $\calO$ dominiert.	Nach Vor"uberlegung gibt es Morphismus $h: T = \Spec R \to X \X_Y X$ mit $h(t_1) = x_1$, $h(t_0) = x_0$.
		...
		\begin{center}\begin{tikzpicture}
			...
			\node (3) at (3,-1) {$Y$};
			\node (4) at (-6,1) {$T$};
			\node (5) at (-4,2.5) {$\textcolor{color1}{U}$};
			...
			\draw[->, font=\scriptsize] (4) to[in=165,out=15] node[above]{$h_2$} (1);
			\draw[->, font=\scriptsize, color1] (5) to[in=150,out=0] node[above]{$h_0$} (1);
		\end{tikzpicture}\end{center}
		...
		\begin{center}\begin{tikzpicture}
			...
			\draw[->, font=\scriptsize] (5) -- node[above]{$h$}(0);
			
			\draw[->, font=\scriptsize] (4) -- node[below]{$\id$} (2);
			\draw[->, font=\scriptsize,preaction={draw,ultra thick,white}] (4) to[in=155,out=25] node[above]{$\id$} (1);
	...
	\proofforward
		...
		\emph{Existenz von $h$}: Im Basiswechseldiagramm
			\begin{center}\begin{tikzpicture}
				...
				\draw[->] (0) -- (1);
				\draw[->,font=\scriptsize] (2) -- node[below]{$h_1$}(3);
				\draw[->,font=\scriptsize] (0) -- node[left]{$f'$}(2);
				\draw[->,font=\scriptsize] (1) -- node[right]{$f$}(3);
			\end{tikzpicture}\end{center}
		...
		Sei $\varphi: U \to X \X_Y T$ der von $h_0$ und $i$ induzierte Morphismus
			\begin{center}\begin{tikzpicture}
				...
				\draw[->,dashed,font=\scriptsize] (4) -- node[above]{$\varphi$}(0);
				\draw[->,font=\scriptsize] (4) -- node[left]{$i$}(2);
				\draw[->,font=\scriptsize] (4) to[in=135,out=45] node[above]{$h_0$}(1);
			\end{tikzpicture}\end{center}
	...
	\proofreverse
		...
		$K|\kappa(y_1)$ ist endliche K\"orpererweiterung (da $f$ von endlichem Typ) $\xRightarrow{\text{Prop.\ref{8.8}}}$ Es gibt Bewertungsring $R$ von $K$, der $\calO$ dominiert $\Rightarrow$ Es gibt Morphismus $h_1: T = \Spec R \to Y'$ mit $h_1(t_i) = y_i$, $i=0,1$. Dann ist
			\begin{center}\begin{tikzpicture}
				...
				\draw[->] (0) -- (1);
				\draw[->,font=\scriptsize] (0) -- node[right,near end]{$f'$} (2);
				\draw[->,font=\scriptsize] (1) -- node[right,near end]{$f$} (3);
				\draw[->] (2) -- (3);
				\draw[->] (4) -- (0);
				\draw[->] (4) -- (5);
				\draw[->] (5) -- (2);
				
				\draw[->,dashed,color1,font=\scriptsize] (5) -- node[above]{$h'$} (0);
				\draw[->,dashed,color2,font=\scriptsize,preaction={draw,ultra thick,white}] (5) -- node[below,near end]{$h$} (1);
			\end{tikzpicture}\end{center}
		ein Bewertungsdiagramm f"ur $f$.	Nach Voraussetzung gibt es $h: T \to X$ mit \ldots
...
\item
	Sind $V, V'$ projektive Variet"aten "uber $k$, $f: V \to V'$ Morphismus, so ist der induzierte Morphismus $t(V) \to t(V')$ eigentlich.
\end{enumerate}\end{Folg}
...
Sei $\underset{\subseteq \IP_k^n}{X:= t(V)}$, $\underset{\subseteq \IP_k^m}{Y:= t(V')}$ (abgeschlossene Unterschemata)
	\begin{center}\begin{tikzpicture}
		...
		\draw[->,color1] (0) -- (2);
		\draw[->,color1] (1) -- (3);
		\draw[->,color1,font=\scriptsize] (2) -- node[below]{a)} (4);
		\draw[->,color1,font=\scriptsize] (3) -- node[below]{a)} (4);
		\draw[->,color2,font=\scriptsize] (0) -- node[right]{$g \circ f$} (4);
		\draw[->,color2,font=\scriptsize] (1) -- node[left]{$g$} (4);
	\end{tikzpicture}\end{center}
...
\item
	Idealgarben
\item
	Sei $X$ eine nichtsingul"are Kurve (\"uber einem algebraisch abgeschlossenen K\"orper $k$), $D := \Sum_{\substack{P \in X \\ \text{(abg.)}}} n_P P$ ein Divisor.	$\calO_X$ soll die Garbe der regul\"aren Differentiale auf $X$ sein.
...
\item
	...
	Es gibt $\left\{\begin{array}{ll} \Omega_{A/R} & A\text{-Modul} \\ d: \begin{array}{rcl} A &\to& \Omega_{A/R} \\ a &\mapsto& da\end{array} & \text{Derivation} \end{array}\right\}$ sodass \begin{tikzpicture}[baseline=0]
		...
		\draw[->,font=\scriptsize,dashed] (O) -- node[right,align=left,xshift=2mm]{$\exists! \varphi$ \\ $A$-linear} (M);
	\end{tikzpicture}
...
\begin{bew}\begin{enumerate}[a)]
\item
	\OE $Y = \Spec R'$ affin, $\calG = \tilde N$ f\"ur einen $R'$-Modul $N$. \OE $X = \Spec R$ affin. Damit folgt die Behauptung aus \ref{9.7} \ref{9.7b}).
	
	\textcolor{gray}{$x_1,\ldots ,x_n$ seien Erzeuger von $N$ also $R'$-Modul $\Rightarrow x \in N$ hat Darstellung $\Sum_{i=1}^n a_i x_i$}
	
	\textcolor{gray}{\emph{Behauptung}: $x_1\otimes 1,\ldots ,x_n \otimes 1$ erzeugen $N \otimes_{R'} R$ als $R$-Modul}
	
	\textcolor{gray}{Sei $x \otimes a \in N \otimes_{R'} R, x = \sum a_i x_i$ ($a_i \in R'$) $\Rightarrow x \otimes a = a \cdot \sum a_i (x_i \otimes 1)$}
\item
...
\item
	...
	ist exakt (weil $\calF$ Garbe ist, \textcolor{color1}{weil $f_*$ linksexakt ist}) $\Rightarrow f_*\calF$ ist als Kern einer Morphismus von quasikoh"arenten Garben selbst quasikoh"arent.
\end{enumerate}\end{bew}
...
\begin{Bsp}
Sei $X$ nichtsingul"are Kurve (\"uber $k$\ldots) und $D$ ein Divisor auf $X$. Dann ist $\calL(D)$ lokal frei vom Rang 1: "Ubung 9, Aufgabe 3b)
...
$(E,p)$ hei\ss t \defterm{Vektorb\"undel} vom Rang $n$ \"uber $X$, wenn es eine offene "Uberdeckung $(U_i)$ von $X$ gibt und Isomorphismen $\varphi_i: p^{-1}(U_i) \to U_i \X \R^n$ sodass
	\begin{center}\begin{tikzpicture}
		...
		\draw[->,font=\scriptsize] (1) -- node[right,xshift=1mm]{$\pr_{U_i}$} (2);
	\end{tikzpicture}\end{center}
...
Sei $X$ ein noethersches, $\underbrace{\text{irreduzibles, reduziertes}}_{=:\text{ \gls{integer} (engl. integral)}}$ Schema.
...
\item
	$\check{H}^k(\calU, \calF) := H^k(C^0(\calU, \calF)) = \FakRaum{\Kern(d^k)}{\Bild(d^{k-1})}$ hei\ss t $k$-te \deftermspec{\v{C}ech-Kohomologie}{Cech-Kohomologie@\v{C}ech-Kohomologie} von $\calF$ bez"uglich $\calU$.
\item
...
\item[c)]
	...
	\begin{center}und \begin{tikzpicture}[baseline= 0ex]
		...
		\node at (f'u|-2) (f'v) {$\calF'(V)$};
		\node (fv) at (fu|-2) {$\calF(V)$};
		\node (f''v) at (f''u|-2) {$\calF''(V)$};
		\node (3) at (1|-2) {$0$};
		...
		\node[font=\scriptsize,color1] (s2) [right=3.7 of s1] {$\tilde s''$};
		\node[font=\scriptsize,color1] (s3) [below=2.3 of s1] {$s$};
		\node[font=\scriptsize,color1] (s4) at (s2|-s3) {$s''$};
		...
		\draw[|->,color1,preaction={draw,ultra thick,white}] (s1) -- (s2); \draw[|->,color1,preaction={draw,ultra thick,white}] (s1) -- (s3); \draw[|->,color1,preaction={draw,ultra thick,white}] (s2) -- (s4); \draw[|->,color1,preaction={draw,ultra thick,white}] (s3) -- (s4);
...
\end{tikzpicture}
\textcolor{gray}{\tiny{($\calI(X)= \Hom_{\calO_X}(\calO_X, \calI)$, $\calI(U)= \Hom_{\calO_X}(j_!(\calO_X(U), \calI))$)}}
...
Sei $\calF$ welke Garbe, $\calI$ injektive Garbe mit $\calF \subseteq \calI \Rightarrow 0 \to \calF \to \calI \to \calG \to 0$ exakt mit $\calG = \FakRaum{\calI}{\calF}$. Die lange exakte Kohomologiesequenz dazu ist:
...
Sei $S = \bigoplus_{i = 0}^\infty S_i$ graduierter Ring und $X = \Proj S$. Sei weiter $M = \bigoplus_{i \in \Z} M_i$ ein graduierter $S$-Modul.
\begin{enumerate}[a)]
\item
...
\end{enumerate}
\textcolor{gray}{($S_f^{\hhom} = \{ \frac{a}{f^n}: a \in S_{n\cdot d}\}$, $D^+(f) \cong \Spec S_f^{\hhom}$, $f$ homogen vom Grad $d$, $M_f^{\hhom} = \{ \frac{m}{f^n}: m \in M_{n \cdot d} \}$)}\end{DefBem}
...
\begin{enumerate}[a)]
\item
	$H^n(\IP^n, \calO(-n-1)) = R \frac{1}{X_0\cdot \ldots \cdot X_n} \cong R$
\item
...
\item[b)]
	...
	\end{enumerate}
\item[c)]
	Sei $1 \le i \le n - 1$, $d \in \Z$, $0 \le k \le n$.
	
	\emph{Behauptung:} Dann ist die Multiplikation mit $X_k$ ein Isomorphismus $H^{i}(\IP^n, \calO(d-1)) \to H^{i}(\IP^n, \calO(d))$.
	
	Jedes $\alpha \in H^{i}(\IP^n, \calO(d))$ wird repr"asentiert von einem Tupel von Linearkombinationen von Monomen $X_{j_0}^{\nu_0} \cdot \ldots \cdot X_{j_i}^{\nu_i}$ mit $\sum \nu_k = d$, $\nu_k < 0$ f"ur alle $k$. Multipliziere mit $X_{j_i}^{-\nu_i}$. Das Bild von $X_{j_0}^{\nu_0} \cdot \ldots \cdot X_{j_n}^{\nu_n}$ in $H^{i}(\IP^n, \calO(d - \nu_i))$ ist $0$. Nach der Behauptung ist damit auch $\alpha = 0$.
	\begin{description}[\setlabelstyle{\itshape}]
	\item[Beweis der Behauptung:] \OE $k = n$
	
		$\cdot X_n$ induziert exakte Sequenz von graduierten $S$-Moduln ($S = R[X_0,\ldots ,X_n]$)
			\[ 0 \to S(d-1) \overset{\cdot X_n}{\to} S(d) \to \underset{\substack{\cong \smallFakRaum{S}{X_n S(d)} \\ \cong R[X_0,\ldots ,X_n](d)}}{\FakRaum{S(d)}{X_n \cdot S(d-1)}} \to 0 \]
		Daraus ergibt sich exakte Sequenz von $\calO_{\IP^n}$-Modulgarben:
			\[ (*) \quad 0 \to \calO(d-1) \overset{\cdot X_n}{\to} \calO(d) \to j_*\calO_{\IP^{n-1}}(d) \to 0 \qquad \textcolor{gray}{(j: \IP^{n-1}=V(X_n) \hookrightarrow \IP^n)} \]
		Es gilt: $H^{i}(\IP^n, j_*\overbrace{\calO_{\IP^{n-1}}(d)}^{=: \calF}) \cong H(\IP^{n-1}, \calO_{\IP^{n-1}}(d))$, \emph{denn:} Sei $0 \to \calF \to \calG^\bullet$ welke Aufl"osung $\Rightarrow 0 \to j_*\calF \to j_*\calG^\bullet$ ist welke Aufl"osung.
		
		\emph{Induktion "uber $n$:} $n = 0 \checkmark$, $n = 1 \checkmark$
		\begin{description}[\setlabelstyle{\normalfont}]
		\item[$n \ge 2$:]
			Lange exakte Kohomologiesequenz zu (*):
				\[ \ldots \to H^{i-1}(\IP^{n-1}, \calO_{\IP^{n-1}}(d)) \to H^{i}(\IP^{n}, \calO(d-1)) \overset{\cdot X_n}{\to} H^{i}(\IP^{n-1}, \calO(d)) \to \ldots \]
			Nach Induktionsvoraussetzung ist $H^{i}(\IP^{n-1}, \calO_{\IP^{n-1}}(d)) = 0$ f"ur $1 \le i \le n-2$. Nach der Behauptung folgt, dass $i = 2,\ldots ,n-2$
			\begin{description}[\setlabelstyle{\normalfont}]
			\item[\underline{$i = 1$:}]
				$0 \to \overset{=S_{d-1}}{H^0(\IP^n, \calO(d-1))} \to \overset{=S_d}{H^0(\IP^n, \calO(d))} \to \overset{=\smallFakRaum{S_d}{X_nS_{d-1}}}{H^0(\IP^{n-1}, \calO(d))} \to 0$ ist exakt $\Rightarrow H^1(\IP^n, \calO(d-1)) \overset{\cdot X_n}{\to} H^1(\IP^n, \calO(d))$ ist injektiv, $H^1(\IP^n, \calO(d-1)) \overset{\cdot X_n}{\to} H^1(\IP^n, \calO(d))$ ist surjektiv f"ur $n \ge 3$ nach Induktionsvoraussetzung. F"ur $n = 2$ ist $1 = n - 1$.
			\item[\underline{$i = n-1$:}]
				Nach Induktionsvoraussetzung ist $H^{n-2}(\IP^{n-1}, \calO(d)) = 0$.
				
				Zu zeigen also: $H^{n-1}(\IP^n, \calO(d)) \to H^{n-1}(\IP^{n-1}, \calO(d))$ ist die Nullabbildung. "Aquivalent: $\delta: \underbrace{H^{n-1}(\IP^{n-1}, \calO(d))}_{\substack{\text{erz. v. den Monomen} \\ X_0^{\nu_0} \cdot \ldots X_{n-1}^{\nu_{n-1}} \\ \text{mit } \sum \nu_i = d \text{, alle } \nu_i < 0}} \to H^n(\IP^n, \calO(d-1))$ ist injektiv.
				
				Das Bild von $\delta$ ist der Kern von $\cdot X_n$, also der freie $R$-Modul mit Basis $X_0^{\nu_0} \cdot \ldots \cdot X_{n-1}^{\nu_{n-1}} \cdot X_n^{-1}$ mit $\sum_{i=0}^{n-1} \nu_i = d$, alle $\nu_i < 0$ $\Rightarrow \Rang(\Bild \delta) = \Rang(H^{n-1}(\IP^{n-1}, \calO(d))) \Rightarrow \delta$ injektiv.	"Ubung: $H^1(\IP^2, \calO(d)) = 0$ f"ur alle $d \in \Z$
			\end{description}
		\end{description}
	\end{description}
\end{enumerate}\end{bew}
...
\begin{Satz}
Sei $X$ projektives $R$-Schema "uber einem noetherschen Ring $R$. Dann ist $H^{i}(X, \calF)$ endlich erzeugter $R$-Modul f"ur jede koh"arente Garbe $\calF$ auf $X$.
\end{Satz}

\begin{bew}
\textcolor{gray}{$\tilde U \subseteq \IP_R^n$ offen, $U = \tilde U \cap X \Rightarrow j_*\calF(\tilde U) = \calF(U)$, $\calF|_U = \tilde M \Rightarrow j_*\calF|_{\tilde U} = \tilde M_U$)}

Sei $j: X \hookrightarrow \IP_R^n$ abgeschlossene Einbettung, $X = \Proj(\FakRaum{R[X_0,\ldots ,X_n]}{I})$. Dann ist $j_*\calF$ koh"arent und $H^{i}(X, \calF) \cong H^{i}(\IP_R^n, j_*\calF)$. Also ohne Einschr"ankung $X = \IP_R^n$.

\emph{Behauptung:} Jede koh"arente Garbe $\calF$ auf $\IP_R^n$ ist isomorph zu $\FakRaum{\bigoplus\limits_{i = 1}^r \calO(d_i)}{\calG}$ f"ur geeignete $r \ge 1$, $d_i \in \Z, \calG$

Dann sei $0 \to \calG \to \bigoplus\limits_j \calO(d_j) \to \calF \to 0$ exakt $\Rightarrow H^{i}(\IP^n, \bigoplus \calO(d_j)) \cong \bigoplus\limits_j H^{i}(\IP^n, \calO(d_j))$ (!)

$\Rightarrow H^{i}(\IP^n, \bigoplus \calO(d_j))$ endlich erzeugte $R$-Moduln

%19. 7.
$\xRightarrow{\text{lange ex. Sequenz}} \ldots \to \underset{\text{endlich erzeugt}}{\bigoplus\limits_j H^{i}(\IP^n, \calO(d_j))} \overset{d^{i}}{\to} H^{i}(\IP^n, \calF) \overset{\delta^{i}}{\to} H^{i+1}(\IP^n, \calG) \to \ldots$

Absteigende Induktion "uber $i$:
	\[ H^{n+1}(\IP^n, \calG) = 0 \text{ weil } n + 1 > m \]
$\Rightarrow H^n(\IP^n, \calF)$ endlich erzeugt, weil $d^n$ surjektiv

$\Rightarrow H^n(\IP^n, \calG)$ endlich erzeugt, weil $\calG$ auch koh"arent

$\Rightarrow H^{n-1}(\IP^n, \calF)$ endlich erzeugt, weil $\Kern(\delta^{n-1}) = \Bild(d^{n-1})$ endlich erzeugt und $\Bild(\delta^{n-1})$ als Untermodul von $H^n(\IP^n, \calG)$ endlich erzeugt

Die Behauptung folgt aus:
\end{bew}

\begin{Prop}
Sei $\calF$ koh"arente Garbe auf $\IP_R^n$. Dann gibt es ein $d_0 \in \Z$, sodass f"ur $d \ge d_0$ $\calF(d) := \calF \otimes_{\calO} \calO(d)$ von globalen Schnitten erzeugt wird, das hei\ss t es gibt $s_1,\ldots ,s_r \in \Gamma(\IP^n, \calF(d))$, sodass f"ur jedes offene $U \subseteq \IP_R^n$ gilt: $\calF(d)(U)$ wird erzeugt von $s_1|_U,\ldots ,s_r|_U$ (als $\calO_{\IP^n}(U)$-Modul!)

Definiere Garbenmorphisus $\epsilon: \left\{ \begin{array}{ccc} \bigoplus_{i=1}^r \calO_{\IP^n} &\to& \calF(d) \\ e_i &\mapsto& s_i \end{array} \right.$

$\epsilon$ ist surjektiv $\Rightarrow e_{-d}: \bigoplus_{i=1}^r \calO(-d) \to \calF$ surjektiv
\end{Prop}

\begin{bew}
Sei $U_i = D(X_i)$, $M_i := \calF(U_i)$. $M_i$ ist endlich erzeugter $R[\frac{X_0}{X_i},\ldots ,\frac{X_n}{X_i}]$-Modul. $\calF|_{U_i} = \tilde M_i$. Seien $s_{i_1},\ldots ,s_{i_r}$ Erzeuger von $M_i$ als $R[\frac{X_0}{X_i},\ldots ,\frac{X_n}{X_i}]$-Modul.

Auf $U_i \cap U_j$ ist $s_{i_\nu}|_{U_i \cap U_j} \in \calF(U_i \cap U_j) = (M_j)_{\frac{X_i}{X_j}} \Rightarrow$ Es gibt $d_{i_\nu}$ mit $s_{i_\nu} \cdot X_i^{d_{i_\nu}} \in \Gamma(U_i \cap U_j, \tilde M_j \otimes \calO(d_{i_\nu}))$ f"ur alle $j$. Sei $d:= \max \{d_{i_\nu}: i, \nu\} \Rightarrow s_{i_\nu} \cdot X_i^d \in \Gamma(\IP^n, \calF \otimes \calO(d))$ f"ur alle $i, \nu \Rightarrow$ die $t_{i_\nu}$ erzeugen $\calF(d)$.
\end{bew}

\begin{satz}[Grothendieck]
Sei $X$ ein $n$-dimensionales noethersches Schema, $\calF$ eine Garbe von abelschen Gruppen. Dann ist $H^{i}(X, \calF) = 0$ f"ur alle $i > n$.
\end{satz}
...
\item
  ...
  \item[Fall 1:]
    ...
    \item{$\bullet$ mit $x\in U_j$:}
      \[s_j = \id_A(s_j) = \rho_{U_i\cap U_j}^{U_j}(s_j) = \rho_{U_i\cap U_i}^{U_j}(s_j) = \id_A(s_i) = s_i = s = \rho_{U_j}^U(s)\]
    \item{$\bullet$ mit $x\notin U_j$:}
...
\proofsubseteq
  ...
  Nach Definition von $\calF^+$ gibt es \"Uberdeckung $U = \bigcup_{i \in I} U_i$ und Schnitte $s_i \in\calF(U_i)$ mit $s|_{U_i} = \bar{s_i}$. Die $\bar{s_i}: U_i \to \Spe(\calF)$ sind stetig. Sei $\widetilde U \subseteq \Spe(\calF)$ offen.
...
\begin{Aufg}[4 Punkte]
 Beweise die Proposition \ref{1.14} aus der Vorlesung:
...
\begin{tikzpicture}[baseline = 0]
	...
	\draw [->,font=\scriptsize] (0) -- node[left]{$\psi_j$} (2);
	\draw [->,font=\scriptsize] (1) -- node[right]{$\psi_k$} (2);
	\draw [->,font=\scriptsize] (0) -- node[above]{$\rho_k^j$} (1);
	\draw [->,font=\scriptsize] (2) -- node[right]{$\exists!$} (3);
	
	\draw [->,font=\scriptsize] (0) to [out=225,in=165] node[left]{$h_j$} (3);
	\draw [->,font=\scriptsize] (1) to [out=315,in=15] node[right]{$h_k$} (3);
\end{tikzpicture}
\begin{tikzpicture}[baseline = 0]
	\node (0) at (-1, 0.5) {$A_j$};
	\node (1) at (1, 0.5) {$A_k$};
	...
	\draw [->,font=\scriptsize] (0) -- node[left]{$h_j$} (2);
	\draw [->,font=\scriptsize] (1) -- node[right]{$h_k$} (2);
	\draw [->,font=\scriptsize] (0) -- node[above]{$\rho_k^j$} (1);
\end{tikzpicture}
\end{center}
...
\begin{tikzpicture}[baseline = 0]
	...
	\draw[->,font=\scriptsize] (0) -- node[above]{$\rho_{V'}^{V''}$} (1);
	\draw[->,font=\scriptsize] (2) -- node[above]{$\rho_{V'}^{V''}$} (3);
	
	\draw[->,font=\scriptsize] (0) -- node[left]{$\beta(V'')$} (2);
	\draw[->,font=\scriptsize] (1) -- node[right]{$\beta(V')$} (3);
	
	\draw[->,font=\scriptsize] (2) -- node[left]{$\widetilde \psi(V'')$}(4);
	\draw[->,font=\scriptsize] (3) -- node[right]{$\widetilde \psi(V'')$}(4);
\end{tikzpicture}
$\Longrightarrow$
\begin{tikzpicture}[baseline = 0]
	\node (0) at (-2, 1) {$\calG(V')$};
	...
	\draw [->,font=\scriptsize] (0) -- node[left]{$\widetilde \psi (V') \circ \beta(V')$} (2);
	\draw [->,font=\scriptsize] (1) -- node[right]{$f^{-1}\beta$} (2);
	\draw [->,font=\scriptsize] (0) -- node[above]{$\psi_{f^{-1}\calG}$} (1);
\end{tikzpicture}
...
\begin{tikzpicture}[baseline = 0]
	...
	\draw [->] (0) -- node[left,font=\scriptsize]{$\rho$} (2);
	\draw [->] (1) -- node[right,font=\scriptsize]{$\rho$} (2);
	\draw [->] (0) -- node[above,font=\scriptsize]{$\rho$} (1);
\end{tikzpicture}
\end{center}
...
\begin{equation*}\begin{array}{rlcccl}
	...
	T_2:& \Hom(\calG, f_*\calF) &\ni &\beta &\mapsto & \varphi_\calF \circ f^{-1} \beta \in \Hom(f^{-1}\calG, \calF)
...
\begin{center}\begin{tikzpicture}
	...
	\node (2) at (-4,2) {$f^{-1}\calG(f^{-1}(V)) = f_*f^{-1}\calG(V)$};
	\node (3) at (2,2) {$\calG(V)$};
	\node (4) at (7,2) {$f^{-1}\calG(U)$};
	
	\node (5) at (-1,-2) {$\calF(U)$};
	
	\node at (1.5,1) {$\schraffiert$};
	\node at (-3,1) {$\schraffiert$};
	\node at (1.5,3) {$\schraffiert$};
	\node at (-2,-1) {$\schraffiert$};
	\node at (4.5,0) {$\schraffiert$};
	
	\draw[->,font=\scriptsize] (1) -- node[below]{$\tilde \psi(V)$} (0);
	\draw[->,font=\scriptsize] (2) -- node[left]{$\alpha(f^{-1}(V))=f_*\alpha(V)$} (1);
	\draw[->,font=\scriptsize] (3) -- node[above]{$\psi_\calG(V)$}(2);
	\draw[->,font=\scriptsize] (3) -- node[above]{$\psi_{f^{-1}\calG}(V)$} (4);
	\draw[->,dashed,font=\scriptsize] (3) -- (1);
	\draw[->,dashed,font=\scriptsize] (4) -- node[below, sloped]{$f^{-1}(f_*\alpha \circ \psi_\calG)(U)$} (0);
	\draw[->,font=\scriptsize] (0) -- node[right]{$\varphi_\calF(U)$} (5);
	\draw[->,font=\scriptsize] (1) to [out=270,in=180] node[below]{$\rho_U^{f^{-1}(V)}$} (5);
	\draw[->,font=\scriptsize] (2) to [out=30,in=150] node[above]{$\rho_U^{f^{-1}}(V)$} (4);
	\draw[->,font=\scriptsize] (4) to [out=270,in=0] node[below]{$\hat \alpha(U)$}(5);
\end{tikzpicture}\end{center}
...
\begin{enumerate}[a)]
\item
	Betrachte den kanonischen Ringhomomorphismus $\varphi\colon R \to R_S$. Sei $I'=\left(\varphi(I)\right)$ das von $\varphi(I)$ erzeugte Ideal in $R_S$, also $I'=\{\frac{f}{a}\in R_S\mid f\in I,\ a\in S\}$.
...
\item
	\begin{twosidedproof}
	\proofsupseteq
		Ist $f \in \sqrt{I}$, so existiert ein $n \in \N$ mit $f^n \in I$. Folglich gilt f\"ur alle $\mathfrak{p} \in \Spec R$ mit $I \subseteq \mathfrak{p}$, dass $f^n \in \mathfrak{p}$, also auch $f \in \mathfrak{p}$. 
	\proofsubseteq
		Sei $a\in R\setminus\!\sqrt{I}$ und $S:=\{a^n\mid n\in\N_0\}$. Dann ist $S$ ein multiplikatives System und $I\cap S=\emptyset$. Nach a) gibt es dann ein Primideal $\mathfrak{p}$ mit $\mathfrak{p} \supseteq I$ und $\mathfrak{p} \cap S=\emptyset$. Damit liegt $a$ nicht im Schnitt \"uber alle Primideale, die $I$ enthalten.
	\end{twosidedproof}
\end{enumerate}
\end{Loes}
...
\begin{center}\begin{tikzpicture}
	...
	\draw[->,font=\scriptsize] (0) -- node[above]{$\Phi$} (1);
	\draw[->,font=\scriptsize] (1) -- node[above]{$\Psi_X$} (2);
	\draw[->,font=\scriptsize] (3) -- node[below]{$\Psi_{\Spec S}$} (4');
	\draw[->,font=\scriptsize] (1) -- node[left]{$\rho_{\Spec S}^X$} (3');
	\draw[->,font=\scriptsize] (2) -- node[right]{$\cong$} (4);
\end{tikzpicture}\end{center}
...
\begin{center}(*)\begin{tikzpicture}[baseline=0]
	...
	\draw[->,font=\scriptsize] (0) -- node[above]{$\Phi$} (1);
	\draw[->,font=\scriptsize] (0) -- node[right]{$i$} (2');
	\draw[->,font=\scriptsize] (1) -- node[right]{$\rho_{\varphi^{-1}(D(f))}^X$} (3');
	\draw[->, dashed,font=\scriptsize] (2) -- node[below]{$h = \varphi^\#(D(f))$} (3);
	\draw[|->] (4) -- (5);
...
\begin{Loes}
$X = \Spec R$, $Y = \Spec S$, $\varphi: R \to S$ Ringhomomorphismus $\leadsto$ Schemamorphismus $(g, g^\#)$
\begin{enumerate}[a)]
\item
	\emph{Zu zeigen:} $f \in R$ nilpotent $\Leftrightarrow D(f) = emptyset$
	
	$f$ nilpotent $\Leftrightarrow f \in \sqrt{(0)} = \bigcap\limits_{\substack{\p \in \Spec R \\ (0) \subset \p}} \p \Leftrightarrow \ \forall \ \p \in \Spec R: f \in \p \Leftrightarrow D(f) = \emptyset$
\item
	$\varphi$ injektiv $\Leftrightarrow g^\#$ injektiv
	\begin{twosidedproof}
	\proofreverse
		$\varphi = g^\#(\Spec R)$
	\proofforward
		\emph{Behauptung:} Es reicht zu zeigen: $g^\#$ ist injektiv auf Basis der Topologie.
		
		\emph{Denn:} $U \in \Off(X)$, $U = \bigcup_{i\in I} U_i$, $g^\#(U_i)$ injektiv, $g^\#(U)(S) = 0$ $\Rightarrow g^\#(U)(S)|_{U_i} = g^\#(U_i)(S|_{U_i}) = 0 \xRightarrow[\text{injektiv}]{g^\#(U_i)} S|_{U_i} = 0 \xRightarrow{\text{Garbe}} S = 0$
		
		Sei $f \in R$, $g^\#(D(f)): \left\{ \begin{array}{ccc} \calO_X(D(f)) = R_f &\to& \calO_Y(g^{-1}(D(f)) = \calO_Y(D(\varphi(f)) \\ \frac{a}{f^n} &\mapsto& \frac{\varphi(a)}{\varphi(f)^n}\end{array} \right.$
		
		\emph{Zu zeigen:} $g^\#(D(f))$ injektiv:
		
		Sei $\frac{g}{f^n} \in \calO_X(D(f))$ mit $g^\#(D(f))(\frac{a}{f^n}) = \frac{\varphi(a)}{\varphi(f)^n} = 0$
		
		$\Rightarrow \exists \ m \in \N: \underset{= \varphi(f^m \cdot a)}{\varphi(f)^m \cdot \varphi(a)} = 0$ in $S$ $\xRightarrow{\varphi \text{ inj.}} f^m \cdot a = 0 \Rightarrow \frac{a}{f^n} = 0$ in $R_f$
		
		\emph{Zu zeigen:} $\varphi$ injektiv $\Rightarrow g$ dominant ($g: Y \to X$)
		
		\emph{Annahme:} $\overline{g(Y)} \ne X$
		
		$\Rightarrow X \setminus \overline{g(Y)} \ne \emptyset$ offen $\Rightarrow \exists \ f \in R$ mit $D(f) \ne \emptyset$ und $D(f) \cap \overline{g(Y)} = \emptyset \Rightarrow g^{-1}(D(f)) = D(\varphi(f)) = \emptyset \Rightarrow \exists \ m \in \N: \underset{= \varphi(f^m)}{\varphi(f)^m} = 0 \xRightarrow{\varphi \text{ inj.}} f^m = 0 \xRightarrow{\text{a)}} D(f) = \emptyset \lightning$
	\end{twosidedproof}
\item
	Ist $\varphi\colon R \to S$ surjektiv, so ist $S \cong R/I$ mit $I = \Kern(\varphi)$. Bei der L"osung von Aufgabe 4 auf \"Ubungsblatt 2 haben wir bereits eingesehen, dass $g = \Spec(\varphi)$ injektiv ist und dass $\Bild(g) = \{ \p \in \Spec R \mid I \subseteq \p \} = V(I)$. Die Umkehrabbildung $g^{-1} \colon V(I) \to \Spec(S), \p \mapsto \varphi(\p) = \p + I$ ist folglich wohldefiniert.
	
	Es bleibt zu zeigen, dass $g^{-1}$ stetig ist. Sei dazu $V(J) \subseteq \Spec S$ abgeschlossen. Dann ist 
	\[ \begin{array}{rcl} (g^{-1})^{-1}(V(J)) = g(V(J)) &=& g(\{\q \in \Spec S \mid J \subseteq \q \})\\
		&=& \{ \p \in V(I) \mid J \subseteq g^{-1}(\p) = \varphi(\p)\}\\
		&=& \{p \in \Spec R \mid \varphi^{-1}(J) \subseteq \p \} = V(\varphi^{-1}(J)) \end{array}
	\]
	abgeschlossen.
...
\begin{Loes}
Zum Beispiel: $R = k[x]_{(x)}$, $Y = \Spec R = \{(0), (x)\}$, $\Off(Y) = \{ \emptyset, U = \underbrace{\{(0)\}}_{=D(x)}, Y\}$
\begin{center}\begin{tikzpicture}
	\matrix (m) [matrix of math nodes, row sep=1em, column sep=0em]
	{ \calO_Y(Y) & = & k[x]_{(x)}\\
	  \calO_Y(U) & = & k(x)\\
	...
	\path[right hook->,font=\scriptsize] (m-1-3) edge[bend right=-75] node[auto] {(inklusion)} (m-2-3);
	\path[->, font=\scriptsize]
		(m-2-3) edge[bend right=-60] node[auto] {Nullabb.} (m-3-3);
\end{tikzpicture}\end{center}
Definiere Idealgarbe: $\calI(Y) = \{0\} \to \calI(U) = k(x) \to \calI(\emptyset) = \{0\}$. W"are $\calI$ quasikoh"arent, so g"alte  $\calI(U) = \rho_U^Y(\calI(Y)) \cdot k(x) = 0$.

$\Rightarrow \calI$ nicht quasikoh"arent.
\end{Loes}
...
\emph{Behauptung:} $\calF$ quasikoh\"arent $\Leftrightarrow \exists$ offene, affine \"Uberdeckung $X = \bigcup_{i\in I} U_i: \calF|_{U_i}$ quasikoh\"arent\begin{twosidedproof}
...
$\hat U = \bigcup_{i\in I'} U_i \Rightarrow $ \OE $X = \Spec R$, also affin und \OE $U_i = D(g_i) \subseteq R$ mit $g_i \in R$ $\leadsto$ Insbesodnere $\calO_X(U_i) = R_g$, $U_i \cong \Spec R_{g_i}$, $\calJ := \calF(X)$
...
\begin{center}\begin{tikzpicture}
	...
	\draw[->,font=\scriptsize,color1,preaction={draw,ultra thick,white}] (2) to[in=195, out=345] (3);
...
\item
	...
	\begin{center}\begin{tikzpicture}
		...
		\draw[->, font=\scriptsize] (4) to[in=145, out=0] (1);
	...
	\begin{center}\begin{tikzpicture}
		...
		\node(5) at (-2,1.25) {$x$};
		\node(6) at (1.75,1.25) {$x$};
		\node(7) at (-2,1.75) {$z$};
		\node(8) at (1.75,1.75) {$y$};
		
		\node(9) at (-3,0.75) {$x$};
		\node(10) at (-3,-0.75) {$x$};
		
		\node(11) at (-2,-1.25) {$x$};
		\node(12) at (1.75,-1.25) {$x$};
		...
		\draw[->, font=\scriptsize] (1) to[in=0, out=115] node[above]{$h_2$} (4);
...
\item
	...
	\begin{center}\begin{tikzpicture}
		...
		\draw[thick,bend right=45,looseness=1.75,dashed,color1,shift=({-2pt,2pt})] (-4.5,0.5) to (-0.5,4.5);
		\draw[thick,bend left=45,looseness=1.75,dashed,color1,shift=({2pt,-2pt})] (0.5,-4.5) to (4.5,-0.5);
	\end{tikzpicture}\end{center}
...
\begin{tikzpicture}[]
	...
	(m-1-1) edge [bend right=15] node[left] {$\varphi_1$}  (m-4-2)
                ...
                edge [dashed] node[above] {$\theta$} (m-2-2)
...
\begin{tikzpicture}
	\node (X) at (0,0) {$X$};
	\node[right=2.5em of X] (Pn) {$\IP_Y^n$};
	\node[right=6em of X] (Pnm) {$\IP_{\Z}^n \X_{\Spec \Z} \IP_Z^m$};

	\node[below=2em of Pn] (Y) {$Y$};
	\node at (Pnm|-Y) (Pm) {$\IP_Z^m$};
	
	\node[below=3em of Pm] (Z) {$Z$};
	
	\draw[right hook->] (X) -- node[font=\scriptsize,above]{$i$} (Pn);
	\draw[right hook->] (Pn) -- node[font=\scriptsize,above]{$\tilde i$} (Pnm);
	\draw[right hook->] (Y) -- node[font=\scriptsize,above]{$i'$} (Pm);
	
	\draw[->] (Pn) -- node[font=\scriptsize,right]{$\pr_Y$} (Y);
	\draw[->] (Pnm) -- (Pm);
	\draw[->] (Pm) -- node[font=\scriptsize,right]{$\pr_Z$} (Z);
	
	\draw[->] (X) -- node[font=\scriptsize,below]{$f$} (Y);
	\draw[->] (Y) -- node[font=\scriptsize,below]{$g$} (Z);
\end{tikzpicture}
\end{minipage}
\begin{minipage}{7cm}
...
\begin{center}\begin{tikzpicture}
	\node (m-1-1) at (0,0) {$X$};
	\node (m-1-2) at ([shift=({2cm,0cm})]m-1-1) {$\IP^n_Y$};
	\node (m-1-3) at ([shift=({3cm,0cm})]m-1-2) {$\IP^n_{\Z} \times_{\Spec \Z} \IP^m_Z$};
	\node (m-1-4) at ([shift=({3cm,0cm})]m-1-3) {$\IP^N_Z$};
	
	\node (m-2-2) at ([shift=({2cm,-1.5cm})]m-1-1) {$Y$};
	\node (m-2-3) at (m-1-3|-m-2-2) {$\IP^m_Z$};
	
	\node (m-3-3) at ([shift=({5cm,-3cm})]m-1-1) {$Z$};
...
\begin{tikzpicture}[baseline=0]
	\node (m-1-1) at (0,0.75) {$X \times_Y Y'$};
	\node (m-1-2) at ([shift=({2.5cm,0cm})]m-1-1) {$X$};
	\node (m-1-3) at ([shift=({2cm,0cm})]m-1-2) {$\IP^n_{Y}$};
	
	\node (m-2-1) at ([shift=({0cm,-1.5cm})]m-1-1) {$Y'$};
	\node (m-2-2) at ([shift=({2.5cm,0cm})]m-2-1) {$Y$};
	\path[->, font=\scriptsize]
...
\item
	...
	$X$ ist noethersch, \gls{integer}, separiert $\checkmark$
...
\begin{Aufg}[zum Nachdenken, keine Abgabe]
...
\begin{Loes}
$\mathcal A$ abelsche Kategorie, $A, B \in \Ob(\mathcal A)$, $0 \to A \to I^\bullet$, $0 \to B \to J^\bullet$ injektive Aufl"osungen, $f: A \to B$
\begin{enumerate}[a)]
\item
	Setze $f$ zu Morphismus $(\alpha^k)_{k\ge0}$ der Kettenkomplexe fort, $\alpha^k: I^k \to J^k$. Seien $\alpha^{-1} = f, \alpha^0, \ldots ,\alpha^n$ gegeben.
	\begin{center}\begin{tikzpicture}
		\def\xdist{1.5}
		\def\ydist{2cm}
		
		\node (01) at (-6,1) {$0$}; \node (02) at ([yshift=-\ydist]01) {$0$};
		\node (a) [right=\xdist of 01] {$A$}; \node (b) at ([yshift=-\ydist]a) {$B$};
		\node (i0) [right=\xdist of a] {$I^0$}; \node (j0) at ([yshift=-\ydist]i0) {$J^0$};
		\node (d1) [right=1 of i0] {$\ldots$}; \node (d2) at ([yshift=-\ydist]d1) {$\ldots$};
		\node (in1) [right=1 of d1] {$I^{n-1}$}; \node (jn1) at ([yshift=-\ydist]in1) {$J^{n-1}$};
		\node (in) [right=\xdist of in1] {$I^{n}$}; \node (jn) at ([yshift=-\ydist]in) {$J^{n}$};
		\node (in+1) [right=\xdist of in] {$I^{n+1}$}; \node (jn+1) at ([yshift=-\ydist]in+1) {$J^{n+1}$};
		\node[font=\scriptsize] (mod) at ([shift=({1.25,-1})]in) {$\smallFakRaum{I^n}{\Kern(d^n)}$};
		\node at ([shift=({1,-1})]a) {$\schraffiert$}; \node at ([shift=({1.1,-1})]in1) {$\schraffiert$}; \node at ([shift=({1.25,-0.4})]in) {$\schraffiert$};
		
		\draw[->](01)--(a); \draw[->](a)--node[above,font=\scriptsize]{$\varphi$}(i0); \draw[->](i0)--(d1); \draw[->](d1)--(in1); \draw[->](in1)--(in); \draw[->](in)--(in+1);
		\draw[->](02)--(b); \draw[->](b)--node[below,font=\scriptsize]{$\psi$}(j0); \draw[->](j0)--(d2); \draw[->](d2)--(jn1); \draw[->](jn1)--node[below,font=\scriptsize]{$e^{n-1}$}(jn); \draw[->](jn)--node[below,font=\scriptsize]{$e^n$}(jn+1);
		
		\draw[->](a)--node[left,font=\scriptsize]{$f$}(b); \draw[->](i0)--node[left,font=\scriptsize]{$\alpha^0$}(j0); \draw[->](in1)--node[left,font=\scriptsize]{$\alpha^{n-1}$}(jn1); \draw[->](in)--node[left,font=\scriptsize]{$\alpha^n$}(jn); \draw[->,dashed](in+1)--node[right,font=\scriptsize]{$\alpha^{n+1}$}(jn+1);
		
		\draw[->>] (in) -- (mod); \draw[->,dashed] (mod) -- (jn+1); \draw[right hook->] (mod) -- (in+1);
	\end{tikzpicture}\end{center}
	F"ur $x \in \Kern(d^n) = \Bild (d^{n-1})$ gilt: es gibt ein $y \in I^{n-1}$ mit $d^{n-1}(y) = x$ $\Rightarrow e^n(\alpha^n(x)) = e^n(\alpha^n(d^{n-1}(y))) = \underbrace{e^n(e^{n-1}}_{=0} (\alpha^{n-1}(y))) = 0 \Rightarrow e^n \circ \alpha^n$ ist auf $\FakRaum{I^n}{\Kern(d^n)}$ wohldefiniert.
...
\item Seien $(\alpha^k)_{k \geq 0}$ und $(\beta^k)_{k \geq 0}$ Fortsetzungen von $f$ auf die Kettenkomplexe. Wir konstruieren diagonale Morphismen $h^k \colon I^k \to J^{k-1}$ mit $\alpha^k - \beta^k = h^{k+1} \circ d^k + e^{k-1} \circ h^k$, wobei $J^{-1} = B$, $I^{-1} = A$ und $J^{-2} = 0$.\\
  \begin{minipage}{7cm}
 Setzen wir $h^{-1} \colon A \to 0$ und $h^0 \colon I^0 \to B$ konstant $0$, so gilt $f-f = h^0 \circ \varphi + 0 \circ h^{-1}$.
 \end{minipage}
...
\begin{tikzpicture}
    ...
    \path[->, font=\scriptsize]
    (m-1-1) edge (m-1-3)
    (m-1-3) edge node[auto] {$\varphi$}  (m-1-4)
		    edge node[auto,swap] {$f-f$}  (m-2-3)
		    edge [bend right = 10] node[above] {$h^{-1}$} (m-2-1)
	(m-1-4) edge node[auto] {$d^0$}  (m-1-5)
		    edge node[auto] {$\alpha^0-\beta^0$}  (m-2-4)
		    edge [bend right = 10] node[above] {$h^0$} (m-2-3)
	(m-2-1) edge (m-2-3)
	(m-2-3) edge node[auto,swap] {$\psi$} (m-2-4)
	(m-2-4) edge node[auto,swap] {$e^0$} (m-2-5)
	;
\end{tikzpicture}
  \end{center}
 \end{minipage} 
 Seien nun $h^{-1}, \dots, h^n$ mit der gew"unschten Eigenschaft gegeben. 
 
 F"ur $x \in \Kern(d^n) = \Bild(d^{n-1})$ gilt: Es gibt ein $y \in I^{n-1}$ mit $d^{n-1}(y) = x$, also gilt
 \[\begin{array}{rcl}
     (\alpha^n-\beta^n-e^{n-1}h^n)(x) &=& (\alpha^n-\beta^n-e^{n-1}\circ h^n)(d^{n-1}(y))\\
     &=& (\alpha^n \circ d^{n-1}-\beta^n\circ d^{n-1}-e^{n-1}\circ h^n\circ d^{n-1})(y)\\
     &=& (e^{n-1} \circ \alpha^{n-1} - e^{n-1} \circ \beta^{n-1} -e^{n-1} \circ h^n \circ d^{n-1})(y)\\
     &=& (e^{n-1} \circ (\alpha^{n-1} - \beta^{n-1} - h^n \circ d^{n-1}))(y)\\
     &=& (e^{n-1} \circ e^{n-2} \circ h^{n-1})(y) = 0
   \end{array}
 \]
 \begin{minipage}{9cm}
  \begin{center}
  \begin{tikzpicture}
     \matrix (m) [matrix of math nodes, row sep=2em, column sep=1.4em, text height=1.5 ex, text depth=0.25 ex]{
		   && I^n &  & I^{n+1} \\
		    && & I^n/\Kern(d^n)\\
		   J^{n-1} && J^n \\
	  };
    \path[->, font=\scriptsize]
    (m-1-3) edge node[auto] {$d^{n}$} (m-1-5)
			edge node[rotate=270,below] {$\alpha^n-\beta^n - e^{n-1} h^n$}  (m-3-3)
	(m-3-1) edge node[auto,swap] {$e^{n-1}$} (m-3-3)
	;
	\path[->,dashed, font=\scriptsize]
	(m-2-4) edge node[auto] {$\tilde{h}^{n+1}$}(m-3-3)
	(m-1-5) edge [bend left=35] node[auto] {$h^{n+1}$}(m-3-3)
	;
	\path[->>, font=\scriptsize]
	(m-1-3) edge (m-2-4)
    ;
	...
	(m-2-4) edge (m-1-5)
    ;
    \draw[->,font=\scriptsize] (m-1-3) to[out=180,in=90] node[left]{$h^n$} (m-3-1);%			edge [bend right=180] node[auto,swap] {$h^n$}  (m-3-1)
  \end{tikzpicture}
 \end{center}
 \end{minipage}
  \begin{minipage}{6.8cm} 
  Also faktorisiert $\alpha^n-\beta^n-e^{n-1}h^n$ "uber $I^n/\Kern(d^n)$ und induziert einen Morphismus $\tilde{h}^{n+1} \colon I^n/\Kern(d^n) \to J^n$. 
   Da $J^n$ injektiv ist und $I^n/\Kern(d^n) \hookrightarrow I^{n+1}$ ein Monomorphismus, gibt es eine Fortsetzung $h^{n+1} \colon I^{n+1} \to J^n$ von $\tilde{h}^{n+1}$. 
   Nach Konstruktion gilt $h^{n+1} \circ d^n  = \alpha^n - \beta^n - e^{n-1} \circ h^n$.
 \end{minipage}

 \item Mit Hilfe von a) und b) zeigen wir nun, dass die Garbenkohomologie nicht von der gew"ahlten injektiven Aufl"osung abh"angt.
 
 Seinen $0 \to \F \to I^\bullet$ und $0 \to \F \to J^\bullet$ zwei injektive Aufl"osungen der Garbe $\F$ auf dem Schema $X$. 
 Nach a) l"asst sich $\id \colon \F \to \F$ zu $(\alpha^k)_{k \geq 0}\colon I^\bullet \to J^\bullet$ und zu $(\beta^k)_{k \geq 0}\colon J^\bullet \to I^\bullet$ fortsetzen. 
...
\begin{tikzpicture}[baseline=0]
    ...
    \path[->, font=\scriptsize]
    (m-1-1) edge (m-1-2)
    (m-1-2) edge node[auto] {$\psi$}  (m-1-3)
		    edge node[auto,swap] {$f$}  (m-2-2)
	(m-1-3) edge node[auto] {$e^0$}  (m-1-4)
		    edge node[auto,swap] {$\beta^0$}  (m-2-3)
	(m-1-4) edge node[auto] {$e^1$}  (m-1-5)
		    edge node[auto,swap] {$\beta^1$}  (m-2-4)
	(m-2-1) edge (m-2-2)
	(m-2-2) edge node[auto,swap] {$\varphi$} (m-2-3)
	(m-2-3) edge node[auto,swap] {$d^0$} (m-2-4)
	(m-2-4) edge node[auto,swap] {$d^1$} (m-2-5)
	;
\end{tikzpicture}\end{center}

Dann sind $(\id)_{k\geq 0}$ und $(\beta^k \circ \alpha^k)_{k \geq 0}$ beides Fortsetzungen von $\id \colon \F \to \F$ nach $I^\bullet \to I^\bullet$. Nach b) gibt es also Morphismen $h^k\colon I^k \to I^{k-1}$ so dass $\beta^k \circ \alpha^k - \id = h^{k+1} \circ d^k + d^{k-1} \circ h^k$. Wir entfernen die erste Spalte, wenden den globalen Schnittfunktor an und erhalten das folgende Diagramm von abelschen Gruppen (die zum Garbenmorphismus $g$ geh"orige Abbildung auf ganz $X$, $g(X)$, nennen wir $\tilde{g}$): 
\begin{center}
\begin{tikzpicture}
     \matrix (m) [matrix of math nodes, row sep=4em, column sep=4.5em, text height=1.5 ex, text depth=0.25 ex]{
		  0 & \Gamma(X, I^0) & \Gamma(X, I^1) & \Gamma(X, I^2) & \cdots\\
		  0 & \Gamma(X, I^0) & \Gamma(X, I^1) & \Gamma(X, I^2) & \cdots\\
	  };
    \path[->, font=\scriptsize]
    (m-1-1) edge (m-1-2)
    (m-1-2) edge node[auto] {$\tilde{d}^0$}  (m-1-3)
		    edge node[auto,swap] {$\tilde{\beta}^0 \circ \tilde{\alpha}^0 - \id$}  (m-2-2)
	(m-1-3) edge node[auto] {$\tilde{d}^1$}  (m-1-4)
		    edge node[auto,swap] {$\tilde{\beta}^1 \circ \tilde{\alpha}^1 - \id$}  (m-2-3)
		    edge [bend right=20] node[above] {$\tilde{h}^0$} (m-2-2)
	(m-1-4) edge node[auto] {$\tilde{d}^2$}  (m-1-5)
		    edge node[auto,swap] {$\tilde{\beta}^2 \circ \tilde{\alpha}^2 - \id$}  (m-2-4)
		    edge [bend right=20] node[above] {$\tilde{h}^1$} (m-2-3)
	(m-1-5) edge [bend right=20] node[above] {$\tilde{h}^2$} (m-2-4)
	(m-2-1) edge (m-2-2)
	(m-2-2) edge node[auto,swap] {$\tilde{d}^0$} (m-2-3)
	(m-2-3) edge node[auto,swap] {$\tilde{d}^1$} (m-2-4)
	(m-2-4) edge node[auto,swap] {$\tilde{d}^2$} (m-2-5)
	;
\end{tikzpicture}
\end{center}
F"ur $k \geq 0$ gilt: Wegen $\alpha^{k} \circ d^{k-1} = d^{k-1} \circ \alpha^{k-1}$ und $\beta^{k} \circ d^{k-1} = d^{k-1} \circ \beta^{k-1}$, ist $\tilde{\beta}^k \circ \tilde{\alpha}^k - \id$ auf $H^k(\Gamma(X,I^\bullet))$ wohldefiniert. Sei $\overline{x} = x + \Bild(\tilde{d}^{k-1}) \in H^k(\Gamma(X,I^\bullet))$, d.h. $x \in \Kern(\tilde{d}^k)$. Dann gilt:
\[ 
(\tilde{\beta}^k \circ \tilde{\alpha}^k - \id)(\overline{x}) 
= \overline{(\tilde{\beta}^k \circ \tilde{\alpha}^k - \id)(x)}
= \overline{\tilde{h}^{k+1} \circ \underbrace{\tilde{d}^k (x)}_{=0} + \underbrace{\tilde{d}^{k-1} \circ \tilde{h}^k(x)}_{\in \Bild(\tilde{d}^{k-1})} }
= 0
\]
Es folgt $\tilde{\beta}^k \circ \tilde{\alpha}^k = \id$. Ganz analog erh"alt man $\tilde{\alpha}^k \circ \tilde{\beta}^k = \id$ und somit $H^k(\Gamma(X,I^\bullet)) = H^k(\Gamma(X,J^\bullet))$.
\end{enumerate}\end{Loes}
...
\begin{Loes}
\v{C}ech-Kohomologie der konstanten Garbe $\Z$ auf der $S^2$
\begin{enumerate}[a)]
\item
	$\mathfrak U = \{U_1, U_2\}$, $U_1, U_2$ echte, offene, \emph{zusammenh"angende} Teilmengen
	\begin{center}\begin{tikzpicture}[fill opacity=0.3]
		\node[fill opacity=1] at (1.75,1.75) {$U_1$}; \node[fill opacity=1] at (1.75,-1.75) {$U_1$};
		\draw[clip] (0,0) circle (2);
		%sqrt(5) ist etwa 2.23606; der Abstand des Kreises, damit er durch die Mitte des Grossen geht
		\draw (0,2.23606) circle (3); \draw[dashed] (0,-2.23606) circle (3);
		\fill[color=red] (0,2) circle (3);
		\fill[color=blue, even odd rule] (0,0) circle (2) (0,2.5) circle (3);
		
		\path[clip](0,2.5) circle (3);
		\fill[color=blue,rotate=10] (0,-1) rectangle (0.5,2.5);
		\begin{scope}[even odd rule]
			\path[clip] (0,0) circle (2) (0,-2.23606) circle (3);
			\fill[color=blue,rotate=-10] (-0.25,-1) rectangle (-0.75,2.5);
		\end{scope}
	\end{tikzpicture}
	
	$U_1 \cap U_2$ ist zusammenh"angend\end{center}
	$\Rightarrow C^0(\mathfrak U, \Z) \cong \Z \X \Z$, $C^1(\mathfrak U, \Z) = \Z$, f"ur alle $k \ge 2$ gilt $C^k(\mathfrak U, \Z) = 0$
		\[ 0 \to C^0 \overset{d^0}{\to} C^1 \to 0 \]
	$d^0 : \left\{ \begin{array}{cccl} C^0 &\to& C^1 & \Kern(d^0) = \Z \\ (f,g) &\mapsto& g-f & \Bild(d^0) = \Z \end{array} \right.$
	
	$\Rightarrow \check{H}^0(\mathfrak U, \Z) = \Z$, $\check{H}^1(\mathfrak U, \Z) = \FakRaum{\Z}{\Z} = 0$, f"ur alle $k \ge 2$ gilt $\check{H}^k(\mathfrak U, \Z) = 0$
\item
	$\mathfrak U = \{U_1, U_2, U_3\}$, $U_i \cap U_j \cong$ Kreisscheibe f"ur $i \ne j$, $U_1 \cap U_2 \cap U_3$ hom"oomorph zu zwei offenen Kreisscheiben.
	\begin{center}\begin{tikzpicture}[fill opacity=0.3]
		\node[fill opacity=1] at (0,2.5) {Vorderseite};
		\draw[clip] (0,0) circle (2);
		\fill[color=red,name path=r circle] (2,2) circle (3); \fill[color=green,name path=g circle] (-2,2) circle (3);
		\path[name intersections={of=r circle and g circle}];
		\fill[color=blue, even odd rule] (0,0) circle (2) (intersection-2) +(0,3) circle (3);
		%sqrt(5) ist etwa 2.23606; der Abstand des Kreises, damit er durch die Mitte des Grossen geht
		\draw (0,2.23606) circle (3); \draw[dashed] (0,-2.23606) circle (3);
	\end{tikzpicture} $\qquad$
	\begin{tikzpicture}[fill opacity=0.3]
		\node[fill opacity=1] at (0,2.5) {R"uckseite};
		\draw[clip] (0,0) circle (2);
		\fill[color=red,name path=r circle] (-2,2) circle (3); \fill[color=green,name path=g circle] (2,2) circle (3);
		\path[name intersections={of=r circle and g circle}];
		\fill[color=blue, even odd rule] (0,0) circle (2) (intersection-2) +(0,3) circle (3);
		\draw (0,2.23606) circle (3); \draw[dashed] (0,-2.23606) circle (3);
	\end{tikzpicture}\end{center}
	$C^0(\mathfrak U, \Z) = \Z(U_1) \X \Z(U_2) \X \Z(U_3) \cong \Z^3$, $C^1(\mathfrak U, \Z) = \Z(U_2 \cap U_3) \X \Z(U_1 \cap U_3) \X \Z(U_1 \cap U_2) \cong \Z^3$, $C^2(\mathfrak U, \Z) = \Z(U_1 \cap U_2 \cap U_3) \cong \Z^3$, f"ur alle $k \ge 3$ gilt $C^k(\mathfrak U, \Z) = 0$
		\[ 0 \to C^0 \overset{d^0}{\to} C^1 \overset{d^1}{\to} C^2 \to 0 \]
	$d^0: \left\{ \begin{array}{ccc} C^0 &\to& C^1 \\ (f,g,h) &\mapsto& (h-g, h-f, g-f) \end{array} \right.$
	
	$\Kern(d^0) = \{(f,f,f) | f \in \Z \} \cong \Z$
	
	$\Bild(d^0) = \{ (h-g, h-f, g-f) | h,g,f \in \Z \} = \{(x,y,z) \in \Z^3 | x = y-z \} \cong \Z^2$
	
	$d^1: \left\{ \begin{array}{ccc} C^1 &\to& C^2 \cong \Z^2 \\ (x,y,z) &\mapsto& (x-y+z, x-y+z) \end{array} \right.$, $\Kern(d^1) \cong \Z^2$, $\Bild(d^1) \cong \Z$
	
	$\check{H}^0(\mathfrak U, \Z) = \check{H}^1(\mathfrak U, \Z) = \FakRaum{\Kern(d^1)}{\Bild(d^0)} = \FakRaum{\Z^2}{\Z^2} = 0$, $\check{H}^2(\mathfrak U, \Z) \cong \FakRaum{\Z^2}{\Z} \cong \Z$, f"ur alle $k \ge 3$ gilt $\check{H}^k(\mathfrak U, \Z) = 0$
\end{enumerate}\end{Loes}
...
\bibliography{GeoSchemata2012Lit}
