...
\usepackage{ngerman, amsmath, amssymb, SIUnits, esvect, graphicx, multirow, braket, dsfont, rotating, pstricks, pst-all, marvosym, nicefrac, longtable}
\include{m2aufz}
\include{m2BoldMath}
\include{m2Math}
...
\begin{figure}[h]
\centering
\includegraphics{001Photoeffekt.eps} 
\caption{Photoeffekt}
\label{Photoeffekt1}
\end{figure}
...
\begin{figure}[h]
\centering
\includegraphics{002Comptoneffekt.eps} 
...
\begin{figure}[h]
\centering
\includegraphics{002Intensitat.eps} 
\label{002Intensitat}
\end{figure}
...
\begin{center}
\includegraphics{003Kalkspat.eps}
\end{center}
Polarisationsfilter $P_\varphi$:
\begin{center}
\includegraphics{003Polarisationsfilter1.eps}
\end{center}
$P_x := P_{\varphi = 0}, \ P_y := P_{\varphi = \frac\pi2}$
\begin{center}
\includegraphics{003Polarisationsfilter2.eps}
\end{center}
...
Kalkspat:
\begin{center}
\includegraphics{003Kalkspat2.eps}
\end{center}
...
\begin{center}
\includegraphics{004Kalkspat.eps}
\end{center}
Beobachtungen:
\begin{itemize}
...
\begin{center}
\includegraphics{030Polarisationsfilter.eps}
\end{center}
...
\begin{center}
\includegraphics{038ZirkularPolPhotonen.eps}\end{center}
...
\begin{center}
\includegraphics{044MessungZirkPol.eps}
\end{center}
\begin{1aufz}
...
\begin{center}
\includegraphics{056SternGerlach.eps}
\end{center}
klassisch:
\begin{align*}
...
\begin{center}
\includegraphics{191Integration.eps}
\end{center}
...
\end{align}
\begin{center}
\includegraphics{203Katenpot.eps}
\end{center}
Anwendung:
\begin{itemize}
...
\begin{center}
\includegraphics{283Drehung.eps}
\end{center}
...
\begin{center}
\includegraphics{419Ubergang.eps}
\end{center}
...
\begin{center}
\includegraphics{431EntartRed.eps}
\end{center}
...
\begin{center}
\includegraphics{470wirkquer.eps}
\end{center}
\eqn{\frac{d\sigma}{d\Omega} = \stackrel{(469)}= \frac1{j_\mathrm{ein}} \frac{j_\mathrm{aus} dF}{d\Omega} = \frac{r^2j_\mathrm{aus}}{j_\mathrm{ein}}}
...
\eqn{\frac{d\sigma}{d\Omega} \propto \frac1{\sin^4 \frac{\vartheta}2}}
Der \emph{totale Streuquerschnitt}
\eqn{\sigma & := \int \dd \Omega \, \frac{d\sigma}{d\Omega}}
...
\begin{center}
\includegraphics{473HartTarget.eps}
\end{center}
Quantenmechanik:
\setcounter{equation}{165}
\eqn{ i \hbar \frac \partial {\partial t} \psi( \vv x, t) = H \psi (\vv x, t)}
Teilchendichte $\leadsto$ Aufenthaltwahrscheinlichkeit eines Teilchens
\setcounter{equation}{473}
\eqn{ \varrho( \vv x, t) & = \vert \psi( \vv x, t) \vert^2}
Gesucht: Strom $\vv j(\vv x, t)$ aus Kontinuit"atsgleichung
\eqn{\frac \partial {\partial t} \varrho( \vv x, t) & = \vv \nabla \vv j ( \vv x, t),\\
\spl{ i \hbar \frac \partial {\partial t} \varrho( \vv x, t) & = (i \hbar \frac \partial {\partial t} \psi^*) \psi + \psi^* i \hbar \frac \partial {\partial t} \psi \\
& = (- H \psi^*) \psi + \psi^* H \psi \\
& = \frac{\hbar^2}{2m} ( \Delta \psi^*) \psi - \frac{\hbar^2}{2m} \psi^* \Delta \psi \\
& = i \hbar \vv \nabla \vv j (\vv x, t),
}}
 mit der Wahrscheinlichkeit 
\eqn{ \vv j ( \vv x, t) & = \frac{\hbar}{2mi} \left[ \psi^*(\vv x, t) \vv \nabla \psi( \vv x, t) - (\vv \nabla \psi^*(\vv x, t)) \psi(\vv x, t) \right]}
Teilchenstromdichte = Teilchendichte $n$ mal $\vv j$.

Quantenmechanische Streuung:
\begin{center}
\includegraphics{477QMStreuung.eps}
\end{center}
Einlaufendes Wellenpaket:
\eqn{\psi(\vv x, t) = \int \frac{\dd^3 \vv {k_0}}{(2\pi)^3} A(\vv{k_0}) \underbrace{ e^{-i \left( \vv k_0 \vv r - \omega(\vv k_0) t \right) }}_{\mathrm{ebene \ Welle}}}
...
\eqn{\psi_s(\vv x, t) & = \int \frac{\dd^3 \vv k}{(2\pi)^3} \underbrace{ \Phi_s (\vv r, \vv k) e^{i \omega(\vv k) t}}_{\mathrm{allgemeiner \ Ansatz}}}
...
\eqn{ \spl{
H \phi (\vv r) & = E \phi (\vv r), \quad E = \frac{\hbar k^2}{2m} > 0 \mathrm{ \ vorgegeben} \\
H & = \frac {\vv P^2}{2m} + V(\vv r) \\
\phi(\vv r) & = e^{i k z} + \phi_s (\vv r)
}}
Weitere Annahme: $V$ ist kugelsymmetrisch:
\eqn{V(\vv r) = V(r)}
$\phi_s(\vv r)$ wird nach Kugelwellen entwickelt:
\eqn{\psi_s(\vv r) \stackrel{r \rightarrow \infty}\sim f(\vartheta) \frac{e^{ik \vv r}}r }
...
\eqn{
\vv {j_s} & = \frac{ \hbar k}m \frac{ \vert f(\vartheta) \vert^2}{r^2} \vv r + \mathcal O \left( \frac1{r^3} \right) \\
\spl{
\frac{d\sigma}{d\Omega} & \stackrel{(171)}= \frac{r^2 j_s}{j_0}  \\
& \stackrel{(483)}= \frac{\hbar k}{m} \frac{\vert f(\vartheta) \vert^2}{\nicefrac{\hbar k}{m}}  \\
& = \vert f(\vartheta) \vert^2 }
}
mit $j_s = j_\mathrm{aus}$, $j_0 = j_\mathrm{ein}$.

$\Longrightarrow$
\eqn{ \spl{
\sigma = \int \frac{d \sigma}{d \Omega} \dd \Omega & = \int_0^{2\pi} \dd \phi \, \int_0^\pi \dd \cos \vartheta \, \vert f(\vartheta) \vert^2 \\
& = 2 \pi \int_0^\pi \dd \vartheta \, \sin \vartheta \vert f(\vartheta) \vert^2
}}
Unsere asymptotische L"osung
\eqn{ \phi(\vv r) \stackrel{r \rightarrow \infty}\sim e^{i k z} + f(\vartheta) \frac{e^{ikr}}r}
erf"ullt die Schr"odinger-Gleichung, sofern
\eqn{\lim_{r \rightarrow \infty} r V(r) = 0.}
(487) ist nicht erf"ullt z.B. f"ur das Coulomb-Potenzial.

Kugelsymmetrie:
\eqn{ \left[ H, \vv L^2 \right] = \left[ H, L_3 \right] = 0 }
$\Longrightarrow$ Entwicklung nach Kugelfl"achenfunktionen
\eqn{ L_3 = \frac \hbar i \frac \partial {\partial \phi}}
...
\eqn{Y_{l0} (\vartheta, \phi) = \sqrt{ \frac{ 2l + \lambda}{4 \pi}} \underbrace{ P_l (\cos \vartheta) }_{\mathrm{Legendre-Pol.}}}
entwickelt werden.

In
\eqn{ \phi(\vv r)= \sum_{l=0}^\infty \frac{U_l(r)}r P_l (\cos \vartheta) }
erf"ullt $U_l(r)$ also die Radialgleichung
$$ \left[ - \frac{ \hbar^2}{2m} \frac{d^2}{dr^2} + \frac{\hbar^2 l(l+1)}{2mr^2} + V(r) \right] U_l(r) = E U_l (r)$$
bzw.
\eqn{ \frac{d^2 U_l}{dr^2} + \left( k^2 - \frac{l(l+1)}{r^2} - \frac{2m}{\hbar^2} V(r) \right) U_l(r) & = 0}
mit $E = \frac{\hbar^2 k^2}{2m}$.
Randbedingung:
\eqn{U_l(0) = 0}
Zun"achst $V=0$, also $\phi_s(\vv r) = 0$ in (480)
\eqn{\phi_0(\vv r) = e^{i k z} = \sum_{l=0}^\infty \frac{U_l^{(0)} (r)}r P_l (\cos \vartheta), \quad z=r \cos \vartheta}
Orthogonalit"at der $P_l$:
$$\int_{-1}^1 \dd t \, P_l(t) P_n (t) = \frac{2}{2l +1} \delta_{l n}$$
(495) $\Longrightarrow$ ($t = \cos \vartheta$)
\eqn{ \spl{
\int_{-1}^1 \dd t \, e^{i k r t} P_l(t) & = \frac2{2n+1} \frac{U_l^{(0)} (r)}r \\
 & \stackrel{(496), P.I.}= \frac1{i k r} \left[ e^{ikr} P_l(t) \right]_{-1}^1 - \frac1{i k r} \int_{-1}^1 \dd t \, e^{i k r t} P_l'(t) \\
 & \stackrel{P_l(1) = 1 \atop P_l(-1) = (-1)^l}= \frac1{i k r} \left[ e^{ikr} - (-1)^l e^{-ikr} \right] + \mathcal{O} \left( \frac1{r^2} \right) \\
 & = \frac1{ikr} i^l \left[ e^{i (kr - l \nicefrac \pi 2)} - e^{- (i kr - l \nicefrac \pi 2)} \right] + \mathcal{O} \left( \frac1{r^2} \right)
}}
Einsetzen in (495):
\eqn{ \phi_0(\vv r) = e^{ikr \cos \vartheta} & \stackrel{r \rightarrow \infty}\sim \sum_{l=0}^\infty (2l + 1) i^l \frac1{2 i k r} \Big( \underbrace{e^{i (kr - l\nicefrac \pi 2)}}_{\mathrm{ausl. \ Kugelw.}} - \underbrace{e^{-i(kr - l \nicefrac \pi 2)}}_{\mathrm{einl. \ Kugelw.}} \Big) P_l (\cos \vartheta)}
...
\eqn{ \spl{
\phi(\vv r) & = e^{ikr \cos \vartheta} + \phi_s(\vv r) \\
& = \sum_{l=0}^\infty \frac{U_l(r)}{r} P_l (\cos \vartheta) \\
& \stackrel{r \rightarrow \infty}\sim \sum_{l=0}^\infty (2l +1) i^l \frac{1}{2 i kr} \left( S_l e^{i(kr - l\nicefrac \pi 2)} - e^{-i(kr - l \frac \pi 2)} \right) P_s (\cos \vartheta)
}}
...
Elastische Streuung:
\begin{center}
\includegraphics{498Kugel.eps}
\end{center}
keine Teilchenvernichtung oder Erzeugung durch $V(\vv r)$!

(477) und (498):
\eqnnon{ 0 = j_r \propto \sum_{l=0}^\infty P_l (\cos \vartheta) \underbrace{ \Im \left[ \frac{U_l^*(0)}r \frac d{dr} \frac{U_l(r)}r \right] }_{ \propto 1 - \vert S_l \vert^2}}
$\Longrightarrow$
\eqn{ \vert S_l \vert = 1}
$\Longrightarrow$
\eqn{ S_l(k) =: e^{2 i \delta_l(k)}}
mit der \emph{Streuphase} $\delta_l(k)$.

Einsetzen in (498) (mit (480), (497)):
\eqn{ f(\vartheta) = \frac1k \sum_{l=0}^\infty (2l +1) \underbrace{ e^{i \delta_l } \sin \delta_l}_{\frac{S_l -1}{2i}} P_l (\cos \vartheta)}
...
und 
\eqn{ \spl{
\sigma & = \sum_{l=0}^\infty \sigma_l \  \mathrm{mit } \\
...
\eqn{ \sigma = \frac{4 \pi}k \Im f(\sigma)}
...
L"osung: 
\eqn{\delta_l(k) = \arctan \frac{ J_l(kR)}{n_l(kR)} {\leftarrow \mathrm{sph. Bessel-Funktion} \atop \leftarrow \mathrm{sph. \ Neumann-Fktn}}}
...
\eqn{\sigma \stackrel{(502)}= 4 \pi R^2}
...
\eqn{\sigma = \ldots = 2 \pi R^2 = 2 \sigma_\mathrm{klassisch}}
\end{1aufz}
\end{document}
