...
$\sigma_{|E}: E \ra L$, also $\sigma_{|E} \in$ Hom$_K(E,L) \subseteq$
...
Dann ist auch $f(\zeta^m) = 0$ für jedes $m$ mit ggT$(m,n) = 1\Ra
...
Ist $L\neq K(\alpha)$, so sei $b_1,\dots,b_m$ eine $K(\alpha)$-Basis
...
19}}}{\Ra}$ es gibt $\alpha \in L^x$ mit $\sigma(\alpha) = \zeta
...
\overset{\mbox{\scriptsize Satz \ref{Satz
19}}}{\Ra}$ es gibt $\alpha \in L$ mit
...
Ist $L_1/K$ vom Typ (iii), so ist $L_1/K$ galoissch und Gal$(L_1/K) =
\mathbb{Z}/p\mathbb{Z}$.

Sei $L_1/K$ vom Typ (ii). Enthält $K$
...
auflösbar}}{\underbrace{\mbox{Gal}(M/L)}} \ra \mbox{Gal}(M/K) \ra
...
\mathbb{Z}/p_i\mathbb{Z}$ mit Primzahlen $p_i,\;i=0,\dots,m-1$ ist.
