...
\[\mathcal{M}:=\{M\subseteq I\mid M \text{ ist Intervall},x_0\in M,F(M)\subseteq K\}\] 
Da $M_0\in\mathcal{M}$ ist, ist $\mathcal{M}\ne\emptyset$. Sei
\[I_{x_0}:=\bigcup_{M\in\mathcal{M}} M\]
dann ist $I_{x_0}\in\mathcal{M}$. Definiere nun $y:I_{x_0}\to\MdR$ durch:
\[y(x):=G^{-1}(F(x))\]
...
D.h. $U\in\mathcal{M}$ und daher ist: $U\subseteq I_{x_0}$.\\
Weiter gilt:
...
&= L \left| \int_{x_0}^x \|y(t)-z(t)\| \text{ d}t \right|\\
&= L \left| \int_{x_0}^x \|y(t)-z(t)\| \varphi_\alpha (t) \cdot \frac1{\varphi_\alpha(t)}\text{ d}t \right|\\
&\le L \left| \int_{x_0}^x \|y-z\|_\alpha \cdot \frac1{\varphi_\alpha(t)}\text{ d}t \right|\\
