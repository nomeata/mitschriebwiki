...
Ohne Beweise geben wir ein \glqq Kochrezept\grqq \, an, wie man zu einem Fundamentalsystem von (H) kommt.
...
Kern$(A-\lambda_1 E) = \left[ \begin{pmatrix} 2i \\ 1 \end{pmatrix} \right]$
\begin{align*}
y(x) &= e^{(1+2i)x} \begin{pmatrix} 2i \\ 1 \end{pmatrix} 
\\ &= e^x (\cos(2x) + i \sin(2x)) \begin{pmatrix} 2i \\ 1 \end{pmatrix} 
\\ &= \underbrace{e^x \begin{pmatrix} -2\sin(2x) \\ \cos(2x) \end{pmatrix}}_{=:  \, y^{(1)}(x)} +\, i \underbrace{e^x \begin{pmatrix} 2\cos(2x) \\ \sin(2x) \end{pmatrix}}_{=: \, y^{(2)}(x)} 
\end{align*}
...
\end{cases}\]
Allgemeine Lösung von $y' = Ay$: Seien $c_1, c_2, c_3 \in \mdr$. \[ y(x) = c_1  e^{2x}\begin{pmatrix} 0 \\ 1\\1 \end{pmatrix} + c_2  e^x \begin{pmatrix} 1\\1\\0 \end{pmatrix} + c_3 e^x \begin{pmatrix} -x \\ -x \\ 1 \end{pmatrix} \]
...
Sei nun $Y(x) := \begin{pmatrix} e^x & 0 \\ 0 & e^{-x} \end{pmatrix}$ \\ Dann ist
\begin{align*}
