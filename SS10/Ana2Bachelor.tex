...
$e_1\da (1,0,\ldots,0),\ e_2\da (0,1,0,\ldots, 0),\ \ldots,\ e_n\da (0,\ldots,0,1) \in \MdR^n$.

\begin{definition}
Seien $x=(x_1, \ldots, x_n), y=(y_1, \ldots, y_n) \in \MdR^n$
\begin{liste}
\item $x\cdot y \da  xy \da  x_1y_1+\cdots+x_ny_n$ hei"st das \textbf{Skalar}\indexlabel{Skalarprodukt}- oder \begriff{Innenprodukt} von $x$ und $y$.
...
\item[(6)] "Ubung.
\item[(4)] O.B.d.A: $y\ne0$ also $\|y\|>0$. $a\da x\cdot x=\|x\|^2,\ b\da xy,\ c\da \|y\|^2=y\cdot y,\ \alpha\da \frac{b}{c}.\ 0\le\sum_{j=1}^n(x_j-\alpha y_j)^2=\sum_{j=1}^n(x_j^2-2\alpha x_jy_j+\alpha^2y_j^2)=a-2\alpha b + \alpha^2 c=a-2\frac{b}{c}b+\frac{b^2}{c^2}c=a-\frac{b^2}{c}\folgt0\le ac-b^2\folgt b^2\le ac\folgt(xy)^2\le\|x\|^2\|y\|^2$.
...
Seien $p,q,l \in \MdN$. Es sei $A$ eine reelle $p\times q$-Matrix.

$$A = \begin{pmatrix}
\alpha_{11} & \cdots & \alpha_{1q}\\
\vdots & & \vdots\\
\alpha_{p1} & \cdots & \alpha_{pq}
\end{pmatrix}\qquad \|A\|\da \left(\sum_{j=1}^p\sum_{k=1}^q\alpha^2_{jk}\right)^\frac{1}{2} \text{\textbf{Norm} von A}$$
Sei $B$ eine reelle $q\times l$-Matrix ($\folgt AB$ existiert). \textbf{"Ubung}: $\|AB\|\le\|A\|\|B\|$\\
Sei $x=(x_1,\ldots,x_q) \in \MdR^q$. $Ax\da A\begin{pmatrix}x_1\\ \vdots \\ x_q\end{pmatrix}$ (\begriff{Matrix-Vektorprodukt}). \\
Es folgt: $$\|Ax\|\le\|A\|\|x\|$$

\begin{definition}
Sei $x_0 \in \MdR^n$, $\delta > 0$, $A, U\subseteq \MdR^n$.
\begin{liste}
 \item $U_\delta(x_0) \da  \{ x \in \MdR^n: \|x-x_0\|<\delta\}$ heißt $\delta$-Umgebung von $x_0$ oder \begriff{offene Kugel} um $x_0$ mit Radius $\delta$.
 \item $U$ ist eine \begriff{Umgebung} von $x_0$ $\dequizu$ $\exists \delta > 0 : U_\delta(x_0) \subseteq U$.
 \item \indexlabel{Beschränktheit!einer Menge}$A$ heißt \textbf{beschränkt} $\dequizu$ $\exists c \ge 0: \|a\|\le c \forall a\in A$.
 \item $x_0\in A$ heißt ein \begriff{innerer Punkt} von A $\dequizu$ $\exists \delta>0: U_\delta(x_0) \subseteq A$. \\
   $A^\circ\da \{ x\in A: x \text{ ist innerer Punkt von }A\}$ heißt das \indexlabel{Inneres einer Menge}\textbf{Innere} von A. Klar: $A^\circ\subseteq A.$
 \item $A$ heißt offen $\dequizu$ $A=A^\circ$. Zur Übung: $A^\circ$ ist offen.
\end{liste}
\end{definition}
...
\begin{definition}
 $A\subseteq \MdR^n$
 \begin{liste}
 \item $x_0\in \MdR^n$ heißt ein \begriff{Häufungspunkt} (HP) von $A$ $\dequizu$ $\forall \delta > 0: (U_\delta(x_0) \backslash \{x_0\}) \cap A \ne \emptyset$. $\H(A) \da  \{ x\in\MdR^n: x \text{ ist Häufungspunkt von } A\}$.
 \item  $x_0\in\MdR^n$ heißt ein \begriff{Berührungspunkt} (BP) von $A$ $\dequizu$ $\forall\delta>0: U_\delta(x_0) \cap A \ne \emptyset$. $\bar{A}\da \{x\in\MdR^n: x \text{ ist ein Berührungspunkt von } A\}$ heißt der \begriff{Abschluss} von $A$.\\
 Klar: $A\subseteq\bar{A}$. Zur Übung: $\bar{A} = A \cup \H(A)$.
 \item \indexlabel{abgeschlossen!Menge}$A$ heißt \textbf{abgeschlossen} $\dequizu$ $A=\bar{A}$. Zur Übung: $\bar{A}$ ist abgeschlossen.
 \item $x_0\in\MdR^n$ heißt ein \begriff{Randpunkt} von $A$ $\dequizu$ $\forall\delta>0: U_\delta(x_0) \cap A \ne \emptyset$ und $U_\delta(x_0) \cap (\MdR^n\backslash A) \ne \emptyset$. $\partial A \da  \{x\in\MdR^n: x \text{ ist ein Randpunkt von } A \}$ heißt der \begriff{Rand} von $A$. Zur Übung: $\partial A = \bar{A}\backslash A^\circ$.
 \end{liste}
\end{definition}
...
\begin{satz}[Offene und abgeschlossene Mengen]
 \begin{liste}
 \item Sei $A\subseteq\MdR^n$. $A$ ist abgeschlossen $\dequizu$  $\MdR^n\backslash A$ ist offen.
...
\begin{beispiel}
  $(n=1)$. $A_t \da  (0,1+t)\ (t>0)$. Jedes $A_t$ ist offen. $\bigcap_{t>0}A_t = (0,1]$ ist nicht offen.
\end{beispiel}

\begin{beweise}
 \item \glqq$\Longrightarrow$\grqq: Sei $x_0\in\MdR^n\backslash A$. Annahme: $\forall \delta>0: U_\delta(x_0) \nsubseteq \MdR^n\backslash A\folgt\forall\delta>0: U_\delta(x_0)\cap A\ne\emptyset \folgt x_0\in\bar{A} \gleichnach{Vor.} A$, Widerspruch \\
 \glqq$\Longleftarrow$\grqq: Annahme: $\subset\bar{A} \folgt \ \exists x_0\in\bar{A}: x_0\notin A$; also $x_0\in\MdR^n\backslash A$. Voraussetzung $\folgt \ \exists \delta > 0: U_\delta(x_0) \subseteq \MdR^n\backslash A \folgt U_\delta(x_0) \cap A = \emptyset \folgt x_0 \notin \bar{A}$, Widerspruch!
 \item Sei $(A_\lambda)_{\lambda\in M}$ eine Familie offener Mengen und $V \da  \bigcup_{\lambda\in M} A_\lambda$. Sei $x_0\in V \folgt \exists\lambda_0\in M: x_0 \in A_{\lambda_0}$. $A_{\lambda_0}$ offen $\folgt \ \exists \delta > 0: U_\delta(x_0) \subseteq A_{\lambda_0} \subseteq V$
 \item folgt aus (1) und (2) (Komplemente!)
 \item $D\da \bigcap_{j=1}^mA_j$. Sei $x_0\in D$. $\forall j\in\{1,\ldots,m\}: x_0\in A_j$, also eixistiert $\delta_j>0: U_\delta(x_0)\subseteq A_j$. $\delta \da  \min\{\delta_j,\ldots,\delta_m\} \folgt U_\delta(x_0) \subseteq D$
 \item folgt aus (1) und (4)
\end{beweise}

\chapter{Konvergenz im $\MdR^n$}

Sei $(a^{(k)})$ eine Folge in $\MdR^n$, also $(a^{(k)}) = ( a^{(1)}, a^{(2)}, \ldots ) $ mit $a^{(k)} = (a_1^{(k)}, \ldots a_n^{(k)}) \in \MdR^n$. Die Begriffe \begriff{Teilfolge} und \begriff{Umordnung} definiert man wie in Analysis I. $(a^{(k)})$ heißt \textbf{beschränkt} $\dequizu$ $\exists c\ge0: \|a^{(k)}\| \le c  \ \forall k\in\MdN$.

\begin{definition*}[Grenzwert und Beschränktheit]
\indexlabel{Konvergenz}$(a^{(k)})$ heißt \textbf{konvergent} $\dequizu$ $\exists a\in\MdR^n: \|a^{(k)} - a\| \to 0 \ (k\to\infty)$ ($\Longleftrightarrow\ \exists a\in\MdR^n: \forall \ep>0\ \exists k_0 \in\MdN: \|a^{(k)} - a\|<\ep \ \forall k\ge k_0$). In diesem Fall heißt $a$ der \begriff{Grenzwert} (GW) oder \begriff{Limes} von $(a^{(k)})$ und man schreibt: $a=\lim_{k\to\infty}a^{(k)}$ oder $a^{(k)} \to a \ (k\to\infty)$
\end{definition*}

\begin{beispiel}
$(n=2)$: $a^{(k)} = (\frac{1}{k}, 1+\frac{1}{k^2})$ (Erinnerung: $\frac{1}{n}$ konvergiert gegen 0); $a \da  (0,1)$; $\|a^{(k)} - a \| = \|(\frac{1}{k} , \frac{1}{k^2})\| = (\frac{1}{k^2} + \frac{1}{k^4})^\frac{1}{2} \to 0 \folgt a^{(k)} \to (0,1)$
\end{beispiel}
%satz 2.1
...
\begin{beweise}
  ...
  \item \glqq$\Longrightarrow$\grqq: wie in Analysis 1. \glqq$\Longleftarrow$\grqq: 1.1(7) $\folgt |a_j^{(k)}-a_j^{(l)}| \le \|a^{(k)}-a^{(l)}\|\ (j=1,\ldots,n)\ \folgt$ jede Folge $(a_j^{(k)})$ ist eine Cauchyfolge in $\MdR$, also konvergent $\overset{(1)}{\folgt} (a^{(k)})$ konvergiert.
\end{beweise}
...
\item[(3)] \glqq$\Longrightarrow$\grqq: Sei $(a^{(k)})$ eine konvergente Folge in $A$ und $x_0\da \lim a^{(k)} \overset{(2)}{\folgt} x_0 \in \bar A \overset{\text{Vor.}}{=}A$. \glqq$\Longleftarrow$\grqq: z.z: $\bar A \subseteq A$. Sei $x_0 \in \bar A \overset{(2)}{\folgt}x_0 \in A$. Also: $A=\bar A$.
\end{beweise}
...
\begin{liste}
\item Sei $y_0 \in \MdR^m$. $\displaystyle\lim_{x\to x_0}f(x)=y_0 \dequizu$ f"ur \textbf{jede} Folge $(x^{(k)})$ in $D\ \backslash\ \{x_0\}$ mit $x^{(k)}\to x_0$ gilt: $f(x^{(k)})\to y_0$. In diesem Fall schreibt man: $f(x)\to y_0\ (x\to x_0)$.
\item $\displaystyle\lim_{x\to x_0}f(x)$ existiert $\dequizu\ \exists y_0 \in \MdR^m: \displaystyle\lim_{x\to x_0}f(x)=y_0$.
\end{liste}
\end{definition*}
...
$r \da  x^2 + y^2.\ (x,y) \to (0,0) \equizu \|(x,y)\| \to 0 \equizu r \to 0$, Sei $(x,y) \ne (0,0)$. Dann gilt:
...
\item Aus der Linearen Algebra ist bekannt: Es gibt einen UVR $V$ von $\MdR^n$ mit: $\MdR^n = U \oplus V$. Definiere $P: \MdR^n \to \MdR^n$ wie folgt: zu $x \in \MdR^n$ existieren eindeutig bestimmte $u \in U,\ v \in V$ mit: $x = u+v;\ P(x) \da  u$.

Nachrechnen: $P$ ist linear.

$P(\MdR^n) = U$ (Kern $P = V,\ P^2 = P$). Sei $(u^{(k)})$ eine konvergente Folge in $U$ und $x_0 \da  \lim u^{(k)}$, z.z.: $x_0 \in U$.
...
Sei $\emptyset \ne A \subseteq \MdR^n,\ x \in \MdR^n.\ d(x,A) \da  \inf\{\|x-a\|:a \in A\}$ heißt der \textbf{Abstand} von $x$ und $A$.
...
\begin{itemize}
\item["`$\Longleftarrow$"':] Sei $x \in \overline{A} \folgtnach{2.2} \exists$ Folge $(a^{(k)})$ in $A: a^{(k)} \to x \folgtnach{(1)} d(a^{(k)},A) \to d(x,A) \folgt d(x,A) = 0$.
\item["`$\Longrightarrow$"':] Sei $d(x,A) = 0.\ \forall k \in \MdN\ \exists a^{(k)} \in A: \|a^{(k)} - x\| < \frac{1}{k} \folgt a^{(k)} \to x \folgtnach{2.2} x \in \overline{A}$.
\end{itemize}
\end{beweise}
...
Die Gerade durch $x_0$ mit der Richtung $e_j$ ist gegeben durch folgende Menge: $\{x_0+te_j:t\in\MdR\}$. $D$ offen $\folgt$ $\exists\delta>0: U_\delta(x_0)\subseteq D$. $\|x_0+te_j - x_0\| = \|te_j\| = |t| \folgt x_0+e_j \in D $ für $t\in(-\delta,\delta)$. $g(t) \da  f(x_0+te_j)$ $(t\in(-\delta,\delta))$
 Es ist $g(t) = f(x_1^{(0)}, \ldots, x_{j-1}^{(0)}, x_j^{(0)} + t, x_{j+1}^{(0)}, \ldots, x_n^{(0)} )$

\begin{definition}
$f$ heißt in $x_0$ \textbf{partiell differenzierbar} nach $x_j$ \dequizu es exisitert 
der Grenzwert 
...
\begin{liste}
\item $f$ heißt in $x_0$ \textbf{partiell differenzierbar} $\dequizu$ $f$ ist in $x_0$ 
partiell differenzierbar nach allen Variablen $x_1,\ldots, x_n$. In diesem Fall heißt 
$\grad f(x_0) \da  \nabla f(x_0) \da  (f_{x_1}(x_0),\ldots, f_{x_n}(x_0))$ der \textbf{Gradient} von 
$f$ in $x_0$. \indexlabel{Gradient}
\item $f$ ist auf $D$ \textbf{partiell differenzierbar} nach $x_j$ oder $f_{x_j}$ ist auf $D$ vorhanden \dequizu $f$ ist in jedem $x\in D$ partiell differenzierbar nach $x_j$. In diesem Fall wird durch $x\mapsto f_{x_j}(x)$ eine Funktion $f_{x_j}: D\to \MdR$ definiert die \textbf{partielle Ableitung} von $f$ auf $D$ nach $x_j$.
\item $f$ heißt \textbf{partiell differenzierbar} auf $D$ \dequizu $f_{x_1},\ldots,f_{x_n}$ sind auf $D$ vorhanden.
\item $f$ heißt auf $D$ \textbf{stetig partiell differenzierbar} \dequizu $f$ ist auf $D$ partiell differenzierbar und $f_{x_1},\ldots,f_{x_n}$ sind auf $D$ stetig. In diesem Fall schreibt man $f\in C^1(D,\MdR)$.
\end{liste}
\end{definition}
...
$$f_{x_jx_k}(x_0) \da  \frac{\partial^2 f}{\partial x_k\partial x_j}(x_0) \da  \left(f_{x_j}\right)_{x_k}(x_0)$$ 
...
\begin{schreibweisen}
$\ds f_{xxyzz} = \frac{\partial^5 f}{\partial z^2\partial y\partial x^2}$, vergleiche: $\ds\frac{\partial^{180} f}{\partial x^{179}\partial y}$
\end{schreibweisen}
...
\begin{definition}
Sei $m\in\MdN$. $f$ heißt auf $D$ \textbf{$m$-mal stetig partiell differenzierbar} \dequizu alle partiellen Ableitungen  von $f$ der Ordnung $\le m$ sind auf $D$ vorhanden und auf $D$ stetig. In diesem Fall schreibt man: $f\in C^m(D,\MdR)$

$$C^0(D, \MdR) \da  C(D,\MdR),\qquad C^\infty(D,\MdR) \da  \bigcap_{k\in\MdN_0}C^{k}(D,\MdR)$$
\end{definition}
...
\begin{beweis}
O.B.d.A: $n=2$ und $x_0=(0,0)$. Zu zeigen: $f_{xy}(0,0)=f_{yx}(0,0)$. $D$ offen $\folgt\exists\delta>0: U_\delta(0,0)\subseteq D$. Sei $(x,y) \in U_\delta(0,0)$ und $x\ne 0\ne y$. $$\Delta\da f(x,y)-f(x,0)-(f(0,y)-f(0,0)),\quad\varphi(t)\da f(t,y)-f(t,0)$$ f"ur $t$ zwischen $0$ und $x$. $\varphi$ ist differenzierbar und $\varphi'(t)=f_x(t,y)-f_x(t,0)$. $\varphi(x)-\varphi(0)=\Delta$. MWS, Analysis 1 $\folgt\exists\xi=\xi(x,y)$ zwischen $0$ und $x$: $\Delta=x\varphi'(\xi)=x(f_x(\xi,y)-f_x(\xi,0))$. $g(s)\da f_x(\xi,s)$ f"ur s zwischen $0$ und $y$; $g$ ist differenzierbar und $g'(s)=f_{xy}(\xi,s)$. Es ist $\Delta=x(g(y)-g(0))\gleichnach{MWS}xyg'(\eta),\ \eta=\eta(x,y)$ zwischen $0$ und $y$. $\folgt \Delta=xyf_{xy}(\xi,\eta).$ (1)\\
$\psi(t)\da f(x,t)-f(0,t)$, $t$ zwischen $0$ und $y$. $\psi'(t)=f_y(x,t)-f_y(0,t)$. $\Delta=\psi(y)-\psi(0)$. Analog: $\exists \bar\eta=\bar\eta(x,y)$ und $\bar\xi=\bar\xi(x,y)$, $\bar\eta$ zwischen $0$ und $y$, $\bar\xi$ zwischen $0$ und $x$. $\Delta=xyf_{yx}(\bar\xi,\bar\eta).$ (2)\\
...
\begin{definition*}
\begin{liste}
\item Sei $k\in\MdN$. $f\in C^k(D,\MdR^m) \dequizu f_j\in C^k(D,\MdR)\ (j=1,\ldots,m)$
\item Sei $x_0\in D$. $f$ hei"st \textbf{partiell differenzierbar} in $x_0 \dequizu$ jedes $f_j$ ist in $x_0$ partiell differenzierbar. In diesem Fall hei"st
$$\frac{\partial f}{\partial x}(x_0)\da \frac{\partial(f_1,\ldots,f_m)}{\partial(x_1,\ldots,x_n)}\da J_f(x_0)\da \begin{pmatrix}
...
$x_0 \dequizu \exists (m \times n)$-Matrix $A$, sodass gilt:
\begin{align*}
\ds\lim_{h\to 0}\frac{f(x_0+h)-f(x_0)-Ah}{\|h\|}=0\ \tag{$*$}
\end{align*}
\item $f$ hei"st differenzierbar auf $D\ \dequizu f$ ist in jedem $x\in D$ differenzierbar.
\end{liste}
\end{definition*}
...
\item $f$ ist in $x_0$ partiell differenzierbar und die Matrix A in $(*)$ ist eindeutig bestimmt: \\$A=J_f(x_0)$. $f'(x_0)\da A=J_f(x_0)$ (\begriff{Ableitung} von $f$ in $x_0$).
\end{liste}
\end{satz}

\begin{beweis}
Sei A wie in $(*)$, $A=(a_{jk})$, $\varrho(h)\da \frac{f(x_0+h)-f(x_0)-Ah}{\|h\|}$, also: $\varrho(h)\to0\ (h\to 0)$. Sei $\varrho=(\varrho_1,\ldots,\varrho_m)$. 2.1 $\folgt \varrho_j(h)\to 0\ (h\to 0)\ (j=1,\ldots,m)$
\begin{liste}
...
Für $h = (h_1,h_2) \ne (0,0):$ $$\rho(h) \da  \frac{1}{\|h\|}(f(h) - f(0,0) - (\underbrace{h_1 f_x(0,0) + h_2 f_y(0,0)}_{= \grad f(0,0)\cdot h}))$$

$f(h) - f(0) = f(h_1,h_2) - f(0,0) = \underbrace{f(h_1,h_2) - f(0,h_2)}_{\ad \Delta_1} + \underbrace{f(0,h_2) - f(0,0)}_{\ad \Delta_2}$

$\varphi(t) \da  f(t,h_2),\ t$ zwischen $0$ und $h_1 \folgt \Delta_1 = \varphi(h_1) - \varphi(0),\ \varphi'(t) = f_x(t,h_2)$
...
$\folgt \rho(h) \da  \frac{1}{\|h\|}(h_1 f_x(\xi,h_2) - h_2 f_y(0,\eta) - (h_1 f_x(0,0) + h_2 f_y(0,0)))\\
= \frac{1}{\|h\|} h(\underbrace{f_x(\xi,h_2) - f_x(0,0),\ f_y(0,\eta) - f_y(0,0)}_{\ad v(h)})
= \frac{1}{\|h\|} h\cdot v(h)$
...
$f$ sei in $x_0 \in D$ db, $\emptyset \ne E \subseteq \MdR^m,\ E$ sei offen, $f(D) \subseteq E$ und $g:E \to \MdR^p$ sei db in $y_0 \da  f(x_0)$. Dann ist $g \circ f: D \to \MdR^p$ db in $x_0$ und $$(g \circ f)'(x_0) = g'(f(x_0))\cdot f'(x_0)\text{ (Matrizenprodukt)}$$
\end{satz}

\begin{beweis}
$A \da  f'(x_0),\ B \da  g'(y_0) = g'(f(x_0)),\ h \da  g \circ f.$
...
$\ds{= \underbrace{\frac{\|f(x)-f(x_0)\|}{\|x-x_0\|}}_{\ad D(x)} \underbrace{\tilde{g}(f(x))}_{\to 0} + \underbrace{B(\underbrace{\frac{f(x)-f(x_0)-A(x-x_0)}{\|x-x_0\|}}_{\overset{f\text{ db}}{\to} 0\ (x \to x_0)})}_{\overset{\text{3.5}}{\to} 0\ (x \to x_0)}}$
...
\begin{beweis}
$\Phi(x)\da Ax (x \in \MdR^n). \text{ Lin. Alg.} \folgt \Phi \text{ ist linear, }
\Phi:\MdR^n \to \MdR^m. \folgtnach{(i)} \Phi \text{ ist injektiv, also }
\kernn \Phi = {0}. \text{ (ii) Sei }z \in \MdR^m, x\da Bz \folgtnach{(ii)} z = ABz = Ax = \Phi(x) \folgt \Phi \text{ ist surjektiv. Dann: } n = \dim \MdR^n \gleichnach{LA} \dim\kernn\Phi + \text{dim}\Phi(\MdR^n) = m.$
\end{beweis}
...
\begin{beweis} von 5.5: $g\da f^{-1}; x_0 \in D, z_0\da f(x_0) (\folgt x_0 = g(z_0))$
Es gilt: $g(f(x)) = x\ \forall x \in D, f(g(z)) = z\ \forall z \in f(D) \folgtnach{5.4} g'(f(x))\cdot f'(x) = I\ \forall x \in D; f'(g(z))\cdot g'(z) = \tilde{I}
\ \forall z \in f(D) \folgt \underbrace{g'(z_0)}_{\ad B}\cdot \underbrace{f'(x_0)}_{\ad A} = I, f'(x_0)\cdot g'(z_0) = \tilde{I} \folgtnach{5.5} m = n$ und $f'(x_0)^{-1} = g'(z_0) = (f^{-1})'(f(x_0))$.
\end{beweis}
...
\begin{liste}
\index{Konvexität}
\item Seien $a,b \in \MdR^n; S[a,b]\da \{a+t(b-a): t\in [0,1]\}$ hei"st
\begriff{Verbindungsstrecke} von $a$ und $b$
\item $M\subseteq \MdR^n$ heißt \textbf{konvex} $\dequizu$\ aus $a,b \in M$ folgt
stets: $S[a,b] \subseteq M$
\item Sei $k \in \MdN$ und $x^{(0)},\ldots,x^{(k)} \in \MdR^n.\ S[x^{(0)},\ldots,x^{(k)}]\da \bigcup_{j=1}^{k}S[x^{(j-1)}, x^{(j)}]$ heißt \begriff{Streckenzug} durch $x^{(0)},\ldots,x^{(k)}$ (in dieser Reihenfolge!)
\item Sei $G \subseteq \MdR^n$. $G\neq\emptyset$ heißt \begriff{Gebiet}$\dequizu\ G$ ist offen und aus $a,b \in G$ folgt: $\exists x^{(0)},\ldots,x^{(k)} \in G: x^{(0)}=a, x^{(k)}=b$ und $S[x^{(0)},\ldots,x^{(k)}] \subseteq G$.
\end{liste}
\end{definition}
...
\begin{beweis}
Sei $g(t)\da a+t\cdot(b-a)$ für $t\in[0,1]$. $g([0,1])=S[a,b]\subseteq D$. $\Phi(t)\da f(g(t)) (t \in [0,1])$ 5.4 $\folgt \Phi$ ist differenzierbar auf $[0,1]$ und $\Phi'(t) = f'(g(t))\cdot g'(t) = f'(a+t(b-a))\cdot(b-a)$. $f(b)-f(a)=\Phi(1)-\Phi(0) \folgtnach{MWS, AI} \Phi'(\eta) = f'(\underbrace{a+\eta(b-a)}_{\ad \xi \in S})\cdot(b-a), \eta \in [0,1]$
\end{beweis}
...
\begin{beweis}
``$\Longrightarrow$'':
``$\Longleftarrow$'': O.B.d.A.: $m=1$. Z.z.: $\exists a \in \MdR^n:\Phi(x)=a\cdot x \forall x \in \MdR^n$.
$a\da \Phi'(0) \Phi(0)=\Phi(2\cdot0)=2\cdot \Phi(0) \folgt \Phi(0)=0$.
...
$g(x)\da (\Phi(x)-ax)^2\ (x \in \MdR^n)$, $ g(0)=(\Phi(0)-a\cdot0)^2=0$.
...
Sei $a \in \MdR^n$ eine Richtung. $D$ offen $\folgt \exists \delta>0: U_\delta(x_0) \subseteq D$. Gerade durch $x_0$ mit Richtung $a:\{x_0+ta:t\in\MdR\}.\  \|x_0+ta-x_0\| = \|ta\| = |t|$. Also: $x_0+ta \in D$ für $t \in (-\delta,\delta),\ g(t) \da  f(x_0+ta)\ (t \in (-\delta,\delta))$.

$f$ heißt \textbf{in $x_0$ in Richtung $a$ db}, gdw. der Grenzwert $$\lim_{t\to 0} \frac{f(x_0+ta)-f(x_0)}{t}$$ existiert und $\in \MdR$ ist. In diesem Fall heißt $$\frac{\partial f}{\partial a}(x_0) \da  \lim_{t\to 0} \frac{f(x_0+ta)-f(x_0)}{t}$$ die \textbf{Richtungsableitung von $f$ in $x_0$ in Richtung $a$}.
...
\item[(ii)] Sei $\grad f(x_0) \ne 0$ und $a_0 \da  \|\grad f(x_0)\|^{-1}\cdot \grad f(x_0)$. Dann: $$\frac{\partial f}{\partial (-a_0)}(x_0) \le \frac{\partial f}{\partial a}(x_0) \le \frac{\partial f}{\partial a_0}(x_0) = \|\grad f(x_0)\|.$$ Weiter gilt: $\frac{\partial f}{\partial a}(x_0) < \frac{\partial f}{\partial a_0}(x_0)$, falls $a \ne a_0$; $\frac{\partial f}{\partial (-a_0)}(x_0) < \frac{\partial f}{\partial a}(x_0)$, falls $a \ne -a_0$.
\end{enumerate}
\end{liste}
\end{satz}
...
\item[(i)] $g(t) \da  f(x_0+ta)$ ($|t|$ hinreichend klein). Aus Satz 5.4 folgt: $g$ ist db in $t=0$ und $g'(0) = f'(x_0) \cdot a \folgt \frac{\partial f}{\partial a}(x_0)$ existiert und ist $= g'(0) = \grad f(x_0)\cdot a$
...
$$\nabla \da  \left( \frac{\partial}{\partial x_1},\ldots,\frac{\partial}{\partial x_n}\right)\ \text{("`Nabla"')};\ \nabla f\da  \left( \frac{\partial f}{\partial x_1},\ldots,\frac{\partial f}{\partial x_n}\right) = \grad f;\ \nabla f(x_0) \da  \grad f(x_0)$$

$$(h\cdot\nabla) \da  h_1 \frac{\partial}{\partial x_1} + \ldots + h_n \frac{\partial}{\partial x_n};\ (h\cdot\nabla) f\da  h_1 \frac{\partial f}{\partial x_1} + \ldots + h_n \frac{\partial f}{\partial x_n} = h \grad f;\ (h\cdot\nabla) f(x_0) \da  h\cdot\grad f(x_0)$$

$(h\cdot\nabla)^{(0)} f(x_0) \da  f(x_0)$. Für $k\in\MdN: (h\cdot\nabla)^{(k)} \da  \left( h_1 \frac{\partial}{\partial x_1} + \ldots + h_n \frac{\partial}{\partial x_n} \right)^k$
...
\begin{beweis}
$\Phi(t)\da f(x_0+th)$ f"ur $t\in[0,1]$. 5.4$\folgt \Phi \in C^{k+1}[0,1],\ \Phi'(t)=f'(x_0+th) \cdot h=(h \cdot \nabla)f(x_0+th)$\\
Induktiv: $\Phi^{(j)}(t)=(h\cdot\nabla)^{(j)}f(x_0+th)\ (j=0,\ldots,k+1, t\in[0,1]).\ \Phi(0)=f(x_0), \Phi(1)=f(x_0+h);\ \Phi^{(j)}(0)=(h\cdot\nabla)^{(j)}f(x_0)$. Analysis 1 (22.2) $\folgt \Phi(1)=\ds\sum_{j=0}^k\frac{\Phi^{(j)}(0)}{j!}+\frac{\Phi^{(k+1)}(\eta)}{(k+1)!}$, wobei $\eta\in[0,1]\folgt f(x_0+h)=\ds\sum_{j=1}^k\frac{(h\cdot\nabla)^{(j)}f(x_0)}{j!}+\frac{(h\cdot\nabla)^{(k+1)}f(x_0+\eta h)}{(k+1)!},\ \xi\da x_0+\eta h$
\end{beweis}

\begin{spezialfall}
Sei $f\in C^2(D,\MdR),x_0\in D, h\in \MdR^n, S[x_0,x_0+h]\subseteq D$. Dann $\exists\eta\in[0,1]$: 
...
\begin{definition*}
$Q_A:\MdR^n\to\MdR$ durch $Q_A(x)\da x(Ax)$. $Q_A$ hei"st die zu $A$ geh"orende \begriff{quadratische Form}. F"ur $x=(x_1,\ldots,x_n):$
$$Q_A(x)=\ds\sum_{j,k=1}^na_{jk}x_jx_k$$
\end{definition*}

\begin{beispiel}
Sei $f\in C^2(D,\MdR),x_0\in D, h\in \MdR^n, S[x_0,x_0+h]\subseteq D$.
$$H_f(x_0)\da \begin{pmatrix}
...
$A$ hei"st $\textbf{positiv definit}$ (pd) & $\dequizu$ $Q_A(x)>0\ \forall x\in\MdR^n\ \backslash\ \{0\}$\\
$A$ hei"st $\textbf{negativ definit}$ (nd) & $\dequizu$ $Q_A(x)<0\ \forall x\in\MdR^n\ \backslash\ \{0\}$\\
$A$ hei"st $\textbf{indefinit}$ (id) & $\dequizu \exists u,v\in\MdR^n: Q_A(u)>0, Q_A(v)<0$
\end{tabular}
\end{definition*}

\begin{beispiele}
\item $(n=2),\ A=\left(\begin{smallmatrix}a&b\\b&c\end{smallmatrix}\right)$\\
$Q_A(x,y)\da ax^2+2bxy+cy^2\ \left((x,y)\in\MdR^2\right)$. Nachrechnen:\\
...
\item \glqq$\impliedby$\grqq: Klar. \glqq$\folgt$\grqq: $K\da \{x\in\MdR^n: \|x\|=1\}=\partial U_1(0)$ ist beschr"ankt und abgeschlossen. $Q_A$ ist stetig auf $K$. 3.3 $\folgt\exists x_0\in K: Q_A(x_0)\le Q_A(x)\ \forall x\in K$. $c\da Q_A(x_0).\ A$ positiv definit, $x_0\ne 0\folgt Q_A(x_0)=c>0$. Sei $x\in\MdR^n\ \backslash\ \{0\};\ z\da \frac{1}{\|x\|}x\folgt z\in K\folgt Q_A(z)\ge c\folgt c \le Q_A\left(\frac{1}{\|x\|}x\right)\gleichnach{(2)}\frac{1}{\|x\|^2}Q_A(x)\folgt Q_A(x)\ge c\|x\|^2$
\end{beweise}
...
\begin{beweise}
\item $A$ sei positiv definit $\folgtnach{7.1}\exists c>0: Q_A(x)\ge c\|x\|^2\ \forall x\in\MdR^n$. $\ep\da \frac{c}{2n^2}$. Sei $B=(b_{jk})$ eine symmetrische Matrix mit $(*)$. F"ur $x=(x_1,\ldots,x_n)\in\MdR^n:\ Q_A(x)-Q_B(x)\le|Q_A(x)-Q_B(x)|=\left|\ds\sum_{j,k=1}^m(a_{jk}-b_{jk})x_jx_k\right|\le\ds\sum_{j,k=1}^n\underbrace{|a_{jk}-b_{jk}|}_{\le\ep}\underbrace{|x_j|}_{\le\|x\|}\underbrace{|x_k|}_{\le\|x\|}\le\ep\|x\|^2n^2=\frac{c}{2n^2}\|x\|^2n^2=\frac{c}{2}\|x\|^2$
\item $A$ sei indefinit. $\exists u,v\in\MdR^n:\ Q_A(u)>0, Q_A(v)<0$. $\alpha\da \min\left\{\frac{Q_A(u)}{\|u\|^2},\ -\frac{Q_A(v)}{\|v\|^2}\right\}\folgt\alpha>0$. $\ep\da \frac{\alpha}{2n^2}$. Sei $B=(b_{jk})$ eine symmetrische Matrix mit $(*)$.\\
...
\begin{liste}
\item
$f$ hat in $x_0$ ein \textbf{lokales Maximum} $\dequizu \exists \delta>0:\ f(x)\le f(x_0)\ \forall x\in D \cap U_\delta(x_0)$.\\
$f$ hat in $x_0$ ein \textbf{lokales Minimum} $\dequizu \exists \delta>0:\ f(x)\ge f(x_0)\ \forall x\in D \cap U_\delta(x_0)$.\\
...
Sei $\delta>0, f:U_\delta(0)\subseteq\MdR^n\to\MdR^n$ stetig, $f(0)=0$ und $V$ sei eine offene Umgebung von $f(0)\ (=0)$. $U\da \{x\in U_\delta(0):f(x)\in V\}$. Dann ist $U$ eine offene Umgebung von $0$.
...
Die Abbildungen $x \mapsto \det f'(x)$ und $x\mapsto \|f'(x) - I\|$ sind auf D stetig, $\det f'(0) \ne 0$, $\| f'(0)- I \| = 0$. Dann existiert ein $\delta > 0$: $K \da  U_\delta(0) \subseteq D$, $\overline{K} = \overline{U_\delta(0)} \subseteq D$ und 
\begin{liste}
...
\item $f^{-1}$ ist stetig auf $f(\overline{K})$: Seien $\xi, \eta \in f(\overline{K})$, $u\da f^{-1}(\xi)$, $v\da  f^{-1}(\eta) \folgt u,v \in \overline{K}$ und $\|f^{-1}(\xi) - f^{-1}(\eta)\| = \|u-v\| \stackrel{\text{(3)}}{\le} 2\|f(u) - f(v)\| = 2\|\xi - \eta\|$
\end{liste}

Beweis zu (3): $h(x) \da  f(x) - x \ (x\in D) \folgt h\in C^1(D,\MdR^n)$ und $h'(x) = f'(x) - I $. Sei $h=(h1,\ldots,h_n)$. Also: $h' = \begin{pmatrix} h_1' \\ \vdots \\ h_n' \end{pmatrix}$. Seien $u,v\in \overline{K}$ und $j\in \{1,\ldots,n\}$.
...
$V\da U_{\frac{\delta}{4}}(0)$ ist eine offene Umgebung von $f(0) \ (=0)$. $U\da \{x\in K: f(x) \in V\}$ Klar: $U\subseteq K \subseteq \overline{K}$, $0\in U$, 9.1 $\folgt$ $U$ ist eine offene Umgebung von 0. (3) $\folgt$ $f$ ist auf $U$ injektiv. (1) $\folgt \det f'(x) \ne 0 \ \forall x\in U$. (4) $\folgt$ $f^{-1}$ ist stetig auf $f(U)$. Klar: $f(U) \subseteq V$. Für (a) ist noch zu zeigen: $V\subseteq f(U)$.

Sei $y\in V$. $w(x) \da  \| f(x) - y\|^2 = (f(x) - y)\cdot(f(x)-y) \folgt w\in C^1(D,\MdR)$ und (nachzurechnen) $w'(x) = 2(f(x)-y)\cdot f'(x)$. $\overline K$ ist beschränkt und abgeschlossen $\folgtnach{3.3} \exists x_1 \in \overline K: \text{ (5) } w(x_1) \le w(x) \ \forall x\in\overline K$.
...
(b): Wegen 5.5 und 9.2 ist nur zu zeigen: $f^{-1}$ ist differenzierbar auf $V$. Sei $y_1 \in V$, $y \in V\backslash\{y_1\}$, $x_1 \da  f^{-1}(y_1)$, $x \da  f^{-1}(y)$; $L(y) \da  \frac{f^{-1}(y) - f^{-1}(y_1) - f'(x_0)^{-1}(y-y_1)}{\|y-y_1\|}$. zu zeigen: $L(y) \to 0 \ (y-y_1)$. $\varrho(x) \da  f(x)-f(x_1)-f'(x_1)(x-x_1)$. $f$ ist differenzierbar in $x_1$ $\folgt \frac{\varrho(x)}{\|x-x_1\|} \to 0 \ (x\to x_1)$.
...
\textbf{Beachte:} $f$ ist auf $D$ "`lokal"'   injektiv (im Sinne von $(*)$), aber $f$ ist auf $D$ \emph{nicht} injektiv, da $f(x,y) = f(x,y+2 k\pi) \ \forall x,y\in\MdR \ \forall k\in\MdZ$.
...
\end{array}
\right)
}_{\ad \frac{\partial f}{\partial y}\ (p\times p)\text{-Matrix}}
...
\begin{beweis}
Definition: $F:D\to\MdR^{n+p}$ durch $F(x,y)\da (x,f(x,y))$. Dann: $F\in C^1(D,\MdR^{n+p})$ und
$$
...
\right)$$
Dann: \begin{liste}
\item[(I)] $\det F'(x,y)\gleichnach{LA}\det\frac{\partial f}{\partial y}(x,y)\ ((x, y) \in D)$, insbesondere: $\det F'(x_0, y_0)\ne 0$. Es ist $F(x_0, y_0)=(x_0, 0)$. 9.3$\folgt\exists$ eine offene Umgebung $\MdU$ von $(x_0, y_0)$ und eine offene Umgebung $\vartheta$ von $(x_0, 0)$  mit: $\MdU\subseteq D, f(\MdU)=\vartheta$. $F$ ist auf $\MdU$ injektiv, $F^{-1}:\vartheta\to\MdU$ ist stetig differenzierbar und
\item[(II)] $\det F'(x,y)\gleichnach{(I)}\det\frac{\partial f}{\partial y}(x,y)\ne 0\ \forall\ (x,y)\in\MdU$
\end{liste}
\textbf{Bezeichnungen}: Sei $(s,t)\in\vartheta\ (s\in\MdR^n, t\in\MdR^p)$, $F^{-1}(s,t)\ad (u(s,t),v(s,t))$, also $u:\vartheta\to\MdR^n$ stetig differenzierbar, $v:\vartheta\to\MdR^p$ stetig differenzierbar. Dann: $(s,t)=F(F^{-1}(s,t))=(u(s,t),f(u(s,t),v(s,t)))\folgt u(s,t)=s\folgt F^{-1}(s,t)=(s,v(s,t))$. F"ur $(x,y)\in\MdU: f(x,y)=0\equizu F(x,y)=(x,0)\equizu(x,y)=F^{-1}(x,0)=(x,v(x,0))\equizu y=v(x,0)$, insbesondere: $y_0=v(x_0,0)$. $U\da \{x\in\MdR^n: (x,0)\in\vartheta\}$. Es gilt: $x_0\in U$. "Ubung: $U$ ist eine offene Umgebung von $x_0$.

\textbf{Definition}: $g:U\to\MdR^p$ durch $g(x)\da v(x,0)$, f"ur $x\in U$ gilt: $(x,0)\in\vartheta\folgt F^{-1}(x,0)=(x,v(x,0))=(x,g(x))\in \MdU$. Dann gelten: (1), (2), (3) und (4). (5) folgt aus (II).

Zu (6): Definition f"ur $x\in U: \psi(x)\da (x,g(x)), \psi\in C^1(U,\MdR^{n+p}),$
...
Seien $M,N$ Mengen $\ne \emptyset,\ f:M\to N$ eine Funktion und $\emptyset \ne T \subseteq M$. Die Funktion $f_{|_T}: T \to N,\ f_{|_T}(x) \da  f(x)\ \forall x \in T$ heißt die \textbf{Einschränkung} von $f$ auf $T$.
\end{definition}

In diesem Paragraphen gelte stets: $\emptyset \ne D \subseteq \MdR^n,\ D$ offen, $f \in C^1(D,\MdR),\ p \in \MdN,\ p<n$ und $\varphi = (\varphi_1,\ldots,\varphi_p) \in C^1(D,\MdR^p)$. Es sei $T\da \{x\in D: \varphi(x) = 0\} \ne \emptyset.$

\begin{definition}
\indexlabel{lokal!Extremum unter einer Nebenbedingung}
$f$ hat in $x_0\in D$ ein \textbf{lokales Extremum unter der Nebenbedingung $\varphi = 0$} $\dequizu x_0 \in T$ und $f_{|_T}$ hat in $x_0$ ein lokales Extremum.
\end{definition}

Wir führen folgende Hilfsfunktion ein: Für $x=(x_1,\ldots,x_n) \in D$ und $\lambda = (\lambda_1,\ldots,\lambda_p) \in \MdR^p$ gilt: $$H(x,\lambda) \da  f(x) + \lambda\cdot\varphi(x) = f(x) + \lambda_1\varphi_1(x) + \cdots + \lambda_p\varphi_p(x)$$
...
$$\gleichnach{(IV)} \underbrace{\frac{\partial f}{\partial y}(x_0) \left(-\frac{\partial \varphi}{\partial y}(x_0)\right)^{-1}}_{\ad  \lambda_0 \in \MdR^p} \frac{\partial \varphi}{\partial z}(x_0) + \frac{\partial f}{\partial z}(x_0) \folgt \frac{\partial f}{\partial z}(x_0) + \lambda_0 \frac{\partial \varphi}{\partial z}(x_0) = 0\text{ (V)}$$
...
\begin{beispiel}
$(n=3,p=2)\ f(x,y,z) = x+y+z,\ T\da \{(x,y,z) \in \MdR^3: x^2+y^2=2,\ x+z=1\},\ \varphi(x,y,z) = (x^2+y^2-2,x+z-1).$
...
\begin{beweis}
$f(x) \da  x\cdot(Ax) = Q_A(x)\ (x \in \MdR^n),\ T\da  \{x \in \MdR^n: \|x\|=1\} = \partial U_1(0)$ ist beschränkt und abgeschlossen.

$\varphi(x) \da  \|x\|^2-1 = x\cdot x-1;\ \varphi'(x) = 2x,\ f'(x) = 2Ax$.
...
\item Sei $\gamma :[a,b] \to \MdR^n$ ein Weg. $\Gamma_\gamma \da  \gamma([a,b])$ heißt der zu $\gamma$ gehörende \textbf{Bogen}, $\Gamma_\gamma \subseteq \MdR^n$. 3.3 \folgt{} $\Gamma_\gamma$ ist beschränkt und abgeschlossen.
...
$\gamma$ heißt \textbf{geschlossen} $\dequizu \gamma(a) = \gamma(b)$.
\item $\gamma^-:[a,b]\to \MdR^n$, definiert durch $\gamma^-(t)\da \gamma(b+a-t)$ heißt der zu $\gamma$ \textbf{inverse Weg}. Beachte: $\gamma^- \ne \gamma$, aber $\Gamma_\gamma = \Gamma_{\gamma^-}$.
\end{liste}
\end{definition}

\begin{beispiele}
\item Sei $x_0, y_0\in\MdR^n$, $\gamma(t) \da  x_0 + t(y_0-x_0)$, $t\in[0,1]$. $\Gamma_\gamma=S[x_0,y_0]$
\item Sei $r>0$ und $\gamma(t) \da  (r \cos t, r \sin t)$, $t\in[0,2\pi]$\\ $\Gamma_\gamma=\{(x,y)\in\MdR^2: x^2+y^2=r^2\} = \partial U_r(0)$ \\
$\tilde\gamma(t) \da  (r \cos t, r \sin t)$, $t\in[0,4\pi]$. $\tilde\gamma \ne \gamma$, aber $\Gamma_{\tilde\gamma} = \Gamma_\gamma$.
\item Sei $f:[a,b] \to \MdR$ stetig und $\gamma(t) \da  (t, f(t)) \quad (t \in [a,b])$. Dann: $\Gamma_\gamma = $ Graph von $f$.
\end{beispiele}
...
$$L(\gamma,Z)\da  \sum_{j=1}^m\|\gamma(t_j) - \gamma(t_{j-1})\|$$
Übung: Sind $Z_1,Z_2\in\Z$ und gilt $Z_1 \subseteq Z_2 \folgt L(\gamma,Z_1) \le L(\gamma,Z_2)$

$\gamma$ heißt \textbf{rektifizierbar} (rb) \indexlabel{Rektifizierbarkeit} $\dequizu$ $\exists M\ge0: L(\gamma,Z)\le M\ \forall Z\in\Z$. In diesem Fall heißt $L(\gamma) \da  \sup\{L(\gamma,Z): Z\in\Z\}$ die \textbf{Länge} von $\gamma$\indexlabel{Länge}.
...
Definiere $\gamma:[a_0,a_l]\to\MdR^n$ durch $\gamma(t)\da \gamma_k(t)$, falls $t\in[a_{k-1},a_k]$. $\gamma$ ist ein Weg im $\MdR^n$, $\Gamma_\gamma = \Gamma_{\gamma_1} \cup \Gamma_{\gamma_2} \cup \cdots \cup \Gamma_{\gamma_l}$. $\gamma$ heißt die Summe der Wege $\gamma_1,\ldots,\gamma_l$ und wird mit. $\gamma = \gamma_1 \oplus \gamma_2 \oplus \cdots \oplus \gamma_l$ bezeichnet. \indexlabel{Summe von Wegen}

\begin{bemerkung}
Ist $\gamma:[a,b]\to\MdR^n$ ein Weg und $Z=\{t_0,\ldots,t_m\}\in\Z$ und $\gamma_k\da \gamma_{|_{[t_{k-1},t_k]}}$ $(k=1,\ldots,m)$ $\folgt$ $\gamma = \gamma_1 \oplus \cdots \oplus \gamma_m$. Aus Analysis I, 25.1(7) und 12.1 folgt: 
\end{bemerkung}
...
$$s(t)\da  \begin{cases}L(\gamma_{|_{[a,t]}}),&\text{falls }t\in(a,b] \\0, &\text{falls }t=a\end{cases}$$ heißt die zu $\gamma$ gehörende \textbf{Weglängenfunktion}.
\end{definition}
...
\item Sei $t_1, t_2 \in [a,b]$ und $t_1<t_2$. $\gamma_1\da \gamma_{|_{[a,t_1]}}$, $\gamma_2\da \gamma_{|_{[t_1,t_2]}}$, $\gamma_3\da \gamma_{|_{[a,t_2]}}$. Dann $\gamma_3 = \gamma_1 \oplus \gamma_2$. 12.2 $\folgt$ $\gamma_1,\gamma_2,\gamma_3$ sind rektifizierbar und $\underbrace{L(\gamma_3)}_{=s(t_2)} = \underbrace{L(\gamma_1)}_{s(t_1)} + \underbrace{L(\gamma_2)}_{\ge 0} \folgt s(t_2) \ge s(t_1)$.
\end{liste}
\end{beweis}

\begin{satz}[Rechenregeln für Integrale im $\MdR^n$]
Sei $f=(f_1,\ldots,f_n):[a,b]\to\MdR^n$ und $f_1,\ldots,f_n\in R[a,b]$.
$$\int_a^bf(t)\dd{t} \da  \left(\int_a^bf_1(t)\dd{t}, \int_a^bf_2(t)\dd{t},\ldots, \int_a^bf_n(t)\dd{t}\right) \quad (\in\MdR^n)$$
Dann: \begin{liste}
\item $$x\cdot \int_a^bf(t)\dd{t} = \int_a^b(x\cdot f(t))\dd{t} \ \forall x\in\MdR^n$$
\item $$\left\|\int_a^bf(t)\dd{t}\right\| \le \int_a^b\|f(t)\|\dd{t}$$
\end{liste}
\end{satz}

\begin{beweis}
\begin{liste}
\item Sei $x=(x_1,\ldots,x_n) \folgt$\\ $x\cdot\int_a^b f(t)\dd{t} = \sum_{j=1}^n x_j\int_a^bf_j(t) \dd{t} = \int_a^b\left(\sum_{j=1}^n x_j f_j(t)\dd{t}\right) = \int_a^b \left(x\cdot f(t)\right) \dd{t}$
\item $y\da \int_a^bf(t)\dd{t}$. O.B.d.A: $y\ne 0$. $x\da  \frac{1}{\|y\|} y \folgt \|x\|=1, y=\|y\|x$. $\|y\|^2 = y\cdot y = \|y\|(x\cdot y) = \|y\|\left(x\cdot \int_a^bf(t)\dd{t} \right) = \|y\|\int_a^b\left(x\cdot f(t)\right) \dd{t} \le \|y\|\int_a^b\underbrace{|x \cdot f(t)|}_{\le\|x\|\|f(t)\| = \|f(t)\|} \dd{t} \le \|y\| \int_a^b\|f(t)\|\dd{t}$
\end{liste}
\end{beweis}
...
\item $L(\gamma)=\int_a^b\|\gamma'(t)\|\dd{t}$
\end{liste}
\end{satz}
...
Sei $t\in (t_0, b];\ \gamma_1\da \gamma_{|_{[a,t_0]}}, \gamma_2\da \gamma_{|_{[t_0,t]}}, \gamma_3\da \gamma_{|_{[a,t]}}$. Dann: $\gamma_3=\gamma_1 \oplus \gamma_2$ und $\underbrace{L(\gamma_3)}_{=s(t)}=\underbrace{L(\gamma_1)}_{=s(t_0)}+L(\gamma_2)\folgt s(t)-s(t_0)=L(\gamma_2)\ (I).$\\
$\tilde{Z}\da \{t_0, t\}$ ist eine Zerlegung von $[t_0,t]\folgt \|\gamma(t)-\gamma(t_0)\|=L(\gamma_2,\tilde{Z})\le L(\gamma_2) \stackrel{\text{(I)}}{\le} s(t) - s(t_0)$ (II)\\
\textbf{Definition}: $F:[a,b]\to\MdR$ durch $F(t)=\ds\int_a^t\|\gamma'(\tau)\|\dd{\tau}$. 2. Hauptsatz der Differential- und Integralrechnung $\folgt F$ ist differenzierbar und $F'(t)=\|\gamma'(t)\|\ \forall t\in[a,b]$. Sei $Z=\{\tau_0,\ldots,\tau_m\}$ eine beliebige Zerlegung von $[t_0, t]$.
$$\ds\int_{\tau_{j-1}}^{\tau_j}\gamma'(\tau)\dd{\tau}=\left(\cdots, \ds\int_{\tau_{j-1}}^{\tau_j}\eta_k'(\tau)\dd{\tau},\cdots\right)\gleichnach{AI}\left(\cdots, \eta_k(\tau_j)-\eta_k(\tau_{j-1}),\cdots\right)=\gamma(\tau_j)-\gamma(\tau_{j-1})$$
$\folgt \|\gamma(\tau_j)-\gamma(\tau_{j-1})\|\overset{12.4}{\le}\ds\int_{\tau_{j-1}}^{\tau_j}\|\gamma'(\tau)\|\dd{\tau}$. Summation $\folgt L(\gamma_2,Z)\le\ds\int_{t_0}^t\|\gamma'(\tau)\|\dd{\tau}=F(t)-F(t_0)\folgt L(\gamma_2)\le F(t)-F(t_0)\ (III)$.\\
...
$(3)\ L(\gamma)=s(b)=s(b)-s(a)\overset{AI}{=}\ds\int_a^b s'(t)\dd{t}\gleichnach{(2)}\ds\int_a^b\|\gamma'(t)\|\dd{t}$
\end{beweise}

\begin{beispiele}
\item $x_0, y_0\in\MdR^n, \gamma(t)\da x_0+t(y_0-x_0)\ (t\in [0,1])$. $\gamma'(t)=y_0-x_0\folgt L(\gamma)=\ds\int_0^1\|y_0-x_0\|\dd{t}=\|y_0-x_0\|$.
\item Sei $f:[a,b]\to\MdR$ stetig differenzierbar und $\gamma(t)\da (t, f(t)), t\in[a,b]$. $\gamma$ ist ein Weg im $\MdR^2$. $\gamma$ ist rektifizierbar $\equizu f \in \text{BV}[a,b]$. $\Gamma_\gamma=$ Graph von $f$. Jetzt sei $f\in C^1[a,b] \folgtnach{12.5} L(\gamma)=\ds\int_a^b\|\gamma'(t)\|\dd{t}=\ds\int_a^b (1+f'(t)^2)^{\frac{1}{2}}\dd{t}$.
\item $\gamma(t)\da (\cos t, \sin t)\ (t\in [0,2\pi])$. $\gamma'(t)=(-\sin t, \cos t)$. $\|\gamma'(t)\|=1\ \forall t\in [0,2\pi]\folgtnach{12.5}s'(t)=1\ \forall t\in[0,2\pi]\folgt s(t)=t\ \forall t\in[0,2\pi]$ (\begriff{Bogenmaß}). \begriff{Winkelmaß}: $\varphi\da \frac{180}{\pi}t$. $L(\gamma)=2\pi$.
\end{beispiele}
...
\item $\gamma$ hei"st \textbf{stückweise stetig differenzierbar} $\dequizu\exists z=\{t_0,\ldots,t_m\}\in\Z$ mit: $\gamma_{|_{[t_{k-1},t_k]}}$ sind stetig differenzierbar $(k=1,\ldots,m)\equizu\exists$ stetig differenzierbare Wege $\gamma_1,\ldots,\gamma_l: \gamma=\gamma_1\oplus\cdots\oplus\gamma_l$.
\item $\gamma$ hei"st \textbf{glatt} $\dequizu \gamma$ ist stetig differenzierbar und $\|\gamma'(t)\|>0\ \forall t\in[a,b]$.
\item $\gamma$ hei"st \textbf{stückweise glatt} $\dequizu\exists$ glatte Wege $\gamma=\gamma_1\oplus\cdots\oplus\gamma_l$
\end{liste}
\end{definition*}
...
\begin{beispiele}
\item $x_0, y_0\in\MdR^n, \gamma_1(t)\da x_0+t(y_0-x_0)\ t\in[0,1],\ \gamma_2(t)\da \gamma_1^-(t)\ t\in[0,1],\ \gamma_3(t)\da x_0+7t(y_0-x_0)\ t\in[0,\frac{1}{7}].\ \gamma_1,\gamma_2,\gamma_3$ sind Parameterdarstellungen von $S[x_0, y_0]$.
\item $\gamma_1(t)=(\cos t, \sin t),\ (t\in [0,2\pi]), \gamma_2(t)\da (\cos t, \sin t), (t\in[0,4\pi])$. $\gamma_1, \gamma_2$ sind Parameterdarstellungen von $K=\{(x,y)\in\MdR^2: x^2+y^2=1\}.$
\end{beispiele}
...
\item[(1)] Es gen"ugt zu zeigen: Aus $\gamma_2$ rektifizierbar folgt: $\gamma_1$ ist rektifizierbar und $L(\gamma_1)\le L(\gamma_2)$. Sei $Z=\{t_0,\ldots,t_m\}\in\Z\folgt\tilde{Z}\da \{h(t_0),\ldots, h(t_m)\}$ ist eine Zerlegung von $[\alpha,\beta]$.
...
Es sei $\gamma:[a,b]\to\MdR^n$ ein \emph{glatter} Weg. 12.5 $\folgt \gamma$ ist rb. $L\da L(\gamma)$. 12.5 $\folgt s \in C^1[a,b]$ und $s'(t) = \|\gamma'(t)\| > 0\ \forall t\in[a,b].\ s$ ist also \emph{streng wachsend}. Dann gilt: $s([a,b]) = [0,L],\ s^{-1}:[0,L]\to[a,b]$ ist streng wachsend und stetig db. $(s^{-1})'(\sigma) = \frac{1}{s'(t)}$ f"ur $\sigma \in [0,L],\ s(t) = \sigma.$

\begin{definition}
$\tilde{\gamma}[0,L] \to \MdR^n$ durch $\tilde{\gamma}(\sigma) \da  \gamma(s^{-1}(\sigma)),$ also $\tilde{\gamma} = \gamma\circ s^{-1};\ \tilde{\gamma}$ ist ein Weg im $\MdR^n$ und $\tilde{\gamma} \sim \gamma;\ \Gamma_\gamma = \Gamma_{\tilde{\gamma}}.$
...
Darum: Sei $\tilde{s}$ die zu $\tilde{\gamma}$ geh"orende Wegl"angenfunktion. $\forall \sigma\in[0,L]: \tilde{\gamma}(\sigma) = \gamma(s^{-1}(\sigma)).$ Sei $\sigma\in[0,L],\ t\da  s^{-1}(\sigma) \in [a,b],\ s(t) = \sigma.$
...
\begin{definition}
Sei $\gamma:[a,b]\to\mdr^n$ ein Weg, $\gamma=(\eta_1,\ldots,\eta_n), \Gamma \da \Gamma_\gamma.$
$g:\Gamma\to\mdr$ stetig und $f=(f_1,\ldots,f_n):\Gamma\to\mdr^n$ stetig.
\begin{enumerate}
\item Für $j\in \{1,\ldots,n\}: \int_\gamma g(x) \dd{x}_j\da \int_a^b g(\gamma(t))\cdot\eta_j'(t)\dd{t}$
\item \begin{align*}
\int_\gamma f(x)\cdot\dd{x} &\da  \int_\gamma f_1(x)\dd{x}_1+\cdots+f_n(x)\text{ d}x_n\\
&\da \sum_{j=1}^n \int_\gamma f_j(x)\dd{x}_j
\end{align*}
Es ist $\int_\gamma f(x)\cdot\dd{x}=\int_a^b f(\gamma(t))\cdot\gamma'(t)\dd{t}$ und heißt das
\textbf{Wegintegral von $f$ längs $\gamma$}.
\end{enumerate} 
\end{definition}

\begin{beispiel}
$f(x,y,z) \da  (z,y,x),\ \gamma(t) = (t,t^2,3t),\ t\in[0,1].\ f(\gamma(t)) = (3t,t^2,t),\ \gamma'(t)=(1,2t,3),\ f(\gamma(t))\cdot\gamma'(t) = 3t+2t^3+3t = 6t+2t^3$.

$\int_\gamma f(x,y,z)\cdot \dd{(x,y,z)} = \int_0^1 (6t+2t^3) \dd{t} = \frac{7}{2}.$
\end{beispiel}
...
\begin{liste}
\item $\ds{\int_\gamma(\xi f(x)+\eta g(x))\cdot \dd{x} = \xi \int_\gamma f(x)\cdot \dd{x}+\eta \int_\gamma g(x)\cdot \dd{x}}$
\item Ist $\gamma = \gamma^{(1)} \oplus \gamma^{(2)} \folgt \ds{\int_\gamma f(x)\cdot \dd{x} = \int_{\gamma^{(1)}} f(x)\cdot \dd{x} + \int_{\gamma^{(2)}} f(x)\cdot \dd{x}}$
\item $\ds{\int_{\gamma^-} f(x)\cdot \dd{x} = -\int_\gamma f(x)\cdot \dd{x}}$
\item $\ds{\left| \int_\gamma f(x)\cdot \dd{x}\right| \le L(\gamma)\cdot \max\{\|f(x)\|:x \in \Gamma\}}$
\item Ist $\ds{\hat{\gamma} \sim \gamma \folgt \int_\gamma f(x)\cdot \dd{x} = \int_{\hat{\gamma}} f(x)\cdot \dd{x}}$.
\end{liste}
\end{satz}
...
$\int_{\gamma^-} f(x)\cdot \dd{x} = \int_a^b f(\gamma(b+a-t))\cdot \gamma'(b+a-t) (-1) \dd{t} =$ (subst. $\tau=b+a-t,\ d\tau = \dd{t}$) $= \int_b^a f(\gamma(\tau))\cdot\gamma'(\tau) \dd{\tau} = -\int_a^b f(\gamma(\tau))\cdot\gamma'(\tau) \dd{\tau} = -\int_\gamma f(x)\cdot \dd{x}.$
\item "Ubung
...
$\int_{\hat{\gamma}} f(x)\cdot \dd{x} = \int_\alpha^\beta f(\gamma(h(t)))\cdot \gamma'(h(t))\cdot h'(t) \dd{t} =$ (subst. $\tau = h(t),\ d\tau = h'(t)\dd{t}$) $= \int_a^b f(\gamma(\tau))\cdot \gamma'(\tau)\dd{\tau} = \int_\gamma f(x)\cdot \dd{x}.$
\end{beweise}

\begin{definition}
$\gamma,\Gamma$ seien wie immer in diesem Paragraphen. $\gamma$ sei stückweise stetig differenzierbar. $s$ sei die zu $\gamma$ geh"orende Wegl"angenfunktion und $g:\Gamma \to \MdR$ stetig. 12.4 $\folgt s$ ist wachsend $\folgtnach{Ana I} s \in BV[a,b];\ g\circ\gamma$ stetig $\folgtnach{Ana I, 26.6} g\circ\gamma \in R_s[a,b]$.

$$\int_\gamma g(x) \dd{s} \da  \int_a^b g(\gamma(t))\|\gamma'(t)\|\dd{t}$$
...
\begin{liste}
\item $\ds{\int_{\gamma^-} g(x) \dd{s} = \int_\gamma g(x) \dd{s}}$
\item Ist $\ds{\gamma = \gamma^{(1)} \oplus \gamma^{(2)} \folgt \int_\gamma g(x)\dd{s} = \int_{\gamma^{(1)}} g(x)\dd{s} + \int_{\gamma^{(2)}} g(x)\dd{s}}$.
\end{liste}
\end{satz}
...
$g(\gamma(t)) = (1+t^2+3t^2)^{1/2} = (1+4t^2)^{1/2},\ \gamma'(t) = (1, 2t),\ \|\gamma'(t)\| = (1+4t^2)^{1/2} \folgt \int_\gamma g(x,y)\dd{s} = \int_0^1 (1+4t^2) \dd{t} = \frac{7}{3}$
\end{beispiel}

\paragraph{Gegeben:} $\gamma_1,\gamma_2,\ldots,\gamma_m$ rektifizierbare Wege, $\gamma_k:[a_k,b_k]\to\MdR^n$ mit $\gamma_1(b_1) = \gamma_2(a_2), \gamma_2(b_2) = \gamma_3(a_3),\ldots , \gamma_{m-1}(b_{m-1}) = \gamma_m(a_m)$. $\Gamma \da  \Gamma_{\gamma_1} \cup \ldots \cup \Gamma_{\gamma_m}$.

$\text{AH}(\gamma_1,\ldots,\gamma_m) \da  \{\gamma:\gamma$ ist ein rektifizierbarer Weg im $\MdR^n$ mit: $\Gamma_\gamma=\Gamma$, $L(\gamma)=L(\gamma_1)+\cdots+L(\gamma_m)$ und $\int_\gamma f(x)\cdot \dd{x} = \int_{\gamma_1}f(x)\cdot \dd{x}+ \cdots + \int_{\gamma_m}f(x)\cdot \dd{x}$ für \emph{jedes} stetige $f:\Gamma\to\MdR^n\}.$
...
\begin{beweis}
O.B.d.A: $m=2$. Sei $c > b_1$ beliebig.

Def. $h:[b_1,c] \to [a_2,b_2]$ linear wie folgt: $h(x)=px+q$, $h(b_1)=a_2$, $h(c)=b_2$. $\hat\gamma_2 \da  \gamma_2\circ h$. Dann: $\gamma_2\sim \hat\gamma_2$. $\gamma \da  \gamma_1\oplus\hat\gamma_2$. 12.2, 12.7, 13.2 $\folgt$ $\gamma\in \text{AH}(\gamma_1,\gamma_2)$.
\end{beweis}
...
Sei $\gamma \in \text{AH}(\gamma_1,\gamma_2)$. Anfangspunkt von $\gamma$ ist (0,0), Endpunkt von $\gamma$ ist (1,1). Nachrechnen: $\int_{\gamma_1}f(x,y)\cdot \dd{(x,y)} = 0$, $\int_{\gamma_2}f(x,y)\cdot \dd{(x,y)} = \frac{1}{2}$. Also: $\int_\gamma f(x,y) \cdot \dd{(x,y)} = \frac{1}{2}$

\item $\gamma_1(t) = (0,t)$, $\gamma_2(t)=(t,1)$.

Sei $\gamma\in \text{AH}(\gamma_1,\gamma_2)$, Anfangspunkt von $\gamma$ ist (0,0), Endpunkt von $\gamma$ ist (1,1). Nachrechnen: $\int_{\gamma}f(x,y)\cdot \dd{(x,y)} = \frac{1}{2}$

\item $\gamma(t)=(t,t^3)$. Anfangspunkt von $\gamma$ ist (0,0), Endpunkt von $\gamma$ ist (1,1). Nachrechnen: $\int_\gamma f(x,y)\cdot \dd{(x,y)} = \frac{1}{2}$
\end{liste}
\end{beispiel}
...
\begin{definition}
Eine Funktion $\varphi:G\to\MdR$ heißt eine \textbf{Stammfunktion (SF) von $f$ auf $G$}\indexlabel{Stammfunktion} $\dequizu$ $\varphi$ ist auf $G$ partiell differenzierbar und $\grad\varphi = f$ auf $G$. Also: $\varphi_{x_j} = f_j$ auf $G$ ($j=1,\ldots,n$).
\end{definition}
...
Ansatz für eine Stammfunktion $\varphi$ von $f$: $\varphi_x=y \folgt \varphi=xy+c(y)$, $c$ differenzierbar, $\folgt$ $\varphi_y=x+c'(y) \stackrel{\text{!}}{=} x-y \folgt c'(y) = -y$, etwa $c(y)=-\frac{1}{2}y^2$. Also: $\varphi(x,y) = xy - \frac{1}{2}y^2$. Probe: $\varphi_x=y$, $\varphi_y=x-y$, also: $\grad\varphi=f$. $\varphi$ ist also eine Stammfunktion von $f$ auf $\MdR^2$.
...
$f$ besitze auf $G$ die Stammfunktion $\varphi$; $\gamma:[a,b]\to\MdR^n$ sei ein stückweise stetig differenzierbarer Weg mit $\Gamma_\gamma\subseteq G$. Dann:
$$ \int_\gamma f(x)\cdot \dd{x} = \varphi\left(\gamma(b)\right) - \varphi\left(\gamma(a)\right) $$
Das heißt: $\int_\gamma f(x)\cdot \dd{x}$ hängt nur vom Anfangs- und Endpunkt von $\gamma$ ab.

Ist $\gamma$ \emph{geschlossen}, das heißt $\gamma(a) = \gamma(b)$, dann gilt $\int_\gamma f(x)\cdot \dd{x} = 0$.

Ist $\hat{\gamma}\colon[\alpha, \beta]\to\MdR^n$ ein weiterer stückweise differenzierbarer Weg mit $\Gamma_{\hat{\gamma}}\subseteq G$ und
$\gamma(a) = \hat{\gamma}(\alpha) \wedge \gamma(b) = \hat{\gamma}(\beta)$, so gilt $\int_\gamma f(x) \cdot \dd{x} = \int_{\hat{\gamma}} f(x) \cdot \dd{x}$.
\end{satz}

\begin{beweis}
O.B.d.A.: $\gamma$ ist stetig differenzierbar. $\Phi(t)\da  \varphi (\gamma(t))$, $t\in[a,b]$. $\Phi$ ist stetig differenzierbar und $\Phi'(t) = \varphi'(\gamma(t))\cdot \gamma'(t) = f(\gamma(t))\cdot\gamma(t)$ Dann: $\int_\gamma f(x)\cdot \dd{x} \gleichnach{13.1} \int_a^bf(\gamma(t))\cdot\gamma'(t)\dd{t} = \int_a^b\Phi'(t)\dd{t} \gleichnach{AI} \Phi(b)-\Phi(a) = \varphi(\gamma(b))-\varphi(\gamma(a))$.
\end{beweis}
...
$\gamma_j(t) \da  z_{j-1} + t(z_j - z_{j-1})$, $(t\in[0,1])$, ($j=1,\ldots,n$). Dann:$\Gamma_{\gamma_j} = S[z_{j-1},z_{j}] \folgt \Gamma_{\gamma_1}\cup\ldots\cup\Gamma_{\gamma_m} = S[z_0,\ldots,z_m] \subseteq G$. 13.4 $\folgt \exists \gamma \in \text{AH}(\gamma_1,\ldots,\gamma_m)$ stückweise stetig differenzierbar $\folgt \Gamma_{\gamma} = S[z_0,\ldots,z_m] \subseteq G$. 
\end{beweis}

\begin{definition*}
\indexlabel{Weg-!unabhängig}
$\int f(x)\cdot \dd{x}$ hei"st \textbf{in G wegunabh"angig} (wu) $\dequizu$ f"ur je zwei Punkte $x_0, y_0\in G$ gilt: f"ur jeden st"uckweise stetig differenzierbaren Weg $\gamma:[a,b]\to\MdR^n$ mit $\Gamma_\gamma\subseteq G$, $\gamma(a)=x_0$ und $\gamma(b)=y_0$ hat das Integral $\ds\int_\gamma f(x)\cdot\dd{x}$ stets denselben Wert. In diesem Fall: $\ds\int_{x_0}^{y_0}f(x)\cdot\dd{x}\da \ds\int_\gamma f(x)\cdot\dd{x}$.
\end{definition*}

\textbf{14.1 lautet dann}: besitzt f auf G die Stammfunktion $\varphi\folgt \ds\int f(x)\cdot\dd{x}$ ist in $G$ wegunabh"angig und $\int_{x_0}^{y_0}=\varphi(y_0)-\varphi(x_0)$ (Verallgemeinerung von Analysis 1, 23.5).

\begin{satz}[Wegunabhängigkeit, Existenz von Stammfunktionen]
$f$ besitzt auf $G$ eine Stammfunktion $\equizu\int f(x)\cdot\dd{x}$ ist in G wegunabh"angig. \\
In diesem Fall: ist $x_0\in G$ und $\varphi:G\to\MdR$ definiert durch: 
\begin{align*}
\varphi(z)=\ds\int_{x_0}^z f(x)\cdot\dd{x}\ (z\in G)\ \tag{$*$}
\end{align*}
...
Es ist:
\begin{eqnarray*}
&&\ds\int_{\gamma}f(z_0)\cdot\dd{x}=\ds\int_0^1 f(z_0)\cdot\underbrace{\gamma'(t)}_{=h}\dd{t}=f(z_0)\cdot h\\
&&\folgt \rho(h)=\frac{1}{\|h\|}\ds\int_{\gamma}(f(x)-f(z_0))\dd{x}\\
&&\folgt |\rho(h)|=\frac{1}{\|h\|}\left|\ds\int_{\gamma}f(x)-f(z_0)\dd{x}\right|\\
...
Sei $\emptyset\ne M\subseteq\MdR^n$. $M$ hei"st \textbf{sternförmig} $\dequizu\exists x_0\in M: S[x_0,x]\subseteq M\ \forall x\in M$.\\
\textbf{Beachte:}
\begin{liste}
...
\begin{eqnarray*}
\varphi(x)&\da &\int_{\gamma_x}f(z)\cdot\dd{z}\ (x\in G)\\
&=&\ds\int_0^1 f(tx)\cdot x\dd{t}\\
&=&\ds\int_0^1(f_1(tx)\cdot x_1+f_2(tx)\cdot x_2+\ldots+f_n(tx)\cdot x_n)\dd{t}\\
\end{eqnarray*}
...
$$\varphi_{x_1}(x)=\ds\int_0^1\frac{\partial}{\partial x_1}(f_1(tx)x_1+\ldots+f_n(tx)\cdot x_n)\dd{t}$$
...
\begin{eqnarray*}
\varphi_{x_1}(x)&=&\int_0^1(f_1(tx)+t(\frac{\partial f_1}{\partial x_1}(tx)x_1+\ldots+\frac{\partial f_n}{\partial x_1}(tx)x_n))\dd{t}\\
&\gleichnach{IB}&\int_0^1(f_1(tx)+t(\frac{\partial f_1}{\partial x_1}(tx)x_1+\frac{\partial f_1}{\partial x_2}(tx)x_2+\ldots+\frac{\partial f_1}{\partial x_n}(tx)x_n))\dd{t}\\
&=&\ds\int_0^1(f_1(tx)+tf_1'(tx)\cdot x)\dd{t}
\end{eqnarray*}

Sei $x\in G$ (fest), $h(t)\da t\cdot f_1(tx)\ (t\in [0,1])$. $h$ ist stetig differenzierbar und $h'(t)=f_1(tx)+tf_1'(tx)\cdot x\folgt \varphi_{x_1}(x)=\ds\int_0^1 h'(t)\dd{t}\gleichnach{A1}h(1)-h(0)=f_1(x)$.
\end{beweis}
...
\[\int_A f(x)\dd{x}=\int_A f(x_1,\ldots,x_n)\dd{(x_1,\ldots,x_n)}\]
...
\[A\da \{(x,y)\in\MdR^2: x\in[a,b],h_1(x)\le y\le h_2(x)\}\]
\[\left(A\da \{(x,y)\in\MdR^2: y\in[a,b],h_1(y)\le x\le h_2(y)\}\right)\]
...
\[\int_A f(x,y)\dd{(x,y)}=\int_a^b\left(\int_{h_1(x)}^{h_2(x)} f(x,y)\dd{y}\right)\dd{x}\]
\[\left(\int_A f(x,y)\dd{(x,y)}=\int_a^b\left(\int_{h_1(y)}^{h_2(y)} f(x,y)\dd{x}\right)\dd{y}\right)\]
\end{satz}

\textbf{Achtung:} $\int_A f(x)\dd{x}$ nicht mit dem Wegintegral $\int_\gamma f(x)\cdot\dd{x}$
verwechseln!
...
\begin{align*}
|A|&=\int_A 1\dd{(x,y)}\\
&\stackrel{15.1}{=}\int_a^b\left(\int_{h_1(x)}^{h_2(x)} 1\dd{y}\right)\dd{x}\\
&=\int_a^b(h_2(x)-h_1(x))\dd{x}
\end{align*}
Ist z.B. $h_1=0$, so folgt:
\[|A|=\int_a^b h_2(x)\dd{x}\]
...
\begin{align*}
\int_A f(x,y)\dd{(x,y)} &= \int_a^b\left(\int_c^d f(x,y)\dd{y}\right)\dd{x}\\
&= \int_c^d\left(\int_a^b f(x,y)\dd{x}\right)\dd{y}
\end{align*}
\item[(3)] Sei $r>0$ und $A\da \{(x,y)\in\MdR^2: x^2+y^2\le r^2\}$. Dann ist
$A$ ein Normalbereich der $x$-Achse mit $h_1(x)\da \sqrt{r^2-x^2}$ und $h_2(x)\da -\sqrt{r^2-x^2}$
(mit $x\in[-r,r]$), und es gilt:
\begin{align*}
|A|&=\int_{-r}^r h_2(x)-h_1(x)\dd{x}\\
&=\int_{-r}^r 2\sqrt{r^2-x^2}\dd{x}\\
&=\pi r^2
\end{align*}
\item[(4)] Sei $A\da \{(x,y)\in\MdR^2: x\in[0,1],x\le y\le \sqrt x\}$ und $f(x,y)=xy$.
Dann gilt für $h_1(x)=x$ und $h_2(x)=\sqrt x$:
\begin{align*}
\int_A xy\dd{(x,y)} &= \int_0^1\left(\int_x^{\sqrt x} xy\dd{y}\right)\dd{x}\\
&= \int_0^1\left(\left[\frac12 xy^2\right]_x^{\sqrt x}\right)\dd{x}\\
&= \int_0^1 \frac12x^2 -\frac12x^3\dd{x}\\
&= \frac12\left[\frac13 x^3-\frac14 x^4\right]_0^1 = \frac1{24}
\end{align*}
Da $A=\{(x,y)\in\MdR^2: y\in[0,1],y^2\le x\le y\}$ außerdem Normalbereich bzgl. der 
$y$-Achse ist, gilt:
\begin{align*}
\int_A f(x,y)\dd{y} &= \int_0^1\left(\int_{y^2}^y xy \dd{x}\right)\dd{y}\\
&=\int_0^1 \left[ \frac12 x^2y\right]_{y^2}^y \dd{y}\\
&=\int_0^1 \frac12y^3-\frac12y^5 \dd{y}\\
...
\[B\da \{(x,y,z)\in\MdR^3: (x,y)\in A, g_1(x,y)\le z\le g_2(x,y)\}\]
...
\[\int_B f(x,y,z) \dd{(x,y,z)} = \int_A\left(\int_{g_1(x,y)}^{g_2(x,y)} f(x,y,z) \dd{z}\right)\dd{(x,y)}\]
\end{satz}
...
$B$ sei wie in 15.2.
\begin{align*}
|B|&\da \int_B 1\dd{(x,y,z)}\\
&\left( = \int_A g_2(x,y)-g_1(x,y)\dd{(x,y)}\right)
\end{align*}
heißt \textbf{Volumen} von $B$.
\end{definition}

\begin{beispiele}
\item[(1)] Sei $B\da [a,b]\times[c,d]\times[\alpha,\beta]$, dann gilt:
\begin{align*}
\int_B f(x,y,z) \dd{(x,y,z)} &= \int_A\left(\int_\alpha^\beta f(x,y,z) \dd{z}\right)\dd{(x,y)}\\
&=\int_a^b\left(\int_c^d\left(\int_\alpha^\beta f(x,y,z) \dd{z}\right)\dd{y}\right)\dd{x}
\end{align*}
Dabei darf die Integrationsreihenfolge beliebig vertauscht werden.
\item[(2)] Sei $B\da \{(x,y,z)\in\MdR^3: x^2+y^2\le 1, 0\le z\le h\}$ für ein
$h>0$. Dann setze $A\da \{(x,y)\in\MdR^2: x^2+y^2\le 1\}, g_1=0, g_2=h$. Es gilt:
\begin{align*}
|B|&= \int_A h\dd{(x,y)}\\
&= h \int_A 1 \dd{(x,y)}\\
&= h\cdot |A| = h\pi
...
\[\int_B \alpha f(X)+\beta g(X)\dd{X}=\alpha\int_B f(X)\dd{X}+\beta\int_B g(X)\dd{X}\]
\item[(2)] Es gilt die bekannte Dreiecksungleichung:
\[\left|\int_B f(X)\dd{X}\right|\le \int_B |f(X)| \dd{X}\le |B|\cdot \max\{|f(X)|: X\in B\}\]
\item[(3)] Ist $f\le g$ auf $B$, so gilt:
\[\int_B f(X)\dd{X} \le \int_B g(X)\dd{X}\]
\end{enumerate}
\end{satz}
\end{enumerate}
...
bezeichnet werden. Mit dem \textbf{komplexen Betrag} $|z|\da \sqrt{x^2+y^2}$ gilt:
\[|z|=\|(x,y)\|\]
...
Sei $(z_n)$ eine Folge in $\MdC$, $s_n\da z_1+\cdots+z_n\ (n\in\MdN)$. $(s_n)$ heißt
...
\[\sum_{n=1}^{\infty} z_n\da \lim_{n\to\infty} s_n\]
\end{definition}
...
\[e^z\da \sum_{n=0}^\infty\frac{z^n}{n!}\]
...
\[\cos{z}\da \sum_{n=0}^\infty (-1)^n\frac{z^{2n}}{(2n)!}\]
\[\sin{z}\da \sum_{n=0}^\infty (-1)^n\frac{z^{2n+1}}{(2n+1)!}\]
\end{beispiele}
...
\[\|x\|_2\da \sqrt{\sum_{j=1}^n |x_j|^2}\]
Dann ist $(\mathbb{K}^n,\|\cdot\|_2)$ ein normierter Raum (vgl. §1).
\item Sei $X=C[a,b]$ und für $f\in X$ seien die folgenden Normen gegeben:
\begin{align*}
&\|f\|_1\da \int_a^b|f(x)|\dd{x}\\
&\|f\|_2\da \sqrt{\int_a^b|f(x)|^2\dd{x}}\\
&\|f\|_\infty\da \max\{|f(x)|:x\in[a,b]\}
\end{align*}
Leichte Übung: $(X,\|\cdot\|_1),(X,\|\cdot\|_2)$ und $(X,\|\cdot\|_\infty)$
sind NRe.
\item Sei $K\subseteq \mathbb{R}^n$ kompakt, $X\da C(K,\mathbb{R}^m)$ und sei für
$f\in X$ die Norm
\[\|f\|_\infty\da \max\{\|f(x)\|:x\in K\}\]
...
und
\[\forall x_1,\ldots,x_k\in X\colon \|x_1+\ldots+x_k\|\leq\|x_1\|+\ldots+\|x_k\|\]
\end{bemerkung}
...
\[U_\delta(x)\da \{x\in X: \|x-x_0\|<\delta\}\]
...
\item Es sei $(Y, \|\cdot\|_Y)$ ein weiterer normierter Raum, $f\colon X\to Y$,
$x_0\in A$. $f$ heißt \textbf{stetig in $x_0$} $\dequizu\forall$ Folge $(a_n)$ in
$A$ mit $a_n \stackrel{\|\cdot\|}{\to} x_0$ gilt $f(a_n) \stackrel{\|\cdot\|_Y}{\to} f(x_0)$.

$f$ heißt genau dann \textbf{stetig auf $A$}, wenn $f$ in jedem $x\in A$ stetig ist.
\end{enumerate}
\end{definition}
...
\begin{beispiel}
Sei $X=C[-1,1]$ mit $\|f\|_2\da \sqrt{\int_{-1}^1 |f(x)|^2\dd{x}}$. Definiere die
Folge $(f_n)$ wie folgt:
...
\item Sei $X=C[-1,1], \|f\|_2=\sqrt{\int_{-1}^1 |f(x)|^2 \dd{x}}$. Dann ist
...
Ist $x_0\in A$ beliebig und $(x_n)$ definiert durch $x_{n+1}\da F(x_n)\ (n\ge 0)$,
...
\begin{beweis}
\begin{enumerate}
\item["`$\Longleftarrow$"'] Es existiere ein $c\in\MdR$, sodass $y(x)=ce^{A(x)}$ für $x\in J$. Dann gilt:
\[\forall x\in J: y'(x)=c\cdot e^{A(x)}\cdot A'(x)=a(x)\cdot c\cdot e^{A(x)}=a(x)y(x)\]
\item["`$\Longrightarrow$"'] Sei $g(x)\da \frac{y(x)}{e^{A(x)}}$. Nachrechnen: $\forall x\in J:g'(x)=0$\\
...
\iff &c(x)=\int s(x)\cdot e^{-A(x)}\dd{x}
\end{align*}
Ist also $c$ eine Stammfunktion von $s\cdot e^{-A}$, so ist $y_s(x)\da c(x)\cdot e^{A(x)}$
...
\[c(x)=\int \sin(x)\cdot e^{\cos(x)}\dd{x} = -e^{\cos(x)}\]
...
\begin{align*}
L_H&\da \{y:I\to\MdR: y\text{ ist eine Lösung von (H)}\}\\
L_{IH}&\da \{y:I\to\MdR: y\text{ ist eine Lösung von (IH)}\}
\end{align*}
16.1$\implies$ $L_H=\{c\cdot e^{A}: c\in \MdR\}$. Bekannt: $L_{IH}\ne\emptyset$.
\end{definition}
...
\begin{align*}
\int_{y_0}^{y(x)}\frac 1{g(t)}\dd{t}=\int_{x_0}^x f(t)\dd{t}\tag{$*$}
\end{align*}
...
\begin{align*}
&G(y)\da \int_{y_0}^y \frac 1{g(t)}\dd{t} &&F(x)\da \int_{x_0}^x f(t)\dd{t}
\end{align*}
...
\[|F(x)|=|F(x)-F(x_0)|<\ep \quad \forall x\in U_\delta(x_0)\cap I\ad M_0\]
$M_0$ ist ein Intervall, $x_0\in M_0\subseteq I$ und $F(M_0)\subseteq K$. Sei
\[\mathfrak{M}\da \{M\subseteq I: M \text{ ist Intervall},x_0\in M,F(M)\subseteq K\}\] 
Da $M_0\in\mathfrak{M}$ ist, ist $\mathfrak{M}\ne\emptyset$. Sei
\[I_{x_0}\da \bigcup_{M\in\mathfrak{M}} M\]
...
\begin{align*}
&f(t)=\frac{u'(t)}{g(u(t))}\\
\implies &F(x)=\int_{x_0}^x f(t)\dd{t}=\int_{x_0}^x \frac{u'(t)}{g(u(t))}\dd{t}\\
&\stackrel{Subst.}{=}
\begin{cases}
s=u(t)\\
\text{ d}s= u'(t)\text{ d}t\\
t=x_0\implies s=u(x_0)=y_0
\end{cases}=\int_{y_0}^{u(x)}\frac 1{g(s)}\dd{s}=G(u(x))
\end{align*}
...
\to\ &\int{\frac{\dd{y}}{g(y)}}=\int f(x)\dd{x}+c\tag{iii}
\end{align*}
...
\to\ &\int e^y\dd{y}=\int 2x\dd{x}+c\\
\to\ & e^y=x^2+c\\
\to\ &y=\log(x^2+c)
\end{align*}
Ist z.B. $c=0$, so ist $y(x)\da \log(x^2)$ eine Lösung auf $(0,\infty)$, oder
...
Sei $I \subseteq \MdR$ ein Intervall, $D \da  I \times \MdR^n, x_o\in I, y_0\in \MdR^n$ und $f:D\to\MdR^n$ sei stetig. Für $y\in C(I,\MdR^n)$ gilt:
\[\text{$y$ ist eine Lösung des AwP (ii) auf $I$} \iff  \forall x\in I:y(x) = y_0 + \int_{x_0}^x f(t, y(t)) \dd{t} \]
In diesem Fall ist $y \in C^1(I, \MdR^n)$.
\end{satz}

\begin{beweis}
\begin{enumerate}
\item["`$\Longrightarrow$"'] Es gilt: $y'(x) = f(x, y(x)) \forall x\in I$; da $y$ und $f$ stetig sind, folgt: $y' \in C(I,\MdR )$. Weiter:
\[\int_{x_0}^x f(t,y(t)) \dd{t} = \int_{x_0}^x y'(t)\dd{t} = y(x) - y(x_0) = y(x) - y_0 \quad \forall x\in I.\]
Bringt man $y_0$ auf die linke Seite, ergibt sich die Behauptung.
\item["`$\Longrightarrow$"'] Es gelte für alle $x\in I$:
...
\[ y'(x) = \frac{\text{d}}{\text{d}x} \int_{x_0}^x f(t, y(t))\dd{t} = f(x, y(x)) \quad \forall x \in I. \]
...
Sei $I = [a,b] \subseteq \MdR, x_o \in I$ und für $y\in C(I, \MdR^n)$ sei $\|y\|_\infty \da  \max \{\|y(x)\| : x\in I \}$ wie in §17 (also ist $(C(I, \MdR^n), \|\cdot \|_\infty )$ ein Banachraum).

Sei $\alpha > 0$ mit $\varphi_\alpha (x) \da  e^{-\alpha |x-x_0|}\ (x \in I)$.

Für $y \in C(I, \MdR^n)$ sei $\|y\|_\alpha \da  \max \{\varphi_\alpha(x)\cdot \|y(x)\| : x\in I \}$.

Dann:
\begin{enumerate}
\item $\|\cdot\|_\alpha$ ist eine Norm auf $C(I,\MdR^n)$.
\item Seien $c_1 \da  \min \{ \varphi_\alpha(x) : x \in I \},\text{ } c_2 \da  \max \{ \varphi_\alpha(x) : x \in I \}$. Es gilt: 
...
\[ g_{k+1}(x) \da  y_0 + \int_{x_0}^x f(t, g_k(t)) \dd{t} \quad (x \in I, k \geq 0), \]
...
Es sei $\alpha \da  2L$; $\varphi_\alpha$ und $\|\cdot\|_\alpha$ seien wie in 21.2, $X \da  C(I, \MdR^n)$. Definiere $F: X \to X$ durch 
\[(F(y))(x) \da  y_0 + \int_{x_0}^x f(t, y(t))\dd{t}\]
Für $y \in X$ gilt dann:
\begin{align*}
F(y) = y &\iff y(x) = y_0 + \int_{x_0}^x f(t, y(t))\dd{t} \quad \forall x \in I \\
...
\begin{align*}
\|(F(y))(x) - (F(z))(x)\|&= \left\|\int_{x_0}^x (f(t, y(t)) - f(t, z(t)))\dd{t} \right\|\\
&\stackrel{12.4}\le \left| \int_{x_0}^x \|f(t, y(t)) - f(t,z(t))\| \dd{t} \right|\\
&\le \left| \int_{x_0}^x L \|y(t)-z(t)\| \dd{t} \right|\\
&= L \left| \int_{x_0}^x \|y(t)-z(t)\| \dd{t} \right| \displaybreak[0]\\
&= L \left| \int_{x_0}^x \|y(t)-z(t)\| \varphi_\alpha (t) \cdot \frac1{\varphi_\alpha(t)}\dd{t} \right| \displaybreak[0]\\
&\le L \left| \int_{x_0}^x \|y-z\|_\alpha \cdot \frac1{\varphi_\alpha(t)}\dd{t} \right| \displaybreak[0]\\
&\le L \|y-z\|_\alpha \left| \int_{x_0}^x \frac1{\varphi_\alpha(t) }\dd{t}\right|\\
...
\begin{align*}
\|(F(y))(x) -(F(z))(x)\| &\stackrel{\text{wie oben}}\leq L \left| \int_{x_0}^x \|y(t)-z(t)\| \dd{t} \right| \\
&\leq L \left| \int_{x_0}^x \|y-z\|_\infty \dd{t} \right| \\
&\leq L \|y-z\|_\infty \left| \int_{x_0}^x 1 \dd{t} \right|\\
&= L \|y-z\|_\infty |x-x_0|\\
...
Sei $a > 0$ und $I \da  [-a, a]; f(x,y) = 2x(1+y).$ Dann gilt $\forall x \in I, \forall y, \bar y \in \mathbb{R}:$
\begin{align*}
...
Setze nun $g_0(x) \da 0$ und $(g_k)$  sei definiert wie in 21.3.
...
\[D \da  \{(x,y)\in\MdR^{n+1} : x \in I, \|y-y_0\| \leq s\}\] 
und $f \in C(D,\MdR^n)$. Weiter sei 
\[M \da  \max\{\|f(x,y)\| : (x,y) \in D \} > 0\] 
und $f$ genüge auf $D$ einer Lipschitz-Bedingung bezüglich $y$. 
Außerdem sei 
\[J \da  I \cap [x_0 - \frac{s}{M}, x_0 + \frac{s}{M}]\]
Dann hat das AwP
\begin{align*} \begin{cases}
y' = f(x,y)\\
y(x_0) = y_0
...
\begin{beweis}
Es sei
\[\mathfrak{M} \da  \{ (y,I_y) : I_y \subseteq \MdR \text{ Intervall, } 
...
$I_{y_1} \cap I_{y_2}$.
\begin{align*}
I \da  \bigcup_{(y, I_y) \in \mathfrak{M}} I_y
\end{align*} 
ist ein Intervall. Definiere $y: I \to \MdR^n$ wie folgt: zu $x \in I$ existiert 
ein $(y_1, I_{y_1}) \in \mathfrak{M}$, sodass für $x \in I_{y_1}$ gilt: $y(x) \da  y_1(x)$.

\textbf{Übung:} $y: I \to \MdR^n$ leistet das Gewünschte.
\end{beweis}

Wie stellt man eine lokale LB fest?
...
\begin{beweis}
O.B.d.A.: $n=1$, also $D\subseteq \MdR^2, f = f(x, y)$. Sei $a\in D$. $D\ \text{offen}\implies
\exists\delta>0\colon \underbrace{\overline{U_\delta(a)}}_{\ad U}\subseteq D$. $U$ ist eine kompakte
Umgebung von $a$. $f_y\ \text{stetig}\implies\exists L_a\da\max\{|f_y(x, y)|:(x, y)\in U\}$.
Seien $(x, y), (x, z) \in U$. $f(x, y) - f(x, z) \stackrel{\text{MWS}}{=} f_y(x, \xi)(y - z)$
mit $\xi$ zwischen $y$ und $z$. Dann: $(x, \xi)\in U\implies|f_y(x, \xi)|\leq L_a\implies
|f(x, y)-f(x, z)|\leq L_a|y-z|$.
\end{beweis}

\chapter{Lineare Systeme}
In diesem Paragraphen sei $I\subseteq\mdr$ ein Intervall, $x_0\in I, y_0\in\mdr^n,
D\da I\times\mdr^n,b:I\to\mdr^n$ stetig und $A:I\to\mdr^{n\times n}$ ebenfalls stetig 
...
Es ist $f(x,y)=A(x)y+b(x)$. Sei $L\da \max\{\|A(x)\|:x\in I\}$. Für alle $(x,y),(x,\overline y)\in D$ gilt:
\begin{align*}
...
Sei $\mathfrak{M}\da \{K\subseteq I: K\text{ ist kompaktes Intervall, } x_0\in K\}$.
...
\item Folgt aus (1).
\item Sei $\xi \in J,\eta\da \hat y(\xi)$. Dann ist $\hat y$ eine Lösung auf $J$ des AwPs
\begin{align*}
\tag{+}
\begin{cases}
y'=A(x)+b(x)\\
...
\textbf{Fall $x>\xi$}:\\
Sei $K\da [\xi,x]$. Da $\hat y$ und $y$ Lösungen des AwPs (+) auf $[\xi,x]$ sind folgt aus
(1), dass $y=\hat y$ auf $K$, also:
\[\hat y(x)=y(x)\]
\textbf{Fall $x<\xi$}:\\
Sei $K\da [x,\xi]$. Da $\hat y$ und $y$ Lösungen des AwPs (+) auf $[x,\xi]$ sind folgt aus
(1), dass $y=\hat y$ auf $K$, also:
\[\hat y(x)=y(x)\]
\item Leichte Übung!	
\end{beweise}
...
\begin{definition}
Setze $\mathbb{L} \da  \{ y: I\to \MdR^n : y $ ist eine Lösung von (H) auf $I$ $\}$\\
...
\item $y^{(1)}, ..., y^{(n)}$ ist ein Fundamentalsystem von (H)\\ $\iff \forall x \in I : Y(x) \text{ invertierbar }$ \\ $\iff \exists \xi \in I: Y(\xi )$ ist invertierbar \\ $\iff \forall x \in I: W(x) \neq 0$ \\ $\iff \exists \xi \in I: W(\xi ) \neq 0$.
\end{enumerate}
\end{satz}
...
$y^{(1)}(x) \da  \begin{pmatrix} \cos(x) \\ \sin(x) \end{pmatrix}$ ist eine L"osung von ($*$) auf $\MdR$.
$y^{(2)}(x) \da  \begin{pmatrix} -\sin(x) \\ \cos(x) \end{pmatrix}$ ist eine weitere L"osung von ($*$) auf $\MdR$.
...
\begin{align*}
y_s(x) \da  Y(x) \int Y(x)^{-1} b(x) \dd{x} \quad (x \in I).
\end{align*}
Dann ist $y_s$ eine spezielle L"osung von (S) auf $I$.
\begin{align*}
W_k(x) \da  \det \left( y^{(1)}(x), ..., y^{(k-1)}(x), b(x), y^{(k+1)}(x), ..., y^{(n)}(x) \right)\quad (k=1,...,n)
\end{align*}
Dann gilt: $y_s(x) = \sum_{k=1}^n \left( \int \frac{W_k(x)}{W(x)} \dd{x}\right) y^{(k)}(x)$.
\end{satz}
...
&y_s(x) \da  \left( \int 1 \dd{x} \right) y^{(2)}(x) = xy^{(2)}(x) = \begin{pmatrix} -x \sin(x) \\ x \cos(x) \end{pmatrix} \text{ ist eine spezielle L"osung von (+).}
\end{align*}
...
\begin{enumerate}
\item Es sei stets $p(\lambda) \da  \det(A - \lambda I)$ das \textbf{charakteristische Polynom}
...
\[M \da  \{\lambda_1, \ldots, \lambda_m\} \cup \{\lambda_j : m + 1 \le j \le r, \Im(\lambda_j) > 0 \}\]

\item Zu $\lambda_j \in M$ bestimme eine Basis von $V_j \da  $ Kern$((A-\lambda_j I)^{k_j})$ 
...
\[y(x) \da  e^{\lambda_j x} (v+\frac{x}{1!} (A-\lambda_j I)v + \frac{x^2}{2!} (A-\lambda_j I)^2 v + \cdots + \frac{x^{k_j - 1}}{ (k_j - 1)! } (A - \lambda_j I)^{k_j - 1} v )\]
...
\end{align*}
mit 
\[A\da \begin{pmatrix} 1 & -4 \\ 1 & 1  \end{pmatrix}\]
Es gilt:
...
k_1 &= 1 &k_2&=1\\
\end{align*}
Also ist $M \da  \{\lambda_1\}$. Aus $\kernn(A-\lambda_1 I) = \left[ \begin{pmatrix} 2i \\ 1 \end{pmatrix} \right]$
folgt:
\begin{align*}
...
Sei also:
\begin{align*}
y^{(1)}(x)&\da e^x \begin{pmatrix} -2\sin(2x) \\ \cos(2x) \end{pmatrix}&
y^{(2)}(x)&\da e^x \begin{pmatrix} 2\cos(2x) \\ \sin(2x) \end{pmatrix}
\end{align*}
...
\end{align*}
mit
\[A\da \begin{pmatrix} 0 & 1 & -1 \\ -2 & 3 & -1 \\ -1 & 1 & 1 \end{pmatrix}\]
Es gilt:
...
\begin{align*}
y^{(2)}(x) \da  e^x \begin{pmatrix} 1\\1\\0 \end{pmatrix} && 
y^{(3)}(x) \da  e^x\left( \begin{pmatrix} 0\\0\\1\end{pmatrix} + x(A-I) \begin{pmatrix} 0\\0\\1\end{pmatrix} \right) = e^x \begin{pmatrix} -x \\ -x \\ 1 \end{pmatrix}
\end{align*}
...
\end{align*}
Mit
\[A\da \begin{pmatrix} 1 & 0 \\ 0 & -1 \end{pmatrix}\]
...
Sei nun $Y(x) \da  \begin{pmatrix} e^x & 0 \\ 0 & e^{-x} \end{pmatrix}$ \\ 
Dann ist
\begin{align*}
y_s(x) &= Y(x) \int Y(x)^{-1} \begin{pmatrix} e^x \\ e^x \end{pmatrix} \dd{x}\\ 
&= Y(x) \int \begin{pmatrix} e^{-x} & 0 \\ 0 & e^x \end{pmatrix} \begin{pmatrix} e^x \\ e^x \end{pmatrix} \dd{x}\\ 
&= Y(x) \int \begin{pmatrix} 1 \\ e^{2x} \end{pmatrix}\dd{x}\\ 
...
stetig. Für $y\in C^n(I,\mdr)$ setze $Ly\da y^{(n)}+a_{n-1}y^{(n-1)}+\cdots+a_0y$.
Die Dgl
\begin{align*}
\tag D Ly=b(x)
\end{align*}
...
Setze $b_0(x)\da (0,\ldots,0,b(x))^\mathsf{T} (\in\mdr^n)$ und
\begin{align*}
A(x)\da 
\begin{pmatrix}
0&1&0&\cdots&0\\
...
\begin{satz}[Lösungen]
\begin{enumerate}
\item Ist $y:I\to\mdr$ eine Lösung von (D) auf $I$, so ist $z\da (y,y',\ldots,y^{(n-1)})$
eine Lösung von (S) auf $I$.
\item Ist $z\da (z_1,\ldots,z_n)$ eine Lösung von (S) auf $I$, so ist $y\da z_1$ eine Lösung von (D) auf $I$.
\end{enumerate}
\end{satz}
...
Die folgenden Sätze 24.2 und 24.3 folgen aus 24.1 und den Sätzen aus §22.
...
\[y_s(x)\da \sum_{k=1}^n\left(y_k(x)\cdot \int \frac{W_k(x)}{W(x)}\dd{x}\right)\]
...
\end{align*}
Sei $y_1$ eine Lsg von (H) mit $y_1\ne 0\ \forall x\in I$. Sei $z\not\equiv0$ eine Lsg von
\[z'=-\left(a_1(x)+\frac{2y_1'(x)}{y_1(x)}\right)z, \text{ \quad so ist}\]
\[y_2(x)\da y_1(x)\cdot\int z(x)\dd{x}\]
eine weitere Lsg von (H) und $y_1,y_2$ ist ein FS von (H).
\end{beispiel}

\begin{beweis}
Nachrechnen: $y_2$ ist Lsg von (H).\\
Aus $y_2'=y_1'\cdot\int z(x)\dd{x}+y_1z(x)$ folgt:
\begin{align*}
W(x)&=\det\begin{pmatrix}
y_1(x)&y_1(x)\cdot\int z(x)\dd{x}\\
y_1'(x)&y_1'(x)\int z(x)\dd{x}+y_1z(x)
\end{pmatrix}\\
&= y_1y_1'\cdot\int z(x)\dd{x}+y_1^2z(x)-y_1y_1'\cdot\int z(x)\dd{x}\\ 
&= y_1^2z(x)
\end{align*}
...
\end{align*}
Es ist $\int\frac2{x(x^2-1)}\dd{x}=\log(1-\frac1{x^2})$, daraus ergibt sich die allgemeine
Lösung von ($**$):
\[z(x)=ce^{\log(1-\frac1{x^2})}=c(1-\frac1{x^2})\quad (c\in\mdr)\]
Sei also:
\[y_2(x)\da y_1(x)\cdot\int 1-\frac1{x^2}\dd{x}=1+x^2\]
...
\[y_s(x)=y_1(x)\cdot\int(-1-x^2)\dd{x}+y_2(x)\cdot\int x\dd{x}=\frac16 x^4-\frac12 x^2\]
...
\[p(\lambda)\da \lambda^n+a_{n-1}\lambda^{n-1}+\cdots+a_1\lambda+a_0\]
...
Sei also:
\begin{align*}
\lambda_1&\da 2&\lambda_2&\da i&\lambda_3&\da -i\\
k_1&\da 1&k_2&\da 1&k_3&\da 1
\end{align*}
Dann ist $M\da \{2,i\}$ und ein FS von ($*$) lautet: $e^{2x},\cos(x),\sin(x)$. Das bedeutet für
