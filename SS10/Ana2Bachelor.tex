...
$$\int_a^bf(t)dt \da  \left(\int_a^bf_1(t)dt, \int_a^bf_2(t)dt,\ldots, \int_a^bf_n(t)dt\right) \quad (\in\MdR^n)$$
Dann: \begin{liste}
...
\item $y\da \int_a^bf(t)dt$. O.B.d.A: $y\ne 0$. $x\da  \frac{1}{\|y\|} y \folgt \|x\|=1, y=\|y\|x$. $\|y\|^2 = y\cdot y = \|y\|(x\cdot y) = \|y\|\left(x\cdot \int_a^bf(t)dt \right) = \|y\|\int_a^b\left(x\cdot f(t)\right) dt \le \|y\|\int_a^b\underbrace{|x \cdot f(t)|}_{\le\|x\|\|f(t)\| = \|f(t)\|} dt \le \|y\| \int_a^b\|f(t)\|dt$
\end{liste}
\end{beweis}
...
$$\ds\int_{\tau_{j-1}}^{\tau_j}\gamma'(\tau)\text{d}\tau=\left(\cdots, \ds\int_{\tau_{j-1}}^{\tau_j}\eta_k'(\tau)\text{d}\tau,\cdots\right)\gleichnach{AI}\left(\cdots, \eta_k(\tau_j)-\eta_k(\tau_{j-1}),\cdots\right)=\gamma(\tau_j)-\gamma(\tau_{j-1})$$
...
\begin{beispiele}
\item $x_0, y_0\in\MdR^n, \gamma(t)\da x_0+t(y_0-x_0)\ (t\in [0,1])$. $\gamma'(t)=y_0-x_0\folgt L(\gamma)=\ds\int_0^1\|y_0-x_0\|\text{d}t=\|y_0-x_0\|$.
\item Sei $f:[a,b]\to\MdR$ stetig differenzierbar und $\gamma(t)\da (t, f(t)), t\in[a,b]$. $\gamma$ ist ein Weg im $\MdR^2$. $\gamma$ ist rektifizierbar $\equizu f \in \text{BV}[a,b]$. $\Gamma_\gamma=$ Graph von $f$. Jetzt sei $f\in C^1[a,b] \folgtnach{12.5} L(\gamma)=\ds\int_a^b\|\gamma'(t)\|\text{d}t=\ds\int_a^b (1+f'(t)^2)^{\frac{1}{2}}\text{d}t$.
...
\begin{enumerate}
\item Für $j\in \{1,\ldots,n\}: \int_\gamma g(x) \text{ d}x_j\da \int_a^b g(\gamma(t))\cdot\eta_j'(t)\text{ d}t$
\item \begin{align*}
\int_\gamma f(x)\cdot\text{d}x &\da  \int_\gamma f_1(x)\text{ d}x_1+\cdots+f_n(x)\text{ d}x_n\\
&\da \sum_{j=1}^n \int_\gamma f_j(x)\text{ d}x_j
\end{align*}
...
$$\int_\gamma g(x) ds \da  \int_a^b g(\gamma(t))\|\gamma'(t)\|dt$$
...
$\text{AH}(\gamma_1,\ldots,\gamma_m) \da  \{\gamma:\gamma$ ist ein rektifizierbarer Weg im $\MdR^n$ mit: $\Gamma_\gamma=\Gamma$, $L(\gamma)=L(\gamma_1)+\cdots+L(\gamma_m)$ und $\int_\gamma f(x)\cdot dx = \int_{\gamma_1}f(x)\cdot dx+ \cdots + \int_{\gamma_m}f(x)\cdot dx$ für \emph{jedes} stetige $f:\Gamma\to\MdR^n\}.$
...
$\gamma(a) = \hat{\gamma}(\alpha) \wedge \gamma(b) = \hat{\gamma}(\beta)$, so gilt $\int_\gamma f(x) \cdot dx = \int_{\hat{\gamma}} f(x) \cdot dx$.
\end{satz}
...
\begin{eqnarray*}
\varphi(x)&\da &\int_{\gamma_x}f(z)\cdot\text{d}z\ (x\in G)\\
...
Sei $x\in G$ (fest), $h(t)\da t\cdot f_1(tx)\ (t\in [0,1])$. $h$ ist stetig differenzierbar und $h'(t)=f_1(tx)+tf_1'(tx)\cdot x\folgt \varphi_{x_1}(x)=\ds\int_0^1 h'(t)\text{d}t\gleichnach{A1}h(1)-h(0)=f_1(x)$.
\end{beweis}
...
\begin{align*}
&\|f\|_1\da \int_a^b|f(x)|\text{ d}x\\
&\|f\|_2\da \sqrt{\int_a^b|f(x)|^2\text{ d}x}\\
...
\begin{beispiel}
Sei $X=C[-1,1]$ mit $\|f\|_2\da \sqrt{\int_{-1}^1 |f(x)|^2\text{ d}x}$. Definiere die
Folge $(f_n)$ wie folgt:
...
\begin{align*}
&G(y)\da \int_{y_0}^y \frac 1{g(t)}\text{ d}t &&F(x)\da \int_{x_0}^x f(t)\text{ d}t
\end{align*}
...
\[\text{$y$ ist eine Lösung des AwP (ii) auf $I$} \iff  \forall x\in I:y(x) = y_0 + \int_{x_0}^x f(t, y(t)) \text{d}t \]
...
\[ g_{k+1}(x) \da  y_0 + \int_{x_0}^x f(t, g_k(t)) \text{d}t \quad (x \in I, k \geq 0), \]
...
\[(F(y))(x) \da  y_0 + \int_{x_0}^x f(t, y(t))\text{d}t\]
...
\begin{align*}
y_s(x) \da  Y(x) \int Y(x)^{-1} b(x) \text{d}x \quad (x \in I).
\end{align*}
...
&y_s(x) \da  \left( \int 1 \text{d}x \right) y^{(2)}(x) = xy^{(2)}(x) = \begin{pmatrix} -x \sin(x) \\ x \cos(x) \end{pmatrix} \text{ ist eine spezielle L"osung von (+).}
\end{align*}
...
\[y_s(x)\da \sum_{k=1}^n\left(y_k(x)\cdot \int \frac{W_k(x)}{W(x)}\text{ d}x\right)\]
...
\[y_2(x)\da y_1(x)\cdot\int z(x)\text{ d}x\]
...
Sei also:
\[y_2(x)\da y_1(x)\cdot\int 1-\frac1{x^2}\text{ d}x=1+x^2\]
