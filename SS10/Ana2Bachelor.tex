...
\[A\da \{(x,y)\in\MdR^2\mid x\in[a,b],h_1(x)\le y\le h_2(x)\}\]
\[\left(A\da \{(x,y)\in\MdR^2\mid y\in[a,b],h_1(y)\le x\le h_2(y)\}\right)\]
...
\end{align*}
\item[(3)] Sei $r>0$ und $A\da \{(x,y)\in\MdR^2\mid x^2+y^2\le r^2\}$. Dann ist
...
&=\pi r^2
\end{align*}
\item[(4)] Sei $A\da \{(x,y)\in\MdR^2\mid x\in[0,1],x\le y\le \sqrt x\}$ und $f(x,y)=xy$.
...
\[B\da \{(x,y,z)\in\MdR^3\mid (x,y)\in A, g_1(x,y)\le z\le g_2(x,y)\}\]
...
\item[(2)] Sei $B\da \{(x,y,z)\in\MdR^3\mid x^2+y^2\le 1, 0\le z\le h\}$ für ein
$h>0$. Dann setze $A\da \{(x,y)\in\MdR^2\mid x^2+y^2\le 1\}, g_1=0, g_2=h$. Es gilt:
\begin{align*}
|B|&= \int_A h\dd{(x,y)}\\
&= h \int_A 1 \dd{(x,y)}\\
&= h\cdot |A| = h\pi
...
\[U_\delta(x)\da \{x\in X\mid \|x-x_0\|<\delta\}\]
...
\begin{align*}
L_H&\da \{y:I\to\MdR\mid y\text{ ist eine Lösung von (H)}\}\\
L_{IH}&\da \{y:I\to\MdR\mid y\text{ ist eine Lösung von (IH)}\}
\end{align*}
...
\[\mathfrak{M}\da \{M\subseteq I\mid M \text{ ist Intervall},x_0\in M,F(M)\subseteq K\}\] 
...
Sei $\mathfrak{M}\da \{K\subseteq I\mid K\text{ ist kompaktes Intervall, } x_0\in K\}$.
...
\[M\da \{\lambda_1,\ldots,\lambda_m\}\cup\{\lambda_j\mid m+1\le j\le r,\Im(\lambda_j)>0\}\]
