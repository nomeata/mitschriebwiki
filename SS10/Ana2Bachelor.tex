...
\begin{definition}
 ...
 \item  $x_0\in\MdR^n$ heißt ein \begriff{Berührungspunkt} (BP) von $A$ $:\equizu$ $\forall\delta>0: U_\delta(x_0) \cap A \ne \emptyset$. $\bar{A}:=\{x\in\MdR^n: x \text{ ist ein Berührungspunkt von } A\}$ heißt der \begriff{Abschluss} von $A$. Klar: $A\subseteq\bar{A}$. Zur Übung: $\bar{A} = A \cup \H(A)$.
...
\item $\MdR^n$ ist abgeschlossen, $\emptyset$ ist abgeschlossen; \\ 
  $\bar{A}=\bar{U_\delta(x_0)} = \{x\in\MdR^n: \|x-x_0\| \le \delta\}$ (\textbf{abgeschlossene Kugel} um $x_0$ mit Radius $\delta$)
...
\begin{beispiel}
$(n=2)$: $a^{(k)} = (\frac{1}{k}, 1+\frac{1}{k^n})$ (Erinnerung: $\frac{1}{n}$ konvergiert gegen 0); $a := (0,1)$; $\|a^{(k)} - a \| = \|(\frac{1}{k} , \frac{1}{k^2})\| = (\frac{1}{k^2} + \frac{1}{k^4})^\frac{1}{2} \to 0 \folgt a^{(k)} \to (0,1)$
\end{beispiel}
%satz 2.1
...
\item $f$ ist auf $D$ \textbf{partiell differenzierbar} nach $x_j$ oder $f_{x_j}$ ist auf $D$ vorhanden :\equizu $f$ ist in jedem $x\in D$ partiell differenzierbar nach $x_j$. In diesem Fall wird durch $x\mapsto f_{x_j}(x)$ eine Funktion $f_{x_j}: D\to \MdR$ definiert die partielle Ableitung von $f$ auf $D$ nach $x_j$.
...
\begin{definition}
Sei $m\in\MdN$. $f$ heißt auf $D$ \textbf{$m$-mal steig partiell differenzierbar} :\equizu alle partiellen Ableitungen  von $f$ der Ordnung $\le m$ sind auf $D$ vorhanden und auf $D$ stetig. In diesem Fall schreibt man: $f\in C^m(D,\MdR)$
...
\Phi:\MdR^n \implies \MdR^m. \folgtnach{(i)} \Phi \text{ ist injektiv, also }
...
$f:D\implies \MdR^n$ sei db auf $D$, es sei $f(D)$ offen, $f$ injektiv auf $D$ und $f^{-1}:f(D)\implies \MdR^n$ sei db auf $f(D)$. Dann:
\item[(1)] $m = n$
...
\textbf{Beachte:}
\begin{itemize}
\item[(1)] Ist $D$ offen und $f:D\implies \MdR^m$ db, so muss i. A. $f(D)$ nicht offen sein. Z.B.: $f(x) = \sin x, D = \MdR, f(D) = [-1,1]$
\item[(2)] Ist $D$ offen, $f:D\implies \MdR^m$ db und injektiv, so muss i.A. $f^{-1}$ \underline{nicht} db sein. Z.B.: $f(x) = x^3, D = \MdR, f^{-1}$ ist in 0 \underline{nicht} db.
\end{itemize}
...
\paragraph{Summe von Wegen:}Gegeben: $a_0, a_1,\ldots a_l \in \MdR$, $a_0<a_1<a_2<\ldots<a_c$ und Wege $\gamma_k:[a_{k-1},a_k] \to \MdR^n$ $(k=1,\ldots,l)$ mit : $\gamma_k(a_k) = \gamma_{k+1}(a_k)$ $(k=1,\ldots,l-1)$.
...
\item $\gamma$ hei"st \textbf{stückweise stetig differenzierbar} $:\equizu\exists z=\{t_0,\ldots,t_m\}\in\Z$ mit: $\gamma_{|_{[t_k-1,t_k]}}$ sind stetig differenzierbar $(k=1,\ldots,m)\equizu\exists$ stetig differenzierbare Wege $\gamma_1,\ldots,\gamma_l: \gamma=\gamma_1\oplus\cdots\oplus\gamma_l$.
...
Außerdem sei 
\[y := I \cap [x_0 - \frac{s}{M}, x_0 + \frac{s}{M}]\]
Dann hat das AwP
\begin{align*} \begin{cases}
y' = f(x,y)\\
y(x_0) = y_0
...
Es sei $D \subseteq \MdR^{n+1}$ \textbf{offen}, $(x_0, y_0) \in D, f\in C(D, \MdR^{n+1})$ und $f$ genüge auf $D$ einer \textbf{lokalen} LB bezüglich $y$.
