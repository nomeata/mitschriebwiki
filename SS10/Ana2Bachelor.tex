...
\begin{definition}
\index{unendlich!Reihe}\index{Reihe!unendliche}
...
\\
\begin{beispiele}
\index{geometrisch!Reihe}\index{Reihe!geometrische}
\index{Exponentialfunktion}
\index{Cosinus}\index{Kosinus}
\index{Sinus}
...
\begin{beispiele}
\index{euklidisch!Norm}\index{Norm!euklidische}
...
\index{gleichmäßig!Konvergenz}\index{Konvergenz!gleichmäßige}
...
\begin{definition}
\index{Banachraum}
\index{vollständig!Raum}\index{Raum!vollständiger}
...
\begin{definition}
\index{gewöhnlich!Differentialgleichung}\index{Differentialgleichung!gewöhnliche}
...
\begin{definition}
\index{explizit!Differentialgleichung}\index{Differentialgleichung!explizite}
...
\index{eindeutig!Lösung}\index{Lösung!eindeutige}
...
\begin{definition}
\index{nicht fortsetzbar}\index{fortsetzbar!nicht}
...
\begin{definition}
\index{L"osungssystem}\index{L"osungsmatrix}\index{Wronskideterminante}\index{Fundamentalmatrix}\index{Fundamentalsystem}
\begin{enumerate}
...
\begin{enumerate}
\item Es sei stets $p(\lambda) := \det(A - \lambda E)$ das charakteristische Polynom von $A$ ($E$ = Einheitsmatrix). Sei $\lambda_0 \in \MdC$ ein Eigenwert (EW) von $A$, dann $p(\lambda_0) = 0$. Die Koeffizienten von $p$ sind reell $\folgt p(\overline{\lambda_0}) = 0 \folgt \overline{\lambda_0}$ ist ein Eigenwert von $A$.
\item Für $\lambda_0 \in \mdc: $ Kern $(A-\lambda_0 E) \subseteq $ Kern$((A-\lambda_0 E)^2) \subseteq $ Kern $((A-\lambda_0 E)^3) \subseteq \ldots$
\end{enumerate} 

\textbf{Kochrezept}:
\begin{enumerate}
\item Bestimme die \textbf{verschiedenen} Eigenwerte $\lambda_1, \ldots, \lambda_r \, (r \le n)$ von $A$ und deren Vielfachheiten $k_1, \ldots, k_r$, also: $p(\lambda) = (-1)^n (\lambda - \lambda_1)^{k_1} (\lambda - \lambda_2)^{k_2} \cdots (\lambda - \lambda_r)^{k_r}$. 
Ordne diese Eigenwerte wie folgt an: $\lambda_1, \ldots, \lambda_m \in \mdr, \lambda_{m+1}, \ldots, \lambda_r \in \mdc \setminus \mdr$.
\\ Aus der Liste $\lambda_{m+1}, \ldots, \lambda_r$ entferne jedes $\lambda_j$ mit $\Im(\lambda_j) < 0$. Es bleibt $M := \{\lambda_1, \ldots, \lambda_m\} \cup \{\lambda_j : m + 1 \le j \le r, \Im(\lambda_j) > 0 \}$

\item Zu $\lambda_j \in M$ bestimme eine Basis von $V_j := $Kern$((A-\lambda_j E)^{k_r}$ wie folgt: Bestimme eine Basis von Kern$(A-\lambda_j E)$, ergänze diese Basis zu einer Basis von Kern$((A-\lambda_j E)^2)$, usw.

\item Sei $\lambda_j \in M$ und $v$ ein Basisvektor von $V_j$. \\
$y(x) := e^{\lambda_j x} (v+\frac{x}{1!} (A-\lambda_j E)v + \frac{x^2}{2!} (A-\lambda_j E)^2 v + \cdots + \frac{x^{k_{j-1}}}{ (k_{j-1})! } (A - \lambda_j E)^{k_{j-1}} v )$ \\
\textbf{Fall 1}: $\lambda_j \in \mdr$. Dann ist $y(x) \in \mdr^n \; \forall \, x \in \mdr$ und \textbf{y ist eine Lösung von (H)}. \\
\textbf{Fall 2}: $\lambda_j \in \mdc \setminus \mdr$. Zerlege $y$ komponentenweise in Real- und Imaginärteil: $y(x) := \underbrace{y^{(1)}(x)}_{\in \mdr^n} + \,i \underbrace{y^{(2)}(x)}_{\in \mdr^n}$.
\textbf{Dann sind $y^{(1)}, y^{(2)}$ linear unabhängige Lösungen von H}.
...
$ [ \; \ldots \; ] $ bezeichne die lineare Hülle.

\textbf{Beispiele}:

\begin{enumerate}
\item \begin{align*}
\tag{$\ast $} y' = \underbrace{\begin{pmatrix} 1 & -4 \\ 1 & 1  \end{pmatrix}}_{=: A} y
\end{align*}
$\det(A-\lambda E) = (\lambda - (\lambda + 2i))(\lambda-(1-2i)), \lambda_1 = \lambda + 2i, \lambda_2 = 1-2i, k_1 = 1 = k_2, M = \{\lambda_1\}$ \\
Kern$(A-\lambda_1 E) = \left[ \begin{pmatrix} 2i \\ 1 \end{pmatrix} \right]$; $y(x) = e^{(1+2i)x} \begin{pmatrix} 2i \\ 1 \end{pmatrix} = e^x (\cos(2x) + i \sin(2x)) \begin{pmatrix} 2i \\ 1 \end{pmatrix} = \underbrace{e^x \begin{pmatrix} -2\sin(2x) \\ \cos(2x) \end{pmatrix}}_{=: y^{(1)}(x)} +\, i \underbrace{e^x \begin{pmatrix} 2\cos(2x) \\ \sin(2x) \end{pmatrix}}_{=: y^{(2)}(x)}$ \\
Dann ist $y^{(1)}, y^{(2)}$ ein Fundamentalsystem von ($\ast$).

\item \begin{align*}
\tag{$\ast $} y' = \underbrace{\begin{pmatrix} 0 & 1 & -1 \\ -2 & 3 & -1 \\ -1 & 1 & 1 \end{pmatrix}}_{=: A} y
\end{align*}
$\det(A-\lambda E) = -(\lambda - 2)(\lambda - 1)^2, \lambda_1 = 2, k_1=1, \lambda_2=1,k_2=2, M = \{\lambda_1, \lambda_2\}$ \\
\boldmath$\lambda_1 = 2$\unboldmath: Kern$(A-2E) = \left[ \begin{pmatrix} 0 \\ 1\\1 \end{pmatrix} \right] \quad y^{(1)}(x) := e^{2x}\begin{pmatrix} 0 \\ 1\\1 \end{pmatrix}$ \\
\boldmath$\lambda_2 = 1$\unboldmath: Kern$(A-E) = \left[ \begin{pmatrix} 0 \\ 1\\1 \end{pmatrix} \right]\subseteq \left[ \begin{pmatrix} 1\\1\\0 \end{pmatrix}, \begin{pmatrix} 0\\0\\1\end{pmatrix} \right] = $\; Kern$((A -E)^2)$ \\
$y^{(2)}(x) := e^x \begin{pmatrix} 1\\1\\0 \end{pmatrix}, y^{(3)}(x) := e^x\left( \begin{pmatrix} 0\\0\\1\end{pmatrix} + x\,(A-E)\! \begin{pmatrix} 0\\0\\1\end{pmatrix} \right) = e^x \begin{pmatrix} -x \\ -x \\ 1 \end{pmatrix}$ \\
$y^{(1)}, y^{(3)}, y^{(3)}$ ist ein Fundamentalsystem von ($\ast$).

\item Sei $A$ wie in Beispiel (2). Löse das AWP \[\begin{cases}
y'=Ay\\
y(0) = \begin{pmatrix} 1 \\ 0 \\ 1 \end{pmatrix}
\end{cases}\]
Allg. Lösung von $y' = Ay:$ Seien $c_1, c_2, c_3 \in \mdr. y(x) = c_1  e^{2x}\begin{pmatrix} 0 \\ 1\\1 \end{pmatrix} + c_2  e^x \begin{pmatrix} 1\\1\\0 \end{pmatrix} + c_3 e^x \begin{pmatrix} -x \\ -x \\ 1 \end{pmatrix}$ \\
$\begin{pmatrix} 1 \\ 0 \\ 1 \end{pmatrix} = y(0) = c_1  \begin{pmatrix} 0 \\ 1\\1 \end{pmatrix} + c_2   \begin{pmatrix} 1\\1\\0 \end{pmatrix} + c_3  \begin{pmatrix} 0 \\ 0 \\ 1 \end{pmatrix} = \begin{pmatrix} c_2 \\ c_1+c_2\\c_1+c_3 \end{pmatrix} \folgt c_2 = 1, 0 = c_1+c_2=c_1+1 \folgt c_1=-1, 1 = c_1+c_3 = -1 + c_3 \folgt c_3 = 2$ \\
 Lösung des AWPs: $y(x) = -e^{2x}\begin{pmatrix} 0 \\ 1\\1 \end{pmatrix} + e^x \begin{pmatrix} 1\\1\\0 \end{pmatrix} + 2e^x \begin{pmatrix} -x \\ -x \\ 1 \end{pmatrix}$

\item Bestimme die allgemeine Lösung von \begin{align*} \tag{$\ast $} y' = \underbrace{\begin{pmatrix} 1 & 0 \\ 0 & -1 \end{pmatrix}}_{=: A} y + \begin{pmatrix} e^x \\ e^x \end{pmatrix}\end{align*}
Allgemeine Lösung von $y' = Ay: \\ \det(A-\lambda E) = (1-\lambda)(1+\lambda), \lambda_1 = 1, \lambda_2 = -1, k_1=k_2=1$ \\
Kern$(A-E) = \left[ \begin{pmatrix} 1 \\ 0 \end{pmatrix} \right], $ Kern$(A+E) = \left[ \begin{pmatrix} 0 \\ 1 \end{pmatrix} \right]$ \\
 Fundamentalsystem von $y' = Ay:  y^{(1)},  y^{(2)}$ mit $y^{(1)}(x) = e^x \begin{pmatrix} 1 \\ 0 \end{pmatrix}, y^{(2)}(x) = e^{-x} \begin{pmatrix} 0 \\ 1 \end{pmatrix} \\
Y(x) := \begin{pmatrix} e^x & 0 \\ 0 & e^{-x} \end{pmatrix}$

\begin{align*}
y_s(x) &= Y(x) \int Y(x)^{-1} \begin{pmatrix} e^x \\ e^x \end{pmatrix} \,dx 
\\ &= Y(x) \int \begin{pmatrix} e^{-x} & 0 \\ 0 & e^x \end{pmatrix} \begin{pmatrix} e^x \\ e^x \end{pmatrix} \,dx 
\\ &= Y(x) \int \begin{pmatrix} 1 \\ e^{2x} \end{pmatrix}dx 
\\ &=  \begin{pmatrix} e^x & 0 \\ 0 & e^{-x} \end{pmatrix} \begin{pmatrix} x \\ \frac{1}{2}e^{2x} \end{pmatrix} = \begin{pmatrix} xe^x \\ \frac{1}{2}e^x \end{pmatrix}
\end{align*}
ist eine spezielle Lösung von ($\ast$).

Allgemeine Lösung von ($\ast$): $y(x) = c_1 e^x \begin{pmatrix} 1 \\ 0 \end{pmatrix} + c_2 e^{-x} \begin{pmatrix} 0 \\ 1 \end{pmatrix}  + \begin{pmatrix} xe^x \\ \frac{1}{2}e^x \end{pmatrix}, c_1, c_2 \in \mdr$
\end{enumerate}
...
\begin{definition}
\index{nullfach!Nullstelle}\index{Nullstelle!nullfache}
