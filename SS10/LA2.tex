...
% Hier noch eine passende Umgebung definieren damit es am Ende auch schoen 
% aussieht
%\begin{vorarbeit}
\paragraph{Vorarbeit:} Klassifiziere die Menge \(\mathcal{N}(f)\) durch
...
% Fuer sowas wie das Ziel auch eine Umgebung?
\paragraph{Ziel:} Klassifiziere die Polynome f\"ur spezielle \(G\).
\begin{itemize}
...
% Hierfuer auch eine eigene Umgebung?
\paragraph{Vorbereitung:} Jedes Polynom \(f\) mit \(\deg(f)=2\) hat die Form
\[
...
\end{comment}
\begin{theo}%(von der quadratischen Erg\"anzung)
...
\]
wobei \(p,q\) durch \(A\) eindeutig bestimmt sind (``Tr\"agheitssatz von 
Sylvester'').
\end{enumerate}
\end{theo}
\begin{proof}
