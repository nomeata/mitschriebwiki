...
\index{Jordank\"astchen}
    \textbf{Jordank\"astchen der L\"ange \(\mathbf{d}\) zum Eigenwert 0}.
\end{remind}
...
\]
\index{Jordank\"astchen}
ein \textbf{Jordank\"astchen der L\"ange \(\mathbf{d}\) zum Eigenwert \(\lambda\)}.
\end{definition}
...
``\(\impliedby\)'': W\"ahle Koordinatensystem \(\mathcal{K}=(0,B)\) und
\(\mathcal{L}=(\vp(0),B)\).\\
...
\begin{itemize}
\item[\(\implies\):] klar
\item[\(\impliedby\):] F\"ur alle \(x\) gilt: \(\phi_{1}(Kx)=\phi_{2}(Kx)\), 
