...
\begin{DefBem}
  ...
  Sei $V\subseteq\mathbb A^n(k)$ eine affine Varietät.
  \begin{enumerate}
  \item Eine rationale Funktion auf $V$ ist eine Äquivalenzklasse von Paaren $(U,f)$, wobei $U\subseteq V$ offen und dicht und $f\in\mathcal O(U)$ ist. Dabei sei $(U,f)\sim(U',f'):\Leftrightarrow f_{|U\cap U'}={f'}_{|U\cap U'}$
  \end{enumerate}
\end{DefBem}
\begin{Bew}
  \begin{enumerate}
  \item $\sim$ ist transitiv: Seien $(U,f)\sim(U',f'),(U',f')\sim(U'',f'')$ dann folgt: $f_{|U\cap U'\cap U''}={f''}_{|U\cap U' \cap U''}$. Da $U\cap U' \cap U''$ dicht in $V$ ist , ist dann auch $f_{|U\cap U''}={f''}_{|U\cap U''}$.
  \end{enumerate}
\end{Bew}
