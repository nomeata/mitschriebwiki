...
\begin{Bew}
 ...
 c) Für jedes homogene Polynom $f \in k[X_1, \dots ,X_n]$ gilt $f \in I(V) \Leftrightarrow f \in I(\tilde{V})$. Es genügt zu zeigen, dass $I(V)$ ein homogenes Ideal ist. Sei also $f \in I(\tilde{V})$ mit $f= \sum_{i=1}^d{f_i}$, $f_i$ homogen vom Grad $i$. Sei $x=(x_1, \dots, x_n) \in \tilde{V}$. Dann ist $(\lambda x_0, \dots , \lambda x_n) = \lambda x \in \tilde{V}$ $\forall \lambda \in k$, also $0 = f(\lambda x) = \sum_{i=1}^d{\lambda^i f_i(x)}$ $\forall \lambda \in k$. Dies ist ein lineares Gleichungssystem mit $|k|$ Zeilen. $\Rightarrow f_i(x)=0$ $\forall i \in \{0, \dots, d\}$.
\end{Bew}

\begin{Prop}[Projektiver Nullstellensatz]
\label{prop:8.9}
 Sei $k$ algebraisch abgeschlossen, $n \ge 0$. Für jedes von $(X_0, \dots, X_n)$ verschiedene Radikalideal $I \subseteq k[X_0,\dots,X_n]$ gilt $I(\underbrace{V(I)}_{\subseteq \mathbb P^n(k)})=\sqrt{I}$.
\end{Prop}
\begin{Bew}
...
\begin{DefBem}
 \label{defbem:8.11}
Sei $V \subseteq \mathbb P^n(k)$ projektive Varietät mit homogenem Verschwindungsideal $I(V)$. Dann heißt $k[V]:=k[X_0, \dots, X_n]/I(V)$ der homogene Koordinatenring von $V$. $k[V]$ ist graduierte k-Algebra. Dabei ist $k[V]_d:=k[X_0, \dots, X_n]/(I(V) \cap k[X_0, \dots, X_n]_d$).
\end{DefBem}
...
Dabei sei $f=\sum_{i=0}^df_i$, $f_i$ homogen vom Grad $i$ und $F:=\sum_{i=0}^df_i\cdot X_0^{d-i}\in k[X_0,\dots,X_n]$. \\
