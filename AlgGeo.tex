...
\end{Bem}
\begin{Bew}
Sei $V^*$ deer Dualraum zu $V$. Dann ist die Bijektion gegeben durch 
\begin{align*}
$G(d,n)(V)&\rightarrow& G(n-d,n)(V^*)\\
U&\mapsto &\{l\in V^*:U\in\Kern(l)\}\\
\bigcap_{l\in U^*}\Kern(l)&\leftarrow & U^*$
\end{align*}
\end{Bew}
...
\begin{align*}
$\psi:=\psi_{d,n}:&G(d,n)(V)&\rightarrow&\mathbb{P}(\bigwedge^dV)\\
&U&\mapsto&[u_1\wedge\cdots\wedgeu_d]$
\end{align*}
wobei $u_1,\cdots,u_d$ eine Basis von $U$ ist. Dann gilt:
\begin{enumerate}
\item $\psi$ ist wohldefiniert.
\item $\psi$ ist injektiv
\item Bild($\psi$) ist Zariski-abgeschlossen in $\mathbb{P}(\bigwedge^dV)$
\end{enumerate}
\end{Prop}
\begin{Bew}
\begin{enumerate}
...
v_n
\end{pmatrix}\\
\Rightarrow v_1\wedge\cdots\wedgev_d=\sum_{i=1}^da_{1i}u_i\wedge\cdots\wedge\sum_{i=1}^da_{di}u_i=\\
(\sum_{\sigma=S_d}(-1)^{sign(\sigma)}a_{1\sigma(1)}:\cdots:a_{d\sigma(d)})\cdot u_1\wedge\cdots\wedgeu_d=\det A\cdot u_1\wedge\cdots\wedge u_d$.
\end{enumerate}
\end{Bew}
\end{Prop}
\begin{Bew}
