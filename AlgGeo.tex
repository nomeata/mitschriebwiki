...
\begin{Lemma}
\label{}
Sei $V\subseteq\mathbb{P}^n(k)$ eine projektive Varietät, $f\in k[V]$ homogen, $l\in \mathcal O_V(D(f))$. Dann besitzt $D(f)$ eine offene Überdeckung $(U_i)_{i\in J}$ mit $U_i=D(h_i)$ für homogene $h_i\in k[V]$, so dass
$$l(x)=\frac{g_i(x)}{h_i(x)}~~~\forall x\in U_i$$
$g_i\in k[V]$ ebenfalls homogen mit $deg(g_i)=deg(h_i)$
\end{Lemma}

\begin{Bew}
Eine offene Überdeckung $(U_i')_{i\in J'}$ mit $l(x)=\frac{G_i(x)}{H_i(x)}~\forall x\in U_i',~G_i,H_i$ vom gleichen Grad, existiert nach Bem 10.3. Seien $g_i'$ und $h_i'$ deren Restklassen in $k[V]$. (Beachte: $D(h_i')$ kann größer als $U_i'$ sein)\\
Nach dem Beweis von 9.9 a) wird $U'_i$ überdeckt von offenen Mengen der Form $D(\tilde{h_i}')$ für homogene $\tilde{h_i'}\in k[V]$ (da die $D(\tilde{h'_i})$ eine Basis der Zariski-Topologie bilden), also
\begin{align*}
...
\end{align*}
mit $N=n(n+1)$ und $x_iy_k:x_jy_k=x_iy_l:x_jy_l$\\
...
\Leftrightarrow v,u_1,...,u_d$ sind linear abhängig\\
$\Leftrightarrow v\in <u_1,...,u_d>=U$
\item Wir brauchen homogene Gleichungen, die in allen Punkten in Bild($\psi$) erfüllt werden.
...
ist eine lineare Abbildung.
  ...
  \begin{Bew}[der Behauptung]
    ...
    \begin{Bew}
      ...
      \item[(b)] ``$\Rightarrow$'' Sei $v_1,\dots,v_m$ eine Basis von $\Kern(\varphi_\omega)$ \\
        $\stackrel{Bew. a)}{\Rightarrow}\omega=\lambda\cdot v_1\wedge\dots\wedge v_d$ für ein $\lambda\in k^\times$ \\
        ``$\Leftarrow$'' Sei $\omega=u_1\wedge\dots\wedge u_d$
        \begin{align*}
          v\in\Kern(\varphi_\omega) \Leftrightarrow & v,u_1,\dots,u_d\text{ linear abhängig } \\
          \Leftrightarrow & v\in<u_1,\dots,u_d> \\
          \Rightarrow & \Kern(\varphi_\omega)=<u_1,...,u_d>\\
...
$$\mathcal{O}_{\mathbb{P}^n(k)}\rightarrow \mathcal{O}_V(\Psi^{-1}(U)),~~~f\mapsto f\circ\Psi$$
...
Für jedes offene $U\subseteq V$ mit $0_1\in U$ und $0_2\in U$ und jedes $f\in \mathcal{O}_V(U)$ ist $f(0_1)=f(0_2)$.\\
...
\OE~$W=U\times V$, da die $U\times V$ eine Basis der Toplogie auf $X\times X$ bilden $\Rightarrow U\cap V=\emptyset$ 
\end{Bew}
...
\begin{Def}
\label{def:13.6}
Für eine Prävarietät $X$ mit affiner Überdeckung $(U_i)_{i=1,...,n}$ sei $X\times X$ die Prävarietät, die durch Verkleben der $U_i\times U_j,~~i,j=1,...,n$ hervorgeht.\\
...
\begin{ErinnDefBem}
  ...
  \item
    ...
    \end{align*}
    heißt \begriff{lokaler Ring} von $V$ in $x$, dabei sei $(U,f)\sim (U',f')\Leftrightarrow f\vert U\cap U'=f'\vert U\cap U'$
  \item $\mathcal{O}_{V,x}$ ist ein lokaler Ring mit maximalem Ideal
    \begin{align*}
      m_x=\{[(U,f)]\in\mathcal{O}_{V,x}:f(x)=0\}.
    \end{align*}
  \item $\mathcal{O}_{V,x}=\varinjlim_{U\subseteq V \text{ offen},\,x\in U}\mathcal{O}_V(U)$ \\
    \begin{align*}
    \begin{xy}
      \xymatrix{
        & & \mathcal{O}_V(U)\ar[3,-2]_{\alpha_U}\ar[2,1]^{\rho_U}\ar[2,0]^{U'\subseteq U} & \\
        & & & \\
        & & \mathcal{O}_V(U')\ar[r]^{\rho_{U'}}\ar[1,-2]_{\alpha_{U'}} & \mathcal{O}_{V,x}\ar@{.>}[1,-3]^{\exists! \alpha} \\
        A & & & \\
      }
    \end{xy}
    \end{align*}
...
\begin{Bew}
  ...
  Aus $(*)$ folgt weiter:
  \begin{align*}
    \varphi_x^\sharp ( \underbrace{m_{\varphi(x)}^W\cdot k[W]_{m_{\varphi(x)}^W}}_{=m_{\varphi(x)}} ) \subseteq m_x^V k[V]_{m_x^V} = m_x
  \end{align*}
\end{Bew}
...
\begin{ErinnDef}
  ...
  \begin{enumerate}
  \item Für ein Primideal $\wp\subseteq R$ heißt $$\height(\wp)\defeqr\sup\{n\in\mathbb{N}:\text{ Es gibt Primideale } \wp_0\subsetneq\ldots\subsetneq \wp_n =\wp\}$$
...
\begin{Bew}
b) Ist $V$ affin (also $V=V_0$), so folgt die Aussage aus a) und Proposition 3.15.4(b). Im allgemeinen Falle überdecke $V$ durch affine Varietäten $V_i \ (i = 1,\ldots,n)$. Da $V$ irreduzibel ist, ist $V_i\cap V_j\neq \emptyset~\forall i,j.\\
\Rightarrow \dim \mathcal{O}_{V,x}$ ist unabhängig von $x$, also gleich dim$V_i$ für jedes $i = 1, \ldots, n$.\\
noch zu zeigen: \ $\dim V_i= \dim V$.\\
...
d) \OE ~sei $V$ affin. Die irreduziblen Komponenten $Z_1,\ldots,Z_n$ von $V$ entsprechen den minimalen Primidealen in $k[V]$. Es gilt $x\in Z_i\Leftrightarrow m_x^V\supseteq I(Z_i)=:\mu_i$. Weiter ist $k[Z_i]=k[V]/\mu_i$. Es folgt:
$\dim\mathcal{O}_{V,x}=\height(m_x^V)=\max_{i=1; \mu_i\subseteq m_x^V}^n\{\text{maximale Länge einer Primidealkette } \mu_i\subsetneq \wp_1\subsetneq\ldots\subsetneq m_x^V\}=\max_{i=1; \mu_i\subseteq m_x^V}^n\{\underbrace{\dim k[Z_i]}_{= \dim Z_i}\}$.
\end{Bew}
...
\begin{enumerate}
\item Für $f\in I$ sei $f^{(1)}:= f^{(1)}_x := \sum_{i=1}^n \frac{\partial f}{\partial X_i}(x)\cdot X_i$. Weiter sei $I_x$ das von den $f^{(1)}$, $f\in I$, erzeugte Ideal in $k[X_1,\ldots ,X_n]$ und $T_x:=T_{V,x}:=V(I_X)$. $T_{V,x}$ heißt \begriff{Tangentialraum} an $V$ in $x$.
\item $T_x$ ist ein linearer Unterraum von $\mathbb{A}^n(k)$.
\item Sind $f_1,\ldots , f_r$ Erzeuger von $I$, so wird $I_x$ erzeugt von $f_1^{(1)},\ldots, f_r^{(1)}$.
\end{enumerate}
\end{DefBem}
\underline{Beispiele von oben:}
\begin{enumerate}
\item[1.)] $I_x=(X),~~~T_x=V(X)$
\item[2.)] $I_x=(0),~~~T_x=\mathbb{A}^2(k)$
\item[3.)] $I_x=(0),~~~T_x=\mathbb{A}^2(k)$
\item[4.)] $I_x=(0),~~~T_x=\mathbb{A}^3(k)$;\\
$I_y=(2X-2Z)=(X-Z),~~~T_y=V(X-Z)$
\end{enumerate}
...
\begin{Bew}
\OE $x=0,~\varphi(x)=0$.\\
Schreibe $\varphi=(\varphi_1,\ldots,\varphi_m)$. Brauche $k$-Algebrenhomomorphismus:
$$(d_x\varphi)^{\sharp}:~~k[Y_1,\ldots,Y_m]/I_{\varphi(x)}\rightarrow k[X_1,\ldots,X_n]/I_x$$
Für $j=1,\ldots, m$ ist $\varphi^{\sharp}(Y_j)=Y_j\circ\varphi=\varphi_j\Rightarrow(\varphi^{\sharp}(Y_j)^{(1)})=\sum_{i=1}^n\frac{\partial\varphi_j}{\partial X_i}(0)\cdot X_i=:(d_x\varphi)^{\sharp}(Y_j)$.\\
...
Dann ist $T_x$ in natürlicher Weise isomorph zu dem Dualraum $(m_x/m_x^2)^{\vee}$ von $m_x/m_x^2$. Der $k$-Vektorraum $(m_x/m_x^2)^{\vee}$ heißt \begriff{Zariski-Tangentialraum} an $V$ in $x$.\\
...
$$\dim T_{V,x}=\dim_xV.$$
\item (Jacobi-Kriterium) Sei $U\subseteq V$ eine offene, affine Umgebung von $x$, $f_1,\ldots,f_r\in k[X_1,\ldots,X_n]$ Erzeuger des Verschwindungsideals $I(U)$. Dann gilt:\\
$$x \text{ nichtsingulär } \Leftrightarrow \Rang\left(\frac{\partial f_i}{\partial X_j}(x)\right)_{i,j}=n-\dim_xV$$
\item Ist $x$ singulär, so ist $\dim T_{V,x}>\dim_xV$.
\end{enumerate}
\end{DefBem}
\begin{Bew}
\begin{enumerate}
\item[b)] Sei $x\in V$, $V=V(f_1,\ldots,f_r)\subseteq\mathbb{A}^n(k)$.
$$\mathcal{J}_f(x):=\left(\frac{\partial f_i}{\partial X_j}(x)\right)_{i=1,\ldots,r\atop j=1,\ldots,n}$$
$T_{V,x}$ ist die Lösungsmenge des LGS $\mathcal{J}_f(x)\cdot X=0$, denn $f_i^{(1)}=\sum_{j=1}^n\frac{\partial f_i}{\partial X_j}(x)\cdot X_j$.
\item[c)] Sei $\mathcal J_f := \left(\frac{\partial f_i}{\partial X_j}\right)_{i,j}$.\\ $\Rightarrow \Rang\left(\mathcal J_f(x)\right)=\max\{d: \exists \, (d\times d)$-Minor $M$ von $\mathcal J_f$ mit $\det M(x)\neq0\}\\
\Rightarrow$ Es gibt eine offene Teilmenge $U$ von $V$, auf der $\Rang(\mathcal J_f(x))$ maximal ist. 
\end{enumerate}
\end{Bew}
...
Rang(\mathcal J_f(x))=\begin{cases}
0 \ \ , \ -3X^2-2X=0 $ und $Y=0\\
1 \ \ , \ $ sonst $
                    \end{cases}$
\item $V=V(f)\subseteq\mathbb A^n(k)$ mit einem Polynom $f\in k[X_1,\ldots, X_n].\\
x\in\mathbb A^n(k)$ singulärer Punkt von $V \Leftrightarrow 0=f(x)=\frac{\partial f}{\partial X_1}(x)=\ldots=\frac{\partial f}{\partial X_n}(x)$
\end{enumerate}
\end{Bsp}
...
\begin{enumerate}
\item Ein lokaler Ring heißt \begriff{regulär}, wenn $\dim R=\dim_{R/m}(\FakRaum{m}{m^2})$ ist.
\item Sei $V$ eine Varietät. Ein Punkt $x\in V$ ist genau dann nichtsingulär, wenn $\mathcal O_{V,x}$ ein regulärer, lokaler Ring ist.
\end{enumerate}
\end{Def}
...
\begin{enumerate}
\item Für $i=1,\ldots,r$ sei
$$f_i^1:=\sum_{j=1}^n\frac{\partial f_i}{\partial X_j}\cdot Y_j\in k[X_1,\ldots,X_n,Y_1,\ldots,Y_n]$$
Dann heißt
$$\mathcal{T}_V=V(f_1,\ldots,f_r,f_1^1,\ldots,f_r^1)\subseteq \mathbb{A}^n\times\mathbb{A}^n=\mathbb{A}^{2n}$$
\begriff{Tangentialbündel} über $V$.
\item Sei $p:\mathbb{A}^n\times\mathbb{A}^n\rightarrow\mathbb{A}^n$ die Projektion auf die ersten $n$ Komponenten. Dann ist $p(\mathcal{T}_V)=V$.
\item Für jedes $x\in V$ ist $p^{-1}(x)\cong T_{V,x}$.
\item Ist $V$ eine beliebige Varietät und $V_1,\ldots,V_m$ eine affine Überdeckung von $V$, so verkleben sich die Tangentialbündel $\mathcal{T}_{V_1},\ldots,\mathcal{T}_{V_m}$ zu einer Varietät $\mathcal{T}_V$, dem \begriff{Tangentialbündel} über $V$.
\end{enumerate}
\end{DefBem}
\begin{Bsp}
