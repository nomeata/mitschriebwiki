...
\underline{$n>1$}:  Zerlege $f$ und $g$ in Primfaktoren (vgl. 1.3) und wähle $a\in k$, so dass $(X_n-a)$ nicht Teiler von $f$ oder $g$ ist. Identifiziere $V(X_n-a)= \{(x_1,\dots,x_n)\in k^n: x_n=a\}$ mit $k^{n-1}$.\\
...
\begin{Def}
  ...
  von homogenen Polynomen gibt, sodass $V=V(\mathcal F):=\{x=(x_0:\dots:x_n)\in\mathbb P^n(k)\vert f(x)=0 \text{ für alle } f\in\mathcal F\}$.
\end{Def}
\begin{Bsp}
  \begin{enumerate}
