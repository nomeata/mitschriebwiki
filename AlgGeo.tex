  ...
  \item $\mathcal{O}_{V,x}=\ilim_{U\subseteq V \text{ offen },x\in U}\mathcal{O}_V(U)$ \\
    ...
    \begin{xy}
      \xymatrix{
        ...
        & & & \\
        & & \mathcal{O}_V(U')\ar[r]^{\rho_U'}\ar[1,-2]^{\alpha_U'} & \mathcal{O}_{V,x}\ar@{.>}[1,-3]^{\exists! \alpha} \\
        A & & & \\
      }
    \end{xy}
    \end{align*}
...
\begin{Bem}
  ...
  \begin{enumerate}
  \item $\mathcal{O}_{V,x}\cong \FakRaum{k[V_0]}{m_x^{V_0}}$, wobei $k[V_0]$ der affine Koordinatenring vin $V_0$
...
\begin{Prop}
  \label{prop:14.3}
  Seien $V,W$ Varietäten, $x\in V, y\in W$. Ist $\mathcal{O}_{V,x}\cong\mathcal{O}_{V,x}$ (als $k$-Algebra), so gibt es (affine)
...
\begin{Bew}
  ...
  \begin{align*}
    \varphi^\sharp:\underbrace{k[W]_{m_x^V}}_{\cong\mathcal{O}_{W,\varphi(x)}}\longrightarrow \underbrace{k[V]_{m_x^V}}_{\cong\mathcal{O}_{V,x}}
  \end{align*}
  Aus $(*)$ folgt weiter:
  \begin{align*}
    \underbrace{\varphi_x^\sharp\left(m_{\varphi(x)}^W\cdot k[W]_{m_{\varphi(x)}^W}\right)}_{=m_{\varphi(x)}}\subseteq m_x^V k[V] m_x^V = m_x
  \end{align*}
\end{Bew}
...
\begin{ErinnDef}
  ...
  \begin{enumerate}
  \item Für ein Primideal $p\subseteq R$ heißt $\height(p)\defeqr\sup\{n\in\mathbb{N}:\text{ es gibt Primideale } p_1\subsetneq\dots\subsetneq =p\}$
    \begriff{Höhe} von $p$.
  \item $\dim R\defeqr\sup\{\height(p):p\subset R \text{ Primideal }\}$ heißt (Krull-) Dimension von $R$.
  \end{enumerate}
\end{ErinnDef}
\begin{Bem}
  \label{bem:15.3}
...
\end{Bem}
\begin{Bew}
  Nach Proposition \ref{prop:3.3} ist eine Teilmenge $Z$ von $V$ genau dann irreduzibel, wenn ihr Verschwindungsideal
  $I(Z)$ ein Primideal ist. Nach \ref{satz:HNS} ist das eine Bijektion.
\end{Bew}
\begin{Prop}
  \label{prop:15.4}
  \begin{enumerate}
...
\begin{BemDef}
  ...
  \item Ist $V$ irreduzibel, so ist 
    ...
    \end{align*}
\item dim$_xV:=$dim$\mathcal{O}_{V,x}$ heißt \begriff{lokale Deimension} von $V$ in $x$.
\item dim$_xV=$max$\{$dim$Z: Z$ irreduzible Komponente von $V,~x\in Z\}$
\end{enumerate}
\end{BemDef}
...
Also: dim$\mathcal{O}_{V,x}=$ht$(m_x^V)=$\\
max$_{i=1; \mu_i\subseteq m_x^V}^n\{\textrm{maximale~Länge~einer~Primidealkette~} \mu_i\subsetneqq \wp_1\subsetneqq\cdots\subsetneqq m_x^V\}$=\\
max$_{i=1; \mu_i\subseteq m_x^V}\{\underbrace{$dim$k[Z_i]}_{= \operatorname{dim}Z_i}\}$.
\end{Bew}
...
Die Tangente in $x$ an $V$ ist die $y$-Achse, also $V(X)$. Der Tangentialraum in $x=(1,0)$ ist der selbe. D.h. der Tangentialraum ist nicht als affiner Raum sonder als Vektorraum zu verstehen.
    \begin{tikzpicture}
      ...
      \draw[very thick] (-0.5,0) circle (0.5);
      \draw[thick, parametric, domain=0:1,samples=10] plot[id=blablub] function{t,t*t};
    \end{tikzpicture}
