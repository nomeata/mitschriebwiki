...
\end{Def}
\begin{Prop}
Eine affine Varietät $V\in k^n$ ist genau dann irreduzibel, wenn $I(V)$ Primidel in $k[X_1,...,X_n]$ ist. Das ist genau dann der Fall, wenn der affine Koordinatenring $A(V)\defeql k(V)$ nullteilerfrei ist.
\end{Prop}
\begin{Bew}
...
\begin{Eri}
 ...
 =&\FakRaum{(k^{n+1}-{0})}{\sim}\text{ mit }(x_0,\dots,x_n)\sim(y_0,\dots,y_n):\Leftrightarrow\exists\lambda\in k^\times:\lambda x_i=y_i\text{ für } i=1,\dots,n
  \end{align*}
Schreibweise $(x_0:\dots:x_n)\defeqr [(x_0,\dots,x_n)]$ (``homogene Koordinate'')
\begin{nnBsp}
  \begin{enumerate}
...
$\Rightarrow\mathbb P^n(k)$ ist mit der Quotientopologie ein kompakter topologischer Raum. \\
...
\item $\varphi_i:\mathbb P^n(k)\backslash U_i\longrightarrow \mathbb P^{n-1}(k),(x_0:\dots:x_n)\mapsto(x_0:\dots:x_{i-1},x_{i+1}:\dots:x_n)$
\end{enumerate}
\end{Bem}
\begin{Folg}
...
\begin{Beo}
  \begin{enumerate}
  \item Ist $f\in k[X_0,\dots,X_n]$ homogen vom Grad $d\geq 0$, do gilt für $(x_0,\dots,x_n)\in k^{n+1}$ und 
    $\lambda\in k f(\lambda x_0,\dots,\lambda x_n)=\lambda^df(x_0,\dots,x_n)$
...
\begin{Def}
  ...
  von homogenen Polynomen gibt, sodass $V=\{x=(x_0:\dots:x_n)\in\mathbb P^n(k)\vert f(x)=0 \text{ für alle } f\in\mathcal F\}$.
\end{Def}
\begin{Bsp}
  \begin{enumerate}
  \item $H_i=V(X_i)=\mathbb P^n(k)-U_i(\gleichwegen{\varphi_i}\mathbb P^{n-1}(k))$ ist eine projektive Varietät (``Hyperebene'').
...
\begin{DefBem}
  ...
  \item $S=k[X_0,\dots,X_n]$ ist \begriff{graduierter Ring} (genau graduierte $k$-Algebra) das heißt:
    ...
    \]
    (hier: $S_d=\{f\in k[X_0,\dots,X_n]\vert \text{ homogen vom Grad }d\}, S_0=k$)
  \item Ein Ideal $I\subseteq S$ heißt homogen, wenn $I$ von homogenen Elementen erzeugt wird.
    Äquivalent: $I=\bigoplus_{d=0}^\infty(I\cap S)$
  \item Summe, Produkt, Durchschnitt und Radikal von homogenen Idealen sind wieder homogen.
  \end{enumerate}
\end{DefBem}
\begin{Bew}
  c.) Seien $I_1,I_2$ homogene Ideale mit homogenem Erzeuger $(f_i)_{i\in \mathcal I}$ beziehungsweise
    ...
    \begin{align*}
      ...
      \Rightarrow & \left(\bigoplus_{d=0}^\infty I_1\cap S_d\right)\cap \left(\bigoplus_{d=0}^\infty I_1\cap S_d\right)=I_1\cap I_2
    \end{align*}
...
\begin{DefBem}
 \begin{enumerate}
  \item Für $V \subseteq \mathbb P^n(k)$ sei $I(V)$ das Ideal in $k[X_0,\dots,X_n]$ das von allen homogenen Polynomen$f$ erzeugt wird für die $f(x)=0 $ $\forall x \in V$ gilt. $I(V)$ heißt Verschwindungsideal von $V$. $I(V)$ ist Radikalideal.
  \item Für eine Menge $F \subset k[X_0,\dots,X_n]$ sei $V(F)=\{ x \in \mathbb P^n(k):f(x)=0 \forall x \in F \}$ die zugehörige projektive Varietät. \\ 
...
\begin{Prop}
 \label{prop:8.8}
 \begin{enumerate}
  \item Die projektiven Varietäten bilden die abgeschlossenen Mengen einer Topologie. Diese heißt die Zariski-Topologie auf $\mathbb P^n(k)$.
...
\begin{DefBem}
 ...
 \begin{enumerate}
  \item Für eine nicht leere projektive Varietät $V \subseteq \mathbb P^n(k)$ heißt \\
   $\tilde{V}:=\{x = (x_0,\dots ,x_n): (x_0: \dots : x_n) \in V \} \cap \{(0, \dots, 0)\}$ der affine Kegel über $V$.
...
\end{DefBem}
\begin{Bew}
 b) Klar ist $(x_0:\dots:x_n) \in V \Leftrightarrow (x_0, \dots , x_n) \in \tilde{V} \backslash \{(0,\dots,0)\}$. Da $V \ne \emptyset$ enthält das Ideal $I$, für das $V = V(I)$ ist, kein Element aus k. Für jedes homogene Element $f \in I$ ist daher $deg(f) \ge 0$ $\Rightarrow f(0, \dots ,0)=0 \Rightarrow \tilde(V)=V(I)$ \\
 c) Für jedes homogene Polynom $f \in k[X_1, \dots ,x_n]$ gilt $f \in I(V) \Leftrightarrow f \in I(\tilde{V})$. Es genügt zu zeigen, dass $I(V)$ ein homogenes Ideal ist. Sei also $f \in I(\tilde{V})$ mit $f= \sum_{i=1}^d{f_i}$, $f_i$ homogen vom Grad i. Sei $x=(x_1, \dots, x_n) \in \tilde{V}$. Dann ist $(\lambda x_0, \dots , \lambda x_n) = \lambda x \in \tilde{V}$ $\forall \lambda \in k$  $\rightarrow 0 = f(\lambda x) = \sum_{i=1}^d{\lambda^i f_i(x)}$ $\forall \lambda \in k$. Dies ist ein lineares Gleichungssystem mit $|k|$ Zeilen. $\Rightarrow f_i(x)=0$ $\forall i \in \{0, \dots, d\}$.
\end{Bew}

\begin{Prop}[Projektiver Nullstellensatz]
\label{prop:8.9}
 Sei $k$ algebraisch abgeschlossen, $n \ge 0$. Für jedes von $(X_0, \dots, X_n)$ verschiedene Radikalideal $I \subseteq k[X_0,\dots,X_n]$ gilt $I(\underbrace{V(I)}_{\mathbb P^n(k)})=\sqrt{I}$
\end{Prop}
\begin{Bew}
Für gegebenes Radikalideal I sei $V \subseteq \mathbb P^n(k)$ die zugehörige projektive Varietät.\\
Ist $I = k[X_1, \dots, X_n]$ so ist $V(I) = \emptyset$ und $I(V(I))=k[X_1, \dots, X_n] = \sqrt{k[X_1, \dots, X_n]}$ \\
Ist $I \subsetneq (X_1, \dots, X_n)$ so ist die affine Nullstellenmenge von $I$ in $\mathbb A^n(k)$ echte Obermenge von $\{(0,\dots, 0)\}$, enthält also einen Punkt $(x_0, \dots, x_n) \ne (0, \dots, 0)$.
Nach \ref{defbem:8.10} b) ist $\tilde{V}$ auch die durch $I$ bestimmte affine Varietät in $\mathbb A^{n+1}(k)$. Nach \ref{defbem:8.10} c) ist ist $I(\tilde{V})=I(V)$. Nach Satz \ref{satz:HNS} (Hilbertscher Nullstellensatz) ist $I(\tilde{V})=\sqrt{I(V)}$.  
\end{Bew}
\begin{DefBem}
 \label{defbem:8.11}
Sei $V \subseteq \mathbb P^n(k)$ projektive Varietät mit homogenem Verschwindungsideal $I(V)$. Dann heißt $k[V]:=k[x_1, \dots, x_n]/I(V)$ der homogene Koordinatenring von $V$. $k[V]$ ist graduierte k-Algebra. Dabei ist $k[V]_d:=k[x_1, \dots, x_n]/(I(V) \cap k[x_1, \dots, x_n]_d$).
\end{DefBem}
\section{Affine und projektive Varietäten}
Es war $U_i =\{(x_0, \dots, x_n)\in \mathbb P^n(k):x_i \ne 0\}=\mathbb P^n(k) \backslash V(x_i)$ offen.
$\rho_i:U_i \rightarrow \mathbb A^n(k)$ $(x_0, \dots, x_n) \mapsto (x_0/x_i, \dots , x_{i-1}/x_i,x_{i+1}/x_i), \dots , x_n/x_i)$ ist bijektiv.
\begin{Prop}
 \label{prop:9.1}
...
\begin{Bew}
$\OE ~~i=0~~\rho_i:=\rho_0$\\
(i) Genügt zu zeigen: Für jedes $f\in k[X_1,...,X_n]$ ist $\rho^{-1} (D(f))$ offen in $U_0$.\\
Äquivalent dazu: $\rho^{-1}(V(f))$ ist abgeschlossen in $U_0$. Dies folgt aus:\\
\begin{BemDef}
\label{defbem:9.2}
Für $f\in k[X_1,...,X_n]$ ist $\rho^{-1}(V(f))=U_0\cap V(F)$.\\
Dabei sei $f=\sum_{i=0}^df_i$, $f_i$ homogen vom Grad i und F:=$\sum_{i=0}^df_i\cdot X_0^{d-i}\in k[X_1,...,X_n]$. \\
F ist homogen vom Grad d und heißt die \begriff{Homogenisierung} von f.
\end{BemDef}
\begin{Bew}
$x=(x_1,...,x_n)\in V(f)\Leftrightarrow f(x_1,...,x_n)=0 \Leftrightarrow \sum_{i=0}^df_i(x_1,...,x_n)=0\Leftrightarrow F(1:x_1:...:x_n)=0\Leftrightarrow \rho^{-1}(x)\in V(F)$.
\end{Bew}
Damit ist gezeigt, dass $\rho$ stetig ist.\\
(ii) $\rho^{-1}$ ist stetig: Wie in (i) genügt zu zeigen: Für jedes homogene $F\in k[X_0,...,X_n]$ ist $\rho(V(F)\cap U_0)$ abgeschlossen in $\mathbb{A}^n(k)$.\\
Beachte: Die $D(F), F\in k[X_0,...,X_n]$ homogen bilden eine Basis der Zariski-Topologie auf $\mathbb{P}^n(k)$ (Bew. wie in Bemerkung \ref{bem:2.7} (ii)).
\end{Bew}
\begin{BemDef}
\label{bemdef:9.3}
$\rho(V(F)\cap U_0)=V(f)$ wobei mit $y_i:=\frac{x_i}{x_0}, i=1,...,n$ $f\in k[Y_1,...,Y_n]$ definiert sei durch $f(Y_1,...,Y_n)=F(1,\frac{x_1}{x_0},...,\frac{x_n}{x_0})$.\\
f heißt \begriff{Dehomogenisierung} von $F$ bzgl. $x_0$.
\end{BemDef}
\begin{Bew}
$x=(x_0:...:x_n)\in V(F)\cap U_0 \Leftrightarrow x_0\neq0$ und $F(x)=0 \Leftrightarrow F(1,\frac{x_1}{x_0},...,\frac{x_n}{x_0}) =0 \Leftrightarrow f(\rho(x)=0\Leftrightarrow \rho(x)\in V(f)$
\end{Bew}
\begin{Bsp}
$F(X_0,X_1,X_2)=X_1^2-X_0X_2~~~~f_{X_0}(Y_1,Y_2)=F(1,\frac{x_1}{x_0},\frac{x_2}{x_0})= Y_1^2-Y_2~~~~f_{X_1}(Y_0,Y_2)=1-Y_0Y_2$\\
\underline{Frage} Wie sieht $F$ aus, wenn $V(F)\cap U_0=\emptyset$ ?\\
Antwort: z.B. $F=X_0^d, \sqrt{(F)}=(X_0)$.
\end{Bsp}
\begin{Bem}
\label{bem:9.4}
\begin{enumerate}
\item Sei $f\in k[X_1,...,X_n]$, $F\in k[X_0,...,X_n]$ die Homogenisierung. Dann gilt für die Dehomogenisierung $\tilde{f}$ von $F$ bzgl. $X_0$: $\tilde{f}=f$.
\item Sei $F\in k[X_0,...,X_n]$ homogen, $f\in k[Y_1,...,Y_n]$ die Dehomogenisierung bzgl. $X_0$, $\tilde{F}$ die Homogenisierung von $f$. Dann gilt. $F=\tilde{F}\cdot X_0^d$ für ein $d\geq 0$.
\end{enumerate}
\end{Bem}
\begin{Bew}
\begin{enumerate}
\item Sei $f=\sum_{i=0}^df_i,~f_d\neq 0 \Rightarrow F=\sum_{i=0}^df_iX_0^{d-i} \Rightarrow \tilde{f}=\sum_{i=0}^df_i\cdot 1=f$.
\item Schreibe $F=X_0^d\cdot \tilde{F}$ mit $X_0\nmid \tilde{F}$. Dann hat die Dehomogenisierung von $\tilde{F}$ bzgl. $X_0$ den selben Grad wie $\tilde{F} \Rightarrow$ ihre Homogenisierung ist $\tilde{F}$
\end{enumerate}
\end{Bew}
\begin{DefBem}
\label{defbem:9.5}
...
(ii) Es gibt eine offene Teilmenge $V\subset\mathbb{P}^n(k)$, so dass $W=U\cap V$
\end{DefBem}
\begin{Bsp} 
$\mathbb{P}^2\setminus \{(0:0:1)\}$ ist quasiprojektiv, aber weder quasiprojektiv noch affin (was zu zeigen wäre)
\end{Bsp}
\begin{Prop}
\label{prop:9.6}
Betrachte $\mathbb{A}^n(k)$ über $\rho_0 : U_0\tilde{\rightarrow} \mathbb{A}^n(k)$ als Teilmenge von $\mathbb{P}^n(k)$. Für ein Radikalideal $I\subseteq k[X_1,...,X_n]$ sei $I^*\subseteq k[X_0,...,X_n]$ das von den Homogenisierungen aller $f\in I$ erzeugte Ideal. Dann ist $V_p(I^*)\subseteq\mathbb{P}^n(k)$ der Zariski-Abschluss von $V_a(I)\subseteq \mathbb{A}^n(k)$
\end{Prop}
\begin{Bew}
(i) "'$V_a(I)\subseteq V_p(I^*)$"': Sei $x=(x_1,...,x_n)\in V_a(I)$ und sei $f\in I$, $F\in I^*$ die Homogenisierung von $f$.\\
Dann ist $F(\rho_0^{-1}(x))=F(1:x_1:...:x_n)=f(x_1,...,x_n)=0$, weil $f\in I =I(V(I))$.\\
...
Sei $F\in \mathcal{J}$ homogen, $f=F(1,\frac{x_1}{x_0},...,\frac{x_n}{x_0})$ die Dehomogenisierung von $F$ bzgl. $x_0$.\\
Sei $y=(y_1,...,y_n)\in V_a(I)$.\\
Dann ist $f(y)=F(1,y_1,...,y_n)=0$, weil $\rho_0^{-1}(y)\in V(\mathcal{J})\Rightarrow f\in I$.\\
Sei $\tilde{F}$ die Homogenisierung von $f$, also $\tilde{F}\in I^*$ dann Folgt mit 9.4: $F=\tilde{F}\cdot X_0^d$ für ein $d\geq0\Rightarrow F\in I^*$.
\end{Bew}
...
\begin{Bew}
a) Sei $W=\bigcup_{i=0}^n(W\cap U_i)$ mit $U_i=\{(x_0:...:x_n)\in\mathbb{P}^n(k):x_i\neq 0\}\cong\mathbb{A}^n(k)$.\\
Also \OE $~W\subseteq\mathbb{A}^n(k)$, $W$ ist offen in einer affinen Varietät, nämlich dem Zariski-Abschluss $V_i$ von $W\cap U_i$ in $U_i$. Nach 2.7.(ii) bilden die $D(f),~f\in k[V_i]$ eine Basis der Zariski-Topologie auf $W\cap U_i$. Nach dem Übungsblatt 4, A3 ist jedes $D(f)$ isomorph zu einer affinen Varietät.\\
b) Sie $(O_j)_{j\in J}$ offene Überdeckung von $W$. Nach dem Beweis von a) wird jedes $O_j$ überdeckt von offenen Teilen der Form $D(f)$ für geeignete $f\in k[\overline{O_j\cap U_i}]$.\\
Also \OE $~O_j=D(f)$ für ein $f_j\in k[X_0,...,\hat{X}_i,...,X_n]$ (im Folgenden bedeutet $\hat{X}_i$: "'die $i$-te Variable streichen"').\\
Sei $F_j\in k[X_0,...,X_n]$ die Homogenisierung von $F_j$. Dann ist
$$W\subseteq \bigcup_{j\in J}D(F_j)=\mathbb{P}^n(k) - \bigcap_{j\in J}V(F_j)=\mathbb{P}^n(k) - V(\underbrace{\sum_{j\in J}(F_j)}_{=:I})$$
$I$ ist endlich erzeugtes Ideal, z.B. von $F_1,...,F_r\Rightarrow W\subseteq\bigcup_{j=1}^rD(F_j)\Rightarrow W\subseteq \bigcup_{j=1}^rD(f_j)$
\end{Bew}
\section{Reguläre Funktionen}
\begin{Def}
\label{def:10.1}
Sei $W\subseteq\mathbb{P}^n(k)$ eine quasiprojektive Varietät. Eine Abbildung $f:W\rightarrow k$ heißt \begriff{reguläre Funktion} auf $W$, wenn $f\mid_{W\cap U_i}$ reguläre Funktion ist für $i=0,...,n$.
\end{Def}
\begin{Bem}
\label{bem:10.2}
...
\begin{Bew} "'$\Rightarrow$"' Sei $P\in U_i$, $g_p,h_p\in k[V_i](V_i=\overline{V\cap U_i})$ wie in 10.1 (d.h., es gibt ein $U_p$ ...).\\
Seien $\tilde{g}_p, \tilde{h}_p$ Repräsentanten von $g_p$ bzw. $h_p$ in $k[x_0,...,\hat{X}_i,...,X_n]$ und $G_p, H_p$ Homogenisierungen.\\
Ist deg$(G_p)\neq$deg$(H_p)$ so ersetze $G_p$ durch $G_p\cdot X_i^{\operatorname{deg}(H_p)-\operatorname{deg}(G_p)}$ (falls deg$(H_p)>$deg$(G_p)$.\\
$\forall x\in U_p$ ist dann $$f(x)=\frac{g_p(x)}{h_p(x)}=\frac{G_p(x_0:...:x_{i-1}:1:x_{i+1}:...:x_n)}{H_p(x_0:...:x_{i-1}:1:x_{i+1}:...:x_n)}$$
"'$\Leftarrow$"' Dehomogenisieren ...
\end{Bew}
\begin{Bem}
...
$\mathcal{O}_V(D(f)\cong k[V]_{(f)}:= \{\frac{g}{f^r}: g\in k[V]~homogen,~\operatorname{deg}(g)=r\cdot\operatorname{deg}(f)\}/\sim$
