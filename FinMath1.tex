...
Die Menge aller $\lambda$-Quantile von $X$ ist ein Intervall $[q_X^-(\lambda),q_X^+(\lambda)]$ mit $q_X^-(\lambda)=\sup\{x:\,  P(X<x)<\lambda\}=\inf\{x:\,  P(X\leq x)\geq \lambda\}$, $q_X^+(\lambda)=\inf\{x:\,  P(X\leq x)>\lambda\}=\sup\{x:\,  P(X < x)\leq \lambda\}$ mit Verteilungsfunktion $F_X(x)= P(X\leq x)$: $q_X^-(\lambda)=inf\{x:\, F_X(x)\geq \lambda\},\, q_X^+(\lambda)=inf\{x:\, F_X(x)> \lambda\}$.
