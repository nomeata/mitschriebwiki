...
% get rid of fourer fontenc warnings
...
\end{align*}
für $t=1,\ldots,T$, wobei $\Delta\alpha_1^k \da \alpha 1^k$ für $k=1,\ldots,d$.
\end{lemma}
...
\end{align*}
Sei $t\in\{2,3,\ldots,T\}$. Da $\left(\begin{smallmatrix} \beta \\ \alpha \end{smallmatrix}\right)$ selbstfinanzierend ist, gilt
\begin{align*}
...
&= (\beta_n - \beta_n) + \alpha_n^\top (\frac{S_n}{B_n} - \frac{S_{n-1}}{B_{n-1}}) \\
...
&= \frac{V_0^\varphi}{B_0} + \sum_{n=1}^t (\frac{V_n^\varphi}{B_n} -  \frac{V_{n-1}^\varphi}{B_{n-1}}) \\
...
\begin{bemerkung}
Ist $\cF = \cF_T = \sigma(S_0,S_1,\ldots,S_T)$, so ist $H = h(S_0,\ldots,S_T)$
\end{bemerkung}

\begin{beispiel}
\begin{itemize}
\item Eurpäische Call-Option: $H=(S_T-K)^+$
\item Eurpäische Put-Option: $H=(K-S_T)^+$
\item Termingeschäft (“short”): $H=K-S_T$
\item Digital Call-Option: $H = 1_{\{S_T > K}\}$
\item Down-and-out-call: $H = (S_T-K_1) 1_{\{\min_{t\in\{0,\ldots,T\}} S_t > K_2\}}$
