...
\begin{center}\begin{tikzpicture}[font=\scriptsize,scale=1,baseline=0]
	...
	\path (0) --coordinate[pos=0.2](a) coordinate[pos=0.8](b) (5);
	\draw[->] (a) to[relative,out=25,in=155] (b);
...
Die Paare  $(u, \{\phi_i\})$ und $(w, \{\phi_i\})$ hei"sen \CmMark{\"aquivalent}, wenn $u|_{\V\G \setminus \bigcup_i \supp \phi_i} = w|_{\V\G \setminus \bigcup_i \supp \phi_i}$ gilt.
...
\marginnote{\begin{tikzpicture}[font=\scriptsize,scale=1,baseline=0]
	...
	\draw (0,0)coordinate(0) --node[below]{$l_1$} (\a: 1.75)coordinate(1) --node[above]{$l_2$} ++(\b:1.5)coordinate(2) --node[above right]{$l_3$} ++(\c:1)coordinate(3) --node[above]{$l_4$} ++(\d:1.25)coordinate(4);
...
Jder solche Punkt ist nicht-entartet und hat den Morse-Index $n - |\calJ|$.
\end{lemma}
