...
% das oben sorgt dafuer dass die dflatexmk engine fuer das Setzten verwendet wird
...
\begin{center}\begin{tikzpicture}[font=\scriptsize,scale=1]
	...
	\def\R{2.5}
	\coordinate (o) at (0,0);
	\coordinate (w1) at (1.5,0); \coordinate (w2) at (-0.5,1); \coordinate (w3) at (-1,-0.5);
	
	\foreach \i in {1,2,3} {
		\draw (o) -- (w\i);
	}
	\draw (w1) -- ++(-0.5,1) -- (w2) -- ++(-1,-0.5) -- (w3) -- ++(2,0) -- cycle;
	\foreach \i in {1,2,3} {
...
Leider erh"alt man im Allgemeinen \emph{nicht} die Topologie von $C_n\G$: Beispiel \textcolor{red}{[BILD]} $\leadsto v \X w$.
Es gilt zumindest
...
\begin{center}\textcolor{red}{[BILD]} $\sim (f,\emptyset)$ mit $f(e) = 2$, $f(v) = f(w) = 0$\end{center}
