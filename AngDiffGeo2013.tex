...
\marginnote{\begin{tikzpicture}[font=\scriptsize,scale=1]
	...
	\def\angle{120}
	\def\len{1}
	\draw (0,0)node[left]{$v$} --node[right]{$e_2$}node[pos=0.4]{$\X$} (30:\len)node[above]{$v_2$} (0,0) --node[above]{$e_3$} (30+\angle:\len)node[above]{$v_3$} (0,0) --node[right]{$e_1$} (30+2*\angle:\len)node[below]{$v_1$};
...
\begin{center}\begin{tikzpicture}[font=\scriptsize,scale=1]
	...
	\draw ($(1)!0.6!(2)$) -- (2)node[right]{$v_1 \X v_2$} --node[right]{$v_1 \X e_2$} (3)node[right]{$v_1 \X v$} -- ($(3)!0.4!(4)$);
	\draw[dashed] (1) -- (2) (3) -- (4);
...
\begin{center}$\G = $ \textcolor{red}{[BILD]}\end{center}
...
\end{itemize}
Dafiniere die Homotopie kantenweise, so dass Punkte in die vorgegebene Standardposition bewegt werden.
\end{emptythm}
