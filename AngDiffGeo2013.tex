...
\usepackage{arydshln} % gestrichelte Linien in Arrays
...
\marginnote{\begin{tikzpicture}[font=\scriptsize,scale=1,baseline=0]
	...
	\foreach \x in {0,1,2,3,4} {\fill (\x) circle(0.05);}
\end{tikzpicture}}
Die kritischen Punkte von $f_l$ sind genau die (kollinearen) Konfigurationen $p_{\calI}$ f"ur lange Teilmengen $\calI \subseteq \{1, \ldots, n\}$
($\sum_{i \in \calI} l_i > \sum _{j \notin \calI} l_j$ und $p_{\calI} = (u_1, \ldots, u_2 \theta_i + 2 \sum_{i n}$ mit $u_i = 1$ f"ur $i \in \calI$, $u_i = -1$ sonst).
Jeder solche kritische Punkt $p_{\calI}$ ist nicht ausgeartet und hat den morse-Index $n - |\calI|$.
