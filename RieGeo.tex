...
Wir definieren $\big(f\cdot V\big)(p) \da f(p) v_p$, wobei $p\in M$, $f\in C^\infty(M)$ sowie $\big(V+W\big)(p) \da v_p + w_p$. Damit ist $\mathcal{V}M$ (die Menge aller Vektorfelder auf $M$) ein $C^\infty(M)$-Modul.
\end{definition}
...
Nach dem Basissatz (Satz \ref{basissatz}) gilt: Jedes Vektorfeld $V\in \mathcal{V}M$ kann auf $U$ geschrieben werden als 
\[
...
Aus den Axiomen (T1), (T2) für Tangentialvektoren folgt, dass $V\in\mathcal V M$ eine Derivation ist.
...
Weiter gilt für alle $p\in M$: $\big(Vf\big)(p) = v_p\big(f\big) = \big(\mathcal D f\big) (p)$, also ist $Vf = \mathcal D f$, insbesondere ist $V$ glatt. Also entspricht $\mathcal V M$ den Derivationen auf $C^\infty M$.
...
\]
keine Derivation ist, da (D2) nicht erfüllt ist, also auch kein Vektorfeld!
...
\[[\cdot,\cdot]:
\begin{aligned}
\mathcal VM \times \mathcal VM &\to \mathcal VM \\
(V,W) &\mapsto [V,W]
\end{aligned}
\]
