...
\newcommand{\ksp}[2]{\left\langle#1,#2\right\rangle} % Konkretes Skalarprodukt mit zwei Parameter
...
\begin{aligned}
U &\to \MdR \\
q &\mapsto g_{ij}(q) \da \left\langle \left.\frac\partial{\partial x^i}\right|_q, \left.\frac\partial{\partial x^j}\right|_q\right\rangle_q
\end{aligned}
\]
...
$M=\MdR^n$ mit Atlas $\{\text{id}\}$ ist eine Riemann’sche Struktur mit dem Standard-Skalarprodukt $\asp$. Dabei ist $g_{ij}(p) = \ksp{\ptv{i}}{\ptv{j}} = \ksp{e_i}{e_j} = \delta_{ij}$, also ist $\left(g_{ij}(p)\right)$ die Einheitsmatrix.
...
\[
\ksp Z {D_YX} = \frac 12 \left( X \ksp Y Z + Y \ksp Z X  - Z \ksp X Y -\ksp {[X,Z]}Y - \ksp {[X,Y]} Z  - \ksp {[Y,Z]} X \right) \mathrlap{\quad (*)}
\]
...
\item Rotationszylinder: Schnittort ist die gegenüberliegende Gerade.
\item Flacher Torus $T^2 = S^1 \times S^1 = \faktor\MdR\MdZ \times \faktor\MdR\MdZ$: Schnittort ist der gegenüberliegende Kreis. $T^2$ kann dargestellt werden als Quadrat mit Seitenlänge 1, dessen gegenüberliegende Seiten miteinander identifiziert werden. Der Rand des Quadrats ist der Schnittort.
\end{enumerate}
\end{beispiel}
