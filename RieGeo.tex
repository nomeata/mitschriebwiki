...
\title{Riemann’sche Geometrie (unvollständig)}
\makeindex
...
\begin{definition}
Ein Vektorfeld $Y$ längs einer Kurve $c$ heißt parallel\index{parallel} bezüglich einem affinen Zusammenhang $D$, falls $D_{c'}Y = 0$.
\end{definition}

\begin{beispiele}
\item Im $\MdR^n$ haben wir $D_{c'}Y = \sum_{j=1}^n {w^j}'X_j = \sum_{j=1}^n 0 X_j= 0$, da bei Vektorfeldern in $\MdR^n$ parallel und konstant gleichwertig ist.
...
{\Huge Mangels Interesse ist kein weitere Mitschrieb online verfügbar. Falls ich mich da geirrt habe, bitte ich um Nachricht! -- Joachim}
