...
\[ \operatorname{Inj(p)} \da \operatorname{dist}(p,\cut(p))\,. \]
Der Injektivitätsradius von $M$ ist dann
\[ \operatorname{Inj(M)} \da \inf_{p\in M} \operatorname{Inj}(p)\,.\]
\end{definition}

\begin{beispiel}
$\operatorname{Inj}(S^n) = \pi$ und $\operatorname{Inj}(p) = \pi$ für jeden Punkt $p \in S^n$.
\end{beispiel}

%$2\cdot \operatorname{Inj}(p)$ ist die Länge der kürzesten von $p$ ausgehenden geschlossenen Geodätischen, falls es diese gibt.
