...
\[
\vol(G) \da \int_{\varphi(G)} \sqrt{\det g_{ij}(\varphi^{-1}(x))} dx^1\ldots dx^n \ad \int_{\varphi(G)}d\vol
\] mit $g_{ij}(p) = \ksp{\frac{\partial}{\partial x^i}|_p}{\frac{\partial}{\partial x^j}|_p}$.
\end{definition}

Nach der Substitutionsregel für Integrale gilt: $\vol(G)$ ist unabhängig von der gewählten Karten.
