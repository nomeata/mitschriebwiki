...
\renewcommand{\thesection}{{\rm §}\,\thechapter.\arabic{section}}
...
$\partial A\da\bar A\setminus \mathring A.$
\end{defini}

\begin{defini}{Dichtheit, Diskretheit} \label{dicht}
Es sei $X$ ein topologischer Raum. Eine Teilmenge $D\subseteq X$ heißt 
...
\begin{defini}{\bf Quotiententopologie} \label{Quotienten}

{\rm Es sei $X$ ein topologischer Raum und $\equiv$ eine Äquivalenzrelation
auf $X.$ Dann wird auf dem Raum $X/\equiv$ der Äquivalenzklassen von $X$ 
...
\begin{bem}{\bf Beispielmaterial} \label{einige Kompakta} 
...
\item[b)]
Ein metrischer Raum, in dem diese Äquivalenz nicht gilt, ist zum Beispiel
der folgende:
...
weitere Elemente aus $\dots U$, um $K\setminus U_\infty$ zu überdecken.

\end{itemize}
}
\end{bem}

\begin{defini}{\bf zusammenhängend}\label{Zusammenhang} 
...
\end{itemize}
Die Äquivalenzklasse ovn $x$ heißt die {\it Zusammenhangskomponente} von 
...
\begin{bem}{Kompakta in Hausdorffräumen}\label{fuer Liouville}
