...
\item \label{Satz10c}$\textrm{dim}\ R= \textrm{dim}\ S$
...
$\delta \leq d$.
\item $\textrm{dim}\ k[x_1, \ldots, x_d]=d$ ($\Rightarrow \textrm{dim}\ A = d$)
\end{enumerate}
\end{Satz}
...
$\textrm{dim}\ A_1=\textrm{dim}\ B_1\stackrel{I.V.}{=}\delta$
...
\textrm{Terme niedriger Ordnung in}\ a_n$$
...
$y_d$ teilt also $g^m$, d.h. $g^m\in (y_d)\stackrel{\textrm{prim}}{\Rightarrow} g\in (y_d)$
...
$$\textrm{trdeg}(\textrm{Quot}(A)) = \dim A$$

Dabei ist $\textrm{trdeg}(A)$ (der \emp{Transzendenzgrad}\index{Transzendenzgrad} von $K$ \"uber $k$) die Maximalzahl \"uber $k$ algebraisch unabh\"angiger Elemente in $K$.
