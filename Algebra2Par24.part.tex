...
in abelsche Gruppen $S_i$ hei\ss t \emp{graduierter Ring}\index{Ring!graduierter}, wenn f\"ur alle $i, j\in \mathbb{N}$
...
\item Ist $I$ homogenes Ideal in $S$, so ist $S/I$ graduierter Ring mit $(S/I)_i=S_i/(I\cap S_i)$
...
{\textrm{noethersch nach Satz 1}}/I$ f\"ur ein $n\geq 0$
...
sodass f\"ur alle $i\in\mathbb{N}, j\in\mathbb{Z}$  gilt:
...
\begin{nnBsp}
Sei $M$ graduierter $S$-Modul (z.B.: $M=S$). Für $d\in\mathbb{Z}$ sei
...
\begin{Bew}
  \begin{enumerate}
    ...
    $d$. Sei $x \in \mbox{ Kern}(\varphi), x = \sum_{i \in \mathbb{Z}} x_i
    \Rightarrow 0 = \varphi(x) = \sum_{i \in \mathbb{Z}} \underset{\in M'_{i +
    d}}{\underbrace{\varphi(x_i)}}$ ist Zerlegung in homogene Komponenten
    $\Rightarrow \varphi(x_i) = 0 \; \forall i \Rightarrow x_i \in \mbox{
    Kern}(\varphi) \; \forall i \Rightarrow \mbox{Kern}(\varphi)$ ist graduiert
  \end{enumerate}
\end{Bew}
...
\begin{Prop}
  ...
  $\mbox{dim }S_d^{(n)} = {n+d-1 \choose d} = \frac{1}{(n-1)!} \cdot (n+d-1)
  \cdot ... 
...
\begin{Bew}
  ...
  $\overset{IV}{\Rightarrow} \mbox{dim } S_d^{(n)} = {n+d-2 \choose d} + {n+d-2
...
\label{Satz6}
  ...
  graduierter $R$-Modul. Dann gibt es ein Polynom $P_M \in \mathbb{Q}[T]$ vom
  Grad $\le n -1 $ und ein $d_0 \in \mathbb{N}$, sodass $P_M(d) = \mbox{dim}_k \;
  M_d$ für alle $d \ge d_0$. $P_M$ heißt das
...
\begin{Bew}
  ...
  >> 0, P_M = 0$ tut's.\\
  $n>1$: Sei $\varphi: M \to M$ die $S$-lineare Abbildung $x \mapsto X_n x,
  ...
  Dann ist \[ 0 \to \underset{=\mbox{ \scriptsize
  Kern}(\varphi)}{\underbrace{K}} \to M(-1) \overset{\varphi}{\to} M \to M/X_n M
  \to 0 \]
  ...
  $\Rightarrow$ für jedes $d \in \mathbb{Z}$ gilt: dim$_k \; K_d -
  \mbox{dim}_k\;M(-1)_d + \mbox{dim}_k\; M_d - \mbox{dim}_k\;(M/X_n M)_d = 0$
  bzw. $\mbox{dim}_k\;M_d - \mbox{dim}_k\; M_{d-1} = \mbox{dim}_k\; (M/X_n M)_d
  - \mbox{dim}_k\; K_d$\\
  \textbf{Beh.:} $M/X_n M$ und $K$ sind (in natürlicher Weise) $k[X_1, \dots,
  X_n]$-Moduln.\\
  \textbf{Bew.:} klar für $X/X_n M$.\\
  für $K$: Seien $y_1, \dots, y_r$ Erzeuger von $K$ als $S$-Modul. Sei $y =
  \sum_{i = 1}^r f_i y_i \in K, f_i \in S$.
  Dann ist O\!\!E $f_i \in k[X_1, \dots, X_{n-1}], da X_n \cdot y = 0$ für alle
  $i$. Nach IV gibt es $\tilde{P} \in \mathbb{Q}[T]$ mit deg$(\tilde{P})
  \le n-2$ und $\tilde{P} = \mbox{dim}_k\; (M/X_n M)_d - \mbox{dim}_k\; K_d =
  \mbox{dim}_k\; M_d - \mbox{dim}_k\; M_{d-1} \defeql H(d) - H(d-1)$.\\
  Sei ${T \choose k} \defeqr \frac{1}{k!} T (T-1) \dots (T-k+1) \in
  \mathbb{Q}[T], \mbox{deg}{T \choose k} = k$.
  Schreibe $\tilde{P} = \sum_{k = 0}^{n-1} c_k {T \choose k}$. Es gilt ${T
  \choose k} - {T-1 \choose k-1} = {T \choose k+1}$. Setze $P_1(T) \defeqr
  \sum_{k=0}^{n-2} c_k {T \choose k+1}, \mbox{ deg}P_1 \le n-1$ und
  $P_1(d)-P_1(d-1) = \tilde{P}(d)$. $P_M \defeqr P_1 +c$, sodass $P_M(d_0) = \mbox{dim}_k\;M_{d_0}$.
\end{Bew}

\begin{Def}
\label{2.18}
  Sei $S$ endlich erzeugte graduierte $K$-Algebra, $S_0 = k,\; M$ endlich
  erzeugbarer $S$-Modul. Dann heißt die formale Potenzreihe $$H_M(t) \defeqr
  \sum_{i=0}^{\infty} (\mbox{dim}_k \; M_i) t^i$$
  \emp{Hilbert-Reihe}\index{Hilbert!-Reihe} zu $M$.
\end{Def}

\begin{nnBsp}
  \begin{enumerate}
    \item[1.)] $M = S = k[X] \Rightarrow \mbox{dim }M_i = 1 \mbox{ für alle }i
    ...
    \item[3.)] $M = S = k[Y] (\cong k[X^d]), \mbox{deg }Y = d > 0 \Rightarrow
               H_M(t) = \sum_{i = 0}^{\infty} t^{d \cdot i} = \frac{1}{1-t^d}$\\
               dim $M_i = \begin{cases}1: & d \mid i\\ 0: & \mbox{sonst} \end{cases}$
  \end{enumerate}
\end{nnBsp}
...
\begin{Bew}
  Induktion über $r$:\\
  $r=0$: $S = S_0 = k \Rightarrow \mbox{dim}_k \; M_i = 0$ für $i >> 0
  ...
  \ge 0$ ist
  $$ -\mbox{dim } M_i + \mbox{dim } M_{i + d_r} = \mbox{dim } Q_{i+d_r} -
  \mbox{dim } K_i$$
  $$\Rightarrow \sum_{i=0}^{\infty} \mbox{dim } M_{i+d_r} t^{i+d_r} - t^{d_r}
  \sum_{i=0}^{\infty} \mbox{dim }M_i t^i = \sum_{i=0}^{\infty} \mbox{dim }
  Q_{i+d_r} t^{i+d_r} - \sum_{i=0}^{\infty} \mbox{dim }K_i t^i$$
  $$\Rightarrow H_M(t) - \sum_{i=0}^{d_r -1} \mbox{dim }M_i t^i - t^{d_r} H_M(t)
  = H_Q(t) - \sum_{i=0}^{d_r - 1} \mbox{dim }Q_i t^i - t^{d_r} H_K(t)$$
  Nach Induktionsvoraussetzung gibt es $F_1(t), F_2(t) \in \mathbb{Z}[t]$ mit
  $$(1-t^{d_1})H_M(t)= \frac{F_1(t)}{\displaystyle\prod_{i=0}^{r-1}(1-t^{d_i})}-
  \frac{t^{d_r} F_2(t)}{\displaystyle \prod_{i=0}^{r-1}(1-t^{d_i})} +
  \underset{G(t)}{\underbrace{\sum_{i=1}^{d_r-1} \mbox{dim }M_i t^i -
  \sum_{i=0}^{d_r - 1} \mbox{dim }Q_i t^i}}$$
