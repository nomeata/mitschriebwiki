...
\begin{Prop}
  ...
  $\mbox{dim }S_d^{(n)} = \binom{n+d-1}{d} = \frac{1}{(n-1)!} \cdot (n+d-1)
  \cdot ... 
...
\begin{Bew}
  ...
  $\overset{IV}{\Rightarrow} \mbox{dim } S_d^{(n)} = \binom{n+d-2}{d} +
...
\label{Satz6}
  ...
  graduierter $S$-Modul. Dann gibt es ein Polynom $P_M \in \mathbb{Q}[T]$ vom
...
\begin{Bew}
  ...
  Dann ist O\!\!E $f_i \in k[X_1, \dots, X_{n-1}]$, da $X_n \cdot y = 0$ für
  alle $i$. Nach IV gibt es $\tilde{P}(d) \in \mathbb{Q}[T]$ mit deg$(\tilde{P})
...
\label{2.18}
  ...
  \defeqr \sum_{i=0}^{\infty} (\mbox{dim}_k \; M_i) t^i$$
...
\begin{Bew}
  Induktion über $r$:\\
  $r=0$: $S = S_0 = k \Rightarrow \mbox{dim}_k \; M_i = 0$ für $i \gg 0
  ...
  Es folgt
  \[
  \sum_{i=0}^{\infty} \mbox{dim } M_{i+d_r} t^{i+d_r} - t^{d_r}
  \sum_{i=0}^{\infty} \mbox{dim }M_i t^i = \sum_{i=0}^{\infty} \mbox{dim }
  Q_{i+d_r} t^{i+d_r} - t^{d_r}\sum_{i=0}^{\infty} \mbox{dim }K_i t^i
  \]
  \[
