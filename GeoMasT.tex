...
$$
d.h. \(\mu^s(M) = \mu^s(B)\), was die Borel-Regularit\"at ergibt.
\end{beweis}
...
\begin{definition}
Seien \((X,d)\) und \((\bar X,\bar d)\) metrische Räume. Eine Abbildung \(f: X \rightarrow \bar X\) heißt Isometrie, falls \(\bar d(f(x),f(y)) = d(x,y)\)f\"ur alle $ x,y\in X$. 
\par
...
\]
Man erklärt \(\Psi^{\ast k} \da \underbrace{\Psi^\ast \circ \dots \circ \Psi^\ast}_{k \text{ mal}}\) f\"ur  $k\in\MdN$. Offenbar gilt \(M \subset M' \Rightarrow \Psi^{\ast k}(M) \subset \Psi^{\ast k}(M')\).
\par
...
\[
\psi_{i_1}\circ \cdots \circ \psi_{i_k}(x_j) = \psi_{i_1}\circ \cdots \psi_{i_k}\circ \psi_j (x_j) \in \Psi^{*(k+1)}(F).
\]
Ferner gilt 
\begin{align*}
...
\]
Sei $s\in E$. Nach der Definition von $E$ gibt es ein $p\in\MdN$, ohne Beschränkung der Allgemeinheit $p\ge k+1$, und ein $y\in\Psi^*(F)$ mit $d(x,y)<\frac\ep2$. Also gibt es ein $i_1,\ldots,i_p\in \{1,\ldots,m\}$ und $j\in\{1,\ldots,m\}$ mit $y = \psi_{i_p}\circ\cdots\circ\psi_{i_1}(x_j)$. Wiederholte Anwendung der Dreiecksungleichung ergibt
\begin{align*}
d(x,\Psi^{*k}(F))
...
\end{align*}
Dies zeigt, dass $E$ total beschränkt ist. Da ferner $E$ als abgeschlossene Menge in einem vollständigen metrischen Raum selbst vollständig ist, ist $E$ sogar kompakt.

Hiermit zeigen wir, dass $E\subset \Psi^*(E)$. Sei $z\in E$. Es existieren dann $z_i \in \bigcup_{k=1}^\infty \Psi^{*k}(F)$ mit $z_i \to z$. Hierbei ist $z_i = \psi_{l(i)}(y)$ mit $y_i\in \bigcup_{k=1}^\infty \Psi^{*k}(F) = \bigcup_{k=1}^\infty \Psi^{*k}(F) \subset E$ und $l(i)\in\{1,\ldots,m\}$. Es existieren Teilfolgen von $(y_i)_{i\in\MdN}$, ohne Beschränkung der Allgemeinheit die Folge selbst, mit $y_i \to y\in E$. Ferner gibt es ein $l\in\{1,\ldots,m\}$ mit $l(i)=l$ für unendlich viele $i\in\MdN$. Hieraus folgt: $z \leftarrow z_i = \psi_l(y_i) \to \psi_l(y)$. Daher ist $z = \psi_l(y) \in \Psi^*(E)$, das heißt $E\subset \Psi^*(E)$.
...
\item Es gibt eine beschränkte, offene Menge $\emptyset \ne U\subset\MdR^n$ derart, dass $\psi_1(U),\ldots,\psi_m(U)$ paarweise disjunkt und $\Psi^*(U) \subset U$.
\end{enumerate}
Dann gilt für die durch $\sum_{i=1}^m r_i^s = 1$ festgelegte Zahl $s>0$ und das $\Psi^*$-invariante Kompaktum $E\subset \MdR^n$ die Abschätzung: $0<\psi^s(E) < \infty$, das heißt insbesondere $\dim_H(E)= s$. Man nennt $s$ die Ähnlichkeitsdimension von $R$.
\end{satz}
...
&> (1-r) - r \ge 0
\end{align*}
das heißt $(\psi_\alpha(x))_k \ne (\psi_\beta(x))_k$. Ferner gilt $|\psi_\alpha(x) - \psi_\beta(x)| = r |x-y|$, $x,y\in \MdR^n$.  Nach Satz \ref{satz:2.25} folgt für ide $\Psi^*$-invariante Menge $E$: $0<\mu^s(E)<\infty$. Genauer: Wegen $r_1=\ldots,r_{2^n}=r\in(0,\frac12)$ ist $1 = \sum_{r=1}^{2^n} r_i^s = 2^n \cdot r^s = 1$ und damit
\[
s = -\frac{n \ln 2}{\ln r}.
\]
