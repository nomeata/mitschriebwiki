...
%Dort ist ein \emph{Wiki} eingerichtet und von Joachim Breitner um die \LaTeX-%
...
\newcommand{\ind}{\mathds 1}
\DeclareMathOperator{\im}{im}
% MaÃŸ-Restriktions-Symbol
\newcommand{\MR}{\lfloor}

\pdfinfo{
	/Author (Die Mitarbeiter von http://mitschriebwiki.nomeata.de/)
	/Title   (Geometrische MaÃŸtheorie)
	/Subject (Geometrische MaÃŸtheorie)
}

\author{PD. Dr. Daniel Hug}
\publishers{Die Mitarbeiter von \url{http://mitschriebwiki.nomeata.de/}}
\title{Geometrische MaÃŸtheorie}
\date{Wintersemester 2009/2010}
\makeindex
...
Dies ist ein Mitschrieb der Vorlesung \glqq Geometrische MaÃŸtheorie\grqq\
von Herrn PD. Dr. Daniel Hug im Wintersemester 2009/2010 an der UniversitÃ¤t Karlsruhe (TH).
 Die Mitschriebe der Vorlesung werden mit ausdrÃ¼cklicher Genehmigung von Herrn Hug hier verÃ¶ffentlicht,
Herr Hug ist fÃ¼r den Inhalt nicht verantwortlich.

\section*{Wer}
Beteiligt am Mitschrieb ist Joachim Breitner.

\section*{Wo}
Alle Kapitel inklusive \LaTeX-Quellen kÃ¶nnen unter \url{http://mitschriebwiki.nomeata.de} abgerufen werden.
Dort ist ein \emph{Wiki} eingerichtet und von Joachim Breitner um die \LaTeX-Funktionen erweitert.
Das heiÃŸt, jeder kann Fehler nachbessern und sich an der Entwicklung
beteiligen. Auf Wunsch ist auch ein Zugang Ã¼ber \emph{Subversion} mÃ¶glich.

\chapter{Grundlagen: MaÃŸ und Integral}

\section{Ã„uÃŸere MaÃŸe und MeÃŸbarkeit}
...
\]
heiÃŸt \emph{Ã¤uÃŸeres MaÃŸ} auf $X$, falls gilt:
\begin{enumerate}
\item $\mu(\emptyset) = 0$
\item FÃ¼r $A,A_n \subset X$, $i\in \MdN$ mit $A\subset \bigcup_{i\ge 1} A_i$ gilt 
\[
...
FÃ¼r die Konstruktion eines Ã¤uÃŸeren MaÃŸes aus Rohdaten ist folgender Satz nÃ¼tzlich:
...
\]
($\inf\emptyset = \infty$) fÃ¼r $A\subset X$ ein Ã¤uÃŸeres MaÃŸ erklÃ¤rt, das von $(\mathcal E, \eta)$ induzierte Ã¤uÃŸere MaÃŸ.
\end{satz}
...
Seien $A, A_i\subset X$ und $A\subset \bigcup_{i\ge1}A_i$. Wir mÃ¼ssen zeigen: $\mu(A) \le \sum_{i\ge 1}\mu(A_i)$.

Ist fÃ¼r ein $i\in\MdN$ bereits $\mu(A_i)=\infty$, so sind wir fertig. Sei also $\mu(A_i)<\infty$ fÃ¼r alle $i\in\MdN$. Sei $\varepsilon>0$. Dann existiert $E_{ij}\in \mathcal E$, $j\in \MdN$ mit $A_i\subset \bigcup_{j\ge 1} E_{ij}$ und
\[
\mu(A_i) + \frac{\varepsilon}{2^i}\ge \sum_{j\ge 1}\eta(E_{ij}) \quad \text{fÃ¼r $i\in \MdN$}
\]
Also gilt
\[
...
\begin{definition}
Sei $\mu$ ein Ã¤uÃŸeres MaÃŸ auf $X$. Eine Menge $A\subset X$ heiÃŸt $\mu$-messbar, falls fÃ¼r alle $M\subset X$ gilt:
\[
...
Es genÃ¼gt bereits: $A$ ist $\mu$-messbar genau dann, wenn fÃ¼r alle $M\subset X$ gilt:
\[
...
\begin{bemerkung}
FÃ¼r $Y\subset X$ ist $\mu \MR Y$ das durch
\[
(\mu \MR Y)(M) \da \mu(M\cap Y),\quad M\subset X
\]
erklÃ¤rte Ã¤uÃŸere MaÃŸ. Ferner ist $\A_\mu \subset \A_{\mu \MR Y}$. Denn fÃ¼r $A\in\A_\mu$ und $M\subset X$ ist
\begin{align*}
\mu_{\MR Y}(M)
...
\begin{proposition}
FÃ¼r ein Ã¤uÃŸeres MaÃŸ $\mu$ auf $X$ gelten die folgenden Aussagen:
\begin{enumerate}[a)]
\item $\emptyset,\, X\in \A_\mu$ sowie $A\in \A_\mu \iff A^c\in\A_\mu$.
\item FÃ¼r $A\subset X$ mit $\mu(A)=0$ gilt $A\in\A_\mu$.
\item FÃ¼r $A_i\in\A_\mu$, $i\in\MdN$ gilt $\bigcup_{i\ge1} A_i\in\A_\mu$ und $\bigcap_{i\ge1} A_i\in\mathcal A_\mu$.
\item FÃ¼r $A\in\A_\mu$, $B\subset X$ gilt
\[
\mu(A\cap B) + \mu(A\cup B) = \mu(A) + \mu(B).
\]
\item FÃ¼r $A_i\in\A_\mu$, $i\in\MdN$, paarweise disjunkt, gilt
\[
\mu(\bigcup_{i=1}^\infty A_i) = \sum_{i=1}^\infty \mu(A_i).
\]
\item FÃ¼r $A_i\in \A_\mu$, $i\in\MdN$ und $A_i\subset A_{i+1}$ fÃ¼r alle $i\in\MdN$ gilt
\[
\mu(\bigcup_{i=1}^\infty A_i) = \lim_{i\to\infty}\mu(A_i).
\]
\item FÃ¼r $A_i \in \A_\mu$, $i\in \MdN$ mit $\mu(A_1) < \infty$ und $A_i\supset A_{i+1}$ fÃ¼r alle $i\in\MdN$ gilt:
\[
...
\end{align*}
Daraus folgt $A_1\cup A_2 \in \A_\mu$. Per Induktion sieht man dann, dass fÃ¼r $A_1,\ldots,A_n\in\A_\mu$ gilt: $\bigcup_{i=1}^n A_i \in \A_\mu$.
...
\item[f)] Wir definieren $B_1 \da A_1$, $B_2 \da A_2\setminus A_1$, $B_3 \da A_3\setminus A_2\ldots$ Nun ist $B_i\in \A_\mu$ fÃ¼r alle $i\in\MdN$ und die $B_i$ sind paarweise disjunkt. Es folgt
\begin{align*}
...
Sei ohne BeschrÃ¤nkung der Allgemeinheit $\mu(M) <\infty$. Dann gilt
\begin{align*}
...
\item[d)] FÃ¼r $A\in\A_\mu$ und $B\subset X$ gilt:
\begin{align*}
\mu(A\cup B) &= \mu( (A\cup B) \cap A) + \mu( (A\cup B)\cap A^c) \\
&= \mu(A) + \mu(B\cap A^c)\\
\intertext{sowie}
\mu(B) &= \mu(B\cap A) + \mu(B\cap A^c).
\intertext{Hiermit so erhÃ¤lt man}
...
Hinweis: Es ist $\A_\mu$ eine (bezÃ¼glich $\mu$ vollstÃ¤ndige) $\sigma$-Algebra und $\mu$ ist ein $\sigma$-additives MaÃŸ auf $\A_\mu$, wobei â€ž$\A_\mu$ ist $\mu$-vollstÃ¤ndigâ€œ heiÃŸt, dass jede $\mu$-Nullmenge in $\A_\mu$ liegt. $(X,\A_\mu)$ ist ein messbarer Raum und $(X,\A_\mu,\mu)$ ist ein MaÃŸraum.

\begin{definition}
Sei $\A$ eine $\sigma$-Algebra auf $X$. Ein Ã¤uÃŸeres MaÃŸ $\mu$ auf $X$ heiÃŸt $\A$-regulÃ¤r, falls $\A\subset \A_\mu$ gilt und zu jeder Menge $M\subset X$ ein $A\in\A$ existiert mit $M\subset A$ und $\mu(M) = \mu(A)$. Das Ã¤uÃŸere MaÃŸ $\mu$ heiÃŸt regulÃ¤r, falls $\mu$ ein $\A_\mu$-regulÃ¤res MaÃŸ ist.
\end{definition}

\begin{proposition}
Sei $\A$ eine $\sigma$-Algebra in $X$, $\mu$ ein $\A$-regulÃ¤res Ã¤uÃŸeres MaÃŸ auf $X$. Dann gilt:
\begin{enumerate}[a)]
\item Ist $M_i\subset X$, $M_i\subset M_{i+1}$ fÃ¼r alle $i\in\MdN$, so ist
\[
\mu(\bigcup_{i\ge 1}M_i) = \lim_{i\to\infty}\mu(M_i).
\]
\item Zu jedem $M\subset X$ mit $\mu(M)<\infty$ existiert ein $A\in\A$, so dass fÃ¼r alle $B\in \A_\mu$ gilt:
\[
...
\end{align*}
woraus Gleichheit in obigern Ungleichungen folgt. Wegen $\mu(M)<\infty$ ist auch $\mu(M\cap B^c)<\infty$, und wir kÃ¶nnen dies von zwei obigen Termen abziehen und erhalten
\[
\mu(M\cap B) = \mu(A\cap B).
\]
\item  Zu $M_1$ existiert $\tilde A_1\in\A$ mit $M_1 \subset \tilde A_1$ und $\mu(M_1) = \mu(\tilde A_1)$. Wir definieren $A_1 \da \tilde A_1 \cap (M_1\cup M_2)$. FÃ¼r diese Menge gilt nun $M_1\subset A_1 \subset M_1\cup M_2$. Wir folgern 
\[
...
\]
also ist $A\in\A_\mu$. Sei nun $M\subset X$ beliebig und ohne BeschrÃ¤nkung der Allgemeinheit $\mu(M)<\infty$. Es existiert also eine Folge $A_i\in\A$, $i\in\MdN$ mit $M\subset A_i$ und $\nu(A_i) \to \mu(M)$. Setze $A\da \bigcap_{i\ge 1} A_i$. Dann gilt $A \in \A$, $M\subset A$, sowie
\begin{align*}
\mu(M) = \lim_{i\to\infty} \nu(A_i) \ge \nu(\bigcap_{i=1}^\infty A_i) = \mu(\bigcap _{i=1}^\infty A_i) = \mu(A) \ge \mu(M),
\end{align*}
woraus $\mu(M) = \mu(A)$ folgt.

Sei schlieÃŸlich $M\in\A_\mu$ mit $\mu(M)<\infty$. Es gibt ein $A\in\A$ mit $M\subset A$ und $\mu(M) = \mu(A)<\infty$. Dann folgt
\begin{align*}
...
\begin{anwendung}
Sei $\vartheta$ ein beliebiges Ã¤uÃŸeres MaÃŸ auf $X$. Dann ist $\vartheta|_{\A_\vartheta}$ ein MaÃŸ. Durch
\[
\mu(M) \da \inf\{\vartheta(A) : A\in\A_\vartheta,\, M\subset A\}
\]
wird also ein $\A_\vartheta$-regulÃ¤res Ã¤uÃŸeres MaÃŸ auf $X$ erklÃ¤rt, das $\vartheta$ fortsetzt (also $\mu|_{\A_\vartheta} = \vartheta|_{\A_\vartheta}$).
\end{anwendung}

\begin{definition}
Seien $X,Y$ Mengen, $\mu$ ein Ã¤uÃŸeres MaÃŸ auf $X$ und $f:X\to Y$. Dann wird durch
\[
(f\mu)(M) \da \mu(f^{-1}(M))
\]
fÃ¼r $M\subset Y$ ein Ã¤uÃŸeres MaÃŸ $f\mu$ auf $Y$ erklÃ¤rt. Man nennt $f\mu$ das \emph{Bild} von $\mu$ unter $f$ oder auch \emph{â€žpush forwardâ€œ} von $\mu$ bezÃ¼glich $f$ und schreibt hierfÃ¼r auch $f_\#\mu$.
\end{definition}

\begin{bemerkung}
FÃ¼r $B\subset Y$ gilt
\[
...
\begin{sprechweisen}
Sei $\mu$ ein Ã¤uÃŸeres MaÃŸ auf $X$. Eine Menge $N\subset X$ heiÃŸt $\mu$-Nullmenge, falls $\mu(N)=0$. Eine Eigenschaft $\mathcal E$ gilt fÃ¼r $\mu$-fast-alle $x\in X$ bzw. $\mu$-fast-Ã¼berall, falls 
\[
\mu(\{x\in X : \mathcal E\text{ gilt fÃ¼r $x$ nicht}\}) = 0.
\]
Mit $\mathbb F_\mu(X,Y)$ wird die Menge aller Abbildungen $f:D\to Y$ bezeichnet mit $D\subset X$ und $\mu(X\setminus D) = 0$.
\end{sprechweisen}

\begin{definition}
Seien $X,Y$ Mengen und $\mu$ ein Ã¤uÃŸeres MaÃŸ auf $X$ und $\mathcal C$ eine $\sigma$-Algebra in $Y$. Dann heiÃŸt $f\in\mathbb F_\mu(X,Y)$ $\mu$-messbar bezÃ¼glich $\mathcal C$, falls $f^{-1}(\mathcal C)\subset \A_\mu$.
\end{definition}

Beachte, dass fÃ¼r $f:D\to Y$ mit $\mu(X\setminus D)=0$ gilt: $D=f^{-1}(Y)\in \A_\mu$.

\begin{lemma}
Seien $X,Y$ Mengen, $\mu$ ein Ã¤uÃŸeres MaÃŸ auf $X$ und $\mathcal E\subset \mathcal P(Y)$.  FÃ¼r $f\in\mathbb F_\mu(X,Y)$ sind Ã¤quivalent:
\begin{enumerate}[a)]
\item $f^{-1}(\mathcal E) \subset \A_\mu$
\item $f$ ist $\mu$-messbar bezÃ¼glich $\sigma(\mathcal E)$.
\end{enumerate}
\end{lemma}
...
\begin{definition}
Sei $X$ eine Menge, $\mu$ ein Ã¤uÃŸeres MaÃŸ auf $X$ und $f\in\mathbb F_\mu(X,\bar\MdR)$. Man nennt $f$ eine $\mu$-messbare Abbildung, falls $f$ dies bezÃ¼glich $\mathfrak B(\bar\MdR)$ ist.
\end{definition}

Im Folgenden schreiben wir fÃ¼r eine Relation $\varrho$ auf $\bar\MdR$, Mengen $D, D'\subset X$ und Abbildungen $f: D\to \bar\MdR$, $g:D'\to\bar\MdR$:
\[
\{ f\mathrel{\varrho} g\} \da \{ x\in D\cap D' : f(x)\mathrel{\varrho} g(x) \}
\]

\begin{lemma}
Sei $\mu$ ein Ã¤uÃŸeres MaÃŸ auf $X$ und $f\in \mathbb F_\mu(X,\bar\MdR)$. Genau dann ist $f$ eine $\mu$-messbare Abbildung, wenn eine der folgenden Bedingungen fÃ¼r alle $a\in\MdR$ erfÃ¼llt ist:
\begin{align*}
...
\begin{lemma}
Sei $\mu$ ein Ã¤uÃŸeres MaÃŸ auf $X$, seien $f,g,f_n\in \mathbb F_\mu(X,\bar\MdR)$, $n\in\MdN$, $\mu$-messbar. Dann gilt
\begin{enumerate}[(a)]
\item $\{f<g\}$, $\{f\le g\}$, $\{f=g\}$, $\{f\ne g\}$ sind $\mu$-messbare Mengen.
\item Die Funktionen
\begin{align*}
&f+ g, && f-g, && f \cdot g \text{ (falls $\mu$-fast-Ã¼berall definiert)}, \\
&\sup_n f_n, && \inf_n f_n, && \\
...
\begin{satz}
Ist $\mu$ ein Ã¤uÃŸeres MaÃŸ auf $X$, so ist $f\in \mathbb F_\mu(X,\bar\MdR)$ genau dann $\mu$-messbar, wenn fÃ¼r alle $M\subset X$, $a,b\in\MdR$ mit $a<b$ gilt
\[
...
\begin{beweis}
Sei $f$ zunÃ¤chst $\mu$-messbar. Dann gilt mit $a<b$, $M\subset X$: 
\begin{align*}
\mu(M) &\ge \mu(M\cap\{f\le a\} ) + \mu(M\cap \{f> a\}) \\
&\ge \mu(M\cap\{f\le a\} ) + \mu(M\cap \{f\ge b\}) 
\end{align*}

Jetzt gelte die Bedingung des Satzes fÃ¼r alle $M\subset X$, $a<b$. Zu zeigen ist: $\{f\le r\}\in \A_\mu$ fÃ¼r beliebige $r\in\MdR$. Sei $M\subset X$ beliebig mit $\mu(M) <\infty$. FÃ¼r $i\in \MdN$ sei
\[
A_i \da M\cap \{r + \frac1{i+1} \le f \le r+\frac 1i\}.
\]
Wir zeigen mit vollstÃ¤ndiger Induktion, dass 
\[
\mu(\bigcup_{i=0}^n A_{2i+1})  \ge \sum_{0=1}^n \mu(A_{2i+1})
\]
gilt.

FÃ¼r $n=0$ ist dies klar. Die Ungleichung gelte fÃ¼r ein $n\in\MdN$.
\begin{align*}
...
\]
und erhÃ¤lt zusammen
\[
\sum_{i=1}^\infty \mu(A_i) \le 2 \mu(M) <\infty.
\]

Sei $\ep>0$. Dann gibt es ein $n\in\MdN$ mit $\sum_{i\ge n}\mu(A_i) <\ep$. ZunÃ¤chst schÃ¤tzen wir ab
\begin{align*}
...
\begin{satz}
Seien $\mu$ ein Ã¤uÃŸeres MaÃŸ auf $X$, $f: X \to [0,\infty]$ eine $\mu$-messbare Abbildung und $(r_n)_{n\in\MdN}$ eine Folge in $[0,\infty)$ mit $\lim_{n\to\infty} r_n = 0$ und $\sum_{n=1}^\infty r_n = \infty$. Dann gibt es eine Folge $(A_n)_{n\in\MdN}$ $\mu$-messbarer Mengen mit
\[
...
\textbf{Behauptung:} Es ist $f \ge \sum_{i=1}^n r_i \ind_{A_i}$, $n\in\MdN$. Dies gilt fÃ¼r $n=1$, und wenn es fÃ¼r ein $n\in\MdN$ gilt, dann folgt: Ist $x\notin A_{n+1}$, dann ist
\begin{align*}
...
\textbf{Annahme:} Es gelte $f(x) > \sum_{i=1}^\infty r_i \ind_{A_i}(x)$ fÃ¼r ein $x\in X$.

Also ist $\sum_{i=1}^\infty r_i \ind_{A_i}(x)<\infty$. Da $\sum_{i=1}^\infty r_i = \infty$ gilt, muss es eine Folge natÃ¼rlicher Zahlen $(j_k)_{k\in\MdN}$ geben mit $\ind_{A_{j_k}}(x)=0$ fÃ¼r alle $k\in\MdN$.
...
In diesem Abschnitt wird generell vorausgesetzt, dass $X$ eine Menge und $\mu$ ein Ã¤uÃŸeres MaÃŸ auf $X$ ist.

\begin{definition}
Eine $\mu$-Treppenfunktion auf $X$ ist eine $\mu$-messbare Abbildung $h\in \mathbb F_\mu(X,\MdR)$ mit abzÃ¤hlbarer Wertemenge $\im(h)$ und
\[
...
\]
das $\mu$-Integral von $h$ erklÃ¤rt, wobei â€ž$0\cdot \infty\da 0$â€œ gelte.
\end{definition}
...
\begin{beweis}
Es gilt zunÃ¤chst $h+g \in \mathbb F_\mu(X,\MdR)$. Zur AdditivitÃ¤t: Wir definieren $A(r,s) \da \{h=r\} \cap \{g=s\}$ fÃ¼r $r,s\in\MdR$. Die Voraussetzungen des Lemmas stellen sicher, 
...
\begin{bemerkung}
Sei $h$ eine $\mu$-Treppenfunktion mit $h\ge 0$, dann gilt $\int hd\mu \ge 0$. Mit Lemma \ref{lem1.10} folgt fÃ¼r $\mu$-Treppenfunktionen $h,g$:
\[
...
\begin{definition}
Sei $f\in\mathbb F_\mu(X,\bar\MdR)$. Eine $\mu$-Oberfunktion (bzw. $\mu$-Unterfunktion) von $f$ ist eine $\mu$-Treppenfunktion $h$ auf $X$ mit $f\le h$ $\mu$-fast-Ã¼berall auf $X$ (bzw. $h\le f$ $\mu$-fast-Ã¼berall auf $X$).

Durch
\[
\int^* fd\mu \da \inf\left\{\int hd\mu : \text{$h$ ist eine $\mu$-Oberfunktion von $f$}\right\}
\]
wird das $\mu$-Oberintegral von $f$ erklÃ¤rt.
Analog wird durch
\[
\int_* fd\mu \da \sup\left\{\int hd\mu : \text{$h$ ist eine $\mu$-Unterfunktion von $f$}\right\}
\]
das $\mu$-Unterintegral von $f$ erklÃ¤rt.
\end{definition}

\begin{lemma}
\label{lem1.11}
FÃ¼r $f,g\in\mathbb F_\mu(X,\bar\MdR)$ gelten die folgenden Aussagen:
\begin{enumerate}
\item $\int_* fd\mu = - \int^* (-f)d\mu$.
\item Gilt $\mu$-fast-Ã¼berall $f\le g$, so ist $\int^* fd\mu \le \int^* gd\mu$ und $\int_* fd\mu \le \int_* gd\mu$.
\item Gilt $\mu$-fast-Ã¼berall $f\ge 0$, so ist $\int^* fd\mu \ge 0$ und $\int_* fd\mu \ge 0$.
\item Gilt $\int^* fd\mu <\infty$, so auch $\int^*f^+d\mu<\infty$ und $f<\infty$ $\mu$-fast-Ã¼berall.
\item FÃ¼r $c\in(0,\infty)$ gilt $\int^*(cf)d\mu = c\cdot \int^*fd\mu$ und $\int_*(cf)d\mu = c\cdot \int_*fd\mu$.
...
\begin{bemerkung}
Ist $h$ eine $\mu$-Treppenfunktion, so ist $h$ eine $\mu$-Oberfunktion und eine $\mu$-Unterfunktion von $h$. Das heiÃŸt insbesondere
\[
...
\begin{definition}
Ist $f\in \mathbb F_\mu(X,\bar\MdR)$ eine $\mu$-messbare Abbildung und stimmt das $\mu$-Oberintegral mit dem $\mu$-Unterintegral von $f$ Ã¼berein, so wird durch
\[
\int fd\mu \da \int^* fd\mu = \int_* fd\mu
\]
das $\mu$-Integral von $f$ erkÃ¤rt. Man sagt in diesem Fall, dass das $\mu$-Integralvon $f$ existiert. Ist $\int fd\mu \in \MdR$, so heiÃŸt $f$ $\mu$-integrierbar.
\end{definition}
...
\begin{beweis}
Ist $\mu(\{f=\infty\})>0$, dann ist fÃ¼r jedes $n\in\MdN$ die Funktion $n\cdot \ind_{\{f=\infty\}}$ eine $\mu$-Unterfunktion von $f$ und 
\[
\int^*fd\mu \ge \int_* fd\mu \ge \int n \cdot\ind_{\{f=\infty\}} d\mu = n \cdot \mu(\{f=\infty\})\to \infty\text{ fÃ¼r }n\to\infty.
\]
Also ist $\int^* fd\mu = \int_* fd\mu = \infty$.

Sei jetzt $f<\infty$ $\mu$-fast-Ã¼berall. FÃ¼r $t\in(1,\infty)$ sei
\[
...
= \int_* (cf) d\mu .
\end{multline*}
\item Seien $f,g$ $\mu$-integrierbar. Dann ist $f,g<\infty$ $\mu$-fast-Ã¼berall, und somit ist $f+g\in \mathbb F_\mu(X,\bar\MdR)$. Ferner gilt
\begin{multline*}
\int fd\mu + \int gd\mu 
= \int^* fd\mu + \int^* gd\mu
\ge \int^*(f+g)d\mu \\
...
Sei nun $\int fd\mu = \infty$. Nach Voraussetzung gilt dann $\int gd\mu >-\infty$ und damit $g>-\infty$ $\mu$-fast-Ã¼berall. Das heiÃŸt $f+g\in\mathbb F_\mu(X,\bar\MdR)$. Ferner gilt
\begin{align*}
...
\item Sei $\int gd\mu <\infty$, das heiÃŸt $f\le g <\infty$ $\mu$-fast-Ã¼berall. Dann ist 
$(f-g)\ind_{\{g>-\infty\}}\in F_\mu(X,\bar\MdR)$ nicht positiv. Wegen $f\le g <\infty$ $\mu$-fast-Ã¼berall 
...
\begin{beweis}
Sei $\varepsilon \in (0,1)$. Sei $h$ eine $\mu$-Unterfunktion von $\liminf_{n\to\infty} f_n$ mit $\im(h) = \{r_1,\ldots,r_m\}\subset [0,\infty)$ (vergleiche Satz \ref{satz1.12}). FÃ¼r $i=1,\ldots,m$ und $n\in\MdN$ sei 
\begin{align*}
A_{i,n}\da \{h=r_i\} \cap \{\inf_{k\ge n} f_k \ge \ep\cdot r_i\} \in A_\mu.
\end{align*}
Es gilt $A_{i,n}\subset A_{i,n+1}$ fÃ¼r $i=1,\ldots,m$ und $n\in\mathbb{N}$. FÃ¼r $\mu$-fast-alle $x\in X$ mit $h(x) = r_i$ gilt:
\begin{align*}
\ep r_i < r_i \le \liminf_{n\to\infty} f_n(x) = \sup_{n\in\MdN} \inf_{k\ge n} f_k(x).
\end{align*}
Es gibt ein $n\in\MdN$ mit $\ep r_i < \inf_{k\ge n} f_k(x),$ das heiÃŸt $x\in A_{i,n}$. Also
\begin{align*}
...
\end{align*}
fÃ¼r ein beliebiges $\ep \in (0,1)$. LÃ¤sst man $\ep$ gegen $1$ gehen, so folgt die Behauptung.
\end{beweis}
...
\begin{satz}[Lebesgue]
\label{satz1.17}
Sei $f_n\in\mathbb F_\mu(X,\bar\MdR)$, $n\in\MdN$ eine konvergente Folge $\mu$-messbarer Funktionen. Es existiere eine $\mu$-integrierbare Funktion $g\in\mathbb F_\mu(X,\bar\MdR)$ mit $|f_n|\le g$ $\mu$-fast-Ã¼berall fÃ¼r jedes $n\in\MdN$. Dann sind $f_n$ und $f\da \lim_{n\to\infty} f_n$ $\mu$-integrierbar und 
\begin{align*}
\lim_{n\to\infty} \int f_n d\mu = \int f d\mu.
\end{align*}
SchÃ¤rfer gilt sogar
\begin{align*}
...
\begin{beweis}
Wegen $|f_n|,|f| \le g$ ist $\int |f_n| d\mu <\infty$ und $\int |f|d\mu <\infty$. Die Folge $(2g-|f_n-f|)_{n\in\MdN}$ nichtnegativer Funktionen in $\mathbb F_\mu(X,\bar\MdR)$ konvergiert fÃ¼r $n\to\infty$ $\mu$-fast-Ã¼berall gegen $2g$. Mit dem Lemma von Fatou (Satz \ref{satz1.15}) folgt:
\begin{multline*}
...
\begin{notation}
FÃ¼r $A\subset X$ und $f\in\mathbb F_\mu (X,\bar\MdR)$ sei
\begin{align*}
...
\end{align*}
 ein Ã¤uÃŸeres MaÃŸ auf $X$ definiert. Es gilt $\A_\mu \subseteq \A_\psi$. Ist $g$ sogar $\mu$-messbar und $f\in\mathbb F_\mu(X,\bar\MdR)$ $\mu$-messbar, dann existiert $\int fd\psi$ genau dann, wenn $\int fgd\mu$ existiert. In diesem Fall gilt
\begin{align*}
...
\begin{beweis}
Es gilt $\psi(\emptyset)=0$. Die SubsigmaadditivitÃ¤t folgt direkt aus Lemma \ref{lem1.18}.
...
Sei nun $g$ $\mu$-messbar und $f\in \mathbb F_\mu(X,\bar\MdR)$ und sei zunÃ¤chst $f\ge 0$. Also existieren $\int fd\psi$ und $\int gfd\mu$. Es gibt eine Folge $(r_j)_{j\in\MdN}$ in $(0,\infty)$ und $(A_j)_{j\in\MdN}$ in $A_\mu$ mit $f = \sum_{j=1}^\infty r_j\ind_{A_j}$. Zweimalige Anwendung des 
...
\begin{satz}
Sei $f\in \mathbb F_\mu(X,\bar\MdR)$ $\mu$-integrierbar. Dann gibt es zu jedem $\ep >0$ ein $\delta > 0$ derart, dass fÃ¼r alle $A\in \A_\mu$ mit $\mu(A)<\delta$ gilt
\[
\int_A |f|d\mu <\ep.
\]
\end{satz}
...
\end{align*}
FÃ¼r $A\in\A_\mu$ mit $\mu(A) < \frac\ep{2N} \ad \delta$ folgt nun
\begin{align*}
\int_A |f| d\mu
...
Seien $X,Y\neq\emptyset$ Mengen mit Ã¤uÃŸeren MaÃŸen $\mu$ auf $X$, $\nu$ auf $Y$. Durch
\[
(\mu \times \nu)(M) \da \inf\{ \sum_{i=1}^\infty \mu(A_i)\nu(B_i): A_i\in \A_\mu, B_i\in \A_\nu,\, i\in\MdN,\, M\subset \bigcup_{i=1}^\infty (A_i \times B_i)\}
\]
wird ein Ã¤uÃŸeres MaÃŸ auf $X\times Y$ erklÃ¤rt, nÃ¤mlich das von $\mathcal E_0 \da \{A \times B : A\in\A_\mu,\, B\in\A_\nu\}$ und $\lambda$ mit $\lambda(A\times B) \da \mu(A)\cdot \nu(B)$ fÃ¼r $A\in\A_\mu$, $B\in\A_\nu$ induzierte Ã¤uÃŸere MaÃŸ.

\begin{satz}
Seien $X,Y\ne \emptyset$ Mengen mit Ã¤uÃŸeren MaÃŸen $\mu$ auf $X$ und $\nu$ auf $Y$. Dann gelten folgende Aussagen:
\begin{enumerate}
\item $\A_\mu \otimes \A_\nu \subset \A_{\mu\times \nu}$ und $(\mu\times \nu)(A\times B) = \mu(A) \cdot \nu(B)$ fÃ¼r $A\in\A_\mu$, $B\in\A_\nu$.
\item Das MaÃŸ $\mu\times\nu$ ist $\A_\mu \otimes \A_\nu$-regulÃ¤r.
...
$f(\cdot,y) \in\mathbb F_\mu(X,\bar\MdR)$ ist $\mu$-messbar fÃ¼r $\nu$-fast-alle $y\in Y$. Es ist $\int f(x,y)\mu(dx)$ $\nu$-messbar und $\iint f(x,y)\mu(dx)\nu(dy)$ existiert (und symmetrisch in $x$ und $y$) und schlieÃŸlich:
\[
\int f d(\mu\times\nu) = \iint f(x,y)\mu(dx)\nu(dy) = \iint f(x,y) \nu(dy) \mu(dx).
\]
\end{enumerate}
...
Wir setzen
\begin{multline*}
\mathcal E \da \{ M\subset X\times Y: \ind_M(\cdot,y) \in \mathbb F_\mu(X,\bar\MdR) \text{ ist $\mu$-messbar fÃ¼r $\nu$-fast-alle $y\in Y$,} \\
\int\ind_M(x,y) \mu(dx) \in \mathbb F_\nu(Y,\bar\MdR)\text{ ist $\nu$-messbar}\}.
\end{multline*}
FÃ¼r $M\in\mathcal E$ sei
\[
...
FÃ¼r $A\times B\in\mathcal E_0$ ist $\ind_{A\times B}(\cdot, y) = \ind_A \cdot \ind_B(y)$ $\mu$-messbar fÃ¼r alle $y\in Y$ und $\int \ind_{A\times B}(x,y) \mu(dx) = \mu(A) \cdot \ind_B(y)$ ist $\nu$-messbar. Also ist $A\times B\in \mathcal E$, und damit $\mathcal E_0\subset\mathcal E$.

FÃ¼r $A\times B\in \mathcal E_0$, $C\times D\in\mathcal E_0$ ist 
\begin{align*}
...
\end{align*}
Jede abzÃ¤hlbare Vereinigung von Mengen aus $\mathcal E_0$ kann als abzÃ¤hlbare Vereinigung von paarweise disjunkten Mengen aus $\mathcal E_0$ erhalten werden, das heiÃŸt $\mathcal E_1\subset \mathcal E$ nach ($\alpha$).
...
\textbf{Behauptung:} FÃ¼r $M\subset X\times Y$ gilt 
\[
...
FÃ¼r ein beliebiges $V\in\mathcal E_1$ mit $A\times B\subset V$ gilt
\[
(\mu\times\nu)(A\times B) =\inf\{\varrho(V):A\times B\subset V,V\in\mathcal{E}_1\}= \varrho(A\times B) = \mu(A) \nu(B).
\]
FÃ¼r $T\subset X\times Y$ und $U\in \mathcal E_1$ mit $T\subset U$ Sind
...
\end{multline*}
Bildet man das Infimum Ã¼ber alle $U\in\mathcal E_1$ mit $U\supset T$, so ergibt diese Ungleichung
\[
(\mu\times \nu) (T\cap (A\times B)) + (\mu\times\nu)(T\cap (A\times B)^c) \le (\mu\times \nu)(T),
\]
woraus $A\times B\in\A_{\mu\times\nu}$ folgt.
\item Ist $M\subset X\times Y$ und $(\mu\times\nu)(M)<\infty$, so gibt es $W\in\mathcal E_2$ mit $\varrho(W)<\infty$ und mit der gewÃ¼nschten Eigenschaft $(\mu\times\nu)(M) = (\mu\times\nu)(W)$.
\item Sei $f=\ind_M$, $M\in\A_{\mu\times\nu}$ und $(\mu\times\nu)(M)<\infty$. Zu $M$ existiert ein $W\in\mathcal E_2$ mit $M\subset W$ und $(\mu\times\nu)(M) = (\mu\times\nu)(W) =\varrho(W)$.

Fall 1: $(\mu\times\nu)(M) = 0$. Dann gilt $\rho(W)=0$ und $\ind_M(\cdot,y)=0$ $\mu$-fast-Ã¼berall fÃ¼r $\nu$-fast-alle $y\in Y$. Insbesondere ist $M\in\mathcal E$ und $\rho(M)=0$.

Fall 2: $(\mu\times\nu)(M) > 0$. Dann gilt $(\mu\times\nu)(W\setminus M) = 0$, $M\subset W$. Fall 1 liefert $W\setminus M\in\mathcal E$ und $\rho(W\setminus M)=0$. Also ist $\ind_M(\cdot, y) = (\ind_W - \ind_{W\setminus M})(\cdot,y)$ $\mu$-messbar fÃ¼r $\nu$-fast-alle $y\in Y$ und  $\ind_M(\cdot,y) = \ind_W(\cdot,y)$ $\mu$-fast-Ã¼berall fÃ¼r $\nu$-fast-alle $y\in Y$. Insbesondere ist $M\in\mathcal E$ und $\varrho(M) = \varrho(W) = (\mu\times\nu)(M)$.
\end{enumerate}
\end{beweis}
