...
\]
Notation: Für \(p>0\) sei \(\alpha(p) \da \frac{\pi^{\frac{p}2}}{\Gamma(\frac{p}2+1)}\).
\end{definition}

Für $p\in\mathbb{N}$ ist dann  \(\alpha(p) = \lambda^p(B^p(0,1))\) mit \(B^p(0,1) \da \{ x \in \mathbb R^p \colon \|x\| = 1 \}\).
...
\]
Dann sind $\epi(\psi)$, $\widetilde\epi(-h)$ abgeschlossene, konvexe, nicht leere Mengen, die keinen gemeinsamen inneren Punkt habe. Dann gibt es eine „nicht vertikale“ trennende Hyperebene, das heißt es gibt ein $\alpha \in \MdR$ und $v\in\tilde\HR$ mit:
\[
...
$\varphi(y_0,x_0) + h(y_0,x_0) \le 0$, das heißt $\varphi^*(y_0,x_0) + h(x_0,y_0) = 0 = \varphi(x_0,y_0)+h(x_0,y_0)$ also $\chi(x_0+y_0, x_0-y_0) + 2\|x_0+y_0\|^2 = 0$ und somit $x_0=-y_0$ und $\chi(0,2x_0)= 0$. Dies liegt wegen Lemma A $(0,2x_0)\in G(f)$, das heißt $0\in D$.

Sei nun $x_1\in\HR$ beliebig und definiere $f_{x_1} : D-x_1 \to \HR$ durch $f_{x_1}(x) \da f(x+x_1)$. Da $f_{x_1}$ keine echte 1-Lipschitzfortsetzung hat, muss $0\in D(f_{x_1}) = D-x_1$, also $x_1\in D$ und somit $D=\HR$.
...
\]
für $(E,g),(\tilde E,\tilde g)\in \mathcal S$ wird auf $\mathcal S$ eine Ordnung eingeführt, in der jede Kette eine obere Schranke besitzt. Nach dem Zornschen Lemma gibt es ein maximales Element in $\mathcal S$, etwa $(E,g)$. Dann ist wegen Lemma D aber $E=\HR$, das heißt $(\HR,g)$ ist die gewählte 1-Lipschitzfortsetzung von $f$.
\end{beweis}

Eine Abbildung $f:\MdR^m \to \MdR^n$ heißt differzierbar an der Stelle $x\in\MdR^m$, falls es eine lineare Abbildung $L:\MdR^m\to\MdR^n$ gibt mit
\[
\lim_{y\to x} \frac{f(y)-f(x)- L(y-x)}{\|y-x\|} = 0.
\]
In diesem Fall schreiben wir $Df(x) \da Df_x \da L$. Ferner ist
\[
Df_x(u) \da L(u) = \lim_{t\to0}\frac{f(x+tu) - f(x)}{t}
\]
für $u\in\MdR^m$.

Ist $f$ in $x$ differzierbar, so gilt speziell $D_iF(x) \da D_{e_i}f(x) \da Df_x(e_i)$.
...
\begin{beweis}
Sei stets ohne Beschränkung der Allgemeinenheit $n=1$.
\begin{enumerate}[{Teil} 1:]
\item $m=1$ und $f$ ist monoton wachsend. Definiere
\[
v_f(M) \da \inf\{\sum_{i=1}^\infty (\underbrace{f(b_i) - f(a_i)}_{\ge 0}): a_i < b_i,\, M\subset \bigcup_{i=1}^\infty (a_i,b_i)\}.
\]
Wie im Fall $\lambda^1$ ist $v_f\in\mathbb M (\MdR)$. Beachte hierzu $v_f(M) \le \Lip(f) \cdot \lambda^1(M)$. Dies zeigt auch $v_f \ll \lambda^1$ und daher $(v_f)_{\lambda^1} = v_f$. Das System
\[
V \da \{ (x,[a,b]): x\in[a,b]\}
\]
ist eine $\lambda^1$-Vitali-Relation. Also ist $\mathbb D(v_f,\lambda^1,V,\cdot) \in [0,\infty)$ $\lambda^1$-fast-überall und
\[
\mathbb D(v_f,\lambda^1,V,x)
= V\text-\lim_{[a,b]\to x} \frac{v_f([a,b])}{\lambda^1([a,b])}
...
&\le g(y) + \Lip(f) \cdot \sup\{\sum_{i=1}^m |a_i-a_{i_1}|: y = a_0\le a_1\le \cdots \le a_m = x,\, m\in\MdN\} \\
&= g(y) + \Lip(f) \cdot |y-x|
\end{align*}
...
\]
also $(g-f)(x) \ge (g-f)(y)$, das heißt $g-f$ ist ebenfalls monoton wachsend und ebenfalls Lipschitz. Nach Teil 1 sind daher $g$ und $g-f$ $\lambda^1$-fast-überall differenzierbar. Somit auch $f=g-(g-f)$.

\textbf{Nebenbemerkung:} $f(b)-f(a) = v_f([a,b]) = (v_f)_{\lambda^1}([a,b]) = \int_{[a,b]} \mathbb D(v_f, \lambda^1, V, x)\lambda(dx) = \int_{[a,b]} f'(x) \lambda(dx)$. Dies ist der Hauptsatz der Differenzial- und Integrationsrechnung für Lipschitzfunktionen.

\item $m\ge 1$. Seien $e\in S^{m-1}$. Sei $N_e$ die Menge aller $x\in\MdR^m$, für die $t \mapsto f(x+te)$ in $t=0$ nicht differenzierbar ist. Dann ist $N_e$ eine Borelmenge und $\HM^1(N_e \cap (x+\MdR e)) = 0$ nach Teil 2. Fubini zeigt $\lambda^m(N_e) = 0$ für beliebige $e\in S^{m-1}$.
...
\]
wobei
$D_i(f) \da D_{e_i}f(x)$ für die Standarbasis $(e_1,\ldots,e_m)$ des $\MdR^m$. Für festes $e\in S^{m-1}$ existiert auch $D_ef(x)$ für $\lambda^m$-fast-alle $x\in\MdR^m$. Sei $\varphi \in\mathcal C^\infty_c(\MdR^m)$. Für $\ep>0$ und $e\in S^{m-1}$ gilt
\[
\int_{\MdR^m} \frac{f(x+\ep e) - f(x)}\ep \cdot \varphi(x) \lambda^m(dx)
= 
-\int f(x) \cdot \frac{\varphi(x) - \varphi(x-\ep e)}\ep\varphi(x) \lambda^m(dx)
\]
wegen $|\varphi| \le \|\varphi\|_\infty \cdot \ind_{\supp(\varphi)}$ und da $f$ Lipschitz-stetig ist erhält mal mit dem Satz von der majorisierten Konvergenz:
\begin{align*}
...
\end{align*}
Hieraus liest man $D_ef(x) = \langle e,\nabla f(x) \rangle$ für $\lambda^m$-fast-alle $x\in\MdR^m$ ab, wobei $e\in S^{m-1}$ fest, aber beliebig ist.
...
\]
eine $\lambda^m$-Nullmenge. Wir zeigen, dass $f$ in $x\in A$ differenzierbar ist. Sei also $x\in A$. Für $\ep>0$ und $e\in S^{m-1}$ sei
\[
\Delta_\ep(e) \da \frac{f(x+\ep e)- f(x)}{\ep} - \langle e,\nabla f(x) \rangle.
\]

Wir zeigen: $\Delta_\ep(e) \to 0$ für $\ep\to 0$ gleichmäßig in $e\in S^{m-1}$.

Setze $M\da 2\max\{\Lip(f), \|\nabla f(x)\|, 1\}$. Dann gilt für $e,\bar e\in S^{m-1}$:
\begin{align*}
|\Delta_\ep (e) - \Delta_\ep(\bar e)|
\le | \frac{f(x + \ep e) - f(x + \ep \bar e)}\ep | + \|\nabla f(x) \| \cdot \|e-\bar e\|
\le M \cdot \|e-\bar e\|.
\end{align*}
Ferner gilt $\delta_\ep(e) \to 0$ für alle $e\in E$, wenn $\ep \to 0$. Es gibt eine endliche Teilmenge $\bar E\subset E$ mit $d(e,\bar E) \le \frac\ep{2M}$ für jedes $e\in S^{m-1}$. Es gibt zu $\ep >0$ ein $\delta>0$ so dass $|\Delta_{\ep'} (\bar e) | \le \frac\ep2$ für alle $\bar e\in\bar E$ und $0<\ep'\le \delta$. Für $e\in S^{m-1}$ und $0<\ep'\le\delta$ gilt dann:
\begin{align*}
|\Delta_{\ep'} (e)|
&\le \min_{\bar e\in\bar E} \{ |\Delta_{\ep'}(e) - \Delta_{\ep'}(\bar e)| + |\Delta_{\ep'}(\bar e)|\} \\
&\le \min_{\bar e\in\bar E} \{ M \cdot \|e-\bar e\| + \frac\ep2\} \\
&\le \frac\ep{2M} \cdot M + \frac\ep2 = \ep.
\end{align*}
Dies zeigt $\Delta_\ep\to 0$ gleichmäßig für $\ep\to 0$ und daraus oflgt die Behauptung.
\end{enumerate}
\end{beweis}
