...
\]
Es ist \(A_{i,n} \subset M_i\), \(A_{i,n}\nearrow M_i \cap T\) und \(A_{1,n} \cup A_{2,n} \subset T\). Somit folgt
\begin{align*}
...
\]
für $\mu$-fast-sicher alle $x\in X$.
\end{satz}

\begin{beweis}
Sei zunächst $f\ge 0$. Sei $A\subset X$ und $\bar\nu(A) \da \int^*_A fd\mu$. Dann ist $\bar\nu$ ein äußeres Maß auf $X$. Setze
\[
\nu(M) \da \inf\{\underbrace{\bar\nu(A)}_{=\int_A fd\mu}: A\supset M,\, A\in \A_\mu\}.
\]
Dann ist $\nu$ ein $\A_\mu$-reguläres äußeres Maß. Ferner ist $\borel(X) \subset \A_\mu \subset A_\nu$. Ist $M\subset X$, so gibt es ein $A\in \A_\mu$ mit $M\subset A$ und $\nu(M)=\nu(A)$. Zu $A\in \A_\mu$ existiert $B\in\borel(X)$ mit $A\subset B$ und $\mu(B\setminus A)=0$. Daraus folgt
\[
...
Für die allgemeine Aussage zerlege $f=f^+-f^-$.
\end{beweis}

\begin{definition}
Sei $\mu\in\mathbb M(X)$, $V$ eine $\mu$-Vitali-Relation, $M\subset X$ und $x\in x$. Dann nennt man
\[
...
\begin{satz}
\label{satz:2.18}
Seien $\mu\in\mathbb M(X)$, $V$ eine $\mu$-Vitali-Relation, $M\subset X$. Dann existiert die $(\mu,V)$-Dichte für $\mu$-fast-alle $x\in X$ Seien
\[
P \da \{x\in X: V\text-\lim_{S\to x}\frac{\mu(S\cap M)}{\mu(S)} = 1\} \quad\text{und}\quad
Q \da \{x\in X: V\text-\lim_{S\to x}\frac{\mu(S\setminus M)}{\mu(S)} = 0\}.
\]
Dann gilt $P,Q\in\A_\mu$, $\mu(M\setminus P)$ und $\mu(Q\setminus M) =0$.

Ferner sind äquivalent:
\begin{enumerate}
\item $M\in \A_\mu$,
\item $\mu(P\setminus M) = 0$,
\item $\mu(M\setminus Q) = 0$.
\end{enumerate}
\end{satz}

\begin{beweis}
Zu $M\subset X$ existiert $A\in\A_\mu$ mit $M\subset A$ und $\mu(M\cap B) = \mu(A\cap B)$ für alle $B\in\A_\mu$. Es gilt $\mu\MR A\in\mathbb M(X)$ und daher ist $\{\mathbb D(\mu\MR A,\mu,V,\cdot)= 1\} \in \A_\mu$. Ferner existiert wegen $S\in\borel(X)$ der Limes
\[
...
\]
Sei $x\notin P$, dann ist $x\notin A$ für $\mu$-fast-alle $x\in x$, das heißt $\mu(A\setminus P) = 0$ und analog $\mu(P\setminus A)=0$. Ist $M\subset A$, so $\mu(M\setminus P) \le \mu(A\setminus P) =0$ und damit $M\setminus P\in\A_\mu$. Analog für $Q$.

Ist $M\in\A_\mu$, so wähle $A=M$, das heißt $\mu(P\setminus M)=0$. Ist  dagegen $\mu(P\setminus M)=0$, so ist $P\setminus M\in\A_\mu$ und $M=(M\setminus P) \cup (M\cap P) = (M\setminus P) \cup (P\setminus (P\setminus M)) \in \A_\mu$. Analog für $Q$.
\end{beweis}
