...
\item $\inf\{\diam(S): S\in C(Z),\, x\in S\} = \inf\{\diam(S) : (z,S) \in C \text{ f\"ur ein $z\in Z$}, x\in S\} \le \inf\{\diam(S): (x,S)\in C\} = 0$ für alle $x\in Z$.
...
\textbf{Nachweis:}
Sei $S=\mathbb B(y,\varrho)\in \mathcal S'^t_{\mathbb B(x,s)}$. Dann ist $\mathbb B(y,\varrho) \cap \mathbb B(x,s) \ne \emptyset$ und au{\ss}erdem gilt $\diam(\mathbb B(y,\varrho)) \le t \cdot \diam(\mathbb B(x,s))$. Sei $\bar z\in \mathbb B(y,\varrho)$ und $z_0\in\mathbb B(y,\varrho)\cap \mathbb B(x,s)$. Dann gilt 
...
Wir zerlegen nun $X$ in der Form $X=\bigcup_{i\ge 1}A_i$ mit beschr\"ankten disjunkten Borelmengen $A_i$. 
Zu jedem $i\in\mathbb{N}$ existiert $C_i\in \borel(X)$ mit $C_i\subset A_i$, $\mu(A_i\setminus C_i)=0$ und  $\nu_\mu(M)=\nu(M\cap C_i)$ f\"ur alle $M\subset A_i$. Setze 
...
$$
Es folgt f\"ur $M\subset X$, wobei f\"ur die zweiten Gleichung $\borel (X)\subset\mathcal{A}_{\nu_\mu}\subset
...
\nu_\mu(M)=\nu_\mu(B^c\cup C).
$$
Dies schlie{\ss}t den Nachweis ab, dass $\nu_\mu$ Borel-regul\"ar ist. Ist  $M$ beschr\"ankt, 
so ist $\nu_\mu(M)=\nu(M\cap B)<\infty$. 

Ist jede $\mu$-Nullmenge eine $\nu$-Nullmenge, so ist $\nu(B^c)=0$, 
also $\nu=\nu\MR B=\nu_\mu$. Ist schlie{\ss}lich $M\subset X$ und $\mu(M)=0$, so ist 
$\nu_\mu(M)=\nu(\emptyset)=0$. Aus $\nu_\mu=\nu$ folgt damit $\nu(M)=0$, so dass jede $\mu$-Nullmenge  eine $\nu$-Nullmenge.
\end{beweis}

\begin{bemerkung}
F"ur \(\mu, \nu \in \mathbb{M}(X)\) schreibt man \(\nu \ll \mu\) (\(\nu\) ist absolut stetig bez"uglich \(\mu\)) falls f"ur alle \(M \subset X\) gilt: \(\mu(M) = 0 \Rightarrow \nu(M) = 0\) \\
Das Ma"s \(\nu_\mu\) ist absolutstetig bez"uglich \(\mu\) und \(\nu_\mu = \mu\MR B\) mit \(\mu(B^c) = 0, B \in \borel(X)\). Also
\[ \nu = \nu_\mu + \underbrace{(\nu - \nu_\mu)}_{\da \nu_\mu^\bot} \]
...
\begin{lemma}
\label{lem2.11}
Seien \(\mu, \nu, \tau \in \mathbb{M}(X), V\) eine \(\mu-\)Vitalirelation, \(c \in (0,\infty)\) und \[A \subset \{ x \in X : V\text{-}\liminf_x \frac{\nu(S)}{\mu(S)} < c \}\]
...
Wegen Satz \ref{satz:2.2} erh"alt man \(U \subset X\) offen mit \(B \subset U\) und \(\tau(U \setminus B) \leq \varepsilon\). Also ist \(\mu(A \setminus U) \leq \mu(A \setminus B)=0\) und
\[ \tau(U) = \tau(U \cap B) + \tau(U \setminus B) \leq \tau(B) + \varepsilon = \tau_\mu(A) + \varepsilon \]
Sei \(C \da \{ (x,S) \in V : S \subset U \wedge \frac{\nu(S)}{\mu(S)} < c \}\). F"ur \(z\in A\cap U\) gilt: \(V\text{-}\liminf\frac{\nu(S)}{\mu(S)} < c\), also \(\inf\{\diam(S):(z,S)\in C\}=0\) f"ur alle \(z \in A \cap U\). \\
Da \(V\) eine \(\mu-\)Vitali-Relation ist, enth"alt \(C(A\cap U)\) eine abz"ahlbare disjunkte \(\mu\)-"Uberdeckung \(\mathcal(S)\) von \(A\cap U\). \\
Aus \(\mu(A \setminus U)=0\) folgt \(\mu(A\cap U \setminus \bigcup_{S\in \mathcal{S}} S) = 0\), \(\nu_\mu \ll \mu\) und \(\nu_\mu \leq \nu\) und damit:
\begin{align*}
\nu_\mu(A) &= \nu_\mu(A\cap U \cap \bigcup_{S\in\mathcal{S}}S) \leq \nu_\mu(\bigcup_{S\in\mathcal{S}} S) \leq \nu(\bigcup_{S\in\mathcal{S}} S) \\
&\leq \sum_{S\in\mathcal{S}}\underbrace{\nu(S)}_{\leq c\tau(S)} \leq c\sum_{S\in\mathcal{S}} \tau(S) = c\cdot \tau(\underbrace{\bigcup_{S\in\mathcal{S}}S}_{\subset U}) \leq c\cdot\tau(U) \\
&\leq c\cdot(\tau_\mu(A)+\varepsilon) = c\cdot\tau_\mu(A)+c\varepsilon
\end{align*}
...
\begin{bemerkung}
\begin{enumerate}[(1)]
	\item Sind \(\nu,\tau \ll \mu\), so gilt \(\nu_\mu=\nu, \tau_\mu=\tau\) und im Lemma \ref{lem2.11} lautet die Folgerung \(\nu(A) \leq c\cdot\tau(A)\)
	\item \(\mu_\mu=\mu\)
\end{enumerate}
\end{bemerkung}
...
\begin{korollar}
\label{kor:2.12}
Seien \(\mu,\nu \in \mathbb{M}(X)\), \(V\) eine \(\mu-\)Vitali-Relation, \(c>0\). Seien ferner \(A \subset \{x\in X:\liminf_x\frac{\nu(S)}{\mu(S)}<c\}\) und \(B\subset\{x\in X:\limsup_x\frac{\nu(S)}{\mu(S)}>c\}\). Dann gilt:
...
und somit mit Lemma \ref{lem2.11}
\begin{align*}
...
\begin{lemma}
\label{lem2.13}
Seien \(\mu,\nu \in \mathbb{M}(X)\) und \(V \mu\)-Vitali-Relation. Dann ist \(\mathbb{D}(\nu,\mu,V,\cdot) \in \mathbb{F}_\mu(X,\bar{\mathbb{R}})\) und \(0 \leq \mathbb{D}(\nu,\mu,V,\cdot) < \infty\) \(\mu-\)fast-"uberall in \(X\).
\end{lemma}
\begin{beweis}
Seien
\begin{align*}
...
\end{align*}
und f"ur \(a,b \in \mathbb{R}\) mit \(a < b\)
\begin{align*} R(a,b) & \da \{ x \in X : V\text{-}\liminf_x\frac\nu\mu<a<b<V\text{-}\limsup_x\frac\nu\mu \} \end{align*}
Da \(V\) eine \(\mu\)-Vitali-Relation ist, enth"alt \(C(P)\) eine abz"ahlbare disjunkte \(\mu\)-"Uberdeckung \(\mathcal{S}\) von \(P\), sodass
...
d.h. \(\mu(P) = 0\) \\
Seien nun \(a<b,\; a,b\in\mathbb{Q}\) und \(A \subset Q\), \(B \subset R(a,b)\) beschr"ankt. F"ur ein beliebiges \(c > 0\) gilt:
\[ \infty > \nu_\mu(A) \geq c\cdot\mu(A),\quad \text{d.h.}\; \mu(A) = 0 \]
Ebenso:
\begin{align*}
& b\cdot\mu(B) \leq \nu_\mu(B) \leq a\cdot\mu(B) \\
& a < b,\quad \mu(B)\leq\infty \\
\end{align*}
d.h. \(\mu(B)=0\) \\
...
\begin{lemma}
\label{lem2.14}
Sei \(\mu,\nu \in \mathbb{M}(X)\), \(V\) eine \(\mu\)-Vitali-Relation. Dann ist \(\mathbb{D}(\nu,\mu,V,\cdot)\) eine \(\mu\)-me"sbare Funktion.
\end{lemma}
\begin{beweis}
Seien \(a<b\) \begin{align*} & M_1 \da \{ \mathbb{D}(\nu,\mu,V,\cdot) < a \} & M_2 \da \{ \mathbb{D}(\nu,\mu,V,\cdot) > b \} \end{align*}
Seien weiter \(A_i \subset M_i\) beschr"ankt (\(i=1,2\)).
Es existieren \(B_i \in \borel(X)\) mit \(A_i \subset B_i\) und
\begin{align*} & \mu(A_i\cap C) = \mu(B_i \cap C),\; \nu_\mu(A_i\cap C) = \nu_\mu(B_i \cap C)\quad(\forall C\in\borel(X)) & (\ast) \end{align*}
Wegen \(a<b\) gilt mit Korollar \ref{kor:2.12} und \((\ast)\)
\begin{align*}
& a \cdot \mu(B_1 \cap B_2) = a\cdot\mu(A_1 \cap B_2) \geq \nu_\mu(A_1 \cap B_2) = \nu_\mu(B_1 \cap B_2) \\
& = \nu_\mu(B_1 \cap A_2) \geq b\cdot\mu(B_1 \cap A_2) = b\cdot\underbrace{\mu(B_1\cap B_2)}_{<\infty} \\
& \Rightarrow \mu(B_1 \cap B_2) = 0
\end{align*}
Nun folgt:
\begin{align*}
...
&= \mu(A_1) + \mu(A_2 \cap B_2) \\
&\overset{A_2\subset B_2}= \mu(A_1) + \mu(A_2)
\end{align*}
Sei \(T \subset X\) beliebig, \(y \in X\). Setze
\[ A_{i,n} \da M_i \cap T \cap B(y,n) \quad(i=1,2) \]
Es ist \(A_{i,n} \subset M_i\), \(A_{i,n}\nearrow M_i \cap T\) und \(A_{1,n} \cup A_{2,n} \subset T\).
\begin{align*}
\mu(T) &\geq \lim_{n\rightarrow\infty} \mu(A_{1,n} \cup A_{2,n}) \overset!\geq \lim_{n\rightarrow\infty} \mu(A_{1,n}) + \lim_{n\rightarrow\infty} \mu(A_{2,n}) \\
&= \mu(M_1 \cap T) + \mu(M_2 \cap T)
\end{align*}
Die Behauptung folgt aus Satz \ref{satz:1.8}
\end{beweis}
