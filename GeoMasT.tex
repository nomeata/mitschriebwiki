...
\begin{beweis}
Es gilt $\psi(\emptyset)=0$. Die Subsigmaadditivität folgt direkt aus dem Lemma \ref{lem1.18}.
...
\[
\int h d\mu \le \int^* \ind_M g \mu + \ep.
\]
...
Sei nun $g$ $\mu$-messbar und $f\in \mathbb F_\mu(X,\bar\MdR)$ und sei zunächst $f\ge 0$. Also existieren $\int fd\psi$ und $\int gfd\mu$. Es gibt eine Folge $(r_j)_{j\in\MdN}$ in $(0,\infty)$ und $(A_j)_{j\in\MdN}$ in $A_\mu$ mit $f = \sum_{j=1}^\infty r_j\ind_{A_j}$. Hiermit gilt:
\begin{align*}
\int fd\psi 
...
&= \int fgd\mu.
\end{align*}
Sei nun $f$ eine beliebige $\mu$-messbare Funktion. Wegen $(fg)^\pm = f^\pm \cdot g$ gilt $\int f^\pm d\psi < \infty$ genau dann, wenn $\int f^\pm g d\mu<\infty$. Somit existiert $\int fd\psi$ genau dann, wenn $\int fdd\mu$ existiert und
\begin{multline*}
...
\begin{satz}
Sei $f\in \mathbb F_\mu(X,\bar\MdR)$ $\mu$-integrierbar. Dann gibt es zu jedem $\ep >0$ mit $\delta > 0$ derart, dass für alle $A\in \A_\mu$ mit $\mu(A)<\delta$ gilt
\[
\int_A |f|d\mu <\ep.
\]
\end{satz}
...
\end{align*}
Es gibt somit zu gegenenem $\ep$ ein $N$ mit 
\begin{align*}
...
\[
\ind_{\bigcup_{i=1}^\infty M_j}(\cdot, y)=\sum_{j=1}^\infty \ind_{M_j}(\cdot, y)
\]
ist $\mu$-messbar für $\nu$-fast-alle $y\in Y$ und \[
\int\ind_{\bigcup_{i=1}^\infty M_j} (x,y)\mu(dx) = \sum_{j=1}^\infty \int \ind_{M_j}(x,y) \mu(dx)
\]
ist $\nu$-messbar.
...
\textbf{Nachweis:} Sei $(A_i\times B_i)  \in \mathcal E_0$ mit $M\subset \bigcup_{i=1}^\infty (A_i\times B_i)\ad V \in\mathcal E_1$. Dann gilt $\ind_V = \sum_{i=1}^\infty \ind_{A_i\times B_i}$, wobei Gleichheit gilt, falls die Mengen $(A_i\times B_i)$ paarweise disjunkten sind, und somit gilt $\varrho(V) \le \sum_{i=1}^\infty \varrho(A_i\times B_i) = \sum_{i=1}^\infty \nu(A_i) \nu(B_i)$, wobei auch hier Gleichheit gilt, falls die Mengen $(A_i\times B_i)$ paarweise disjunkt sind.

Hiermit gilt der erste Teil der Behauptung
\begin{align*}
...
Ist $(\mu\times \nu)(M)<\infty$, so existieren $V_1\in\mathcal E_1$, $i\in\MdN$, $M\subset V_i$ mit
\[
...
\begin{enumerate}
\item Sei $A\times B\in\mathcal E_0$. Für ein beliebiges $V\in\mathcal E_1$ mit $A\times B\subset V$ gilt
\[
(\mu\times\nu)(A\times B) = \varrho(A\times B) = \mu(A) \nu(B)
\]
Für $T\subset X\times Y$ und $U\in \mathcal E_1$ mit $T\subset U$ Sind
$U\cap (A\times B)$ und $U\cap (A\times B)^c$ disjunkt Mengen in $\mathcal E_1$ und erhalten
\begin{multline*}
(\mu\times \nu) (T\cap (A\times B)) + (\mu\times\nu)(T\cap (A\times B)^c)
\\ \le \varrho(U\cap (A\times B)) + \varrho(U\cap (A\times B)^c) = \varrho(U).
\end{multline*}
Bildet man das Infimum über alle alle $U\in\mathcal E_1$ mit $U>T$, so ergibt diese Ungleichung
\[
