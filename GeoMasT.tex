...
\]
Aber es ist $f([-1,1]) = [0,1]$ und $\int_{f(A)}dt = \lambda(f(A)) = 1$. Man beachte
\[
...
\begin{lemma}
\label{lem:3.4}
Sei $m\le n$ und $f:\MdR^m \to \MdR^n$ eine lineare Abbildung. Dann gibt es eine symmetrische lineare Abbildung $\sigma: \MdR^m\to\MdR^n$ und eine lineare Isometrie $\varrho:\MdR^m\to\MdR^n$ mit $f =\varrho \circ \sigma$. Ist $(a_1,\ldots,a_m)$ eine Orthonormalbasis von $\MdR^m$, so gilt
\[
\llbracket f \rrbracket \da |\det(\sigma)| \da \sqrt{ \det(\langle f(a_i),f(a_j)\rangle)_{i,j=1,\ldots,m}},
\]
...
\begin{align*}
\langle f(a_i), f(a_j)\rangle = \langle \varrho\circ \sigma(a_i), \varrho\circ\sigma(a_j) \rangle = \langle \varrho(a_i),\varrho(a_j)\rangle.
\end{align*}
Sei $S$ die beschriebene Matrix von $\sigma$ bezüglich $(a_1,\ldots,a_m)$. Dann:
\begin{align*}
(\det(\sigma))^2
...
&= \det(\rho(a_i),\ldots,\rho(a_m)) \\
&\le \|\rho(a_i)\|\cdot \cdots \cdot \|\rho(a_m)\| \\
&= \|f(a_i)\|\cdot \cdots \cdot \|f(a_m)\|.
\end{align*}
\end{bemerkungen}
...
\begin{bemerkung}
Eine entsprechende Aussage gilt allgemein für $f:(X,d)\to (Y,\bar d)$, wobei $(X,d)$ ein polnischer, das heißt seperabler vollständiger metrischer, Raum ist.
\end{bemerkung}

\begin{beweis}
Betrachte Folgen von Zerlegungen
\begin{align*}
\mathcal C_i &\da  \{[0,2^{-i})^m + 2^i \cdot z \in \MdZ^m\},\\
...
\]
für $i\to \infty$ und ferner ist jedes $C\in\mathcal C_i$ ist disjunkte Vereinigung von $\tilde C\in\mathcal C_{i+1}$.
...
\end{align*}
für $i\to \infty$. Hieraus folgt die Messbarkeitsbehauptung.
...
\item $t^{-1} \cdot\|\tau_E(v)\| \le \|Df_b(V)\| \le t \cdot\|\tau_E(v)\|$ für ein $b\in B$ und ein $v\in \MdR^m$.
...
\begin{beweis}
Sei $\ep>0$ mit $t^{-1}+ \ep < 1< t-\ep$. Wähle abzählbare dichte Teilmengen $C\subset \MdR^m$ und $T\subset \GL(m,\MdR)$. Für $c\in C$ und $\tau\in T$ sowie $i\in\MdN$ sei $E(c,\tau,i)$ die Menge aller $b\in B \cap B(c,\frac1i)$, für die gilt:
\begin{enumerate}[(a)]
\item $ (t^{-1} +\ep)\cdot \|\tau(v)\| \le \|Df_b(v)\| \le (t-\ep) \cdot \|\tau(v)\|$ für alle $v\in\MdR^m$ und
\item $\|f(a)- f(b) - Df_b(a-b)\| \le \ep \cdot \|\tau(b-a)\|$ für alle $a\in B(c,\frac1i)$.
\end{enumerate}
\end{beweis}
