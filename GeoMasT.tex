...
\begin{motivation}
\begin{itemize}
\item Verallgemeinterte Mannigfaltigkeit
\item Allgemeine Fassung der Flächenformel und Koflächenformel
\item Sind Trägermengen für rektifizierbare Ströme und rektifizierbare Varifalktigkeiten
\end{itemize}
\end{motivation}

\begin{definition}
Eine Menge $M\subset \MdR^p$ heißt abzählbar $n$-rektifizierbar, falls es eine Menge $M_0\subset \MdR^p$ mit $\HM^n(M_0)=0$ und Lipschitzabbildungen $f_j:\MdR^n \to \MdR^p$ für $j\in \MdN$, so dass gilt:
\[
...
\item Teilmengen und abzählbare Vereinigungen von abzählbaren $n$-rektifizierbaren Mengen sind abzählbare $n$-rektifizierbare Mengen.
\item Sei $(x_i)_{i\in \MdN} \subset \MdR^2$ abzählbar, dicht. Es ist
\[
M \da \bigcup_{i\in\MdN} S^1(x_1,2^{-1})
\]
abzählbar 1-rektifizierbar. $\HM^1(M) <\infty$. $\overline M = \MdR^2$.
\item Sei $K\subset \MdR^p$ kompakt und konvex, dann ist der Rand $\partial K$ $(p-1)$-rektifizierbar.
...
\item $D$ sei ein gleichseitiges Dreieck in $\MdR^2$ mit Kantenlänge 1. $D_1$ sei die Vereinigung von drei gleichseitigen Dreiecken in $\MdR^2$ mit Kantenlänge $\frac13$ „in den Ecken von $D$“. Definiere $D_i$, $i>1$, analog und 
\[
\hat D \da \bigcap_{i=1}^\infty D_i \subset \MdR^2.
\]
Es ist $\HM^1(\hat D)=1$, aber $\HM^1(\pi_1(\hat D))=0$, wobei $\pi_1$ die Orthogonalprojektion auf die $y$-Achse ist. Seien $\pi_2$, $\pi_3$ Orthogonalprojektionen auf die Geraden, die mit der $j$-Achse den Winkel 120$^\circ$ einschließen. Dann gilt $\HM^1(\pi_i(\hat D)) = 0$, $i=1,2$. Somit ist $\hat D$ nicht abzählbar 1-rektifizierbar.
\end{beispiele}

\begin{satz}[Struktursatz]
Sei $A\subset\MdR^p$ mit $\HM^n(A)<\infty$. Dann gibt es eine disjunkte Zerlegung $A= R \mathop{\dot\cup} N$ mit 
\begin{enumerate}[\quad(a)]
\item $R= A\cap R_0$ mit einer abzählbare $n$-rektifizierbare Borelmenge $R_0\subset \MdR^p$.
\item Für jede abzählbare $n$-rektifizierbare Menge $F\subset \MdR^p$ gilt $\HM^n(N\cap F)=0$.
\end{enumerate}
\end{satz}
...
\]
ist abzählbar $n$-rektifizierbar. Setze $N\da A\setminus R_0$. Sei $F\subset \MdR^p$ abzählbar $n$-rektifizierbar. Es gibt $\tilde F \supset F$ mit $\tilde F\in \borel(\MdR^p)$ und abzählbar $n$-rektifizierbar.
...
\begin{definition}
Eine Menge $N\subset \MdR^p$ heißt rein nicht $n$-rektifizierbar, falls $\HM^n(N\cap F)=0$ für alle abzählbare $n$-rektifizierbare Mengen $F\subset \MdR^p$.
\end{definition}
...
\HM^n(N\mid E) = 0.
\]
\item  Ist $A\subset \MdR^p$, $A= \bigcup_{i=1}^\infty A_i$, $\HM(A_i)<\infty$ und gilt für alle $B\subset A$ mit $\HM^n(B)>0$ stets $\HM^n(B\mid E) = 0$ für eine Menge von $E\in G(p,n)$ positiven Maßes, so ist $A$ abzählbar $n$-rektifizierbar.
...
\]
\item Es gibt $N_0\subset \MdR^p$ mit $\HM^n(N_0)=0$ und $n$-dimensionalen Untermannigfaltigkeiten $N_j\subset \MdR^p$ der Klasse $C^1$ mit $\HM^n(N_j)<\infty$ und 
\[
...
\begin{beweis}
\begin{itemize}
\item (a)$\implies$(c): Wir verwenden den folgenden Lipschitz-Approximationssatz:
...
Wiederholte Anwendung dieses Approximationssatz zeigt: Zu $j\in\MdN$ existiert eine Menge $E_j\subset \MdR^p$ und $C^1$-Abbildungen $g_i^{(j)}\colon \MdR^n\to \MdR^p$ mit
\[
f_j(\MdR^n) \subset E_j \cup \bigcup_{i=1}^\infty g_i^{(j)} (\MdR^n),
\]
wobei $\HM^n(E_j)=0$.

Setze $C_{ij}\da \{x\in \MdR^n : \operatorname{rg}(Dg_i^{(j)} (x)) < n \}$. (Ohne Beschränkung der Allgemeinheit ist $p\ge n$). Die Flächenformel ergibt $\HM^n(g_i^{(j)}(C_{ij})) = 0$. Für $N_0 \da \bigcup_{j=1}^\infty E_j \cup \bigcup_{i=1}^\infty g_i^{(j)}(C_{ij})$ ist $\HM^n(N_0)=0$.

Zu $x\in \MdR^n\setminus C_{ij}$ gibt es eine Umgebung $U_{ij}(x)$, so dass $g_j^{(i)}(U_{ij}(x))\subset \MdR^p$ eine $n$-dimensionale Untermannigfaltigkeiten der Klasse $C^1$ ist. Da $\MdR^n\setminus C_{ij}$ offen ist, kann es durch abzählbar viele solcher Umgebungen überdeckt werden.
\end{itemize}
\end{beweis}
...
\item $[0,1]\setminus \MdQ $ ist abzählbar 1-rektifizierbar.
\end{beispiele}

\begin{definition}
Sei $M\subset \MdR^p$ $\HM^n$-messbar und $(\HM^n\MR M)\in \mathbb M(\MdR^p)$, das heißt $\HM^n(M\cap K) < \infty$ für alle kompakten Mengen $K\subset \MdR^p$. Sei $x\in\MdR^p$ und $P\in G(p,n)$. Dann heißt $P$ approximativer Tangentialraum von $M$ in $x$, falls
\[
\HM^n\MR \left(\frac{M-x}{\lambda}\right) \tonach{v} \HM\MR P
\]
für $\lambda \mathop\searrow0$, das heißt für jede Funktion $f\in C_0(\MdR^p)$ gilt
\[
\lim_{\lambda \mathop\searrow0} \int_{\frac{M-x}\lambda} f(y)\, \HM^n(dy) = \int_P f(y) \,\HM^n(dy).
\]
\end{definition}
...
\[
\lim_{\lambda \mathop\searrow0} \int_{\frac{M-x}\lambda} f(y)\, \HM^n(dy) =  \lim_{\lambda\mathop\searrow0} \lambda^{-n}\int_M f(\frac{z-x}\lambda)\,\HM^n(dz)
\]
...
\item Ist $N\subset \MdR^p$ eine $n$-dimensionale $C^1$-Untermannigfaltigkeit von $\MdR^p$ und $x_0 \in N$, dann existiert der Tangentialraum und stimmt mit dem differenzialgeometrischen Tangentialraum $\widetilde {T_{x_0}N}$ überein.

\textbf{Ansatz:} $N\cap V = \{x + g(x) \ad G(x) : x\in (x_0 + \widetilde{T_{x_0}N}) \cap U\}$ mit $V,U$ Umgebungen von $x_0$, $g:(\widetilde{T_{x_0}N} + x_0) \cap U \to (\widetilde{T_{x_0}N}^\bot)$, wobei $g(x_0)=0$.
\end{bemerkungen}
\end{document}
