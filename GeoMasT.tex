...
\begin{definition}[Masse von Differntialformen und Strömen]
\begin{itemize}
...
\emph{Nachtrag: Ströme mit minimaler Masse} \\
$T = \llbracket B^2 \rrbracket \in \mathcal D_2 (\MdR^2)$, d.h. $T(\omega) = \int_B \langle\omega(x),e_1\wedge e_2\rangle \HM^2(dx)$, $\omega \in \mathcal D^2(\MdR^2)$. Frage: Hat $T$ minimaler Masse unter allen 2-Strömen $S \in \mathcal D_2(\MdR^2)$ mit $\partial S = \partial T = \llbracket \partial B^2 \rrbracket$.

\begin{lemma}
Sei $T \in \mathcal D_n(U)$, $U \subset \MdR^n$ offen und $\MN(T) < \infty$. Es gebe ein $\Omega = d\varphi \in \mathcal D^n(U)$ mit $\|\Omega\|\leq1$ und $\MN(T) = T(\Omega)$. Dann gilt:
$$
	\MN(T) \leq \MN(S)\quad (\forall S, \partial S = \partial T).
$$
\end{lemma}
...
\emph{Zurück zum Beispiel}: $\Omega \da dx_1 \wedge dx_2 \in \mathcal D_2(\MdR^2)$. Dann ist $\|\Omega\|=1$ und
$$
...
$$
Ferner gilt für $\varphi(x) = \frac12 (x_1 d_x2 - x_2 d_x1)$ gerade $d\varphi_x = \frac12 \cdot (dx_1 \wedge dx_2 - dx_2 \wedge dx_1) = \Omega$. Mit obigem Lemma folgt, dass $T$ minimierend ist. Beachte $\eta = \eta_1 dx_1 + \eta_2 dx_2 \in \mathcal D^1(\MdR^2)$.
\begin{align*}
...
\begin{satz}
Seien $T_i \in \mathcal D_k(U)$, $U \subset \MdR^n$ offen, $i \in \MdN$ mit
$$
	\sup_{i\in\MdN} \MN_V(T_i) < \infty \quad \text{ für alle } V \subset U \text{ offen }, \bar V \text{ kompakt }, \bar V \subset U.
$$
Dann gibt es eine Teilfolge $(T_{n_i})_{i\in\MdN}$ und $T\in\mathcal D_k(U)$ mit $T_{n_i}\stackrel s\rightharpoonup T$.
\end{satz}

\begin{beweis}[Skizze]
Verwende, dass Ströme stetige, lineare Funktionale auf $\mathcal D^k(U)$ (topologischer Vektorraum) bilden. Jetzt kann man den Satz von Banach-Alaoglu anwenden, der besagt, dass die Menge $(T_i)_i$ eine schwach* konvergente Teilfolge hat, zumindest auf $V$.
\end{beweis}

Sei $\mu$ ein borelreguläres Maß auf $U \subset \MdR^n$ offen mit $\mu(K)<\infty$ für $K\subset \MdR^n$ kompakt (Radonmaß). Sei $\xi: U\to\MdR^m$ eine $\mu$-messbare Abbildung und $\|\xi\| = 1$ $\mu$-fast-überall. Dann wird durch
$$
...
$$
falls $f \in \mathcal C_c(U,\MdR^m)$, $\supp(f) \subset K$, $K\subset U$ kompakt, $\|f\|\leq1$. D.h. $\sup\{L(f) : f\in \mathcal C_c(U,\MdR^m), \|f\|\leq 1, \supp(f)\subset K\} < \infty$ für alle $K \subset U$, $K$ kompakt.

\begin{satz}[Riesz]
Sei $L \colon \mathcal C_c(U,\MdR^m)\to\MdR$, $U \subset \MdR^n$ offen, ein lineares Funktional, das $\sup\{L(f): f\in \mathcal C_c(U,\MdR^m), \|f\|\leq 1, \supp(f)\subset K\} < \infty$ erfüllt. Dann existiert ein Radonmaß $\mu$ auf $U$ und eine $\mu$-messbare Abbildung $\xi: U\to\MdR^m$ mit $\|\xi(x)\| = 1$ für $\mu$-f.a. $x\in U$ und
$$
...
\begin{beweis}
Siehe L. Simon, Lecture Notes ANU, GMT
\end{beweis}

Als Folge erhält man für Ströme lokalendliche Masse.

\begin{satz}
Sei $U \subset \MdR^n$ offen und $T \in \mathcal D_k(U)$. Dann sind äquivalent:
\begin{enumerate}[(1)]
	\item $T \in \MN_{k,\loc}(U)$.
	\item Es gibt ein Radonmaß $\mu_T$ auf $U$ und eine $\mu_T$-messbare Abbildung $\xi: U \to \bw_k \MdR^n$ mit $|\xi(x)|=1$ für $\mu_T$-fast-alle $x\in U$, so dass:
	$$
...
\begin{enumerate}[(1)]
	...
	\emph{Zusammenhänge:}:
	$$
		|\omega| \geq \|\omega\| \geq {\binom n k}^{-\frac12} \cdot |\omega|,
	$$
	$\omega \in \bw^k\MdR^n$; $\|\omega\|=|\omega|$ für $\omega$ einfach ($\omega \in \bw^k\MdR^n$ heißt einfach, falls es $\eta_1,\dots,\eta_k \in \bw^1 \MdR^n$ gibt mit $\omega=\eta_1\wedge\dots\wedge\eta_k$.).
	$$
		|\xi| \leq \|\xi\| \leq {\binom n k}^{\frac12} |\xi|,\, \xi \in \bw_k \MdR^n,\, \|\xi\|=|\xi|
	$$
	für $\xi$ einfach.
	$$
	...
	\item Im vorangehenden Satz setzt man $\overset\rightarrow T(x) \da \frac{\xi(x)}{\|\xi(x)\|}$ und $\|T\| \da \|\xi\| \cdot \mu_T$ ($\|T\|(M) = \int \bot_M(x) \|\xi\| \mu_T(dx)$) und erhält so:
	$$
	...
	\item Ist $T$ durch Integral darstellbar, so erklärt man für $A \subset U$, $A$ Borelsch:
	$$
