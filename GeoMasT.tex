...
Dann ist für \(\alpha(p) = \lambda^p(B^p(0,1))\) mit \(B^p(0,1) \da \{ x \in \mathbb R^d \colon \|x\| = 1 \}\).
...
\end{align*}
Ist nun \(M \subseteq \mathbb R^n\) mit \(\lambda^n(M)=0\). Sei \(\delta > 0\) fest. Zu \(\varepsilon > 0\) gibt es \(A_i \subseteq \mathbb R^n\), wie oben beschrieben, d.h. \(\diam(A_i) < \delta\) und mit \((\ast)\), \(M \subseteq \bigcup_{i=1}^\infty A_i\) und \(\sum_{i=1}^\infty \lambda(A_i) < \varepsilon \Rightarrow \sum_{i=1}^\infty \diam(A_i)^n \leq 2^n \cdot n^{\frac{n}2} \cdot \varepsilon \Rightarrow \HM^n_\delta(M) \leq \alpha(n) \cdot 2^{-n}\cdot 2^n \cdot n^{\frac{n}2} \cdot \varepsilon\). Da \(\varepsilon\) beliebig war, ist \(\HM^n_\delta(M=0\). Folglich ist auch \(\HM^n(M)=0\).
\end{beweis}
...
\par
Sei \(\mathcal S\) die Menge aller Kugeln in \(\mathbb R^n\) mit Radius kleiner als \(\frac{\delta}2\). Wegen Korollar \ref{cor:2.9} und Übung Nr. 1 auf Blatt 4 gibt es eine abzählbare Folge \((B_i)_{i\in\mathbb N}\) in \(\mathcal S\) paarweise disjunkter Kugeln in \(U\) mit \(\lambda^n(M \setminus \bigcup_{i=1}^\infty B_i) = 0\). Mit Lemma \ref{lem:2.28} folgt
\begin{align*}
...
&\leq \Lip(f) \cdot (d d(x,y)^2)^{\frac12} \\
&= \sqrt n \Lip(f) d(x,y)
\end{align*}
\end{enumerate}
\end{beweis}
...
\begin{lemma}
Die Abbildung \(\varphi: \HR^2 \rightarrow (-\infty, \infty]\), \(\varphi(x,y) \da \frac14 \chi(x+y,x-y) + \langle x,y \rangle, x,y\in\HR\).
\begin{enumerate}[(1)]
