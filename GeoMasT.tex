...
Wegen Satz \ref{satz:2.2} erhält man \(U \subset X\) offen mit \(B \subset U\) und \(\tau(U \setminus B) \leq \varepsilon\). Also ist \(\mu(A \setminus U) \leq \mu(A \setminus B)=0\) und
\[
\tau(U) = \tau(U \cap B) + \tau(U \setminus B) \leq \tau(B) + \varepsilon = \tau_\mu(A) + \varepsilon.
\]
Sei \(C \da \{ (x,S) \in V : S \subset U \wedge \frac{\nu(S)}{\mu(S)} < c \}\). Für \(z\in A\cap U\) gilt: \(V\text{-}\liminf\frac{\nu(S)}{\mu(S)} < c\), also \(\inf\{\diam(S):(z,S)\in C\}=0\) für alle \(z \in A \cap U\). 

Da \(V\) eine \(\mu-\)Vitali-Relation ist, enthält \(C(A\cap U)\) eine abzählbare disjunkte \(\mu\)-Überdeckung \(\mathcal(S)\) von \(A\cap U\). Aus \(\mu(A \setminus U)=0\) folgt \(\mu(A\cap U \setminus \bigcup_{S\in \mathcal{S}} S) = 0\), \(\nu_\mu \ll \mu\) und \(\nu_\mu \leq \nu\) und damit:
\begin{align*}
...
\]
und gilt nach Lemma \ref{lem2.11} und
...
\end{align*}
und für \(a,b \in \mathbb{R}\) mit \(a < b\)
\begin{align*}
R(a,b) & \da \{ x \in X : V\text{-}\liminf_x\frac\nu\mu<a<b<V\text{-}\limsup_x\frac\nu\mu \}.
\end{align*}

Da \(V\) eine \(\mu\)-Vitali-Relation ist, enthält \(C(P)\) eine abzählbare disjunkte \(\mu\)-Überdeckung \(\mathcal{S}\) von \(P\), sodass
\[
...
Seien nun \(a<b,\; a,b\in\mathbb{Q}\) und \(A \subset Q\), \(B \subset R(a,b)\) beschränkt. Für ein beliebiges \(c > 0\) gilt:
\[
...
\end{align*}
für $a < b,$ $\mu(B)\leq\infty$, d.h. \(\mu(B)=0\).
...
Es existieren \(B_i \in \borel(X)\) mit \(A_i \subset B_i\) und für alle $\forall C\in\borel(X)$ sei 
\begin{align*}
...
\]
Es ist \(A_{i,n} \subset M_i\), \(A_{i,n}\nearrow M_i \cap T\) und \(A_{1,n} \cup A_{2,n} \subset T\). Es gilt
\begin{align*}
\mu(T) &\geq \lim_{n\rightarrow\infty} \mu(A_{1,n} \cup A_{2,n})\\
&\overset!\geq \lim_{n\rightarrow\infty} \mu(A_{1,n}) + \lim_{n\rightarrow\infty} \mu(A_{2,n}) \\
