...
Für einen metrischen Raum $(X,d)$, f\"ur Mengen $A,B\subset X$ und $x\in X$ sei 
$$
...
\]
Für $M\subset X$ erh\"alt man nun
\begin{align*}
...
\item Für $Z\subset X$, $C\subset V$ mit $\inf\{\diam(S): (z,S)\in C\}=0$ für alle $z\in Z$ enthält die Familie $C(Z) \da \{S: (z,S)\in C\}$ für ein $z\in Z$ eine abzählbare disjunkte $\mu$-Überdeckung von $Z$.
\end{enumerate}
\end{definition}

Im Folgenden ist das Ziel, Methoden und Kriterien zum Nachweis von Eigenschaft (2) kennen zu lernen und das Arbeiten mit Vitali-Relationen bei der Differentation von Maßen.
...
\item Der Raum $(X,d)$ ist spezieller, geeignet ist etwa $(\MdR^n,\|\cdot\|)$, $\mu$ ein beliebiges Radonmaß.
\end{enumerate}
...
Definiere
\begin{multline*}
\Omega \da \{\mathcal R\subset \mathcal S: \mathcal R \text{ ist eine disjunkt Familie von Mengen, für alle $S\in\mathcal S$ gilt:}\\
...
&= \mu(\bigcup_{j=k}^\infty (W_j \cap A_j \setminus \bigcup_{C\in\mathcal C_j}C)) \\
...
\item $\inf\{\diam(S): S\in C(Z),\, x\in S\} = \inf\{\diam(S) : (z,S) \in C, x\in S\} \le \inf\{\diam(S): (x,S)\in C\} = 0$ für alle $x\in Z$.
...
\textbf{Nachweis:}
Sei $S=\mathbb B(y,\varrho)\in \mathcal S'^t_{\mathbb B(x,s)}$. Dann ist $\mathbb B(y,\varrho) \cap \mathbb B(x,s) \ne \emptyset$, $\diam(\mathbb B(y,\varrho)) \le t \cdot \diam(\mathbb B(x,s))$. Sei $\bar z\in \mathbb B(y,\varrho)$ und $z_0\in\mathbb B(y,\varrho)\cap \mathbb B(x,s)$. Dann gilt $d(\bar z, x)\le d(\bar z, z_0) + d(z_0,x)\le \diam(\mathbb B(y,\varrho)) + s \le t \cdot 2s +s = (1+2t)s$. Hiermit:
\[
...
In Satz \ref{satz:2.8} werden wir einen Überdeckungssatz formulieren, mit dessen Hilfe zum Beispiel für $X\in\MdR^n$, $d=\|\cdot\|$, gezeigt werden kann dass $V$ für jedes Maß $\mu\in\mathbb M(\MdR^n)$ eine $\mu$-Vitali-Relation ist.
...
&= s^n \cdot u \frac vu \cdot (2t+1)^n \\
...
\begin{korollar}
\label{kor:2.9}
Seien $\mu$ ein Borel-reguläres Maß auf $\MdR^n$ und $\mathcal F$ eine Familie abgeschlossener Bälle mit positiven Radien in $\MdR^n$. Sei $A$ die Menge der Mittelpunkte dieser Bälle. Sei $\mu(A)<\infty$ und $\inf\{r>0: \mathbb B(a,r)\in\mathcal F\}=0$ für alle $a\in A$. Dann gibt es zu jeder offenen Menge $\mathcal U \subset\MdR^n$ eine abzählbare disjunkte Teilfamilie $\mathcal G\subset\mathcal F$, so dass $B\subset\mathcal U$ für alle $B\in \mathcal G$ und $\mu(A\cap U \setminus \bigcup_{B\in\mathcal G}B)=0$, das heißt $\mathcal G$ ist eine $\mu$-Überdeckung von $A\cap U$.
\end{korollar}

\begin{beweis}
Wähle $\theta \in (1-\frac1{N_n}, 1)$.

\textbf{Behauptung:} Es gibt $M_1\in\MdN$ und disjunkte Bälle $B_1,\ldots,B_{M_1}\in\mathcal F$ mit $B_i \subset \mathcal U$ und $\mu(A\cap U\setminus \bigcup_{i=1}^{M_1} B_i) \le \theta \cdot \mu(A\cap U)$.
\end{beweis}

\textbf{Nachweis:} Sei $\mathcal F_1 \da \{B\in\mathcal F : \diam(B) \le 1,\, B\subset U\}$. Die Menge der Mittelpunkte von Bällen aus $\mathcal F_1$ ist gerade $A\cap U$. Wegen Satz \ref{satz:2.8} gibt es zu $\mathcal F_1$ Teilfamilien $\mathcal G_1,\ldots,\mathcal G_n\subset\mathcal F_1$, wobei $\mathcal G_i$ abzählbar viele disjunkte Bälle enthält mit $A\cap U\subset A\cap U \cap \bigcup_{i=1}^{N_n}\bigcup_{B\in\mathcal G_i}B$. Hieraus folgt
\begin{align*}
\mu(A\cap U)
...
\section{Differentation von Maßen}

\begin{definition}
Seien $\mu\in\mathbb (X)$, $V$ eine $\mu$-Vitali-Relation auf $X$, $x\in X$. Seien $D\subset \mathcal P(X)$ und $f:\mathcal D\to \MdR$. Dann wird für jede Teilfamilie $C\subset V$ mit $\inf\{\diam(S): (x,S)\in C\}=0$ erklärt:
\begin{align*}
C\text{-}\limsup_x f &\da C\text{-}\limsup_{s\to x} f(S) \da
\lim_{x\searrow0} \sup\{f(S):(x,S)\in C,\, S\in\mathcal D,\, \diam(S)<\ep\}\\
\intertext{und}
C\text{-}\limsup_x f &\da C\text{-}\liminf_{s\to x} f(S) \da
\lim_{x\searrow0} \inf\{f(S):(x,S)\in C,\, S\in\mathcal D,\, \diam(S)<\ep\}.
\end{align*}
Falls $C\text{-}\limsup_x f = C\text{-}\liminf_x f$ gilt, so wird erklärt:
\begin{align*}
C\text{-}\lim_x f \da C\text{-}\lim_{s\to x} \da C\text{-}\limsup_x f = C\text{-}\liminf_x f.
\end{align*}
...
0 \le \nu(A_k) - \nu_\mu(M) \le \frac1k.
\]
Setze $A\da\bigcup_{k=1}^\infty A_k \in \borel(X)$. Es gilt:
\begin{align*}
...
&\le \mu( (\bigcup_{i=1}^\infty M_i) \setminus (\bigcup_{i=1}^\infty A-i))\\
...
\intertext{und ebenso}
\mu(M^c\setminus \underbrace{(C\cap M^c)}_{\in\borel(X)}) &= \mu( (M^c\cap A)\setminus (C\cap M^c)) \le \mu(A\setminus C) = 0.
\end{align*}
...
&\ge \nu_\mu(M) + \nu_\mu(M^c\cap A)
\end{align*}
Somit gilt überall Gleichheit, insbesondere ist
\begin{align*}
\infty > \nu(C\cap M) + \nu(C\cap M^c) = \nu_\mu(M) + \nu_\mu(C^c\cap A)
\end{align*}
Da der erste (bzw. zweite) Summand links größer gleich dem ersten (bzw. zweiten) Summand rechts ist, folgt $\nu_\mu(M) = \nu(C\cap M)$ und $\nu_\mu(M^c \cap A) = \nu(M^c \cap C)$.
\end{beweis}

Fall 2: $M\subset A$ beliebig. Dann existieren $D_1,D_2\in\borel(X)$ mit $M\subset D_i$ und $\nu(M\cap S) = \nu(D_1\cap S)$ und $\mu(M\cap S) = \mu(D_2\cap S)$ für alle $S\in\borel(X)$, da $\borel(X) \subset \mathcal A_\mu,A_\nu$. Setze $D\da D_1\cap D_2\cap A\in\borel(X)$. Es gilt $M\subset D\subset A$. Ferner ist
\begin{align*}
\nu(M\cap S) \le \nu(D\cap S) \le \nu(D_1\cap S) = \nu(M\cap S)
\end{align*}
also $\nu(D\cap S) = \nu(M\cap S)$ und ebenso $\mu(D\cap S) = \mu(M\cap S)$. Insbesondere auch $\mu(M\setminus S) = \mu(D\setminus S)$. Wegen $D\in\borel(X)$, $D\subset A$ ist $\nu_\mu(D) = \nu(C\cap D)$ nach Fall 1. Zusammen gilt
\begin{align*}
\nu_\mu(M)
