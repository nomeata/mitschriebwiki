...
\begin{equation}\label{C1trafo}
\int_U g\circ f(x) Jf(x) \lambda^n(dx)=\int_{f(U)}g(y)\lambda^n(dy),
\end{equation}
wobei $Jf(x):=|\det(Df(x)|$. 
...
Ist speziell $A\subset U$ eine Borelmenge und $g(y):=\ind_{f(A)}(y)$. Dann geht \eqref{C1trafo} über in
$$
...
Wie sieht es dagegen in der folgenden Situation (Bild) aus?
