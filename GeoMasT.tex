...
\item $\bigwedge^k V$ wird in kanonischer Weise zu einem Vektorraum. Die Elemente von $\bigwedge^k V$ nennt man $k$-Kovektoren, falls $V=\MdR^n$. Ist $V=(\MdR^n)^*$, so werden die Elemente von $\bigwedge^k (\MdR^n)^* =:  \bigwedge_k\MdR^n$ als $k$-Vektoren bezeichnet.
\end{enumerate}
\end{definition}

\begin{bemerkungen}
\item $\bigwedge^1\MdR^n = (\MdR^n)^*$
\item $\bigwedge_1\MdR^n = ((\MdR^n)^*)^* =\MdR^n$ (mit der üblichen Identifikation).
...
\[
\eta_1\wedge \ldots\wedge \eta_k \in \bigwedge\limits^k\MdR^n
\]
durch
\[
(\eta_1\wedge \ldots\wedge \eta_k)(v_1,\ldots,v_k) \da \det\left( \langle \eta_i,v_j\rangle_{i,j=1,\ldots,k}\right)
\]
erklärt. Man beachte hierbei $\langle\eta_i,v_j\rangle=\eta_i(v_j)$.
...
\textbf{Ergänzung:} Mit $\mathcal T^k(V)\da \{ (T\colon V^k\to\MdR) : T \text{ ist $k$-linear}\}$ bezeichnet man die Tensoren der Stufe $k$ über dem Vektorraum $V$. Es gibt eine Abbildung
\begin{align*}
...
\]
Man bezeichnet $S\otimes S$ als das Tensorprodukt von $T$ und $S$. 
Ferner betrachten wir für $p\in\MdN$ die Abbildung
\begin{align*}
\operatorname{Alt}\colon \mathcal T^p(V) &\to \bigwedge^p V\\
T & \mapsto \operatorname{Alt}(T),
\end{align*}
wobei
\[
\operatorname{Alt}(T)(v_1,\ldots,v_p) \da \frac{1}{p!} \cdot \sum_{\pi\in S_p} \operatorname{sgn}(\pi) \cdot T(v_{\pi(1)},\ldots,v_{\pi(p)}).
\]
...
\[
\omega \wedge \eta \da \frac{(k+l)!}{k!l!} \operatorname{Alt}(\omega\otimes \eta) \in \bigwedge^{k+l}\MdR^n.
\]
...
\[
v_1\wedge \ldots\wedge v_k \in \bigwedge\limits_k\MdR^n
\]
durch
\[
...
$\eta_1\wedge\ldots\wedge \eta_k\in\bigwedge^k\MdR$ als Linearform auf $\bigwedge_k\MdR^n$ 
definiert werden kann, indem man
$$
(\eta_1\wedge\ldots\wedge \eta_k)(v_1\wedge\ldots\wedge v_k):=(\eta_1\wedge\ldots\wedge \eta_k)(v_1,\ldots, v_k)
$$
für $v_i\in\MdR^n$ erklärt. Die Wohldefiniertheit ist leicht einzusehen. 

\medskip

In gleicher Weise wird 
$v_1\wedge \ldots\wedge v_k \in \bigwedge_k\MdR^n$ als Linearform auf $\bigwedge^k\MdR^n$ erklärt, 
indem man 
$$
...
$$
e_{i_1}^*\wedge\ldots\wedge e_{i_k}^*,\qquad 1\le i_1<\ldots<i_k\le n
$$
eine Basis von $\bigwedge^k\MdR^n$ ist. Ebenso ist 
$$
e_{i_1}\wedge\ldots\wedge e_{i_k},\qquad 1\le i_1<\ldots<i_k\le n
$$
eine Basis von $\bigwedge_k\MdR^n$. Diese Basen sind zueinander dual in Bezug auf obige 
Deutung von $k$-Kovektoren als Linearformen auf $k$-Vektoren. 


\bigskip

{\bf Differentialformen} 
...
\in {\mathbb N}$ sei $V^{p} := V \times \dots \times V$ das $p$-fache 
...
\begin{eqnarray*}
(S + T)(v_{1},\dots,v_{p}) & := & S(v_{1},\dots,v_{p}) + 
T(v_{1},\dots,v_{p}) \\
(\lambda S)(v_{1},\dots,v_{p}) & := & \lambda S(v_{1},\dots,v_{p})
\end{eqnarray*}
für $v_{1},\dots,v_{p} \in V$ 
ein reeller Vektorraum. Wir setzen ${\cal T}^{0}(V) := {\mathbb R}$.\\
...
\[ (S \otimes T)(v_{1},\dots,v_{p},v_{p+1},\dots,v_{p+q}) := 
...
definiert durch
\[ (f^{*}T)(v_{1},\dots,v_{p}) := T(f(v_{1}),\dots,f(v_{p})) \]
...
aller alternierenden $p$-Tensoren; sei ferner $\Omega^{0}(V) := 
{\mathbb R}$ und $\Omega^{1}(V) := {\cal T}^{1}(V)$. Dann ist 
$\Omega^{p}(V)$ ein linearer Unterraum von ${\cal T}^{p}(V)$; seine 
Elemente heißen auch $p$-Kovektoren über $V$.\\

\noindent
{\bf Beispiel.} Inst $n:=\text{dim}(V)$, so ist $\det\in\Omega^n(V)$.\\
...
$\sigma$ ab. Im geraden Fall nennt man $\sigma$ gerade und setzt ${\rm 
sgn}(\sigma) = 1$, im ungeraden Fall heißt $\sigma$ ungerade und man 
setzt ${\rm sgn}(\sigma) = -1$. Die Abbildung sgn ist ein 
Gruppenhomomorphismus von $(S_{p},\circ)$ nach $(\{-1,1\},\cdot)$.

\bigskip

\noindent {\bf Definition.} Für $T \in {\cal T}^{p}(V)$ sei
\[ {\rm Alt}(T)(v_{1},\dots,v_{p}) := \frac{1}{p!} \sum_{\sigma \in 
S_{p}} {\rm sgn}(\sigma) T(v_{\sigma(1)},\dots,v_{\sigma(p)}) \]
...
Transposition und $\sigma' := \sigma \circ (i,j)$ für $\sigma \in 
S_{p}$. Dann ist
\begin{eqnarray*} 
&   & {\rm Alt}(T) (v_{1},\dots,v_{j},\dots,v_{i},\dots,v_{p}) \\
& = & \frac{1}{p!} \sum_{\sigma \in S_{p}} {\rm sgn}(\sigma) 
...
-\omega(v_{1},\dots,v_{p}) = {\rm sgn}(\sigma) 
...
\[ \omega \wedge \eta := \frac{(p+q)!}{p!q!} {\rm Alt}(\omega \otimes 
\eta). \]
...
& = & \sum_{\sigma \in S_{p+q}} {\rm sgn}(\sigma) 
S(v_{\sigma(1)},\dots,v_{\sigma(p)}) 
T(v_{\sigma(p+1)},\dots,v_{\sigma(p+q)}) \\
...
\[ \varphi_{i}(e_{j}) := \delta_{ij} = \left\{ \begin{array}{ll} 1, & 
\mbox{ für } i = j, \\
...
die Menge
\[ B := \{ \varphi_{i_1} \wedge \dots \wedge \varphi_{i_p} : 1 \le 
i_{1} < \dots < i_{p} \le n\} \]
...
\varphi_{i_{\sigma(p)}} \]
sowie
\[ \omega(e_{i_1},\dots,e_{i_p}) = {\rm sgn}(\sigma) 
...
\varphi_{i_p}) (e_{j_1},\dots,e_{j_p}) \\
& = & \sum_{\sigma \in S_{p}} {\rm sgn}(\sigma) 
...
\[ \eta(v_{1},\dots,v_{n}) := {\rm det} (a_{ij})_{i,j=1}^{n}. \]
...
\[ M_{p}^{n} := \{(i_{1},\dots,i_{p}) \in \{1,\dots,n\}^{p}: 1 \le 
i_{1} < \dots < i_{p} \le n\}. \]
Insbesondere sei
\[ \varphi_{I} := \varphi_{i_1} \wedge \dots \wedge \varphi_{i_p} \]
...
\begin{eqnarray*}
(\omega_{1} + \omega_{2})(X) & := & \omega_{1}(X) + \omega_{2}(X), \\
(f\omega) (X) & := & f(X)\omega(X) =: (f \wedge \omega)(X), \\
...
\[ (L^{*}T)(v_{1},\dots,v_{p}) := T(Lv_{1},\dots,Lv_{p}), 
...
$M$ erklärt durch
\[ (F^{*}\omega)_{X} := (DF_{X})^{*}\omega_{F(X)}, \qquad X \in M. \]
...
\wedge dx^{\sigma(n)} \\
& = & a \circ F \sum_{\sigma \in S_{n}} {\rm sgn}(\sigma) 
...
die $(p+1)$-Form
\begin{eqnarray*}
d\omega & := & \sum_{1 \le i_{1} < \dots < i_{p} \le n} da_{i_{1} 
...
definieren hierzu
\begin{eqnarray*}
(V) & := & v_{1} dx^{1} + v_{2} dx^{2} + v_{3}dx^{3} \\
((V)) & := & v_{1} dx^{2} \wedge dx^{3} + v_{2} dx^{3} \wedge 
...
\[ (I\omega)_{X} := \sum_{1 \le i_{1} < \dots < i_{p} \le n} 
...
abgeschlossene Einheitswürfel. Speziell setzen wir $[0,1]^{0} := \{0\}$.
...
\[ \int_{[0,1]^{p}} \omega := \int_{[0,1]^{p}} f = \int_{0}^{1} \dots 
...
\[ \int_{c} \omega := \int_{[0,1]^{p}} c^{*}\omega, \quad p \ge 1 \]
und
\[ \int_{c} \omega := \omega(c(0)), \quad p = 0. \]
...
im ${\mathbb R}^{1}$. Mit $V := (v_{1},\dots,v_{n})$ erhält man für 
...
\[ \partial I^{2} := - (I_{1,0}^{2} - I_{1,1}^{2}) + (I_{2,0}^{2} - 
I_{2,1}^2), \]
...
[0,1]^{p} \to {\mathbb R}^{p}$ sei erklärt durch $I^{p}(X) := X$ 
für $X \in [0,1]^{p}$. 
...
erklärt durch
\begin{eqnarray*}
&   & I_{i,0}^{p}(X) := (x_{1},\dots,x_{i-1},0,x_{i},\dots,x_{p-1}), 
\\
&   & I_{i,1}^{p}(X) := (x_{1},\dots,x_{i-1},1,x_{i},\dots,x_{p-1}) 
\end{eqnarray*}
für $X = (x_{1},\dots,x_{p-1}) \in [0,1]^{p-1}$. 
\item Setze 
$I_{1,0}^{1}(0) := 0$ und $I_{1,1}^{1}(0) = 1$. 
\item Der {\em Rand des 
singulären $p$-Würfels}\index{Rand} $c$ ist erklärt durch
\[ \partial c := \sum_{i=1}^{p} (-1)^{i} \left[ c \circ I_{i,0}^{p} - 
c \circ I_{i,1}^{p}\right]. \]
\item Der Rand der $p$-Kette $\sum a_{i}c_{i}$ ist erklärt durch
\[ \partial\left(\sum a_{i}c_{i}\right) := \sum a_{i}\partial c_{i}. \]
\end{itemize}
...
\begin{eqnarray*}
{\mathbb R}^{n}_{k} & := & \{X \in {\mathbb R}^{n}: x_{k+1} = \dots = 
x_{n} = 0\}, \\
H_{k}^{n} & := & \{X \in {\mathbb R}^{n}: x_{1} \ge 0, x_{k+1} = 
\dots = x_{n} = 0\}, \\
\partial H^{n}_k & := & \{X \in {\mathbb R}^{n}:x_{1} = 0, x_{k+1} = 
\dots = x_{n} = 0\}.
\end{eqnarray*}
...
\item Eine Hemisphäre $S^{n-1}_{+}:=S^{n-1}\cap E_n^\perp$ ist eine 
...
\item {\em Möbiusband}.\index{M{ö}biusband} „Ein Zeigervektor $a(\varphi)$ beschreibe 
...
Sei
\[ a(\varphi) := (\cos \varphi, \sin \varphi, 0), \qquad \varphi 
\in [-\pi,\pi] \]
und
\[ b_{\varphi}(\psi) := \cos (\psi) a(\varphi)  + \sin( \psi) e_{3} , 
\qquad \psi \in \left[ -\frac{\pi}{2}, \frac{\pi}{2}\right]. \]
Sei $R > 1$ und $\phi: [-\pi, \pi] \times (-1,1) \to {\mathbb R}^{3}$ 
mit
\begin{eqnarray*}
\phi(\varphi,t) & := & Ra(\varphi) + t b_{\varphi} 
...
H_{k}^{n} \setminus \partial H_{k}^{n}$. Die Menge $U' := h^{-1}(V')$ 
...
R}^{n}$ mit $F(W_{1}) = U_{2} \cap F(W)$. Setze $U_{0}:= U \cap U_{1} 
\cap U_{2}$ und $V := g^{-1}(U_{0})$. Dann gilt wegen (a)
\begin{eqnarray*}
...
\[ TM_{X} := DF_{F^{-1}(X)} ({\mathbb R}^{k}) \subset {\mathbb R}^{n} 
\]
...
\[ (F^{*}\omega)_{Z} := (DF_{Z})^{*} \omega_{X}. \]
Auf diese Weise wird eine $p$-Form $F^{*}\omega$ auf $W$ erklärt. 
Nach Satz \ref{Satz3.6.4} ist die Abbildung $G := F^{-1} \circ \overline{F}$ in 
...
mit $F(Z) = \overline{F}(\overline{Z}) = X$. Sei $G := F^{-1} \circ 
...
\[ \Delta := {\rm det}\, (\alpha_{ij})_{i,j=1}^{n}. \]
...
\end{eqnarray*}
wobei
\[ f(z) := {\rm sgn}({\rm det}(D(\tilde{F} \circ G)_{Z})). \]
...
\[ W(X, a_{X}) := \{Y \in {\mathbb R}^{n}: \|Y - X\|_{\max} \le 
a_{X}\} \subset U. \]
...
\[ g(X) := \prod_{j=1}^{n} f\left(\frac{x_{j}-z_{j}}{a}\right). \]
...
ist 
\[ C_{k+1} := M \setminus \left(D_{1}^{0} \cup \dots \cup D_{k}^{0} 
...
\[ \varphi_{i}(X) := \left\{ \begin{array}{ll}
...
und $G := c_{2}^{-1} \circ c_{1}$ auf $c_{1}^{-1}(B)$. Dann gilt auf $c_{1}^{-1}(B)$:
\begin{eqnarray*}
...
\tilde{F}(x_{1},\dots,x_{n}) := F(x_{1},\dots,x_{k-1},1 - x_{k}) \]
und $\tilde{c} := \tilde{F} \mid [0,1]^{k}$. Dann ist $\tilde{F}$ und 
...
\]
erfüllt. Die Einschränkung $c := F \mid [0,1]^{k}$ ist dann ein normaler 
...
\[ \int_{M} \omega := \sum_{i=1}^{m} \int_{M} \varphi_{i}\omega. \]
...
von $H^{k}$. Definiere
\[ W' := \{(x_{1},\dots,x_{k-1}) \in {\mathbb R}^{k-1}: 
(0,x_{1},\dots,x_{k-1}) \in W\} \]
und $F'(x_{1},\dots,x_{k-1}) := F(0,x_{1},\dots,x_{k-1})$ mit 
...
eine Abbildung. Dann erklären wir die Abbildung
 $G(x,t) := x + t(F(x) - x)$ für $x \in M$ und 
...
\[ \varphi(t) := \int_{M} dg_{1} \wedge \dots \wedge dg_{n} = 
...
\noindent
Als Folgerung erhalten wir für $B := \{X \in {\mathbb R}^{n}: \|X\| 
\le 1\}$:
