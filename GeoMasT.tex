...
\textbf{Ansatz:} $N\cap V = \{x + g(x) \ad G(x) : x\in (x_0 + \widetilde{T_{x_0}N}) \cap U\}$ mit $V,U$ Umgebungen von $x_0$, $g:(\widetilde{T_{x_0}N} + x_0) \cap U \to (\widetilde{T_{x_0}N}^\bot)$, wobei $g(x_0)=0$ und \(Dg(x_0)=0\). Weiter folgt, falls \(\lambda>0\) hier klein ist (\(f \in C_0(\MdR^p)\)):
\begin{align*}
...
&\to \int_{T_{x_0}N} f(z) \HM^n(dz) & (\lambda \to 0)
\end{align*}
...
\par
Ist \(\theta^{n*}(\mu,A,x) = \theta^n_*(\mu,A,x)\), so nennt man \(\theta(\mu,A,x) \da \theta^n_*(\mu,A,x)\) die Dichte von \(A\) in \(x\) bezüglich \(\mu\).
\end{definition}

\begin{lemma}\label{lem:3.21}
Ist \(\mu\) ein Borelreguläres äußeres Maß auf \(\MdR^p\), \(A \subset \MdR^p\) \(\mu\)-messbar und \(\mu\MR A\in \mathbb{M}(\MdR^p)\). Dann gilt \(\theta^n(\mu,A,x)=0\) für \(\HM^n\)-fast-alle \(x\in\MdR^p\setminus A\).
\end{lemma}
Ohne Beweis (ist kompliziert).

\begin{proposition}\label{prop:3.22}
Ist \(M \subset \MdR^p\) abzählbar \(n\)-rektifizierbar, \(\HM^n\)-messbar und \(\HM^n\MR M \in \mathbb{N}(\MdR^p)\), dann existiert \(T_x M\) für \(\HM^n\)-fast-alle \(x \in M\).
\end{proposition}
...
\]
Für jedes solches \(x\) existiert \(T_x M\) und \(T_x N_j\). Sei \(f \in C_0(\MdR^p)\) mit \(\supp(f) \subset B(0,R)\) (für ein \(R \in (0,\infty)\)). Dann gilt
\begin{align*}
...
\end{align*}
Also: \(\lim_{\lambda \downto 0} \lambda^{-n} \int_{M\setminus M_j} f(\frac{z-x}\lambda) \HM^n (dz) = 0\). Ebenso folgt
\[
...
\begin{definition}
Sei \(M \subset \MdR^p\) \(\HM^n\)-messbar. Man nennt die Funktion \(\theta: M \to (0,\infty)\), die \(\HM^n\)-messbar ist und
\[
\int_{M\cap K} \theta d\HM^n < \infty
\]
für alle \(K \subset \HM^n\) mit \(K\) ist \(\HM^n\)-messbar und beschränkt erfüllt, eine Gewichtsfunktion zu \(M\).
\end{definition}

\begin{bemerkung}
Sind \(M, \theta\) wie in der Definition, so ist
\[
\mu\MR\theta \da \int_\cdot \theta(x) \HM^n(dx) \in \mathbb M(\MdR^p).
\]
\end{bemerkung}
...
\begin{definition}
Sei \(M \subset \MdR^p \HM^n\)-messbar und \(\theta: M \to (0,\infty)\) eine Gewichtsfunktion zu \(M\). Dann heißt \(P \in G(p,n)\) \(n\)-dimensionaler \emph{approximativer Tangentialraum} von \(M\) in \(x\) bezüglich \(\theta\), falls
\[
(\eta_{x\lambda})_\# \left(\lambda^{-n}(\HM^n\MR \theta)\right) \overset{v}\to \theta(x) (\HM^n \MR P) \quad (\lambda\downto0),
\]
...
\begin{bemerkung}
\begin{enumerate}[(1)]
\item Ist \(\theta\) fest, so ist der approximative Tangentialraum \(P\) von \(M\) in \(x\) bezüglich \(\theta\) eindeutig bestimmt, falls er existiert. Bezeichnung: \(T_x^\theta M \da P\).
...
\]
\item Sind \(\theta\) und \(\tilde\theta\) Gewichtsfunktionen zu \(M\), so gilt für \(M_\theta \da \{x\in M \colon T_x^\theta M \text{ existiert }\}\), \(M_{\tilde\theta} \da \{x\in M \colon T_x^{\tilde\theta} M \text{ existiert }\}\):
\[
\HM^n(M_\theta \mathop\Delta M_{\tilde\theta}) = \HM^n\left( (M_\theta \setminus M_{\tilde\theta}) \cup (M_{\tilde\theta} \setminus M_\theta) \right) = 0
\]
und für \(\HM^n\)-fast-alle \(x \in M_\theta \cap M_{\tilde\theta}\) gilt \(T_x^\theta M = T_x^{\tilde\theta}M\).
\end{enumerate}
\end{bemerkung}

\begin{satz}\label{satz:3.24}
Sei \(M \subset \MdR^p\) \(\HM^n\)-messbar. Dann sind äquivalent:
\begin{enumerate}[(a)]
\item \(M\) ist abzählbar \(n\)-rektifizierbar.
...
\begin{beweis}[Skizze]
(a) \(\Rightarrow\) (b): Zu \(M\) betrachtet man eine Zerlegung \(M = M_0 \mathop{\dot\cup} {\dot\bigcup}_{j\geq1} M_j\) mit \(\HM^n\)-messbaren Mengen \(M_j \subset \MdR^p\), wobei \(\HM^n(M_0)=0\) und \(M_j\subset N_j\), \(N_j\) ist \(n\)-dimensionale \(C^1\)-Untermannigfaltigkeit von \(\MdR^p\) mit \(\HM^n(N_j)<\infty\). Sei \(\theta\) eine beliebige Gewichtsfunktion zu \(M\) und \(\mu \da \HM^n \MR \theta\). Betrachte \(x\in M_j\) mit folgenden Eigenschaften:
\begin{enumerate}[(1)]
...
\end{enumerate}
Mit Hilfe des Satzes von Lusin folgt, dass \(\HM^n\)-f.a. \(x\in M_j\) diese Eigenschaften erfüllen. Dann gilt:
\[
...
\begin{beispiel}
\(M = [-e_1, e_1] \cup \bigcup_{n \in \MdN} \left( [-e_1, e_1] + \frac1n e_2 \right)\). Setze \(\theta|_{M_i} = \frac12 i\). Zeige \(T_0^\theta M = \MdR e_1\). \(\theta|_{[-e_1,e_1]} \da \theta(0)\).
\end{beispiel}
