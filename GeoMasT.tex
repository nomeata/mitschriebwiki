...
Sei $(X,d)$ ein metrischer Raum mit der von $d$ induzierten Topologie $\mathcal T$. Sei $\borel(X)$ die Borel-$\sigma$-Algebra. Ist $\mu$ ein äußeres Maß auf $X$ und sind alle offenen (alle abgeschlossenen) Mengen in $\A_\mu$, so gilt $\borel(X)\subset \A_\mu$. Wir nennen $\mu$ Borel-regulär, wenn $\mu$ $\borel(X)$-regulär ist, das heißt $\borel(X)\subset \A_\mu$ und zu jedem $M\subset X$ existiert ein $B\in\borel(X)$ mit $\mu(M)=\mu(B)$.


\begin{lemma}
\label{lem:2.1}
Sei $(X,d)$ ein metrischer Raum und $\A\subset \mathcal P(X)$ und für jede Folge $(A_i)_{i\in\MdN}$ in $\A$ gelte $\bigcup_{i=1}^\infty A_i \in \A$ und $\bigcap_{i=1}^\infty A_i \in \A$. Enthält $\A$ ferner alle offenen Mengen oder alle abgeschlossenen Mengen, so gilt $\borel(X)\subset X$.
\end{lemma}
...
\[
\mu(A) = \inf\{\mu(U): U\supset A,\, C\text{ offen}\}.
\]
\end{enumerate}
\end{satz}
...
\begin{definition}
Seien $X$ eine Menge, $(Y,\mathcal T)$ ein topologischer Raum, $\mu$ ein äußeres Maß auf $X$ und $f\in\mathbb F_\mu(X,Y)$. Man nennt $f$ $\mu$-messbar, falls $f$ $\mu$-messbar bezüglich $\borel(X)$ ist, das heißt $f^{-1}(\borel(Y))\subset \A_\mu$.
\end{definition}

Für einen metrischen Raum $(X,d)$, $A,B\in X$ und $x\in X$ sei $d(x,A) \da \inf\{d(x,y): y\in A\}$ sowie $d(A,B)\da \inf\{d(x,y),x\in A,\, y\in B\}$.
...
\item Für jedes $M\subset X$ und alle Mengen $A,B\subset Y$ mit $d(A,B)>0$ gilt
\[
...
(2)$\implies$(1): Zeige $f^{-1}(C) \in \A_\mu$ für abgeschlossene $C\in Y$. Für $y\in Y$ sei $g(y) \da d(y,C)$. Ohne Beschränkung der Allgemeinheit sei $C\ne \emptyset$. Für $a,b\in \MdR$ mit $a<b$ gilt 
\[
d(\underbrace{\{g\le a\}}_{\ad A}, \underbrace{\{g\ge b\}}_{\ad B})  \ge b - a > 0.
\]
Für $M\subset X$ gilt nun
\begin{align*}
\mu(M) \ge \mu(M\cap f^{-1}(A)) + \mu(M\cap f^{-1}B(B)) 
= \mu(M \cap \{g\circ f\le a\}) + \mu(M\cap \{g\circ f \le b\}).
\end{align*}
...
\]
Mit $\lambda$ wird das äußere Lebesguemaß bezeichnet, das heißt
\[
...
\[
\lambda( A \setminus  \underbrace{\bigcup_{i=1}^\infty [x_i, x_i+h]}_{\ad S}) = 0,
\]
dann folgt
\begin{multline*}
...
\end{multline*}
und da $t>1$ beliebig ist, folgt $\lambda(A) = 0$.
...
\begin{definition}
Sei $\mu$ ein äußeres Maß auf $(X,d)$, $M\supset X$, $\A\subset \mathcal P(X)$. Man nennt $\A$ eine (offene, abgeschlossene, Borelsche) $\mu$-Überdeckung von $M$ wenn $\mu(M\setminus \bigcup_{A\in\A} A) = 0$ und $\A$ aus (offenen, abgeschlossenen, Borelschen) Mengen besteht.
\end{definition}
