...
Seien schlie{\ss}lich $A,B$ nichtleer und kompakt. Setze für $p\in\MdN$
\begin{align*}
...
des Lebesguema{\ss}es (vgl. Satz \ref{satz:2.2}), die allgemeine Aussage erhält man 
...
Dann ist für \(\alpha(p) = \lambda^p(B^p(0,1))\) mit \(B^p(0,1) \da \{ x \in \mathbb R^p \colon \|x\| \leq 1 \}\).

\begin{lemma}\label{lem:2.28}
Im euklidischen Raum \(\mathbb R^n\) gilt: \(\HM^n \ll \lambda^n\)
\end{lemma}
\begin{beweis}
Allgemeines Argument folgt aus Übungsblatt 6, Nr. 1.
\par
...
\begin{align*}
\tag{\(\ast\)}& |a_i-b_i| \leq 2 \min\{|b_j-a_j| \colon 1 \leq j \leq n\} = 2|b_{i_0}-a_{i_0}|, \; i_0 \in \{1,\dots,n\}
\end{align*}
Dann gilt:
\[
...
&\leq 2^n n^{\frac{n}2} |b_{i_0}-a_{i_0}|^n \\
&\leq 2^n n^{\frac{n}2} \lambda^n(A)
\end{align*}
Ist nun \(M \subseteq \mathbb R^n\) mit \(\lambda^n(M)=0\). Sei \(\delta > 0\) fest. Zu \(\varepsilon > 0\) gibt es \(A_i \subseteq \mathbb R^n\), wie oben beschrieben, d.h. \(\diam(A_i) < \delta\) und mit \((\ast)\):
\[
M \subseteq \bigcup_{i=1}^\infty A_i \quad \text{ und } \quad \sum_{i=1}^\infty \lambda(A_i) < \varepsilon
\]
Daraus folgt:
\[
...
\]
\[
\Rightarrow \; \HM^n_\delta(M) \leq \alpha(n) \cdot 2^{-n}\cdot 2^n \cdot n^{\frac{n}2} \cdot \varepsilon
\]
...
\end{satz}
\begin{beweis}
Sei \(M \subseteq \mathbb R^n\) und \(\varepsilon > 0\). Dann existiert eine offene Menge \(U\) mit \(M \subseteq U\) und \(\lambda^n(U) \leq \lambda^n(M) + \varepsilon\) (vgl. Satz \ref{satz:2.2} c))
\par
...
\]
\(f\) heißt \emph{Lipschitz-Abbildung}, falls \(\Lip(f) < \infty\).
\end{definition}
...
\begin{bemerkung}
\begin{enumerate}[(1)]
\item Johnson, Lindenstrauss und Schechtman zeigen, dass im Allgemeinen nicht \(\Lip(h) = \Lip(f)\) möglich ist.
\item Ist \(f: A \subseteq \mathbb R^n \rightarrow \mathbb R^n\) Lipschitzfunktion, so existiert eine Lipschitz-Fortsetzung mit gleicher Lipschitzkonstante (Satz von Kirszbraun, Valentine)
\end{enumerate}
\end{bemerkung}
...
&\leq \left( \sum_{i=1}^n \Lip(h)^2 d(x,y)^2 \right)^{\frac12} \\
&\leq \Lip(f) \cdot (d(x,y)^2)^{\frac12} \\
&= \sqrt n \cdot \Lip(f) \cdot d(x,y)
\end{align*}
\end{enumerate}
\end{beweis}

\begin{satz}[Kirszbraun, Valentine]
Seien \((\HR_1, \langle,\rangle)\) und \((\HR_2, \langle,\rangle)\) Hilberträume, \(\emptyset \neq D \subseteq \HR_1\),
...
\par
\emph{Skizze}: \(\HR \da \HR_1 \otimes \HR_2 \da \{ (x,y) \colon x \in \HR_1, y \in \HR_2\}, \langle (x_1,y_1), (x_2,y_2) \rangle \da \langle x_1,x_2\rangle_1 + \langle y_1,y_2 \rangle_2\), \((x_i,y_i) \in \HR_1 \otimes \HR_2\).
Zu \(f:D \subseteq \HR_1 \rightarrow \HR_2\) betrachte \(\tilde f: D \otimes \HR_2 \rightarrow \HR_1 \otimes \HR_2\), \((x,y) \mapsto (0,f(x))\), usw. (Übung)
\end{beweis}

Sei nun \(f: D \subseteq \HR \rightarrow \HR\) mit \(\Lip(f)=1\). Zu \(f\) gebe es keine echte 1-Lipschitz-Fortsetzung. Seien
\[\HR^2 \da \HR \otimes \HR = \HR \times \HR\]
und \[\chi: \HR^2 \rightarrow (-\infty, \infty], \chi(x,y) \da \sup\{ \|y-f(z)\|^2 - \|x-z\|^2 \colon z \in D\}\]
...
\[
\chi(x,y) \geq \|y-f(x)\|^2 - \|x-y\|^2 = \|y-f(x)\|^2 \geq 0
\]
...
\[
\|y-f(z)\|^2 = \|\tilde f(x) - \tilde f(z)\|^2 > \|x-y\|^2
\]
\(\Rightarrow \chi(x,y) > 0\). Also: \(\chi \geq 0 \) und \(\{\chi = 0\} \subseteq G(f)\).
\par
Sei nun \((x,y) \in G(f)\), d.h. \(x\in D\) und \(y=f(x)\). Also
\[
\|y-f(z)\|^2 = \|f(x)-f(z)\|^2 \leq \|x-z\|^2
\]
\(\Rightarrow \chi(x,y) \leq 0 \Rightarrow \chi(x,y) = 0\), d.h. \(G(f) \subseteq \{\chi=0\}\).
\end{beweis}

\begin{lemma}
Die Abbildung \(\varphi: \HR^2 \rightarrow (-\infty, \infty]\) sei definiert durch
\[
\varphi(x,y) \da \frac14 \chi(x+y,x-y) + \langle x,y \rangle \quad (x,y\in\HR)
\]
\begin{enumerate}[(1)]
...
\]
\item \(\varphi\) ist eine eigentliche (\(\varphi \not\equiv \infty\)), unterhalbstetige, konvexe Funktion mit \(\varphi^\ast(x,y) \geq \varphi(x,y)\) für \(x,y\in \HR\). Hierbei ist \(\varphi^\ast\) die zu \(\varphi\) konjugierte Funktion, explizit
\[
\varphi^\ast(\zeta) \da \sup\{ \langle \zeta,\xi \rangle - \varphi(\xi) \colon \xi \in \HR^2 \}, \zeta \in \HR^2
\]
\end{enumerate}
\end{lemma}
\begin{beweis}
...
\begin{align*}
\|x-y-f(z)\|^2 - \|x+y-z\|^2 &= -4\langle x,y\rangle - 2\langle x,y, f(z)\rangle + 2\langle x+y, z\rangle + \|f(z)\|^2 - \|z\|^2 \\
&= -4\langle x,y\rangle + 2\langle x,z-f(z)\rangle + 2\langle y,z-f(z)\rangle + \|f(z)\|^2-\|z\|^2
\end{align*}
Bildung des Supremums ergibt die Behauptung.
\item Direkt durch Einsetzen in die Definition:
\[
4\varphi(\frac{z+f(z)}2, \frac{z-f(z)}2) = \underbrace{\chi(z,f(z))}_{=0} + \langle z-f(z),z+f(z)\rangle = \|z\|^2 - \|f(z)\|^2
\]
\item Als Supremum von stetigen, konvexen Funktionen ist \(\varphi^\ast\) konvex und unterhalbstetig.
\end{enumerate}
\end{beweis}
