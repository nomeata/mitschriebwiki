...
\[
\eta_1\wedge \ldots\wedge \eta_k \in \bw\limits^k\MdR^n
\]
durch
\[
...
\[
v_1\wedge \ldots\wedge v_k \in \bw\limits_k\MdR^n
\]
durch
\[
...
$\eta_1\wedge\ldots\wedge \eta_k\in\bw^k\MdR$ als Linearform auf $\bw_k\MdR^n$ 
...
\]
Für $I\in I_k^n$ ist $e_I \da e_{i_1},\ldots,e_{i_k}$, $dx_I \da dx_{i_1}\wedge \cdots \wedge dx_{i_k}$ und so weiter.
\end{notation}
...
\begin{align*}
\Phi &= \sum_{\mathclap{1\le i_1<\cdots < i_k\le n}} \Phi_{(i_1,\ldots,i_k)} dx_{i_1} \wedge \cdots \wedge dx_{i_k} \\
...
\begin{definition}
Sei $W\subset \MdR^n$ offen. Eine Abbildung $\Phi\colon W\to \bw^k\MdR^n$ heißt Differentialform vom Grad $k$ (kurz: $k$-Form).

Die $k$-Form $\Phi$ ist von der Klasse $\mathcal C^r$, $r\ge 1$, falls $p\mapsto \Phi(p)(v_1,\ldots,v_k)$ von der Klasse $\mathcal C^r$ ist für jede Wahl von $v_1,\ldots,v_k\in \MdR^n$.
\end{definition}

\begin{bemerkungen}
\item $\Phi$ ist von der Klasse $\mathcal C^r$ genau dann, wenn
\[
p\mapsto \Phi(p)_I = \langle \Phi(p), e_I\rangle = \langle \Phi(p) , e_{i_1}\wedge \cdots \wedge e_{i_k}\rangle
\]
von der Klasse $\mathcal C^r$ ist für alle $I\in I_k^n$.
\item $\Phi$ lässt sich schreiben als
\[
...
Für $\Phi \in \bw^k\MdR^n$, $\eta \in \bw^l\MdR^n$ sei $\Phi\wedge \eta \in \bw^{k+l}\MdR^n$ erklärt durch:
Es sei $\Phi = \sum_{I\in I_k^n} \Phi_Idx_I$ und $\eta = \sum_{J\in I_l^n} \eta_J dx_J$. Dann ist
\[
\Phi \wedge \eta \da \sum_{I\in I^n_k,\, J\in I^n_k} \Phi_I\cdot \eta_J \underbrace{dx_I \wedge dx_J}_{\mathclap{dx_{i_1} \wedge \cdots \wedge dx_{i_k} \wedge dx_{j_1} \wedge \cdots \wedge dx_{j_l}}}.
\]
\end{definition}

\begin{bemerkungen}
\item Assoziativgesetz: Für $\Phi\in \bw^k\MdR^n$, $\eta\in\bw^l\MdR^n$, $\Theta\in \bw^r\MdR^n$ gilt $(\Phi\wedge \eta) \wedge \Theta =\Phi \wedge (\eta\wedge \Theta)$.
\item Distributivgesetz: Für $\alpha_1,\alpha_2\in \MdR$, $\Phi_1,\Phi_2 \in \bw^k\MdR^n$ und $\eta\in\bw^l\MdR^n$ gilt $(\alpha_1 \Phi_1 + \alpha_2\Phi_2) \wedge \eta = \alpha_1(\Phi_1\wedge \eta) + \alpha_2(\Phi_2\wedge \eta)$.
\end{bemerkungen}
\pagebreak[2]
\textbf{Ausblick:}
\begin{itemize}
...
\begin{beweis}
\begin{enumerate}
\item[\quad(2)] Seien $\Phi = fdx_I$, $\Psi = gdx_J$. Dann erhält man
\begin{align*}
...
\begin{lemma}
\label{lem:4.2}
Ist die $k$-Form $\Phi\colon U\to \bw^k\MdR^n$ von der Klasse $\mathcal C^r$, $r\ge 2$, so gilt $dd\Phi = 0$ ($(k+2)$-Form).
\end{lemma}
...
\begin{bemerkung}
Eine $k$-Form $\Phi$ mit $d\Phi = 0$ heißt geschlossen. Eine $k$-Form $\Phi$, zu der es eine $k-1$-Form $\eta$ gibt mit $d\eta =\Phi$ heißt exakt. Lemma \ref{lem:4.2} besagt, dass $d\Phi$ stets geschlossen ist.

\textbf{Frage:} Ist $\Phi$ geschlossen, das heißt $d\Phi =0$, ist dann $\Phi$ exakt?

Im Allgemeinen gilt dies nicht. Für ein einfach zusammenhängendes $U$ ist dies jedoch richtig (Lemma von Poincaré).
\end{bemerkung}
...
\]
wobei das $U$ im linkten Integral als Menge mit Orientierung (durch die rechts verwendete geordnete Standardbasis) zu verstehen ist. So legt man auch fest, dass
\[
...
Sei $n\ge 1$.
\begin{enumerate}
\item Ein $(n-1)$-dimensionaler Quader $F$, der zur $i$-ten Koordinatensystemachse orthogonale ist, ist von der Form
\[
...
Sei $R=[a_1,b_1]\times\dots [a_n,b_n]$ ein Quader mit $a_i<b_i$. Dann sei für $1\le i\le n$
\begin{align*}
...
\begin{satz}
Seien $R=[a_1, b_1] \times \dots \times [a_n,b_n]$, $a_i < b_i$ und $\varphi$ eine $(n-1)$-Form der Klasse $C^k$, $(k \ge 1)$, auf einer offenen Menge $U$ mit $R\subset R$. Dann gilt
\[
...
\begin{align*}
\int_{\partial_o R}\varphi = \int_{\partial_o [a_1,b_1]}\varphi = \int_{\{b\}-\{a\}} \varphi = \varphi(b)-\varphi(a)
\end{align*}
und
\begin{multline*}
...
&= \sum_{j=1}^n (-1)^{j-1}\Big( \int_{R_j^+} \varphi_i \cdot dx_1\wedge \dots \wedge \cancel{dx_i} \wedge \dots\wedge dx_n - \int_{R_j^-} \varphi_i \cdot dx_1\wedge \dots \wedge \cancel{dx_i} \wedge \dots\wedge dx_n \Big) \\
&= \sum_{j=1}^n (-1)^{j-1}\Big( \int_{R_j^+} \underbrace{\langle \varphi_i \cdot dx_1\wedge \dots \wedge \cancel{dx_i} \wedge \dots\wedge dx_n , \hat e_j\rangle}_{=
\begin{cases}
0, &\text{für } j\ne i\\
1, &\text{sonst}
\end{cases}} \, \HM^{n-1}(dx)\\
&\hspace{4cm} - \int_{R_j^-} \langle \varphi_i \cdot dx_1\wedge \dots \wedge \cancel{dx_i} \wedge \dots\wedge dx_n , \hat e_j\rangle \, \HM^{n-1}(dx)\Big) \\
&= (-1)^{i-1} \Big( \int_{R_i^+} \varphi \, d\HM^{n-1} - \int_{R_i^+} \varphi \, d\HM^{n-1}\Big).
\end{align*}
Dies zeigt die Gleichheit.
\end{beweis}

\textbf{Spezialfall:} Divergenzsatz/Satz von Gauß-Gren.
...
Ist $\varphi$ eine $(n-1)$-Form auf $\MdR^n$, das heißt
\[
\varphi = \sum_{i=1}^n \varphi_1\cdot dx_1\wedge \dots \wedge \cancel{dx_i} \wedge \dots\wedge dx_n
\]
so setze
\[
