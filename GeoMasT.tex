...
Sei $b\in E(c,\tau,i)$. Sei $(e_1,\ldots,e_m)$ die Standardbasis. Da $Df_b$ injektiv ist, ist in der Zerlegung $Df_b=\rho\circ\sigma$ mit $\sigma:\MdR^m\to\MdR^m$ symmetrisch, $\varrho:\MdR^m\to\MdR^n$ eine lineare Isometrie die Abbildung $\sigma$ ein Automophismus. Wir schätzen ab
\begin{align*}
...
Zu zeigen ist noch, dass die Mengen $B$ überdecken. Sei hierzu $b\in B$. Dann ist $Df_b = \varrho \circ \tau$ mit $\varrho, \sigma$ wie oben. Wähle $\delta>0$ so, dass
\begin{align*}
(1-\delta\cdot\|\sigma^{-1}\|)^{-1} < t - \ep \quad\text{und} \quad 1<1+\delta\cdot\|\sigma^{-1}\| < (t^{-1}+\ep)^{-1}
\end{align*}
und $\tau\in T$ so dass $\|\sigma-\tau\|< \delta$. Dann gilt:
\begin{align*}
\|\tau\circ\sigma^{-1}\| 
&= \|\tau \circ \sigma^{-1} - \id_{\MdR^m} + \id_{\MdR^m}\|\\
...
Der Beweis der Borel-Messbarkeits wird hier nicht ausgeführt.
\end{beweis}
...
\item $A\subset \{x\in\MdR^m: Jf(x)\ne 0\}$. Sei $t>1$ beliebig. Wähle eine Borel-Überdeckung $\mathcal E$ von $B\da \{x\in\MdR^m: Jf(x) \ne 0\}$ wie in Lemma \ref{lem:3.7}. Sei $\mathcal G$ eine abzählbare Zerlegung von $A$ derart, das für jedes $G\in\mathcal G$ gilt: $G\in \borel(\MdR^m)$ und $G\subset E$ für ein $E\in\mathcal E$ und $\tau_E$ wie in Lemma \ref{lem:3.7}. Dann folgt
\begin{align*}
t^{-2m}\cdot\HM^m(f(G)) 
...
\end{align*}
Da $f|_G$ injektiv ist für alle $G\in\mathcal G$ erhält man zunächst
\begin{align*}
\sum_{G\in\mathcal G} \HM^m(f(G))
&= \sum_{G\in\mathcal G} \int_{\MdR^m} \ind_{f(G)} d\HM^m \\
&= \sum_{G\in\mathcal G} \int_{\MdR^m} \#(G\cap f^{-1}(\{y\}))\, \HM^m(dy) \\
&= \int_{\MdR^m} \#(A\cap f^{-1}(\{y\}))\, \HM^m(dy)
\end{align*}
und hiermit
\begin{align*}
t^{-2m}\cdot \int_{\MdR^m} \#(A\cap f^{-1}(\{y\}))\, \HM^m(dy) 
&\le \int_A Jfd\lambda^m \\
&\le t^{2m} \cdot \int_{\MdR^m} \#(A\cap f^{-1}(\{y\})) \, \HM^m(dy).
\end{align*}
Dies gilt für jedes $t>1$, so folgt die Gleichheit.
