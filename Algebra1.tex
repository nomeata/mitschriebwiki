...
\item (Gauß) Für $f,g\in K[X]\setminus\{0\}$ gilt: $\nu_p(f\cdot g) = \nu_p(f) + \nu_p(g)$ für alle $p\in \mathbb P$.
\sbew{
Sei $f=\sum_{i=0}^na_iX^i$, $g=\sum_{j_0}^mb_jX^j$, $f\cdot g = \sum_{k=0}^{m\cdot n} c_kX^k$, also $c_k=\sum_{i+j=k}a_ib_j$.

\paragraph{1. Fall:} Sei $m=0$. Dann ist $c_k=a_+b_0$ für $k=0,\ldots,n$ und \begin{align*}
\nu_p(f\cdot g) & = \min_{i=0}^n(\nu_p(a_ib_0)) \\&= \min_{i=0}^n(\nu_p(a_i) + \nu_p(b))\\ &= \min_{n=0}^n(\nu_p(a_n)) + \nu_p(b_0) = \nu_p(f) + \nu_p(g)
\end{align*}

\paragraph{2. Fall:} Sei $f,g\in R[X]$ und primitiv, also $\nu_p(f)=\nu_p(g)=0$. Sei $i_0 \defeqr \min_{i=0}^n\{i: p\nmid a_i\}$ und $j_0 \defeqr \min_{i=0}^n\{j: p\nmid b_j\}$. Es ist:
\[
c_{i_0+j_0}
...
Es ist $c\in R$, enn: für $p\in \tilde{\mathcal P}$ ist nach \ref{nu-multiplikativ}
\[
\underbrace{\nu_p(f)}_{\ge 0} = \nu_p(c) + \sum_{i=1}^n \underbrace{\nu_p(f_i)}_{=0}\,,
\]
also ist $\nu_p(0)\ge 0$.

Schreibe also $c=e\cdot p_1\cdots p_n$ mit $e\in R^\times$ und $p_i\in \tilde{\mathcal P}$.
...
Die Restklassenabbildung $\pi: R\to \bar R$ ist surjektiv und induziert einen surjektiven Ringhomomorhpismus $\tilde \pi: R[x]\to \bar R[X]$. Es ist $\Kern{\tilde\pi} = \{f=\sum_{i=0}^na_iX^i\in R[X], p\mid a_i, i=0,\ldots,n\} = p\cdot R[X]$, also ist $\bar R[X] \cong R[X]/(p)$.
