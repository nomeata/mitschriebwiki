...
\item $Urb:$ \underline{Mengen} $\ra$ \underline{Mengen}, $Im(X) = \mathcal P(x)$, für $f: X \to Y$ ist $Urb(f): \mathcal P(Y) \to \mathcal P(X)$, $Im(f)(V) = f^{-1}(V)$, $V\in \mathcal P(Y)$ ist kontravariant.
\end{enumerate}
...
\begin{Def}
Seien $\mathcal A$, $\mathcal B$ Kategorien und $F,G:A\to B$ kovariante Funktoren. $F$ und $G$ heißen \emp{isomorph}, wenn es zu jedem Objekt $A\in Ob\mathcal A$ einen Isomorphismus $\alpha_A: F(A) \to G(A)$, also $\alpha_A\in Mor_{\mathcal B}(F(A),G(A))$ gibt, so dass für alle Morphismen $f:A \to A'$ in $\mathcal A$ das folgende Diagramm kommutiert:
\[
\begin{CD}
F(A) @>\alpha_A>> G(A) \\
@VF(f)VV         @VVG(f)V \\
...
\sbew{
Nach Satz \ref{Satz 4} gibt es für jede Gruppe $G$ eine bijektive Abbildung $\alpha_G:F_X(G)=Abb(X,C) \to Hom(F(X),\cdot)$, $f\mapsto \varphi = \hat f$. Sei $\rho:G\to G'$ ein Gruppenhomomorphismus. Dann kommutiert:
\[\begin{CD}
...
\begin{Satz}[Sylow]
    ...
    \bew{}{
        ...
        $\mathcal{S} \defeqr \{S' \subset G:S' = gSg^{-1}$ für ein $g \in G$ \}
        \newline
        \textbf{Beh.2}:
            $p \nmid |\mathcal{S}|$.
        \newline
...
\bsp{$G=A_5$ hat 60 Elemente, z.B.: $(12345)$, konjugiert dazu
$(13245)$ } 
\end{Folg}
...
\newline
\sbew{Es genügt zu zeigen: $N$ enthält ein $\sigma \neq id$ mit
