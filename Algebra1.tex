...
Nullteiler in $R$ ist. (Das heißt: Aus $xy = 0$ folgt, daß $x=0$ oder
$y=0$.)
\item $R$ heißt \emp{Integritätsbereich} (engl. \emp{integral}), wenn er
...
\bsp{Ist $R$ ein kommutativer Ring mit Eins und $R^{n \times n}$ der Ring der $n \times n$-Matrizen mit
...
\bsp{In $R=K[X]$, $K$ Körper, ist $(X) = \{f=\sum_i=0^n, n\in \mathbb N, a_0=0\}$ maximal.}
...
\item[(b)] Seien $x_1,x_2\in R$ mit $x\cdot y=I$, also $\bar x \cdot \bar y =0$ in $R/I$. $I$ Primideal $\iff x \in I$ oder $y\in I \iff \bar x = 0 $ oder $\bar y=0 \iff R/I$ ist nullteilerfrei.
