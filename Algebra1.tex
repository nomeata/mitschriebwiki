...
\newline \sbew{$c_g$ ist Homomorphismus: $c_g(x_1 x_2) = g(x_1
...
Gruppenhomomorphismus. \newline \sbew{$\forall x \in G: c(g_1 g_2)(x) = (g_1
...
in $G$. \newline \sbew{Die Zuordnung \[\begin{array}{lcl}
...
\begin{DefBem}
	...
	\item $\varphi(1) = 1 = \varphi(2)$, $\varphi(p) = p-1$ für $p$ Primzahl, $\varphi(m\cdot n) = \varphi(m) \cdot \varphi(m)$, falls $m,n$ teilerfremd, $\varphi(p^k) = p^{k-1}(p-1)$, für $p$ Primzahl.
...
\label{Satz 2}
Jede Untergruppe einer freien abelscen Gruppe von endlichem Rang $n$ ist frei mit Rang $r\le n$. Genauer:
    ...
    \sbew{
        Noch zu zeigen: Die Eindeutigkeit de $a_i$:
...
in $F^a(X^{\pm})$
\newline \textbf{Def}.: Zwei Wörter in $F^a(X^{\pm})$ heißen
\emp{äquivalent}, wenn sie durch endliches Einfügen oder Streichen
von Wörtern der Form $(x,i(x)), x\in X^{\pm}$ auseinander
hervorgehen.
...
$F(S)$ die freie Gruppe mit Basis $S$, $f:S \ra G$ die Identität und
...
\bsp{\begin{enumerate}
\renewcommand{\labelenumi}{(\theenumi)}
\item Gruppen $\ra$ Mengen, $(G,\cd) \mapsto G$ (''Vergißfunktor'')
\item $\mathcal{P}$: Mengen $\ra$ Mengen : $X\mapsto \mathcal{P}(X)$
(Potenzmenge) \newline Für $f:X\ra Y$ sei
...
Für $f:Y \ra Z$ ist Hom$(X,\cd)(f): Mor(X,Y) \ra Mor(X,Z)$ gegeben durch $g
\mapsto f \circ g$ und
Hom$(\cd, X)(f): Mor(Z,X) \ra Mor(Y,X),\;g \mapsto g \circ f$
\item Sei $X$ Menge, $F_X$: Gruppen $\ra$ Mengen, $G \mapsto Abb(X,G) =
Mor_{Mengen}(X,G)$ (mit Vergißfunktor). Für
jedes $f:X \ra G$ gibt es $\varphi:F(X) \ra G$ (Satz \ref{Satz 4}) \newline
Für jedes $f:X \ra G$ gibt es $\varphi: F(X) \ra G$, also Bijektion: $\alpha_G:
...
\label{1.22}
    ...
    \begin{enum}
        ...
        \sbew{$gx = \rho(g)(x)$ ergibt die gewünschte Bijektion }
        \sbsp{0.9}{
            \begin{enumerate}
                \renewcommand{\labelenumi}{(\theenumi)}
...
\newline\sbew{Ist $y \in Gx$, so ist $Gy = Gx$, denn $y=gx \Ra
...
\newline\sbew{ Für $h \in G$ gilt: $h \in G_{gx} \lra h(gx) = gx \lra
...
1$ in $R$ \newline \sbew{0.9}{Wäre $0 = 1$, so gälte für jedes $x
...
\sbew{1.0}{Seien $L,L'$ Zerfällungskörper, $L =
