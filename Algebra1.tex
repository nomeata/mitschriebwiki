...
Die Restklassenabbildung $\pi: R\to \bar R$ ist surjektiv und induziert einen surjektiven Ringhomomorphismus $\tilde \pi: R[x]\to \bar R[X]$. Es ist $\Kern{\tilde\pi} = \{f=\sum_{i=0}^na_iX^i\in R[X], p\mid a_i, i=0,\ldots,n\} = p\cdot R[X]$, also ist $\bar R[X] \cong R[X]/(p)$.
...
Nach Folgerung \ref{Satz 3} ist $G \cong \ds\bigoplus_{i=1}^r \mathbb{Z}/a_i
