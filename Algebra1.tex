...
\begin{enum}
	    ...
	    \implies g^m = g^{m-n}h^n \in \langle g^n\rangle$ Wid!
...
Jede Untergruppe einer freien abelschen Gruppe von endlichem Rang $n$ ist frei mit Rang $r\le n$. Genauer:
    ...
    so daß $a_1 x_1, \dots, a_r x_r$ eine Basis von $H$ ist. Die $a_i$ sind eindeutig bestimmt.
...
Jede endlich erzeugte abelsche Gruppe $A$ ist die direkte Summe von zyklischen Gruppen:
    ...
    mit $m,n,a_1,\dots,a_m \in \mathbb{N}$, $\forall i: a_i \geq 2$, $a_i \mbox{
    teilt }
...
$(x_1,\dots,x_n,y_1,\dots,y_m,z_1,\dots,z_l)$ Das gleiche gilt für
$x(yz)$.
...
\bew{}{
% TODO müsste es hier nicht eigentlich \Varphi heißen, denn in der Aussage gibt
% es kein \varphi oder andersrum weil später in Bezug hierauf von \varphi die
% Rede ist
...
\item $\mathbb Z^2 \da \mathbb Z \times \mathbb Z$, $S=\{(0,1) \ad x, (1,0) \ad y\}$. Der Homomorphismus $\varphi : F(s) \to \mathbb Z^2$, $x\mapsto (0,1)$, $y\mapsto (1,0)$ bildet $w = x^{n_1}y^{m_1}\cdots y^{m_d} \in F(S)$ auf $\varphi(w) = (\sum_{i=1}^s n_i, \sum_{i=1}^s m_i)$ ab, also ist $\Kern\varphi = \{ w = x^{n_1}y^{m_1}\cdots y^{m_d} \in F(S) \mid \sum_{i=1}^s n_i = \sum_{i=1}^s m_i = 0\} = \langle \{ w_1w_2w_1^{-1}w_2^{-1}, w_1, w_2\in F(S)\}\rangle = G^{\text{ab}}$. $\Kern\varphi$ ist kleinster Normalteiler von $F(\{x,y\})$, der $xyx^{-1}y^{-1}$ enthält, daher ist $\mathbb Z^2 \cong F(\{x,y\}) / \langle xyx^{-1}y^{-1}\rangle_\text{NT}$.
\end{enumerate}
...
\begin{Def}
    ...
    einer Menge $Mor_{\varphi}(A,B)$ von \emp{Morphismen} von $A$ nach $B$, für
...
$F: Mor_{\mathcal{A}}(X,Y) \ra Mor_{\mathcal{B}}(F(X),F(Y))$, so daß
gilt:
\begin{enumerate}
...
Für $f\in $Hom$(Y,Z)$ ist Hom$(X,\cd)(f): Mor(X,Y) \ra Mor(X,Z)$ gegeben durch $g
\mapsto f \circ g$ und
...
\bsp{\begin{enum}
\item ist effektiv
\item Der Ineffektivitätskern ist das Zentrum $Z(G)$
\item ist effektiv für $|X| \geq 2$
\end{enum}
}
...
\item $\alpha_i$ ist injektiv. Ist $gG_{x_i} = hG_{x_i}$, so ist $g\in hG_{x_i}\implies h^{-1}g\in G_{x_i} \implies (h^{-1})x_i = x_i \implies g\cdot x_i = h\cdot x_i$
...
Daher gibt es genau zwei Elemente der Ordnung 3 und vier Elemente der Ordnung 5. Übrib gleiben 8 Elemente, die Ordnung 15 haben müssen, also ist $G\cong \mathbb Z/15\mathbb Z$.
}
...
Es genügt auch zu zeigen, dass $N$ as Produkt von Transpositionen enthält, denn ist $\sigma=(1\ 2)(3\ 4)\in N$, so ist auch $(3\ 4\ 5)=\sigma(\tau \sigma^{-1} \tau)\in N$, mit $\tau = (1\ 2)(3\ 5)$.

Das Ziel ist also zu zeigen, dass $N$ ein Element $\sigma$ enthält mi $\sigma(i)\ne i$ für höchstens vier $i$, denn dann ist $\sigma\in A_4$, also 3-Zykel oder Produkt von zwei Transpositionen.
...
Annahme: $k_0\ge 5$. 1. Fall: $\sigma$ enthält einen Zyklus der Länge $\ge 3$, also $\sigma(1)=2$, $\sigma(2)=3$, $\sigma(4)\ne 4$, $\sigma(5)\ne 5$. Sei $\alpha \da \sigma^{-1}(3\ 4\ 5)\sigma(5\ 4\ 3)\in N$. Ist $\sigma(i)=i$, so ist $\alpha(i)=i$ für $i\ge 6$. Außerdem ist $\alpha(1)=1$ und $\alpha(2)=\sigma^{-1}(4)\ne 2$, also ist $\alpha\ne \text{id}$ und $k_\alpha < k_\sigma$.
...
\item $G$ heißt \emp{einfach}, wenn $G\triangleright \{1\}$ die einzige Normalreihe ist, das hießt: $G$ besitzt nur die trivialen
...
Normalteiler in $\bar{H_{i-1}}$ für $i=0,\ldots,l$, denn für 
$\bar{h_i} \in \bar H_i$, $\bar
g \in \bar{H_{i-1}}$ ist $\bar g \bar{h}_i \bar{g}^{-1} = \pi(gh_ig^{-1}) \in \bar{H_i}$ 
...
ist Kompositionsreihe für $G$ wenn $j\le l-2$, bzw .
\[G_1 = C_0 \triangleright C_1 \triangleright \dots \triangleright C_j  = \{1\}\]
ist Kompositionsreihe für $G$, wenn $j=l-2$.
\newline Dann gilt: 
\[G_1 \triangleright G_2 \triangleright G_2
...
\text{ für } i\in\{0,\ldots,j,j+2,\ldots,l-1\}\,.\]% und $\ds C_j/C_{j+1} \cong G_{\sigma(j)}/G_{\sigma(j)+2}$.
\newline\textbf{Beh.2}
\begin{enum}
\item $C_j = C_{j+1}$
...
\[
G= G_0 = H_{0,0} \triangleright H_{0,1} \triangleright \cdots H_{0,d_0} = G_1 = H_{1,0} \triangleright \cdots \triangleright H_{1,d_1} = G_2 \cdots G_m = \{1\}
\]
Dabei ist 
\[
\faktor{H_{i,j}}{H_{i,j+1}} \cong \faktor{ H_{i,j}/G_{i+1} }{ H_{i,j+1}/H_{i+1}} \subseteq \faktor{ G_i/G_{i+1} }{ H_{i,j+1}/H_{i+1}} 
\]
also ist $H_{i,j}/{H_{i,j+1}}$ kongruent zu einer Untergruppe einer Faktorgruppe einer abelschen Gruppe, also selbst auch abelsch.
}
\end{enum}
\end{DefBem}
...
\bsp{
\begin{itemize}
\item $D_n = \{ 1, \tau, \ldots, \tau^{n-1}, \sigma, \sigma\tau, \sigma\tau^{n-1}\} \triangleright \{1, \tau, \ldots, \tau^{n-1}\} \cong \mathbb Z / n\mathbb Z \triangleright \{1\}$, also ist $D_n$ auflösbar.
...
\bsp{
In $R=\mathbb Z$ sind $n\mathbb Z$ für jedes $n\in \mathbb Z$ Ideale. In $R=\mathbb Q$ dagegen sin diese für $n\ne 0$ keine Ideale.
}

\item $x\in R$ heißt (Links-/Rechts-)\emp{Nullteiler}, wenn es ein
$y \in R \setminus \{0\}$ gibt mit $xy = 0$. (bzw. $yx = 0$)
\item $R$ heißt \emp{nullteilerfrei}, wenn $0$ der einzige
Nullteiler in $R$ ist. (dh. wenn aus $xy = 0$ folgt, daß $x=0$ oder
$y=0$)
\item $R$ heißt \emp{Integritätsbereich} (integral[domain]), wenn er
...
char($K$)$=0$, so ist der kleinste Teilkörper von $K$ isomorph zu
$\mathbb{Q}$
\end{enum}
\bsp{ $R$ Ring, $R^{n \times n}$ Ring der $n \times n$-Matrizen mit
Einträgen in $R$ \newline Für $n \geq 2$ ist $R^{n \times n}$ nicht
...
Zur Definition von $\det A$ muß $R$ kommutativ sein!
...
\item Ein (Rechts-)Ideal $I$ in $R$ heißt \emp{Hauptideal}, wenn es
ein $a \in R$ gibt mit $I = (a)$
\item Ein kommutativer Ring mit Eins heißt \emp{Hauptidealring}, wenn
jedes Ideal in $R$ ein Hauptideal ist.
\bsp{Sei $I \subseteq \mathbb{Z}$ Ideal, $a \in I$ mit $|a| \leq |b| \forall b
\in I \setminus \{0\}$.
...
\sbew{(a), (b), (d) $\chk$\begin{enumerate}
...
\bsp{\begin{enum} \item[(1)]$p$ prim $\lra p \mathbb{Z}$
...
\begin{DefBem}
\begin{enum}
\item Ein Integritätsbereich $R$ heißt \emp{euklidisch}, wenn es
...
\item $x \in R\setminus R^x$ heißt \emp{irreduzibel}, wenn aus $x=y_1
...
\min\{\nu_p(a_i)\;: i=0,\dots,n\}$
\newline $f$ heißt \emp{primitiv}, wenn $\nu_p(f) = 0$ für alle $p
\in \mathcal{P}$
\end{Def}
...
\sbew{Sei $f = gh$ mit $\ds g=\sum_{i=0}^r b_i X^i$, $\ds h =
...
\end{enum}
\bew{}{
\item $R \ra \bar R$ induziert surjektiven Homomorphismus $\varphi: R[X]
\ra \bar R[X]$, $\Kern(\varphi$)$= \{f \in \sum_{i=0}^n a_i X^i,\; p
\mid a,\;$ für $i=0,\dots,n\} = p R[X]$ Mit dem Homomorhpiesatz folgt
die Behauptung.
\item Sei $f = gh \Ra \bar f = \bar g \bar h$ (Schreibe $h =
\sum_{i=0}^s c_i X^i$). Also ist \OE $\bar h \in (\bar R[X])^x =
\bar{R}^x \Ra p \mid c_i$ für $i=1,\dots,s$. Wäre $s \geq 1$, so wäre
$c_s$ durch $p$ teilbar, also auch $b_r c_s = a_n \blitza$
}

\bsp{$f = X^2 - 5 \in \mathbb{Z}[X]$ und
...
\sbew{Sei $K \defeqr$ Quot($R$). Dann ist $K[X]$ faktoriell,
$R[X] \subseteq K[X]$ Unterring.
\newline Sei $0 \neq f \in R[X]$ läßt sich als Produkt von
...
\begin{Bem}[Vorarbeit]
\label{2.29}
Für jedes Primelement $p \in R$ und alle $f,g \in K[X]:
\nu_p(fg) = \nu_p(f) + \nu_p(g)$
\newline Wähle dabei System $\mathcal{P}$ von Vertretern der
Primelemente, zerlege $x \in R$ als $\ds x = e \prod_{p \in
\mathcal{P}} p^{\nu_p(x)}$, beachte $\nu_p(xy) = \nu_p(x) +
\nu_p(y)$.

Für $\ds f = \sum_{i=0}^n a_i X^i$ ist $\ds \nu_p(f) = \min_{i=0}^n
\nu_p(a_i)$.

Für $\ds x = \frac{a}{b} \in K$ sei $\ds \nu_p(x) = \nu_p(a) -
\nu_p(b) \in \mathbb{Z}$ (wohldefiniert!).

\bew{}{
\item[(1)] Grad($f$)$= 0$, dh. $f = a_0 \in K$, $\ds g\in
...
\item[(2)] Wir dürfen annehmen: $f,g \in R[X]$ primitiv.
\newline \textbf{denn}: Wähle $a \in R$ mit $af \in R[X]$. (''Hauptnenner'')
...
\item[(3)] Für primitive $f,g \in R[X]$ gilt $\nu_p(fg) = \nu_p(f) +
...
K^x$) \textbf{Beob.}: $c \in R$, denn: für $p \in \mathcal{P}$ ist
\[0 \leq \nu_p(f) = \nu_p(c) + \sum_{p=1}^n \nu_p(f_i) = \nu_p(c)
...
Abschluß $\bar K$ bis auf Isomorphie eindeutig bestimmt.
...
\begin{enum}
\item Hom$_K(L,L') = \{\sigma:L\ra L'$ Körperhom., $\sigma_{|K}$ $=$
id$_K\}$
\newline Aut$_K(L) = $ Aut$(L/K)$ = Hom$_K(L,L)$.

\item Ist $L/K$ endlich, $\bar K$ algebraischer Abschluß von $K$, so
...
\sbew{Ist $K$ endlich, so folgt aus \ref{3.17}, daß $L^x$ zyklische
...
\sbew{In $\bar K[X]$ zerfällt $f$ in Linearfaktoren:
