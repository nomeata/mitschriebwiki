...
\end{xy}
\]
  Mit $S^0(M) := R =: \Lambda^0(M)$ heisst $S(M):= \bigoplus_{n\geq 0} S^n(M)$ die \emp{symmetrische Algebra}\index{Algebra!symmetrische} über $M$\\
  $\Lambda (M) := \bigoplus_{n\geq 0} \Lambda^n(M)$ die \emp{äu\ss ere Algebra}\index{Algebra!\"au\ss ere} über $M$ ( oder \emp{Graßmann-Algebra}\index{Gra\ss mann-Algebra} )
 \end{Satz}
